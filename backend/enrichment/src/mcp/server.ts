...
// -----------------------------------------------------------------------------
// Tool definitions — hand-written JSON Schemas so we never go through the
// SDK's Zod-based generic overload resolution (which recursively blows up tsc
// with TS2589 on SDK 1.29 + Zod 3.24). JSON Schema is the protocol's own wire
...
function optionalNumber(args: Record<string, unknown>, key: string, def: number, min: number, max: number): number {
  const v = args[key];
...
const TOOLS: ToolDef[] = [
  ...
  // ---------------------------------------------------------------------------
  {
    ...
    description:
      'Lists all active enrichment entity descriptors (platform-scoped). Each descriptor defines what entity the engine can enrich, which target table it writes to, and which extraction schema and default merge policy it uses.',
  ...
  // ---------------------------------------------------------------------------
  {
    ...
    description:
      'Returns the full descriptor + extraction schema + default merge policy for a given entity_name. Use list_descriptors to discover valid entity_name values.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  // ---------------------------------------------------------------------------
  {
    name: 'list_policies',
    description:
      'Returns all active merge policies and trust rules. Platform-scoped rules apply to every tenant; tenant-scoped rules apply only when tenant_code is provided.',
    inputSchema: {
      type: 'object',
      properties: {
        tenant_code: {
    ...
    handler: async (args) => {
      ...
      const policiesResult = await pool.query(
        `SELECT id, tenant_id, code, version, budget_cap_eur, current_usage_eur, is_active, created_at
           FROM enrichment_merge_policies
          WHERE is_active = true
            AND (tenant_id IS NULL ${tenantId ? 'OR tenant_id = $1::uuid' : ''})
          ORDER BY tenant_id NULLS FIRST, code ASC`,
        tenantId ? [tenantId] : [],
      );
      const trustResult = await pool.query(
        `SELECT id, tenant_id, source_type, domain_pattern, trust_score, notes
           FROM enrichment_trust_rules
          WHERE tenant_id IS NULL ${tenantId ? 'OR tenant_id = $1::uuid' : ''}
          ORDER BY tenant_id NULLS FIRST, trust_score DESC`,
        tenantId ? [tenantId] : [],
      );
      return textResult({
  ...
  // ---------------------------------------------------------------------------
  {
    name: 'get_job_status',
    description:
      'Returns full status of a single enrichment job: row from enrichment_jobs, ordered event timeline, and all candidate facts produced so far. Scoped to the given tenant — cross-tenant lookups return not found.',
    inputSchema: {
      type: 'object',
      properties: {
    ...
    handler: async (args) => {
      ...
      const data = await withTenantClient(tenant.id, async (c) => {
        const jobRow = await c.query(
          `SELECT j.id, j.tenant_id, j.status, j.mode, j.target_table, j.target_record_id,
        ...
        const events = await c.query(
          `SELECT event_type, payload_jsonb AS payload, created_at
        ...
        const candidates = await c.query(
          `SELECT id, entity_type, entity_anchor, field_name, candidate_value, confidence,
  ...
  // ---------------------------------------------------------------------------
  {
    ...
    description:
      'Paginated view of recent enrichment jobs for a tenant. Useful for triage: filter by status to find stuck or failed jobs.',
    inputSchema: {
      type: 'object',
      properties: {
        tenant_code: { type: 'string', description: 'Tenant code (e.g. "rtl-bank") or tenant UUID' },
        ...
        status: {
          type: 'string',
          description:
            'Optional status: pending|discovering|crawling|extracting|resolving|previewing|committed|failed|cached|rolled_back',
        },
      },
    ...
    handler: async (args) => {
      ...
      const result = await withTenantClient(tenant.id, (c) =>
        c.query(
          `SELECT j.id, j.status, j.mode, j.target_record_id,
                  d.entity_name AS descriptor_name,
...
export function buildServer(): Server {
  const server = new Server(
    { name: 'heuresys-enrichment-mcp', version: '0.3.0-fase3' },
    { capabilities: { tools: {} } },
  );
