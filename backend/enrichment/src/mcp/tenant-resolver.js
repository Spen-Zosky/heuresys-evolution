 ...
 * Throws if no match.
 */
export async function resolveTenant(codeOrId) {
    ...
    if (isUuid) {
        ...
        const result = await pool.query(`SELECT id, code, name FROM tenants WHERE id = $1::uuid LIMIT 1`, [codeOrId]);
        if (result.rows.length === 0) {
            throw new Error(`tenant with id '${codeOrId}' not found`);
        }
        return result.rows[0];
    }
    const result = await pool.query(`SELECT id, code, name FROM tenants WHERE code = $1 LIMIT 1`, [codeOrId]);
    if (result.rows.length === 0) {
