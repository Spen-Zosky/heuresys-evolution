...
type ProviderCode = 'anthropic' | 'openai' | 'gemini';

function providerOrder(): ProviderCode[] {
  const anthropicOk =
    typeof env.anthropicApiKey === 'string' && env.anthropicApiKey.startsWith('sk-ant-');
  ...
  const available: ProviderCode[] = [];
  if (env.defaultLlmProvider === 'openai') {
    if (openaiOk) available.push('openai');
    if (anthropicOk) available.push('anthropic');
    if (geminiOk) available.push('gemini');
...
function buildPrompt(entityName: string, schema: Record<string, unknown>, markdown: string): string {
...
function cleanAndParseJson(raw: string): Record<string, unknown> {
  ...
  try {
    ...
    throw new Error(`LLM output not valid JSON: ${(err as Error).message}\nraw: ${cleanJson.slice(0, 500)}`);
  }
}

function computeConfidence(parsed: Record<string, unknown>, schema: Record<string, unknown>): number {
  const filledFields = Object.values(parsed).filter((v) => v !== null && v !== undefined && v !== '').length;
  const totalFields = Object.keys((schema as { properties?: Record<string, unknown> }).properties ?? {}).length || 1;
 ...
 */
function computeConfidenceByField(
  parsed: Record<string, unknown>,
  schema: Record<string, unknown>,
): Record<string, number> {
  const props = (schema as { properties?: Record<string, { type?: string; format?: string }> })
    .properties ?? {};
  ...
  for (const key of Object.keys(props)) {
    ...
    if (typeof value === 'string') {
      if (/^https?:\/\/.+/i.test(value)) score = 0.9;
      else if (/^[A-Z]{2}$/.test(value)) score = 0.9; // ISO country
      else if (/^[A-Z]{2}[0-9A-Z]{2,12}$/.test(value)) score = 0.9; // VAT-like
    }
    out[key] = score;
  }
  return out;
...
function toStrictJsonSchema(
  ...
  function sanitise(value: Record<string, unknown>): Record<string, unknown> {
    ...
    for (const [k, v] of Object.entries(value)) {
      if (OPENAI_STRIPPED_KEYS.has(k)) continue;
      out[k] = v;
    }
    return out;
  }
  ...
  for (const [k, v] of Object.entries(props)) {
    const propSchema = sanitise((v ?? {}) as Record<string, unknown>);
    ...
    if (typeof existingType === 'string') {
      ...
      strictProps[k] = {
        ...propSchema,
        type: Array.from(new Set([...(existingType as string[]), 'null'])),
      };
    } else {
...
async function extractWithOpenAI(input: ExtractionInput): Promise<ExtractionResult> {
  ...
  const strictSchema = toStrictJsonSchema(
    input.entityName.replace(/[^a-zA-Z0-9_]/g, '_'),
    input.schema,
  );
  const response = await client.chat.completions.create({
    model,
    max_tokens: input.maxOutputTokens ?? 1024,
    response_format: { type: 'json_schema', json_schema: strictSchema } as never,
    messages: [
      {
        role: 'system',
        content:
          'You are a precise extraction engine. Extract facts from the provided web content into the given schema. Return null for any field that is not explicitly stated in the content — never guess or invent values.',
      },
      { role: 'user', content: buildPrompt(input.entityName, input.schema, input.markdown) },
    ],
  });
...
function toGeminiResponseSchema(schema: Record<string, unknown>): Record<string, unknown> {
  ...
  function convertProp(prop: Record<string, unknown>): Record<string, unknown> {
    const out: Record<string, unknown> = {};
  ...
  for (const [k, v] of Object.entries(props)) {
    geminiProps[k] = convertProp((v ?? {}) as Record<string, unknown>);
  }
  return {
    type: 'OBJECT',
...
async function extractWithGemini(input: ExtractionInput): Promise<ExtractionResult> {
  ...
  const model = client.getGenerativeModel({
    ...
    generationConfig: {
      ...
      responseSchema: toGeminiResponseSchema(input.schema) as never,
...
async function callProvider(
  ...
  if (provider === 'anthropic') return extractWithAnthropic(input);
  if (provider === 'gemini') return extractWithGemini(input);
  return extractWithOpenAI(input);
}

function zodIssuesToPromptHint(err: ZodError): string {
...
async function extractAndValidate(
  ...
  for (let attempt = 0; attempt <= MAX_VALIDATION_RETRIES; attempt++) {
    try {
      ...
      logger.info(
        { provider, attempt: attempt + 1, issueCount: err.issues.length },
        'extraction zod validation failed, retrying with corrective prompt',
      );
...
export async function extract(input: ExtractionInput): Promise<ExtractionResult> {
  ...
  for (const provider of order) {
    try {
      ...
      logger.warn(
        { provider, err: { message: lastError.message.slice(0, 200) } },
        'llm provider failed, trying next in order',
      );
    }
  }
