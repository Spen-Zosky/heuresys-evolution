...
let queue = null;
let worker = null;
export async function startBudgetResetScheduler() {
    if (worker)
        return;
    ...
    await queue.upsertJobScheduler('budget-reset-scheduler', { pattern: env.budgetResetCron }, { name: 'budget-reset' });
    worker = new Worker(QUEUE_NAME, async (_job) => {
        logger.info('[budget-reset] starting scheduled budget reset check');
        try {
            const result = await pool.query(`UPDATE enrichment_merge_policies
          ...
          RETURNING id, code, current_usage_eur::text AS previous_usage`);
            ...
            for (const p of policies) {
                logger.info({ policyId: p.id, code: p.code, previousUsageEur: p.previousUsageEur }, `[budget-reset] reset policy ${p.code} (was €${p.previousUsageEur.toFixed(4)})`);
            }
            ...
            const error = err;
            logger.error({ err: error.message, stack: error.stack }, `[budget-reset] failed: ${error.message}`);
            throw err;
        }
    ...
    worker.on('failed', (job, err) => {
        ...
        logger.error({ jobId: id, attempts: job?.attemptsMade, error: err.message }, `[budget-reset] job=${id} failed (${job?.attemptsMade}/${job?.opts?.attempts ?? 1}): ${err.message}`);
    });
    logger.info(`[budget-reset] scheduler started with cron="${env.budgetResetCron}"`);
}
export async function stopBudgetResetScheduler() {
    if (worker) {
        await worker.close();
        worker = null;
    }
