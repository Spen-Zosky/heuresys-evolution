...
export interface EnrichmentMetrics {
  ...
  acquisition: {
    sourcesTotal: number;
    freshnessHits: number; // discovered_via='l2_cache'
  };
  budget: {
    capEur: number;
    usedEur: number;
...
export async function getEnrichmentMetrics(
  ...
  // ---- jobs ----
  const jobsRes = await db.query<{ status: string; c: string; last_completed: string | null }>(
    `SELECT status,
            COUNT(*)::text AS c,
  ...
  // ---- candidates ----
  const candRes = await db.query<{ c: string; avg_conf: string | null }>(
  ...
  // ---- writes (L4 ledger) ----
  const writesRes = await db.query<{ total: string; active: string; rolled: string }>(
  ...
  const identityBlockRes = await db.query<{ c: string }>(
    `SELECT COUNT(*)::text AS c
  ...
  const srcRes = await db.query<{ total: string; l2: string }>(
    `SELECT COUNT(*)::text AS total,
  ...
  // ---- budget ----
  const policyRes = await db.query<{ cap: string; used: string }>(
    `SELECT
