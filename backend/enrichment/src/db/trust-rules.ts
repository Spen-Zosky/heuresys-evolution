 ...
 */
export async function getTrustScoreForSourceType(sourceType: string): Promise<number> {
  const result = await pool.query<{ trust_score: string }>(
  ...
  const fallback = await pool.query<{ trust_score: string }>(
    `SELECT trust_score::text
       FROM enrichment_trust_rules
