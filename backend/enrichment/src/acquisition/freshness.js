 ...
 */
export async function findFreshSnapshot(client, tenantId, canonicalUrl, freshnessDays) {
    const result = await client.query(`SELECT s.id AS source_id,
      ...
      LIMIT 1`, [tenantId, canonicalUrl, freshnessDays]);
 ...
 */
export async function findFreshSnapshotForTenant(tenantId, canonicalUrl, freshnessDays) {
    return withTenantClient(tenantId, (client) => findFreshSnapshot(client, tenantId, canonicalUrl, freshnessDays));
}
export function logFreshnessHit(canonicalUrl, fresh) {
    logger.info({
        url: canonicalUrl,
