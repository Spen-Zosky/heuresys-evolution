...
import { findFreshSnapshot, logFreshnessHit, type FreshSnapshot } from './freshness.js';
...
export function resolveBackend(): Backend {
  ...
  if (configured === 'firecrawl') return 'firecrawl';
  if (configured === 'plain_fetch') return 'plain_fetch';
...
interface AcquisitionPayload {
  ...
  language: string;
  title: string | null;
  /** Original page URL — populated by crawl path for per-page provenance. */
  pageUrl?: string;
}

async function acquireViaFirecrawl(url: string): Promise<AcquisitionPayload> {
...
async function acquireViaPlainFetch(url: string): Promise<AcquisitionPayload> {
  const response = await fetch(url, {
    headers: {
      'user-agent':
        'Mozilla/5.0 (compatible; HeuresysEnrichmentBot/0.1; +https://heuresys.com)',
...
async function acquireOne(url: string, preferred: Backend): Promise<AcquisitionPayload> {
  try {
    ...
    if (preferred === 'firecrawl' && err instanceof FirecrawlError && err.code !== 'missing_key') {
      logger.warn(
        { err: { code: err.code, message: err.message, status: err.status }, url },
        'firecrawl acquisition failed, falling back to plain_fetch',
      );
...
function buildDiscoveryUrls(seedUrl: string): string[] {
...
async function persistSnapshot(
...
): Promise<{ sourceId: string; snapshotId: string; contentHash: string }> {
  ...
  const sourceId = sourceRes.rows[0].id as string;

  const snapshotRes = await client.query(
    `INSERT INTO enrichment_source_snapshots
  ...
  return { sourceId, snapshotId: snapshotRes.rows[0].id as string, contentHash };
}
 ...
 */
async function reuseFreshSnapshot(
  tenantId: string,
  jobId: string,
  rawUrl: string,
  canonical: string,
  ...
  return withTenantClient(tenantId, async (client) => {
    ...
    return {
      sourceId: sourceRes.rows[0].id as string,
 ...
 */
async function acquireViaCrawl(
  url: string,
  crawlConfig: Record<string, unknown>,
): Promise<AcquisitionPayload[]> {
  const opts: CrawlOptions = {};
  if (typeof crawlConfig.max_depth === 'number') opts.maxDepth = crawlConfig.max_depth;
  if (typeof crawlConfig.max_pages === 'number') opts.limit = crawlConfig.max_pages;
  if (Array.isArray(crawlConfig.include_paths)) opts.includePaths = crawlConfig.include_paths as string[];
  if (Array.isArray(crawlConfig.exclude_paths)) opts.excludePaths = crawlConfig.exclude_paths as string[];
...
export async function fetchAndStore(
  ...
  if (requestedSourceType === 'official_website') {
    ...
    if (classification.verified) {
      logger.info(
        { url: canonical, matched: classification.matchedDomain },
  ...
  const cachedFresh = await withTenantClient(tenantId, (c) =>
    findFreshSnapshot(c, tenantId, canonical, freshnessDays),
  );
  if (cachedFresh) {
    ...
    try {
      ...
      if (crawlPages.length === 0) {
        ...
        return withTenantClient(tenantId, async (client) => {
          ...
          const primarySaved = await persistSnapshot(
            client, tenantId, jobId, url, canonical,
            sourceType, trustScore, primaryPayload,
          );
          ...
          for (let i = 1; i < crawlPages.length; i++) {
            ...
            await persistSnapshot(
              client, tenantId, jobId, pageRawUrl,
              pageRawUrl, `${sourceType}_crawl`, Math.max(trustScore - 0.1, 0.1),
              page,
            );
          }
      ...
      logger.warn(
        { err: errInfo, url: canonical, fallback: 'scrape+discovery' },
        'crawl failed, falling back to single-page scrape + discovery',
      );
    }
  }
  ...
  const discoveryResults = await Promise.allSettled(
    discoveryUrls.map((u) => acquireOne(u, preferred)),
  );

  const successfulDiscovery: Array<{ url: string; payload: AcquisitionPayload }> = [];
  discoveryResults.forEach((r, idx) => {
    if (r.status === 'fulfilled' && r.value.httpStatus < 400 && r.value.bytesSize > 200) {
      ...
      logger.debug(
        { url: discoveryUrls[idx], err: (r.reason as Error).message.slice(0, 120) },
        'discovery page skipped',
      );
    }
  });

  logger.info(
    { seed: canonical, discovered: successfulDiscovery.length, backend: preferred },
    'multi-page acquisition complete',
  );
  ...
  return withTenantClient(tenantId, async (client) => {
    ...
    for (const disc of successfulDiscovery) {
      await persistSnapshot(
        ...
        disc.url,
        disc.url,
        `${sourceType}_discovery`,
        Math.max(trustScore - 0.1, 0.1),
        disc.payload,
      );
    }
