...
import { assertBudgetAvailable, BudgetExceededError, estimateCostEur, incrementBudgetUsage, } from '../db/budget.js';
...
export async function createJobRow(input) {
    ...
    const job = await withTenantClient(input.tenantId, (c) => createJob({
 ...
 */
export async function runPipeline(jobId, input, descriptor) {
    ...
    await withTenantClient(tenantId, (c) => recordJobEvent(jobId, tenantId, 'acquisition.done', { snapshotId: fetched.snapshotId, httpStatus: fetched.httpStatus, bytes: fetched.bytesSize }, c));
    ...
    if (descriptor.default_merge_policy_id) {
        try {
            ...
            if (err instanceof BudgetExceededError) {
                await withTenantClient(tenantId, async (c) => {
                    await updateJobStatus(jobId, 'failed', { code: 'BUDGET_EXCEEDED', message: err.message }, c);
                    await recordJobEvent(jobId, tenantId, 'budget.exceeded', { policyId: err.policyId, capEur: err.capEur, usedEur: err.usedEur }, c);
                });
                throw err;
            }
            throw err;
    ...
    if (descriptor.default_merge_policy_id) {
        const cost = estimateCostEur(extracted.providerCode, extracted.inputTokens, extracted.outputTokens);
        if (cost > 0) {
    ...
    const candidates = [];
    await withTenantClient(tenantId, async (c) => {
        await recordJobEvent(jobId, tenantId, 'extraction.done', {
        ...
        for (const [fieldName, fieldValue] of Object.entries(extracted.raw)) {
            ...
            const fieldConfidence = extracted.confidenceByField?.[fieldName] ?? extracted.confidenceOverall;
 ...
 */
export async function markJobFailed(tenantId, jobId, error) {
    await withTenantClient(tenantId, async (c) => {
