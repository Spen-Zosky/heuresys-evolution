...
import { decideMerge } from './strategies.js';
/**
...
async function loadJob(client, jobId, tenantId) {
    const r = await client.query(`SELECT j.id, j.tenant_id, j.descriptor_id, j.target_table, j.target_pk_field,
...
async function loadCandidatesForJob(client, jobId, tenantId) {
    ...
    const r = await client.query(`SELECT c.id, c.tenant_id, c.entity_type, c.entity_anchor, c.field_name,
      ...
      ORDER BY c.created_at`, [jobId, tenantId]);
    return r.rows;
}
async function loadTargetRow(client, targetTable, pkField, targetRecordId, columns) {
    if (columns.length === 0)
        return null;
    const quoted = columns.map((c) => `"${c}"`).join(', ');
    const bareTable = stripSchema(targetTable);
    const r = await client.query(`SELECT ${quoted} FROM "${bareTable}" WHERE "${pkField}" = $1 LIMIT 1`, [targetRecordId]);
    return r.rows[0] ?? null;
}
...
export async function commitJob(input) {
    ...
    return withTenantClient(tenantId, async (client) => {
        ...
        const fieldMap = ENTITY_FIELD_MAP[job.target_table.replace(/^public\./, '')] ??
            ...
            {};
        const mappedColumns = Array.from(new Set(candidates
            .map((c) => fieldMap[c.field_name])
            .filter((c) => !!c)));
        const currentRow = mappedColumns.length > 0
            ? await loadTargetRow(client, job.target_table, job.target_pk_field, job.target_record_id, mappedColumns)
            : null;
        ...
        const tenantIdentityRow = await client.query(`SELECT tax_id, verified_website, name
         FROM tenants WHERE id = $1 LIMIT 1`, [job.target_record_id]);
        ...
        for (const cand of candidates) {
            ...
            const alreadyCommittedFlag = await isWriteAlreadyCommitted(tenantId, job.target_table, job.target_record_id, cand.field_name, factHash, client);
            ...
            // to apply.
            const realTrustScore = cand.source_trust_score !== null ? Number(cand.source_trust_score) : 0.1;
            ...
            try {
              ...
              SET "${mappedColumn}" = $1,
                  updated_at = NOW()
            WHERE "${job.target_pk_field}" = $2`, [decision.newValue, job.target_record_id]);
                await client.query('RELEASE SAVEPOINT sp_merge');
            }
            catch (err) {
                await client.query('ROLLBACK TO SAVEPOINT sp_merge');
                logger.warn({ err: err.message, column: mappedColumn, value: decision.newValue }, 'merge apply UPDATE failed, continuing with next candidate');
                summaries.push({
                    candidateId: cand.id,
                    fieldName: cand.field_name,
                    mappedColumn,
        ...
        if (applied > 0) {
            await client.query(`UPDATE enrichment_jobs SET status = 'committed', completed_at = NOW() WHERE id = $1`, [jobId]);
        }
        return {
            jobId,
            policyId: policy.id,
