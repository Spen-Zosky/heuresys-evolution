 ...
 */
export function checkEntityMismatch(input) {
    ...
    if (overlap < MIN_OVERLAP) {
        return {
            mismatch: true,
            overlap,
            reason: `entity_mismatch: token overlap ${overlap.toFixed(2)} below ${MIN_OVERLAP} (candidate="${String(candidateValue).slice(0, 40)}", current="${String(currentValue).slice(0, 40)}")`,
        };
    }
    return { mismatch: false, overlap, reason: `token overlap ${overlap.toFixed(2)} OK` };
}
export const __test = { tokenise, MIN_OVERLAP };
//# sourceMappingURL=entity-mismatch.js.map