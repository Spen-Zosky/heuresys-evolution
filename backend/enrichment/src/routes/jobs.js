...
function resolveTenantId(req) {
    ...
    try {
        ...
        throw createError(400, 'INVALID_TENANT', err.message);
    }
    return candidate;
}
...
function parseCreateBody(body) {
    ...
    return { entity_name: b.entity_name, target_record_id: b.target_record_id, url: b.url, mode: mode };
}
export async function createJobHandler(req, res) {
    try {
        ...
        await withTenantClient(tenantId, (c) => recordJobEvent(jobId, tenantId, 'job.queued', { queue: 'enrichment-jobs' }, c));
        res.status(202).json({
            success: true,
...
jobsRouter.get('/:id', async (req, res) => {
    try {
        ...
        const result = await withTenantClient(tenantId, (c) => c.query(`SELECT id, tenant_id, descriptor_id, target_table, target_record_id, mode, status,
...
jobsRouter.get('/', async (req, res) => {
    try {
        ...
        const clauses = ['j.tenant_id = $1::uuid'];
        ...
        const listResult = await withTenantClient(tenantId, (c) => c.query(`SELECT j.id, j.status, d.entity_name AS descriptor_name, j.target_record_id, j.mode,
        ...
        const countResult = await withTenantClient(tenantId, (c) => c.query(`SELECT COUNT(*)::int AS total
           FROM enrichment_jobs j
           ${where}`, countParams));
        res.json({
            success: true,
...
jobsRouter.delete('/:id', async (req, res) => {
    try {
        ...
        const result = await withTenantClient(tenantId, async (c) => {
            const existing = await c.query(`SELECT id, status FROM enrichment_jobs
          WHERE id = $1::uuid AND tenant_id = $2::uuid LIMIT 1`, [id, tenantId]);
...
jobsRouter.post('/:id/commit', async (req, res) => {
    try {
        ...
        const approved = Array.isArray(body.approved_candidate_ids)
            ? body.approved_candidate_ids.filter((x) => typeof x === 'string')
            : undefined;
        const forceMode = body.force_mode === 'merge' || body.force_mode === 'suggest' || body.force_mode === 'observe'
            ? body.force_mode
            : undefined;
