...
import { createSurveySchema, updateSurveySchema, submitSurveyResponseSchema, } from '../schemas/surveys.js';
...
router.get('/', asyncHandler(async (req, res) => {
    ...
    const [countResult, dataResult] = await Promise.all([
        ...
        req.dbClient.query(`SELECT s.id, s.title, s.description, s.survey_type, s.start_date, s.end_date,
...
router.get('/', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { status, survey_type, is_anonymous, limit = '100', offset = '0', } = req.query;
    let query = 'SELECT id, tenant_id, title, description, survey_type, start_date, end_date, is_anonymous, is_active, status, questions, total_invitations, created_at, updated_at FROM surveys WHERE tenant_id = $1';
    const params = [tenantId];
    let paramIndex = 2;
    if (status) {
    ...
    const countResult = await req.dbClient.query('SELECT COUNT(*) FROM surveys WHERE tenant_id = $1', [tenantId]);
    res.json({
        success: true,
        data: result.rows,
        meta: {
...
router.get('/:id', asyncHandler(async (req, res) => {
    ...
    const result = await req.dbClient.query('SELECT id, tenant_id, title, description, survey_type, start_date, end_date, is_anonymous, is_active, status, questions, total_invitations, created_at, updated_at FROM surveys WHERE id = $1 AND tenant_id = $2', [id, tenantId]);
...
router.get('/:id/responses', asyncHandler(async (req, res) => {
    ...
    const survey = await req.dbClient.query('SELECT is_anonymous FROM surveys WHERE id = $1 AND tenant_id = $2', [id, tenantId]);
 ...
 * POST /surveys
 */
router.post('/', validate(createSurveySchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { title, description, survey_type = 'engagement', start_date, end_date, is_anonymous = true, questions, } = req.body;
    if (!title) {
 ...
 * PATCH /surveys/:id
 */
router.patch('/:id', validate(updateSurveySchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'];
    const existing = await req.dbClient.query('SELECT id FROM surveys WHERE id = $1 AND tenant_id = $2', [id, tenantId]);
    ...
    const result = await req.dbClient.query(`UPDATE surveys SET ${updates.join(', ')} WHERE id = $${paramIndex} AND tenant_id = $${paramIndex + 1} RETURNING *`, [...values, id, tenantId]);
 ...
 */
router.post('/:id/respond', validate(submitSurveyResponseSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'];
...
router.delete('/:id', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'];
    const result = await req.dbClient.query('DELETE FROM surveys WHERE id = $1 AND tenant_id = $2 RETURNING id', [id, tenantId]);
    if (result.rows.length === 0) {
        throw Errors.notFound('Survey');
    }
