...
import { createSocialPostSchema, updateSocialPostSchema, socialLikeSchema, createSocialCommentSchema, createClubSchema, joinClubSchema, } from '../schemas/social.js';
...
router.get('/feed', asyncHandler(async (req, res) => {
    ...
    const { post_type, author_id, visibility, limit = '50', offset = '0', } = req.query;
    let query = `
      SELECT
        sp.*,
    ...
    const countResult = await req.dbClient.query('SELECT COUNT(*) FROM social_posts WHERE tenant_id = $1', [tenantId]);
    res.json({
        success: true,
        data: result.rows,
        meta: {
 ...
 * Create new post
 */
router.post('/posts', validate(createSocialPostSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { author_id, content, post_type = 'update', attachments, poll_options, visibility = 'all', target_org_units, } = req.body;
 ...
 * Update post
 */
router.patch('/posts/:id', validate(updateSocialPostSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'];
    const existing = await req.dbClient.query('SELECT id FROM social_posts WHERE id = $1 AND tenant_id = $2', [id, tenantId]);
    ...
    const result = await req.dbClient.query(`UPDATE social_posts SET ${updates.join(', ')}
...
router.delete('/posts/:id', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'];
    const result = await req.dbClient.query('DELETE FROM social_posts WHERE id = $1 AND tenant_id = $2 RETURNING id', [id, tenantId]);
    if (result.rows.length === 0) {
        throw Errors.notFound('Post');
    }
 ...
 * Like a post
 */
router.post('/posts/:id/like', validate(socialLikeSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'];
    ...
    // Verify post exists
    const postCheck = await req.dbClient.query('SELECT id FROM social_posts WHERE id = $1 AND tenant_id = $2', [id, tenantId]);
    if (postCheck.rows.length === 0) {
        throw Errors.notFound('Post');
    }
    // Check if already liked
    const likeCheck = await req.dbClient.query('SELECT id FROM social_likes WHERE post_id = $1 AND employee_id = $2', [id, employee_id]);
...
router.delete('/posts/:id/like', asyncHandler(async (req, res) => {
    ...
    const deleteResult = await withTransaction(async (client) => {
        const delResult = await client.query('DELETE FROM social_likes WHERE post_id = $1 AND employee_id = $2 RETURNING id', [id, employee_id]);
        if (delResult.rows.length === 0) {
            return null;
        }
        await client.query('UPDATE social_posts SET likes_count = GREATEST(likes_count - 1, 0) WHERE id = $1', [id]);
 ...
 * Add comment to post
 */
router.post('/posts/:id/comments', validate(createSocialCommentSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'];
    ...
    const result = await withTransaction(async (client) => {
      ...
      `, [id, author_id, parent_comment_id || null, content]);
        await client.query('UPDATE social_posts SET comments_count = comments_count + 1 WHERE id = $1', [id]);
 ...
 * Create new club
 */
router.post('/clubs', validate(createClubSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { name, description, category, icon, cover_image_url, is_public = true, requires_approval = false, max_members, owner_id, } = req.body;
    if (!name || !owner_id) {
 ...
 * Join a club
 */
router.post('/clubs/:id/join', validate(joinClubSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'];
    const { employee_id } = req.body;
    if (!employee_id) {
    ...
    // Verify club exists
    const clubCheck = await req.dbClient.query('SELECT id, requires_approval, max_members, members_count FROM employee_clubs WHERE id = $1 AND tenant_id = $2', [id, tenantId]);
    ...
    const memberCheck = await req.dbClient.query('SELECT id FROM club_memberships WHERE club_id = $1 AND employee_id = $2', [id, employee_id]);
    ...
    await withTransaction(async (client) => {
      ...
      `, [id, employee_id, status]);
        if (status === 'active') {
            await client.query('UPDATE employee_clubs SET members_count = members_count + 1 WHERE id = $1', [id]);
        }
    }, tenantId);
    res.json({
        success: true,
 ...
 * Leave a club
 */
router.delete('/clubs/:id/leave', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'];
    const { employee_id } = req.body;
    if (!employee_id) {
    ...
    const leaveResult = await withTransaction(async (client) => {
        const delResult = await client.query('DELETE FROM club_memberships WHERE club_id = $1 AND employee_id = $2 AND role != $3 RETURNING id', [id, employee_id, 'owner']);
        if (delResult.rows.length === 0) {
            return null;
        }
        await client.query('UPDATE employee_clubs SET members_count = GREATEST(members_count - 1, 0) WHERE id = $1', [id]);
        return delResult.rows[0];
    }, tenantId);
    if (!leaveResult) {
        res
