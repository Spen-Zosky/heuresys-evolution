 ...
 */
router.get('/stats', authMiddleware, requirePermission('PLATFORM', 'VIEW'), asyncHandler(async (req, res) => {
  ...
  `, queryParams),
    ...
    sendSuccess(res, {
        ...
        byCategory: byCategoryResult.rows.reduce((acc, row) => {
 ...
 */
router.get('/recent', authMiddleware, requirePermission('PLATFORM', 'VIEW'), asyncHandler(async (req, res) => {
    const tenantId = req.user?.tenantId;
    const isSuperuser = req.user?.role === 'SUPERUSER';
    const page = safeParseInt(req.query.page, { fallback: 1 });
    const pageSize = safeParseInt(req.query.pageSize, { fallback: 20, max: 100 });
 ...
 */
router.get('/:errorId', authMiddleware, requirePermission('PLATFORM', 'VIEW'), asyncHandler(async (req, res) => {
 ...
 */
router.get('/trends/hourly', authMiddleware, requirePermission('PLATFORM', 'VIEW'), asyncHandler(async (req, res) => {
    const tenantId = req.user?.tenantId;
    const isSuperuser = req.user?.role === 'SUPERUSER';
    const hours = safeParseInt(req.query.hours, { fallback: 24, max: 168 }); // Max 7 days
    const result = await pool.query(`
    SELECT
      hour_start,
      category,
 ...
 */
router.get('/spikes/detect', authMiddleware, requirePermission('PLATFORM', 'VIEW'), asyncHandler(async (req, res) => {
    const tenantId = req.user?.tenantId;
    const isSuperuser = req.user?.role === 'SUPERUSER';
    const threshold = parseFloat(req.query.threshold) || 2.0;
    const minErrors = safeParseInt(req.query.minErrors, { fallback: 5 });
    const result = await pool.query(`
 ...
 */
router.get('/patterns/list', authMiddleware, requirePermission('PLATFORM', 'VIEW'), asyncHandler(async (req, res) => {
    const tenantId = req.user?.tenantId;
    const isSuperuser = req.user?.role === 'SUPERUSER';
 ...
 */
router.patch('/patterns/:id/resolve', authMiddleware, requirePermission('PLATFORM', 'EDIT'), validate(resolveErrorPatternSchema), asyncHandler(async (req, res) => {
    const { id } = req.params;
 ...
 */
router.get('/aggregate', authMiddleware, requirePermission('PLATFORM', 'VIEW'), asyncHandler(async (req, res) => {
    const tenantId = req.user?.tenantId;
    const isSuperuser = req.user?.role === 'SUPERUSER';
    const period = req.query.period || '24h';
    const periodMap = {
 ...
 */
router.post('/aggregate', authMiddleware, requirePermission('PLATFORM', 'CREATE'), asyncHandler(async (_req, res) => {
    const result = await pool.query(`
 ...
 */
router.get('/by-status', authMiddleware, requirePermission('PLATFORM', 'VIEW'), asyncHandler(async (req, res) => {
    const tenantId = req.user?.tenantId;
    const isSuperuser = req.user?.role === 'SUPERUSER';
    const period = req.query.period || '24h';
    const periodMap = {
...
//# sourceMappingURL=error-analytics.js.map