...
router.get('/', asyncHandler(async (req, res) => {
    ...
    const { is_active, location_type, city, search, limit = '100', offset = '0', } = req.query;
    ...
    const fetchData = async () => {
      ...
      `;
        ...
        const countResult = await req.dbClient.query('SELECT COUNT(*) FROM locations WHERE tenant_id = $1', [tenantId]);
        return {
            success: true,
            data: result.rows,
            meta: {
    ...
    const responseData = hasFilters
        ? await fetchData()
        : await cachedForTenant(tenantId, `locations:list:${limitNum}:${offsetNum}`, fetchData, CACHE_TTL.REFERENCE);
...
router.get('/types', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const result = await req.dbClient.query('SELECT DISTINCT location_type FROM locations WHERE tenant_id = $1 AND location_type IS NOT NULL ORDER BY location_type LIMIT 100', [tenantId]);
...
router.get('/:id', asyncHandler(async (req, res) => {
    ...
    const result = await req.dbClient.query(`SELECT l.*,
        (SELECT COUNT(*) FROM employees e WHERE e.location_id = l.id AND e.is_active = true) as employee_count,
        (SELECT COUNT(DISTINCT e.org_unit_id) FROM employees e WHERE e.location_id = l.id AND e.is_active = true AND e.org_unit_id IS NOT NULL) as org_unit_count
       FROM locations l WHERE l.id = $1 AND l.tenant_id = $2`, [id, tenantId]);
 ...
 * POST /locations
 */
router.post('/', requirePermission('ORGANIZATION', 'CREATE'), validate(createLocationSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { code, name, location_type, address, city, province, postal_code, country, latitude, longitude, phone, email, capacity_headcount, square_meters, opening_date, } = req.body;
    if (!code || !name) {
        res
            .status(400)
    ...
    const existing = await req.dbClient.query('SELECT id FROM locations WHERE code = $1 AND tenant_id = $2', [code, tenantId]);
    ...
    const result = await req.dbClient.query(`INSERT INTO locations (tenant_id, code, name, location_type, address, city, province, postal_code, country,
 ...
 * PATCH /locations/:id
 */
router.patch('/:id', requirePermission('ORGANIZATION', 'EDIT'), validate(updateLocationSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'];
    const existing = await req.dbClient.query('SELECT id FROM locations WHERE id = $1 AND tenant_id = $2', [id, tenantId]);
    if (existing.rows.length === 0) {
        res
            .status(404)
            .json({ success: false, error: 'Location not found', code: 'LOCATION_NOT_FOUND' });
    ...
    const result = await req.dbClient.query(`UPDATE locations SET ${updates.join(', ')} WHERE id = $${paramIndex} AND tenant_id = $${paramIndex + 1} RETURNING *`, [...values, id, tenantId]);
    await invalidateCachePattern(`t:${tenantId}:locations:*`);
...
router.delete('/:id', requirePermission('ORGANIZATION', 'DELETE'), asyncHandler(async (req, res) => {
    ...
    const existing = await req.dbClient.query('SELECT id, name FROM locations WHERE id = $1 AND tenant_id = $2', [id, tenantId]);
    if (existing.rows.length === 0) {
        res
            .status(404)
            .json({ success: false, error: 'Location not found', code: 'LOCATION_NOT_FOUND' });
    ...
    const employeeCount = await req.dbClient.query('SELECT COUNT(*) FROM employees WHERE location_id = $1 AND tenant_id = $2', [id, tenantId]);
    ...
    if (hard) {
        ...
        await req.dbClient.query('UPDATE locations SET is_active = false, closing_date = CURRENT_DATE, updated_at = NOW() WHERE id = $1 AND tenant_id = $2', [id, tenantId]);
        await invalidateCachePattern(`t:${tenantId}:locations:*`);
...
router.get('/:id/employees', asyncHandler(async (req, res) => {
    ...
    const result = await req.dbClient.query(`SELECT id, first_name, last_name, email, job_title, department, hire_date, is_active
       FROM employees WHERE location_id = $1 AND tenant_id = $2 ORDER BY last_name, first_name`, [id, tenantId]);
