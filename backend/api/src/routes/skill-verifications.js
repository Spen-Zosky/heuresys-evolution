...
import { approveSkillSchema, rejectSkillSchema, overrideSkillSchema, bulkApproveSchema, } from '../schemas/skills-assessment.js';
...
// =============================================================================
router.get('/team/:managerId', asyncHandler(async (req, res) => {
    const dbClient = req.dbClient;
    const { managerId } = req.params;
    const { includeExpired = 'false', limit = '50', offset = '0', } = req.query;
    // Verify manager exists
    const tenantId = req.headers['x-tenant-id'];
    const managerResult = await dbClient.query(tenantId
        ? 'SELECT id, tenant_id, first_name, last_name FROM employees WHERE id = $1 AND tenant_id = $2'
        : 'SELECT id, tenant_id, first_name, last_name FROM employees WHERE id = $1', tenantId ? [managerId, tenantId] : [managerId]);
    ...
    WHERE e.manager_id = $1
      AND esp.tenant_id = $2
  `;
    const params = [managerId, manager.tenant_id];
    let paramIndex = 3;
  ...
  `, [managerId, manager.tenant_id]);
    ...
    res.json({
        ...
        data: {
            ...
            meta: buildMeta(parseInt(counts.pending_count) + parseInt(counts.expired_count), safeParseInt(limit, { fallback: 50 }), safeParseInt(offset, { fallback: 0 })),
        },
    });
}));
// =============================================================================
...
router.get('/pending', asyncHandler(async (req, res) => {
    const dbClient = req.dbClient;
    const tenantId = req.headers['x-tenant-id'];
    const { orgUnitId, employeeId, limit = '50', offset = '0', } = req.query;
    if (!tenantId) {
...
// =============================================================================
router.post('/:profileId/approve', validate(approveSkillSchema), asyncHandler(async (req, res) => {
    const dbClient = req.dbClient;
  ...
  `, [profileId]);
    ...
    const expiryDate = expiresAt || new Date(Date.now() + 365 * 24 * 60 * 60 * 1000).toISOString().split('T')[0];
...
// =============================================================================
router.post('/:profileId/reject', validate(rejectSkillSchema), asyncHandler(async (req, res) => {
    const dbClient = req.dbClient;
    const { profileId } = req.params;
...
// =============================================================================
router.post('/:profileId/override', validate(overrideSkillSchema), asyncHandler(async (req, res) => {
    const dbClient = req.dbClient;
    const { profileId } = req.params;
    const { verifierId, knowledge, skill, ability, behavior, attitude, reason, autoApprove = true, } = req.body;
    if (!verifierId) {
        throw Errors.badRequest('verifierId is required');
    }
    if (!reason) {
...
// =============================================================================
router.get('/audit/:profileId', asyncHandler(async (req, res) => {
    const dbClient = req.dbClient;
    const { profileId } = req.params;
  ...
  `, [
    ...
    // Get total count
    const countResult = await dbClient.query('SELECT COUNT(*) as total FROM employee_skill_history WHERE profile_id = $1', [profileId]);
    res.json({
        success: true,
        data: {
            profile: {
            ...
            auditTrail: historyResult.rows.map((row) => ({
                ...
                previous: row.previous_composite_score !== null
                    ? {
                        ksaba: {
...
// =============================================================================
router.post('/bulk-approve', validate(bulkApproveSchema), asyncHandler(async (req, res) => {
    const dbClient = req.dbClient;
...
//# sourceMappingURL=skill-verifications.js.map