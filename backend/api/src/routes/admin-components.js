...
router.get('/', asyncHandler(async (req, res) => {
    ...
    const { functional_area, scope, context } = req.query;
    const params = [tenantId];
    const conditions = ['(acr.tenant_id = $1 OR acr.tenant_id IS NULL)'];
    if (functional_area) {
 ...
 */
router.get('/by-area/:functional_area_code', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
