...
import { createArticleSchema, updateArticleSchema, createReactionSchema, createCommentSchema, } from '../schemas/engagement.js';
...
router.get('/', asyncHandler(async (req, res) => {
    ...
    const [countResult, dataResult] = await Promise.all([
        req.dbClient.query(`SELECT COUNT(*) as total FROM news_articles WHERE tenant_id = $1 AND status = $2`, [tenantId, status]),
        req.dbClient.query(`SELECT na.id, na.title, na.slug, na.excerpt, na.status, na.is_featured,
...
router.get('/articles', asyncHandler(async (req, res) => {
    ...
    const pageNum = safeParseInt(page, { fallback: 1 });
 ...
 */
router.post('/articles', validate(createArticleSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const dbClient = req.dbClient;
    const userId = getUserId(req);
    const { title, content, excerpt, cover_image_url, category_id, tags, is_featured, is_pinned, allow_comments, requires_acknowledgment, audience_type, audience_ids, publish_at, status, } = req.body;
    if (!title || !content) {
 ...
 * Update article
 */
router.put('/articles/:id', validate(updateArticleSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const dbClient = req.dbClient;
    const { id } = req.params;
    const { title, content, excerpt, cover_image_url, category_id, is_featured, is_pinned, allow_comments, requires_acknowledgment, audience_type, audience_ids, publish_at, status, } = req.body;
    // Check if publishing
    let publishedAt = null;
    if (status === 'published') {
        const existing = await dbClient.query('SELECT published_at FROM news_articles WHERE id = $1 AND tenant_id = $2', [id, tenantId]);
 ...
 */
router.post('/articles/:id/reactions', validate(createReactionSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const dbClient = req.dbClient;
    const userId = getUserId(req);
    const { id } = req.params;
 ...
 * Add comment
 */
router.post('/articles/:id/comments', validate(createCommentSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const dbClient = req.dbClient;
    const userId = getUserId(req);
    const { id } = req.params;
 ...
 */
router.post('/articles/:id/acknowledge', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const dbClient = req.dbClient;
    const userId = getUserId(req);
    const { id } = req.params;
