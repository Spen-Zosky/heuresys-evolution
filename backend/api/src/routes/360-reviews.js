...
router.get('/', asyncHandler(async (req, res) => {
    ...
    const { limit = '50', offset = '0', target_employee_id, reviewer_employee_id, status, review_cycle_id, } = req.query;
    let query = `
      SELECT
...
router.get('/summary', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const result = await req.dbClient.query(`SELECT * FROM v_360_feedback_summary WHERE tenant_id = $1`, [tenantId]);
    res.json({
        success: true,
        data: result.rows,
    });
 ...
 */
router.get('/response-rates', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const result = await req.dbClient.query(`SELECT * FROM v_360_response_rates WHERE tenant_id = $1`, [tenantId]);
    res.json({
        success: true,
        data: result.rows,
    });
 ...
 */
router.post('/', validate(Create360ReviewSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { target_employee_id, reviewer_employee_id, review_cycle_id, relationship_type, overall_rating, strengths, areas_for_improvement, is_anonymous, status, questionnaire_id, performance_review_id, request_id, question_responses, sentiment_score, submission_time_seconds, } = req.body;
    const query = `
 ...
 * Update 360 review
 */
router.put('/:id', validate(Update360ReviewSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { id } = req.params;
    const { relationship_type, overall_rating, strengths, areas_for_improvement, is_anonymous, status, completed_at, question_responses, sentiment_score, submission_time_seconds, } = req.body;
    const query = `
...
//# sourceMappingURL=360-reviews.js.map