...
function sendFile(res: Response, buffer: Buffer, filename: string, format: string): void {
  const contentType =
    format === 'pdf'
      ? 'application/pdf'
...
router.get(
  '/skill-inventory',
  requirePermission('ORGANIZATION', 'VIEW'),
  validate(skillInventoryQuerySchema, 'query'),
  asyncHandler(async (req: Request, res: Response) => {
    const {
      format = 'xlsx',
      orgUnitId,
    ...
    const parsedFormat = (format as 'xlsx' | 'pdf') || 'xlsx';
    const filters = {
      orgUnitId,
      verificationStatus,
