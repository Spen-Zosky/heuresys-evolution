    ...
    const data = await cachedForTenant(
      ...
      async () => {
        const result = await dbClient.query(
          `SELECT skill_label, skill_type, reuse_level,
        ...
        const summary = {
          ...
          critical: result.rows.filter(
            (r: Record<string, unknown>) => r.risk_level === 'CRITICAL_GAP'
          ).length,
          scarce: result.rows.filter((r: Record<string, unknown>) => r.risk_level === 'SCARCE')
            .length,
          healthy: result.rows.filter((r: Record<string, unknown>) => r.risk_level === 'HEALTHY')
            .length,
          widespread: result.rows.filter(
            (r: Record<string, unknown>) => r.risk_level === 'WIDESPREAD'
          ).length,
        };

        return {
          summary,
          skills: result.rows.map((r: Record<string, unknown>) => ({
            skillLabel: r.skill_label,
            skillType: r.skill_type,
            reuseLevel: r.reuse_level,
            employeesWithSkill: parseInt(r.employees_with_skill as string, 10),
            avgProficiency: r.avg_proficiency ? parseFloat(r.avg_proficiency as string) : null,
            penetrationRate: parseFloat(r.penetration_rate as string),
            openDemand: parseInt(r.open_demand as string, 10),
...
router.get(
  '/similar-occupations/:occupationUri',
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const data = await cached(
      `org:simocc:${uriHash(occupationUri)}`,
      async () => {
        // Find occupation ID
        const occResult = await dbClient.query(
          `SELECT id, preferred_label_en FROM esco_occupations WHERE uri = $1`,
          [occupationUri]
        );
        if (occResult.rowCount === 0) throw Errors.notFound('Occupation', occupationUri);
        ...
        return {
          ...
          uri: occupationUri,
          similarOccupations: result.rows.map((r: Record<string, unknown>) => ({
            occupationId: r.occupation_id,
            occupationLabel: r.occupation_label,
            iscoCode: r.isco_code,
            embeddingSimilarity: parseFloat(r.embedding_similarity as string),
            skillOverlap: parseFloat(r.skill_overlap as string),
            combinedScore: parseFloat(r.combined_score as string),
          })),
        };
      },
      CACHE_TTL.STATIC
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const data = await cachedForTenant(
      ...
      async () => {
        ...
        const empCount = await dbClient.query(
          `SELECT count(*) FROM employees WHERE tenant_id = $1 AND deleted_at IS NULL`,
          [tenantId]
        );
        ...
        const result = await dbClient.query(
          `SELECT
           ...
           ORDER BY count(DISTINCT es.employee_id) ASC`,
          [tenantId]
        );

        const risks = result.rows.map((r: Record<string, unknown>) => {
          const empCnt = parseInt(r.employee_count as string, 10);
          const penetration = totalEmployees > 0 ? empCnt / totalEmployees : 0;
          let riskLevel: string;
          if (empCnt <= 2) riskLevel = 'critical';
          else if (empCnt <= 4 || penetration < 0.05) riskLevel = 'high';
          else if (penetration < 0.1) riskLevel = 'moderate';
          ...
          return {
            ...
            riskLevel,
            holders: (r.holders as string[]).slice(0, 10), // limit to 10 names
          };
        });
