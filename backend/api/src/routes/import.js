...
import { ImportEngineService } from '../services/import-engine.js';
 ...
 */
async function acquireImportClient(tenantId) {
 ...
 */
router.post('/upload', requirePermission('ORGANIZATION', 'CREATE'), upload.single('file'), handleMulterError, validate(uploadQuerySchema, 'query'), asyncHandler(async (req, res) => {
    ...
    try {
        logger.info({ importType, fileName: req.file.originalname, size: req.file.size, tenantId }, 'Import upload started');
        const service = new ImportEngineService(dbClient);
        // 1. Parse file
        const parsed = await service.parseFile(req.file.buffer, req.file.mimetype, req.file.originalname);
        ...
        const insertResult = await dbClient.query(`INSERT INTO import_jobs
 ...
 */
router.post('/execute/:importId', requirePermission('ORGANIZATION', 'CREATE'), validate(importIdParamSchema, 'params'), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const userId = req.user?.userId;
    ...
    try {
        ...
        const jobResult = await dbClient.query('SELECT * FROM import_jobs WHERE id = $1 AND tenant_id = $2', [importId, tenantId]);
        if (jobResult.rows.length === 0) {
            throw Errors.notFound('Import job', importId);
        }
        const job = jobResult.rows[0];
        if (job.status !== 'pending_review') {
            throw Errors.badRequest(`Import job is in status '${job.status}' — only 'pending_review' jobs can be executed.`);
        }
        ...
        // 4. Execute import
        logger.info({ importId, importType, rowCount: mapped.entities.length }, 'Import execution started');
        ...
        await dbClient.query(`UPDATE import_jobs SET
          status = $1,
          created_rows = $2,
          updated_rows = $3,
          skipped_rows = $4,
 ...
 */
router.get('/history', requirePermission('ORGANIZATION', 'VIEW'), validate(historyQuerySchema, 'query'), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { status, importType, limit, offset } = req.query;
    ...
    const countResult = await pool.query(`SELECT COUNT(*) FROM import_jobs WHERE ${whereClause}`, params);
    const total = parseInt(countResult.rows[0]?.count, 10);
    const dataResult = await pool.query(`SELECT id, import_type, file_name, file_size_bytes, status,
 ...
 */
router.get('/history/:importId', requirePermission('ORGANIZATION', 'VIEW'), validate(importIdParamSchema, 'params'), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const importId = req.params.importId;
    if (!isValidUUID(importId)) {
        throw Errors.badRequest('Invalid importId format');
    }
    const result = await pool.query(`SELECT id, import_type, file_name, file_size_bytes, mime_type, status,
              total_rows, valid_rows, error_rows,
              created_rows, updated_rows, skipped_rows,
              errors, warnings, preview_data,
              uploaded_by, executed_at, completed_at, created_at, updated_at
       FROM import_jobs
       WHERE id = $1 AND tenant_id = $2`, [importId, tenantId]);
 ...
 */
router.post('/auto-link/:importId', requirePermission('ORGANIZATION', 'CREATE'), validate(importIdParamSchema, 'params'), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const importId = req.params.importId;
    if (!isValidUUID(importId)) {
        throw Errors.badRequest('Invalid importId format');
 ...
 */
router.get('/auto-link/:importId/results', requirePermission('ORGANIZATION', 'VIEW'), validate(importIdParamSchema, 'params'), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const importId = req.params.importId;
    if (!isValidUUID(importId)) {
        throw Errors.badRequest('Invalid importId format');
    }
    const result = await pool.query(`SELECT isl.id, isl.input_text, isl.esco_skill_id,
       ...
       ORDER BY isl.similarity DESC NULLS LAST`, [importId, tenantId]);
