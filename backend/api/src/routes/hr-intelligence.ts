...
import {
  hrIntelligenceAddSkillSchema,
  hrIntelligenceUpdateSkillSchema,
  hrIntelligenceExtractSkillsSchema,
  hrIntelligenceCreateAliasSchema,
  hrIntelligenceGapAnalysisSchema,
  hrIntelligenceSkillMatrixSchema,
  hrIntelligenceBenchmarkReportSchema,
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const statsResult = await dbClient.query(
      `SELECT
        ...
        (SELECT COUNT(DISTINCT employee_id) FROM employee_skills WHERE tenant_id = $1) as employees_with_skills`,
      [tenantId]
    );
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const { q, skillType, limit, offset } = req.query as Record<string, string>;

    const result = await service.searchSkills((q as string) || '', {
      skillType: skillType as string,
      limit: limit ? safeParseInt(limit as string, { fallback: 50 }) : 20,
      offset: offset ? safeParseInt(offset as string, { fallback: 0 }) : 0,
    });
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    const service = new HRIntelligenceService('system');
    const skill = await service.getSkill(req.params.id as string);
 ...
 */
router.get(
  '/skills/:uri/hierarchy',
  asyncHandler(async (req: Request, res: Response) => {
    const service = new HRIntelligenceService('system');
    const hierarchy = await service.getSkillHierarchy(decodeURIComponent(req.params.uri as string));
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const result = await service.searchOccupations((q as string) || '', {
      limit: limit ? safeParseInt(limit as string, { fallback: 50 }) : 20,
      offset: offset ? safeParseInt(offset as string, { fallback: 0 }) : 0,
    });
 ...
 */
router.get(
  '/occupations/:uri/skills',
  asyncHandler(async (req: Request, res: Response) => {
    const service = new HRIntelligenceService('system');
    const result = await service.getOccupationSkills(decodeURIComponent(req.params.uri as string));
...
router.post(
  ...
  authMiddleware,
  requireTenant,
  validate(hrIntelligenceAddSkillSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const service = new HRIntelligenceService(tenantId);
    const { employeeId } = req.params as Record<string, string>;

    const skillId = await service.addEmployeeSkill(employeeId as string, req.body);
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const skills = await service.getEmployeeSkills(employeeId as string, { includeESCODetails });
...
router.patch(
  ...
  authMiddleware,
  requireTenant,
  validate(hrIntelligenceUpdateSkillSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const service = new HRIntelligenceService(tenantId);
    const { skillId } = req.params as Record<string, string>;

    await service.updateEmployeeSkill(skillId as string, req.body);
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    await service.verifyEmployeeSkill(skillId as string, authReq.user?.userId ?? 'system');
...
router.delete(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    await service.deleteEmployeeSkill(skillId as string);
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (skills) searchOptions.skills = (skills as string).split(',');
    if (location) searchOptions.location = location as string;
    if (countryCode) searchOptions.countryCode = countryCode as string;
    if (industry) searchOptions.industry = industry as string;
    if (experienceLevel) searchOptions.experienceLevel = experienceLevel as string;
    if (salaryMin) searchOptions.salaryMin = parseInt(salaryMin as string);
    if (salaryMax) searchOptions.salaryMax = parseInt(salaryMax as string);
    if (employmentType) searchOptions.employmentType = employmentType as string;
    if (locationType) searchOptions.locationType = locationType as string;
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    const service = new HRIntelligenceService('system');
    const { countryCode, industry, period, days } = req.query as Record<string, string>;
 ...
 */
router.get(
  '/job-market/trending-skills',
  authMiddleware,
  requireTenant,
  asyncHandler(async (req: Request, res: Response) => {
    const service = new HRIntelligenceService('system');
    const { countryCode, industry, days, limit } = req.query as Record<string, string>;
...
router.post(
  ...
  authMiddleware,
  requireTenant,
  validate(hrIntelligenceExtractSkillsSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const service = new HRIntelligenceService(tenantId);
...
router.post(
  ...
  validate(hrIntelligenceCreateAliasSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const service = new HRIntelligenceService('system');
...
router.post(
  ...
  authMiddleware,
  requireTenant,
  validate(hrIntelligenceGapAnalysisSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const service = new HRIntelligenceService(tenantId);
    ...
    const analysis = await service.createSkillGapAnalysis(
      employeeId,
      targetPositionId,
      analysisName
    );
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const analysis = await service.getSkillGapAnalysis(req.params.id as string);
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const { entityType, limit = '20', offset = '0' } = req.query as Record<string, string>;
...
router.post(
  ...
  authMiddleware,
  requireTenant,
  validate(hrIntelligenceSkillMatrixSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const service = new HRIntelligenceService(tenantId);
...
router.post(
  ...
  authMiddleware,
  requireTenant,
  validate(hrIntelligenceBenchmarkReportSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const service = new HRIntelligenceService(tenantId);
    ...
    if (!['salary_benchmark', 'skill_demand', 'talent_availability'].includes(reportType)) {
      throw Errors.badRequest(
        'reportType must be "salary_benchmark", "skill_demand", or "talent_availability"'
      );
    }
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const service = new HRIntelligenceService(tenantId);
    const { reportType, limit, offset } = req.query as Record<string, string>;
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const report = await service.getBenchmarkReport(req.params.id as string);
