...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const { category, active_only } = req.query as Record<string, string>;
...
router.post(
  '/templates',
  validate(createEngagementTemplateSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
...
router.put(
  '/templates/:id',
  validate(updateEngagementTemplateSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const { id } = req.params as Record<string, string>;
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const limitNum = safeParseInt(limit as string, { fallback: 20 });
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const questionResults = questions.map((q: Record<string, any>) => {
      const answers = responsesResult.rows
        .map((r: Record<string, any>) =>
          r.answers.find((a: Record<string, any>) => a.question_id === q.id)
        )
        .filter((a: Record<string, any>) => a !== undefined);
      ...
      if (q.type === 'scale_5' || q.type === 'scale_10' || q.type === 'nps') {
        const numericAnswers = answers
          .map((a: Record<string, any>) => parseInt(a.value))
          .filter((v: number) => !isNaN(v));

        if (numericAnswers.length > 0) {
          result.average = (
            numericAnswers.reduce((a: number, b: number) => a + b, 0) / numericAnswers.length
          ).toFixed(2);
          result.distribution = {};
          numericAnswers.forEach((v: number) => {
            result.distribution[v] = (result.distribution[v] || 0) + 1;
          });
        }

        if (q.type === 'nps') {
          const promoters = numericAnswers.filter((v: number) => v >= 9).length;
          const detractors = numericAnswers.filter((v: number) => v <= 6).length;
        ...
        result.distribution = {};
        answers.forEach((a: Record<string, any>) => {
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const pending = surveysResult.rows.filter((s: Record<string, any>) => !s.is_complete);
    const completed = surveysResult.rows.filter((s: Record<string, any>) => s.is_complete);
...
router.post(
  '/pulse',
  validate(createPulseConfigSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const userId = (req as AuthenticatedRequest).user?.userId;
    const {
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const { period = '6' } = req.query as Record<string, string>; // months
