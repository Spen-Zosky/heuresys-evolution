...
router.get('/', asyncHandler(async (req, res) => {
    ...
    const [countResult, dataResult] = await Promise.all([
        req.dbClient.query(`SELECT COUNT(*) as total FROM employee_benefits eb WHERE eb.tenant_id = $1 ${whereClause}`, [tenantId]),
        req.dbClient.query(`SELECT eb.id, eb.benefit_name, eb.benefit_type, eb.description,
...
router.get('/', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { benefit_type, is_active, limit = '100', offset = '0', } = req.query;
    ...
    const fetchData = async () => {
      ...
      `;
        ...
        const countResult = await req.dbClient.query('SELECT COUNT(*) FROM employee_benefits WHERE tenant_id = $1', [tenantId]);
        return {
            rows: result.rows,
    ...
    const data = hasFilters
        ? await fetchData()
        : await cachedForTenant(tenantId, `benefits:list:${limitNum}:${offsetNum}`, fetchData, CACHE_TTL.MODERATE);
    res.json({
        success: true,
        data: data.rows,
        meta: {
 ...
 * POST /benefits
 */
router.post('/', requirePermission('COMPENSATION', 'CREATE'), validate(createBenefitSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { benefit_name, benefit_type, description, monthly_cost, coverage_options, is_active = true, } = req.body;
 ...
 * PATCH /benefits/:id
 */
router.patch('/:id', requirePermission('COMPENSATION', 'EDIT'), validate(updateBenefitSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'];
    const existing = await req.dbClient.query('SELECT id FROM employee_benefits WHERE id = $1 AND tenant_id = $2', [id, tenantId]);
    ...
    const result = await req.dbClient.query(`UPDATE employee_benefits SET ${updates.join(', ')} WHERE id = $${paramIndex} AND tenant_id = $${paramIndex + 1} RETURNING *`, [...values, id, tenantId]);
...
router.delete('/:id', requirePermission('COMPENSATION', 'DELETE'), asyncHandler(async (req, res) => {
    ...
    const activeEnrollments = await req.dbClient.query('SELECT COUNT(*) FROM employee_benefit_enrollments WHERE benefit_id = $1 AND is_active = true', [id]);
    if (parseInt(activeEnrollments.rows[0].count) > 0) {
        throw Errors.badRequest('Cannot deactivate benefit with active enrollments');
    }
    const result = await req.dbClient.query('UPDATE employee_benefits SET is_active = false WHERE id = $1 AND tenant_id = $2 RETURNING id', [id, tenantId]);
    if (result.rows.length === 0) {
        throw Errors.notFound('Benefit');
    }
