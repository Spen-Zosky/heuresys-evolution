...
import { getSkillExtractionService, } from '../services/skill-extraction/index.js';
import { asyncHandler } from '../errors/middleware.js';
import { buildMeta } from '../utils/pagination.js';
import { Errors } from '../errors/factory.js';
import { validate } from '../middleware/validate.js';
import { skillExtractionExtractSchema, skillExtractionExtractBatchSchema, skillExtractionMapSingleSchema, } from '../schemas/semantic.js';
 ...
 */
router.post('/extract', validate(skillExtractionExtractSchema), asyncHandler(async (req, res) => {
 ...
 */
router.post('/extract-batch', validate(skillExtractionExtractBatchSchema), asyncHandler(async (req, res) => {
 ...
 */
router.post('/map-single', validate(skillExtractionMapSingleSchema), asyncHandler(async (req, res) => {
    const { skillName, language, minConfidence } = req.body;
...
router.get('/search', asyncHandler(async (req, res) => {
    ...
    const labelColumn = validateEmbeddingColumn(lang === 'it' ? 'preferred_label_it' : 'preferred_label_en', 'skill-extraction.search-skills');
    const dbClient = req.dbClient;
    const result = await dbClient.query(`
    SELECT
      id,
    ...
    res.json({
        ...
        data: {
            ...
            skills: {
                ...
                mappingRate: stats.total_extracted > 0
