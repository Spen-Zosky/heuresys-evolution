...
router.post('/generate-prototype', validate(GeneratePrototypeSchema), asyncHandler(async (req, res) => {
    ...
    if (nacePrimary || companySize || countryCode) {
        ...
        if (nacePrimary) {
            ...
            await dbClient.query(`INSERT INTO tenant_industry_classifications (tenant_id, classification_code, classification_role, is_active)
           VALUES ($1, $2, 'primary', true)
           ON CONFLICT (tenant_id, classification_code) DO UPDATE SET classification_role = 'primary', is_active = true`, [tenantId, nacePrimary]);
        }
        if (companySize) {
        ...
        if (setClauses.length > 0) {
            ...
            await dbClient.query(`UPDATE tenants SET ${setClauses.join(', ')} WHERE id = $${params.length}`, params);
        }
    }
    // Check if prototype already exists
    const existing = await dbClient.query(`SELECT id, generated_at, departments_generated, positions_generated
    ...
    const naceCheck = await dbClient.query(`SELECT classification_code FROM tenant_industry_classifications
       WHERE tenant_id = $1 AND classification_role = 'primary' AND is_active = true
       LIMIT 1`, [tenantId]);
    if (naceCheck.rowCount === 0) {
        throw Errors.badRequest('Tenant has no NACE code configured. Provide nacePrimary in request body.');
    }
    // Generate prototype
    const result = await dbClient.query(`SELECT fn_generate_org_prototype($1::uuid, false) AS result`, [tenantId]);
...
router.post('/setup', validate(SetupSchema), asyncHandler(async (req, res) => {
    ...
    if (orgStructureType) {
        await dbClient.query(`INSERT INTO tenant_onboarding_profiles (tenant_id, prototype_config)
         VALUES ($1, $2::jsonb)
         ON CONFLICT DO NOTHING`, [tenantId, JSON.stringify({ orgStructureType })]);
    }
    // Trigger blueprint generation
    let blueprintResult = null;
    try {
        const result = await dbClient.query(`SELECT fn_generate_org_prototype($1::uuid, true) AS result`, [tenantId]);
    ...
    const summary = (blueprintResult?.summary ?? null);
    res.status(201).json({
        success: true,
        data: {
            nacePrimary,
...
router.get('/status', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const dbClient = req.dbClient;
    const data = await cachedForTenant(tenantId, 'onboard:status', async () => {
        const result = await dbClient.query(`SELECT top.*, tic.classification_code AS nace_primary, t.company_size, t.onboarding_completed_at
           FROM tenants t
...
//# sourceMappingURL=tenant-onboarding.js.map