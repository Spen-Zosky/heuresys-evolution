...
import { createInterviewSchema, updateInterviewSchema, completeInterviewSchema, cancelInterviewSchema, } from '../schemas/recruitment.js';
...
router.get('/', requirePermission('RECRUITMENT', 'VIEW'), applyScopeFilter('RECRUITMENT'), asyncHandler(async (req, res) => {
    ...
    const [countResult, dataResult] = await Promise.all([
        req.dbClient.query(`SELECT COUNT(*) as total FROM recruiting_interviews ri ${whereClause}`, params),
        req.dbClient.query(`SELECT ri.id, ri.candidate_id, ri.job_posting_id, ri.interview_type,
...
router.get('/', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { candidate_id, status, interview_type, limit = '100', offset = '0', } = req.query;
    let query = `
    SELECT i.*,
      c.first_name || ' ' || c.last_name as candidate_name,
      c.email as candidate_email,
  ...
  `;
    ...
    const countResult = await req.dbClient.query('SELECT COUNT(*) FROM recruiting_interviews WHERE tenant_id = $1', [tenantId]);
    res.json({
        success: true,
        data: result.rows,
        meta: {
 ...
 * POST /interviews
 */
router.post('/', requirePermission('RECRUITMENT', 'CREATE'), validate(createInterviewSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { candidate_id, job_posting_id, interview_type, title, description, scheduled_at, duration_minutes = 60, timezone = 'Europe/Rome', location_type = 'video', location, meeting_link, dial_in, notes, created_by, } = req.body;
 ...
 * PATCH /interviews/:id
 */
router.patch('/:id', requirePermission('RECRUITMENT', 'EDIT'), validate(updateInterviewSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'];
    const existing = await req.dbClient.query('SELECT id FROM recruiting_interviews WHERE id = $1 AND tenant_id = $2', [id, tenantId]);
    ...
    const result = await req.dbClient.query(`UPDATE recruiting_interviews SET ${updates.join(', ')} WHERE id = $${paramIndex} AND tenant_id = $${paramIndex + 1} RETURNING *`, [...values, id, tenantId]);
    res.json({ success: true, data: result.rows[0] || null, message: 'Interview updated' });
}));
/**
 * POST /interviews/:id/complete
 */
router.post('/:id/complete', requirePermission('RECRUITMENT', 'EDIT'), validate(completeInterviewSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'];
 ...
 */
router.post('/:id/cancel', requirePermission('RECRUITMENT', 'EDIT'), validate(cancelInterviewSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'];
...
router.delete('/:id', requirePermission('RECRUITMENT', 'DELETE'), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'];
    const result = await req.dbClient.query('DELETE FROM recruiting_interviews WHERE id = $1 AND tenant_id = $2 RETURNING id', [id, tenantId]);
    if (result.rows.length === 0) {
        throw Errors.notFound('Interview', id);
    }
