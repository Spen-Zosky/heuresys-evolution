...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const [dataResult, countResult] = await Promise.all([
      client.query(
        `SELECT id, process_code, process_name, process_category, description,
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const [phases, roles, skillReqs] = await Promise.all([
      ...
      client.query(
        `SELECT pr.id, pr.role_name, pr.role_type, pr.phase_id, pr.min_headcount,
      ...
      client.query(
        `SELECT psr.id, psr.phase_id, psr.esco_skill_id, psr.proficiency_level,
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const [dataResult, countResult] = await Promise.all([
      client.query(
        `SELECT id, parent_id, code, name, org_type, org_level, is_active
         FROM org_units
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const [dataResult, countResult] = await Promise.all([
      client.query(
        `SELECT es.id, es.preferred_label, es.description, es.skill_type,
      ...
      client.query(
        `SELECT COUNT(DISTINCT es.id)::int AS total
         FROM employee_skill_profiles esp
         JOIN esco_skills es ON es.id = esp.skill_id
         JOIN employees e ON e.id = esp.employee_id
         WHERE e.org_unit_id = $1`,
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const [dataResult, countResult] = await Promise.all([
      client.query(
        `SELECT id, template_id, run_mode, status, started_at, completed_at, created_at
         FROM blueprint_runs
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const [dataResult, countResult] = await Promise.all([
      client.query(
        `SELECT br.id, br.result_type, br.severity, br.title, br.description,
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const [dataResult, countResult] = await Promise.all([
      pool.query(
        `SELECT id, preferred_label, description, skill_type, reuse_level
         FROM esco_skills
      ...
      pool.query(
        `SELECT COUNT(*)::int AS total FROM esco_skills
         WHERE preferred_label ILIKE '%' || $1 || '%'
            OR description ILIKE '%' || $1 || '%'`,
        [q]
      ),
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const result = await pool.query(
      `SELECT id, preferred_label, alt_labels, description, skill_type,
