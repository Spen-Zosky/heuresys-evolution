...
import {
  ...
  ExportJobConfig,
  JobStatus,
} from '../services/payroll-integration.js';
import {
  createPayrollIntegrationSchema,
  updatePayrollIntegrationSchema,
  createExportJobSchema,
  transmitExportSchema,
  acknowledgeExportSchema,
} from '../schemas/compensation.js';
 ...
 */
router.get('/export-sections', async (_req: Request, res: Response) => {
  const sections = [
    {
      code: 'anagrafica',
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const { year, limit = '24' } = req.query as Record<string, string>;
...
router.post(
  '/integrations',
  checkPermission(PERMISSIONS.TENANT_CONFIGURE),
  validate(createPayrollIntegrationSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const config: PayrollIntegrationConfig = req.body;
...
router.patch(
  '/integrations/:id',
  checkPermission(PERMISSIONS.TENANT_CONFIGURE),
  validate(updatePayrollIntegrationSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params.id as string;
    const updates: Partial<PayrollIntegrationConfig> = req.body;
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const { year, month, status, limit = '20', offset = '0' } = req.query as Record<string, string>;
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const authReq = req as AuthenticatedRequest;
    const config: ExportJobConfig = req.body;
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const { integrationId } = req.query as Record<string, string>;

    const service = new PayrollIntegrationService(tenantId);
    const mappings = await service.getFieldMappings(integrationId as string | undefined);
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!confirmTransmission) {
      throw Errors.badRequest(
        'È richiesta la conferma esplicita della trasmissione (confirmTransmission: true)'
      );
    }
...
router.post(
  '/jobs/:id/acknowledge',
  checkPermission(PERMISSIONS.TENANT_CONFIGURE),
  validate(acknowledgeExportSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params.id as string;
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const { year, status, limit = '20', offset = '0' } = req.query as Record<string, string>;

    const service = new PayrollIntegrationService(tenantId);
    const historyFilters: { year?: number; status?: JobStatus; limit: number; offset: number } = {
      limit: safeParseInt(limit as string, { fallback: 50 }),
      offset: safeParseInt(offset as string, { fallback: 0 }),
    };
    if (year) historyFilters.year = parseInt(year as string);
    if (status) historyFilters.status = status as JobStatus;
