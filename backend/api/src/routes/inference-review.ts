...
import {
  approveInferenceSchema,
  rejectInferenceSchema,
  bulkInferenceSchema,
  bulkByFilterInferenceSchema,
...
interface ReviewMetrics {
  ...
  byRelationType: Record<string, { pending: number; approved: number; rejected: number }>;
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const { limit, offset, relationType, minConfidence, sortBy } = req.query as Record<
      string,
      string
    >;
    const limitNum = safeParseInt(limit as string, { fallback: 20, max: 100 });
    const offsetNum = safeParseInt(offset as string, { fallback: 0 });
    const minConf = parseFloat(minConfidence as string) || 0;
    ...
    const validSortKey = Object.hasOwn(sortOptions, sortBy as string)
      ? (sortBy as string)
      : 'confidence_desc';
    ...
    const relations: PendingRelation[] = result.rows.map((row) => ({
      id: row.id,
...
router.post(
  '/:id/approve',
  validate(approveInferenceSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const dbClient = req.dbClient!;
    const { id } = req.params as Record<string, string>;
    const { reviewerId, notes } = req.body as {
      reviewerId?: string;
      notes?: string;
    };
    ...
    const checkResult = await dbClient.query(
      `SELECT id, approval_status FROM ontology_skill_relations WHERE id = $1`,
      [id]
    );
...
router.post(
  '/:id/reject',
  validate(rejectInferenceSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const dbClient = req.dbClient!;
    const { id } = req.params as Record<string, string>;
    const { reviewerId, reason } = req.body as {
      reviewerId?: string;
      reason?: string;
    };
...
router.post(
  '/bulk',
  validate(bulkInferenceSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const dbClient = req.dbClient!;
    const { action, relationIds, reviewerId, notes } = req.body as {
...
router.post(
  '/bulk-by-filter',
  validate(bulkByFilterInferenceSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const dbClient = req.dbClient!;
    const { action, filter, reviewerId, notes, limit } = req.body as {
      action: 'approve' | 'reject';
      filter: {
        relationType?: string;
    ...
    let query = `
      ...
      WHERE id IN (
        ...
        WHERE source = 'ai_inferred'
          AND approval_status = 'pending'
    `;
    const params: unknown[] = [newStatus, reviewerId || null];
    let paramIndex = 3;
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const byRelationType: Record<string, { pending: number; approved: number; rejected: number }> =
      {};
    for (const row of typeResult.rows) {
      ...
      if (typeEntry) {
        const status = row.approval_status as 'pending' | 'approved' | 'rejected';
    ...
    const approvalRate =
      totalReviewed > 0 ? Math.round((approvedCount / totalReviewed) * 100) / 100 : 0;
