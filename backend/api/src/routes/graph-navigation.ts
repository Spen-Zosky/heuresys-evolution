...
import type { KpiCascadeResult } from '../services/graph-navigation.js';
import { z } from 'zod';
...
router.get(
  '/employee/:employeeId/process-qualification',
  asyncHandler(async (req: Request, res: Response) => {
...
router.get(
  '/org-unit/:orgUnitId/coverage',
  asyncHandler(async (req: Request, res: Response) => {
    const { orgUnitId } = req.params as { orgUnitId: string };
...
router.get(
  '/industry/:industryCode/process-map',
  validate(industryCodeSchema, 'params'),
  asyncHandler(async (req: Request, res: Response) => {
    const { industryCode } = req.params as { industryCode: string };
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const service = new GraphNavigationService(req.dbClient!);
    const data: KpiCascadeResult | null = await service.getProcessKpiCascade(processId, tenantId);
