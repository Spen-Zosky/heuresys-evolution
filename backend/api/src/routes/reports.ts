...
import type { ReportDefinitionInput } from '../services/report-builder.js';
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const [countResult, dataResult] = await Promise.all([
      ...
      req.dbClient!.query(
        `SELECT rd.id, rd.name, rd.description, rd.data_source, rd.is_template,
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const report = await reportBuilderService.cloneReport(
      tenantId,
      id,
      name,
      created_by || 'system'
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const history = await reportBuilderService.getExecutionHistory(
      tenantId,
      id,
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const existing = await req.dbClient!.query(
      'SELECT COUNT(*) FROM report_definitions WHERE tenant_id = $1 AND is_template = true',
      [tenantId]
    );
    ...
    // Seed system reports
    type SeedReport = Omit<
      ReportDefinitionInput,
      'tenant_id' | 'created_by' | 'is_system' | 'is_public'
    >;
    const systemReports: SeedReport[] = [
      {
