...
import { createEmbeddingPipeline, } from '../services/embedding-pipeline.js';
import { validate } from '../middleware/validate.js';
import { queueEmbeddingSchema, processEmbeddingsSchema, embeddingSearchSchema, reindexEmbeddingSchema, } from '../schemas/skills-extended.js';
 ...
 */
async function ensurePipeline(_req, _res, next) {
    try {
...
router.get('/queue', asyncHandler(async (req, res) => {
    ...
    const countResult = await dbClient.query(`SELECT COUNT(*) FROM embedding_queue WHERE tenant_id = $1${status ? ' AND status = $2' : ''}`, status ? [tenantId, status] : [tenantId]);
    res.json({
        success: true,
        data: result.rows,
        meta: {
 ...
 */
router.post('/queue', validate(queueEmbeddingSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { entity_type, entity_id, text_content } = req.body;
 ...
 */
router.post('/process', requirePermission('AI_SERVICES', 'CREATE'), validate(processEmbeddingsSchema), asyncHandler(async (req, res) => {
    const { batch_size } = req.body;
    const batchSize = typeof batch_size === 'number' && batch_size > 0 ? Math.min(batch_size, 200) : 50;
    const service = getPipeline();
 ...
 */
router.post('/search', validate(embeddingSearchSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
...
router.get('/stats', asyncHandler(async (req, res) => {
    ...
    const indexResult = await dbClient.query(`SELECT COUNT(*) AS total_indexed,
 ...
 */
router.post('/reindex', requirePermission('AI_SERVICES', 'CREATE'), validate(reindexEmbeddingSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
