...
const SIZE_CONFIG: Record<
  string,
  { headcount: string; departments: number; layers: number; label: string }
> = {
...
router.get(
  ...
  asyncHandler(async (_req, res) => {
    const result = await pool.query(
      `SELECT ip.nace_class_code AS "naceCode",
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    const { naceCode, companySize, customProcesses } = req.body as z.infer<typeof generateSchema>;

    // 1. Trova il profilo industria (match esatto, fallback stesso NACE qualsiasi size)
    const profileResult = await pool.query(
      `SELECT ip.id, ip.code, ip.name, ip.nace_class_code, ip.company_size_code,
    ...
    let orgUnits: Record<string, unknown>[] = [];

    if (profile) {
      // 2. Processi del profilo
      const procResult = await pool.query(
        `SELECT id, process_code AS "processCode", process_name AS "processName",
         ...
         WHERE profile_id = $1
         ORDER BY value_chain_position, process_name`,
        [profile.id]
      );
      processes = procResult.rows;

      if (processes.length > 0) {
        const processIds = processes.map((p) => p.id as string);

        // 3. Ruoli
        const roleResult = await pool.query(
          `SELECT pr.process_id AS "processId", pr.role_name AS "roleName",
           ...
           ORDER BY pr.process_id, pr.role_type`,
          [processIds]
        );
        ...
        const kpiResult = await pool.query(
          `SELECT pk.process_id AS "processId", pk.kpi_code AS "kpiCode",
        ...
        const skillResult = await pool.query(
          `SELECT DISTINCT ON (psr.esco_skill_id)
      ...
      const templateResult = await pool.query(
        `SELECT id FROM blueprint_templates
      ...
      if (templateResult.rows.length > 0) {
        const ouResult = await pool.query(
          `SELECT code, name_it AS "nameIt", name_en AS "nameEn",
    ...
    // 7. Processi custom
    const customList: Record<string, unknown>[] = (customProcesses ?? []).map((name, i) => ({
      id: `custom-${i}`,
