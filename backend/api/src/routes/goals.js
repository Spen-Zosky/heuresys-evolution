...
router.get('/', requirePermission('PERFORMANCE', 'VIEW'), applyScopeFilter('PERFORMANCE'), asyncHandler(async (req, res) => {
    ...
    const { employee_id, status, priority, category, goal_type, parent_goal_id, limit = '100', offset = '0', } = req.query;
    let query = `
      SELECT
    ...
    if (parent_goal_id) {
        if (parent_goal_id === 'null') {
            ...
            params.push(parent_goal_id);
            paramIndex++;
        }
    }
    const MAX_LIMIT = 500;
    query += ` ORDER BY g.due_date ASC NULLS LAST, g.priority DESC LIMIT $${paramIndex} OFFSET $${paramIndex + 1}`;
    params.push(Math.min(safeParseInt(limit, { fallback: 100 }), MAX_LIMIT), safeParseInt(offset, { fallback: 0 }));
    ...
    if (parent_goal_id) {
        if (parent_goal_id === 'null') {
            ...
            countParams.push(parent_goal_id);
        }
    }
...
router.get('/hierarchy', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { employee_id, status, root_only } = req.query;
...
router.get('/templates', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { category, difficulty_level, org_unit_id, is_company_wide } = req.query;
    let query = `
...
router.get('/team', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { manager_id, status, category, include_overdue } = req.query;
    if (!manager_id) {
 ...
 */
router.get('/cascade', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { root_id, org_unit_id, max_level } = req.query;
    let query = `
...
function buildGoalTree(rows) {
    ...
    for (const row of rows) {
        ...
        if (row.parent_goal_id === null) {
            ...
            const parent = nodeMap.get(row.parent_goal_id);
            if (parent) {
...
router.post('/', validate(createGoalSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { employee_id, title, description, goal_type = 'individual', status = 'draft', priority, start_date, due_date, parent_goal_id, category, weight, owner_id, } = req.body;
 ...
 * Update a goal
 */
router.put('/:id', validateUUID(), validate(updateGoalSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'];
    const { title, description, goal_type, status, priority, progress_percent, start_date, due_date, parent_goal_id, category, weight, } = req.body;
    // Check goal exists and belongs to tenant
    const existingGoal = await req.dbClient.query('SELECT id, status FROM goals WHERE id = $1 AND tenant_id = $2', [id, tenantId]);
...
router.delete('/:id', validateUUID(), asyncHandler(async (req, res) => {
    ...
    const childCheck = await req.dbClient.query('SELECT COUNT(*) FROM goals WHERE parent_goal_id = $1', [id]);
 ...
 * Get goal alignments
 */
router.get('/:id/alignments', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'];
 ...
 */
router.post('/:id/alignments', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'];
    ...
    const alignment = await performanceManagementService.createGoalAlignment(tenantId, id, aligned_goal_id, alignment_type, weight);
...
router.get('/:id/updates', asyncHandler(async (req, res) => {
    ...
    const updates = await performanceManagementService.getGoalUpdates(tenantId, id, safeParseInt(limit, { fallback: 50 }));
 ...
 */
router.post('/:id/milestones/:milestoneId/complete', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const goalId = req.params['id'];
    const milestoneId = req.params['milestoneId'];
    // Verify goal exists
    const goalCheck = await req.dbClient.query('SELECT id FROM goals WHERE id = $1 AND tenant_id = $2', [goalId, tenantId]);
    if (goalCheck.rows.length === 0) {
        throw Errors.notFound('Goal', goalId);
    }
    // Verify milestone exists
    const milestoneCheck = await req.dbClient.query('SELECT id FROM goal_milestones WHERE id = $1 AND tenant_id = $2', [milestoneId, tenantId]);
 ...
 */
router.get('/:id/cascaded-progress', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'];
...
router.post('/templates', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { name, description, category, goal_type, suggested_metrics, suggested_duration_days, suggested_weight, difficulty_level, org_unit_id, is_company_wide, created_by, } = req.body;
    if (!name) {
 ...
 */
router.post('/from-template', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
 ...
 */
router.get('/:id/check-ins', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const goalId = req.params['id'];
    const result = await req.dbClient.query(`
...
router.post('/:id/check-in', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const goalId = req.params['id'];
    const { employee_id, new_progress, status_update, notes, blockers, next_steps, confidence_level, } = req.body;
 ...
 */
router.post('/:id/validate-smart', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const goalId = req.params['id'];
    // Get goal
    const goalResult = await req.dbClient.query(`
    ...
    const smartCriteria = {
        ...
        measurable: {
            ...
            feedback: goal.progress_percent !== null
