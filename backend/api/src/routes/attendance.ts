...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const [countResult, dataResult] = await Promise.all([
      req.dbClient!.query(
        `SELECT COUNT(*) as total FROM employee_attendance ea JOIN employees e ON ea.employee_id = e.id ${whereClause}`,
        params
      ),
      req.dbClient!.query(
        `SELECT ea.id, ea.employee_id, ea.attendance_date, ea.clock_in, ea.clock_out,
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const targetYear = year ? parseInt(year as string, 10) : new Date().getFullYear();
    const targetMonth = month ? parseInt(month as string, 10) : null;

    let dateCondition = `EXTRACT(YEAR FROM ea.attendance_date) = $3`;
    const params: (string | number)[] = [employeeId, tenantId, targetYear];
