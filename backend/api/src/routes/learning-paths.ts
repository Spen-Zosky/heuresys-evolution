...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const hasFilters =
      is_active !== undefined || target_role || skill_level || is_mandatory !== undefined;
    ...
    const fetchData = async () => {
      ...
      if (target_role) {
        ...
        params.push(target_role as string);
        paramIndex++;
      }

      if (skill_level) {
        query += ` AND lp.skill_level = $${paramIndex}`;
        params.push(skill_level as string);
        paramIndex++;
      }
      ...
      const countResult = await req.dbClient!.query(
        'SELECT COUNT(*) FROM learning_paths WHERE tenant_id = $1',
        [tenantId]
      );
    ...
    const data = hasFilters
      ...
      : await cachedForTenant(
          tenantId,
          `learning-paths:list:${limitNum}:${offsetNum}`,
          fetchData,
          CACHE_TTL.MODERATE
        );
...
router.patch(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const existing = await req.dbClient!.query(
      'SELECT id FROM learning_paths WHERE id = $1 AND tenant_id = $2',
      [id, tenantId]
    );
    ...
    const result = await req.dbClient!.query(
      `UPDATE learning_paths SET ${updates.join(', ')} WHERE id = $${paramIndex} AND tenant_id = $${paramIndex + 1} RETURNING *`,
      [...values, id, tenantId]
    );
...
router.delete(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const result = await req.dbClient!.query(
      'UPDATE learning_paths SET is_active = false, updated_at = NOW() WHERE id = $1 AND tenant_id = $2 RETURNING id',
      [id, tenantId]
    );
