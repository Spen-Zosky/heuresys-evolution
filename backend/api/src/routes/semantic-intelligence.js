...
import { createUnifiedSemanticService, } from '../services/unified-semantic-service.js';
import { createEmbeddingQueueProcessor, } from '../services/embedding-queue-processor.js';
...
import { semanticSearchSchema, semanticDirectSearchSchema, semanticEmbeddingsGenerateSchema, semanticTalentSearchSchema, semanticPerformanceSearchSchema, semanticLearningSearchSchema, semanticOrgSearchSchema, semanticAskSchema, semanticQueueProcessSchema, semanticQueueCleanupSchema, } from '../schemas/semantic.js';
 ...
 */
async function ensureService(req, _res, next) {
    try {
        ...
        req.semanticService = service;
        req.queueProcessor = processor;
        next();
    }
    catch (error) {
        next(error);
 ...
 */
router.post('/search', validate(semanticSearchSchema), asyncHandler(async (req, res) => {
    const service = req.semanticService;
 ...
 */
router.post('/search/direct', validate(semanticDirectSearchSchema), asyncHandler(async (req, res) => {
    const service = req.semanticService;
    const tenantId = req.headers['x-tenant-id'];
    if (!tenantId) {
        throw Errors.badRequest('Tenant ID required');
 ...
 */
router.post('/embeddings/generate', validate(semanticEmbeddingsGenerateSchema), asyncHandler(async (req, res) => {
    const service = req.semanticService;
    const tenantId = req.headers['x-tenant-id'];
 ...
 */
router.post('/embeddings/generate-all', asyncHandler(async (req, res) => {
    const service = req.semanticService;
    const tenantId = req.headers['x-tenant-id'];
    ...
    res.json({
        ...
        data: {
            ...
            summary: {
                totalRecords,
                totalEmbedded,
                overallPercentage: totalRecords > 0 ? Math.round((totalEmbedded / totalRecords) * 100) : 0,
            },
        },
    });
}));
 ...
 */
router.post('/talent-search', validate(semanticTalentSearchSchema), asyncHandler(async (req, res) => {
    const service = req.semanticService;
    const tenantId = req.headers['x-tenant-id'];
    if (!tenantId) {
        throw Errors.badRequest('Tenant ID required');
    }
    const { query, limit = 15, similarityThreshold = 0.5, includeInternal = true, includeExternal = true, } = req.body;
    if (!query) {
        throw Errors.badRequest('Query is required');
    }
    const entityTypes = [];
 ...
 */
router.post('/performance-search', validate(semanticPerformanceSearchSchema), asyncHandler(async (req, res) => {
    const service = req.semanticService;
    const tenantId = req.headers['x-tenant-id'];
    if (!tenantId) {
        throw Errors.badRequest('Tenant ID required');
    }
    const { query, limit = 20, similarityThreshold = 0.5, includeReviews = true, includeCheckIns = true, includeFeedback = true, } = req.body;
    if (!query) {
        throw Errors.badRequest('Query is required');
    }
    const entityTypes = [];
    if (includeReviews)
        entityTypes.push('performance_review');
    if (includeCheckIns)
        entityTypes.push('check_in');
    if (includeFeedback)
        entityTypes.push('feedback_360');
    const result = await service.unifiedSearch({
        query,
        tenantId,
        entityTypes,
 ...
 */
router.post('/learning-search', validate(semanticLearningSearchSchema), asyncHandler(async (req, res) => {
    const service = req.semanticService;
    const tenantId = req.headers['x-tenant-id'];
    if (!tenantId) {
        throw Errors.badRequest('Tenant ID required');
    ...
    const entityTypes = ['learning_path', 'course', 'skill'];
    const result = await service.unifiedSearch({
        query,
        tenantId,
        entityTypes,
 ...
 */
router.post('/org-search', validate(semanticOrgSearchSchema), asyncHandler(async (req, res) => {
    const service = req.semanticService;
    const tenantId = req.headers['x-tenant-id'];
    if (!tenantId) {
        throw Errors.badRequest('Tenant ID required');
    ...
    const entityTypes = ['department', 'org_unit', 'location', 'employee'];
    const result = await service.unifiedSearch({
        query,
        tenantId,
        entityTypes,
 ...
 */
router.post('/ask', validate(semanticAskSchema), asyncHandler(async (req, res) => {
    const service = req.semanticService;
    const tenantId = req.headers['x-tenant-id'];
    if (!tenantId) {
        throw Errors.badRequest('Tenant ID required');
    ...
    if (questionLower.includes('dipendent') ||
    ...
    if (questionLower.includes('dipartiment') ||
    ...
    // Career related
    if (questionLower.includes('career') ||
...
router.get('/queue/status', asyncHandler(async (req, res) => {
    const processor = req.queueProcessor;
 ...
 */
router.post('/queue/process', validate(semanticQueueProcessSchema), asyncHandler(async (req, res) => {
    const processor = req.queueProcessor;
 ...
 */
router.post('/queue/cleanup', validate(semanticQueueCleanupSchema), asyncHandler(async (req, res) => {
    const processor = req.queueProcessor;
...
//# sourceMappingURL=semantic-intelligence.js.map