...
import { performanceDistributionQuerySchema, performanceTrendsQuerySchema, performanceDeptComparisonQuerySchema, performanceManagerConsistencyQuerySchema, performanceExecSummaryQuerySchema, performanceHeatmapQuerySchema, performanceCompletionRatesQuerySchema, } from '../schemas/analytics-extended.js';
 ...
 */
router.get('/distribution', validate(performanceDistributionQuerySchema, 'query'), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { review_cycle_id, org_unit_id, year } = req.query;
    let query = `
      SELECT
 ...
 */
router.get('/trends', validate(performanceTrendsQuerySchema, 'query'), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { org_unit_id, employee_id, years = '3' } = req.query;
    ...
    `;
    const params = [tenantId, yearsNum];
    let paramIndex = 3;
    if (org_unit_id) {
        query += ` AND EXISTS (SELECT 1 FROM employees e WHERE e.id = pr.employee_id AND e.org_unit_id = $${paramIndex})`;
        params.push(org_unit_id);
 ...
 */
router.get('/department-comparison', validate(performanceDeptComparisonQuerySchema, 'query'), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    ...
    const comparisonData = result.rows.map((row) => ({
        ...row,
        deviation_from_org: (parseFloat(row.avg_rating) - parseFloat(orgAvg.rows[0]?.org_avg)).toFixed(2),
    }));
    res.json({
        success: true,
        data: {
 ...
 */
router.get('/manager-consistency', validate(performanceManagerConsistencyQuerySchema, 'query'), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { review_cycle_id } = req.query;
    let query = `
      SELECT
    ...
    const managerData = result.rows.map((row) => ({
        ...row,
        consistency_score: Math.max(0, 100 - parseFloat(row.avg_adjustment || 0) * 20 - parseFloat(row.rating_spread || 0) * 10).toFixed(1),
    }));
    res.json({
        success: true,
        data: managerData,
 ...
 */
router.get('/executive-summary', validate(performanceExecSummaryQuerySchema, 'query'), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { review_cycle_id } = req.query;
    // Overall metrics
    let overallQuery = `
 ...
 */
router.get('/heatmap', validate(performanceHeatmapQuerySchema, 'query'), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { review_cycle_id } = req.query;
    let query = `
      SELECT
 ...
 */
router.get('/completion-rates', validate(performanceCompletionRatesQuerySchema, 'query'), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { review_cycle_id, group_by = 'department' } = req.query;
