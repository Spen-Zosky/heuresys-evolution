 ...
 */
router.get('/industry/:naceCode', requirePermission('ORGANIZATION', 'VIEW'), asyncHandler(async (req, res) => {
 ...
 */
router.get('/my-position', requirePermission('ORGANIZATION', 'VIEW'), asyncHandler(async (req, res) => {
    const service = new BenchmarkingService(req.tenantId, req.tenant?.name ?? '');
