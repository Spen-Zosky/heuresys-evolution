...
import { createPredictionModelSchema, updateModelStatusSchema, generatePerformancePredictionsSchema, validatePredictionsSchema, } from '../schemas/admin.js';
...
router.get('/', asyncHandler(async (req, res) => {
    ...
    try {
        const result = await req.dbClient.query(`SELECT
          ...
          (SELECT COUNT(*) FROM performance_predictions WHERE tenant_id = $1 AND is_current = true AND risk_level IN ('high', 'critical')) as high_risk_count`, [tenantId]);
        stats = result.rows[0] || {};
    }
    catch (_err) {
 ...
 */
router.post('/models', validate(createPredictionModelSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
 ...
 * Update model status
 */
router.patch('/models/:id/status', validate(updateModelStatusSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
 ...
 */
router.get('/turnover/high-risk', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
 ...
 */
router.post('/turnover/batch', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
 ...
 */
router.get('/turnover/:employeeId', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
 ...
 */
router.get('/performance/:employeeId', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const employeeId = req.params['employeeId'];
...
router.get('/performance', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { risk_level, is_high_potential, org_unit_id, limit = '100', offset = '0', sort_by = 'risk_score', sort_order = 'desc', } = req.query;
    let query = `
      SELECT
        pp.id,
        pp.employee_id,
    ...
    const sortColumn = validateIdentifier(validSortColumns.includes(sort_by) ? sort_by : 'risk_score', 'predictions.performance-sort');
 ...
 */
router.post('/performance/generate', validate(generatePerformancePredictionsSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
 ...
 */
router.get('/performance/:employeeId/factors', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const employeeId = req.params['employeeId'];
 ...
 */
router.get('/risk-distribution', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const result = await req.dbClient.query(`SELECT tenant_id, org_unit_id, department_name, total_employees,
              low_risk, medium_risk, high_risk, critical_risk,
              high_potentials, avg_risk_score, avg_predicted_rating
       FROM v_risk_distribution WHERE tenant_id = $1 ORDER BY department_name LIMIT 100`, [tenantId]);
    // Calculate totals
 ...
 */
router.get('/high-potentials', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { org_unit_id, min_score = '3.0', limit = '50' } = req.query;
    let query = `
      SELECT
        pp.employee_id,
        e.first_name || ' ' || e.last_name as employee_name,
      ...
      WHERE pp.tenant_id = $1
        ...
        AND pp.hipo_score >= $2
    `;
    const params = [tenantId, parseFloat(min_score)];
    let paramIndex = 3;
    if (org_unit_id) {
        query += ` AND e.org_unit_id = $${paramIndex}`;
        params.push(org_unit_id);
 ...
 */
router.post('/validate', validate(validatePredictionsSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { prediction_period } = req.body;
...
router.get('/accuracy', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { model_name = 'heuresys-perf-v1', limit = '10' } = req.query;
    const result = await req.dbClient.query(`SELECT
        model_name,
        model_version,
        prediction_period,
      ...
      LIMIT $3`, [tenantId, model_name, safeParseInt(limit, { fallback: 50 })]);
    // Calculate aggregate metrics
    const validRows = result.rows.filter((r) => r.mean_absolute_error);
    const aggregateMetrics = validRows.length > 0
        ? {
            avg_mae: validRows.reduce((sum, r) => sum + parseFloat(r.mean_absolute_error), 0) /
                validRows.length,
            avg_accuracy_05: validRows.reduce((sum, r) => sum + parseFloat(r.accuracy_within_05 || 0), 0) /
                validRows.length,
            avg_accuracy_10: validRows.reduce((sum, r) => sum + parseFloat(r.accuracy_within_10 || 0), 0) /
                validRows.length,
            total_validated: validRows.reduce((sum, r) => sum + parseInt(r.validated_predictions), 0),
        }
        : null;
    res.json({
        success: true,
...
router.get('/actions', asyncHandler(async (req, res) => {
    const { category, risk_level, for_hipo } = req.query;
    let query = `
      SELECT
        action_code,
        action_name,
...
router.get('/factors', asyncHandler(async (req, res) => {
    ...
    const result = await req.dbClient.query(query, params);
    // Group by category
    const byCategory = result.rows.reduce((acc, factor) => {
 ...
 */
router.get('/flight-risk/:employeeId', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
 ...
 */
router.get('/flight-risk/department/:deptId', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    ...
    const employees = result.rows.map((e) => ({
        ...e,
        risk_level: e.calculated_risk_score >= 75
            ? 'critical'
    ...
    const avgRisk = employees.length > 0
        ? Math.round(employees.reduce((sum, e) => sum + e.calculated_risk_score, 0) / employees.length)
        : 0;
    res.json({
        success: true,
        data: {
 ...
 */
router.get('/skill-demand', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
...
router.get('/ai-recommendations', asyncHandler(async (req, res) => {
    ...
    const turnoverRate = m.total_employees > 0 ? Math.round((m.terminations_12m / m.total_employees) * 100) : 0;
    if (turnoverRate > 15) {
        recommendations.push({
            id: 'rec-turnover-1',
            category: 'retention',
