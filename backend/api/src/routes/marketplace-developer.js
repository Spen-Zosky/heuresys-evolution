...
import { createDeveloperPluginSchema, updateDeveloperPluginSchema, createPluginVersionSchema, createApiKeySchema, } from '../schemas/marketplace.js';
 ...
 */
router.get('/plugins/:id', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
 ...
 */
router.post('/plugins', validate(createDeveloperPluginSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { name, slug, short_description, description, category_id, pricing_model = 'free', price_cents, currency = 'EUR', tags, icon_url, homepage_url, repository_url, license, screenshot_urls, banner_url, } = req.body;
    if (!name) {
 ...
 */
router.put('/plugins/:id', validate(updateDeveloperPluginSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'];
    // Verify ownership
    const existing = await req.dbClient.query('SELECT id FROM plugins WHERE id = $1 AND publisher_tenant_id = $2', [id, tenantId]);
    if (existing.rows.length === 0) {
        throw Errors.notFound('Plugin');
    }
    const { name, short_description, description, category_id, pricing_model, price_cents, currency, tags, icon_url, homepage_url, repository_url, license, visibility, screenshot_urls, banner_url, } = req.body;
    const result = await req.dbClient.query(`
      UPDATE plugins SET
 ...
 */
router.delete('/plugins/:id', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'];
    const existing = await req.dbClient.query('SELECT id, status, total_installations FROM plugins WHERE id = $1 AND publisher_tenant_id = $2', [id, tenantId]);
    if (existing.rows.length === 0) {
        throw Errors.notFound('Plugin');
    }
 ...
 */
router.patch('/plugins/:id/submit', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'];
    const result = await req.dbClient.query(`
      UPDATE plugins
 ...
 */
router.post('/plugins/:id/versions', validate(createPluginVersionSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const pluginId = req.params['id'];
    // Verify ownership
    const pluginCheck = await req.dbClient.query('SELECT id FROM plugins WHERE id = $1 AND publisher_tenant_id = $2', [pluginId, tenantId]);
    ...
    await req.dbClient.query('UPDATE plugin_versions SET is_latest = false WHERE plugin_id = $1 AND is_latest = true', [pluginId]);
    const result = await req.dbClient.query(`
 ...
 */
router.get('/plugins/:id/versions', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const pluginId = req.params['id'];
    // Verify ownership
    const pluginCheck = await req.dbClient.query('SELECT id FROM plugins WHERE id = $1 AND publisher_tenant_id = $2', [pluginId, tenantId]);
    ...
    const result = await req.dbClient.query('SELECT id, plugin_id, version, release_notes, changelog, min_platform_version, max_platform_version, config_schema, permissions_required, entry_point, package_url, package_size_bytes, checksum, status, is_latest, published_at, created_at, updated_at FROM plugin_versions WHERE plugin_id = $1 ORDER BY created_at DESC LIMIT 50', [pluginId]);
 ...
 */
router.patch('/plugins/:id/versions/:versionId/yank', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const pluginId = req.params['id'];
    ...
    const versionResult = await req.dbClient.query('SELECT id, is_latest FROM plugin_versions WHERE id = $1 AND plugin_id = $2', [versionId, pluginId]);
    if (versionResult.rows.length === 0) {
        throw Errors.notFound('Version');
    }
    const wasLatest = versionResult.rows[0]?.is_latest;
    // Yank the version
    await req.dbClient.query("UPDATE plugin_versions SET status = 'yanked', is_latest = false, updated_at = NOW() WHERE id = $1", [versionId]);
 ...
 */
router.get('/plugins/:id/reviews', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const pluginId = req.params['id'];
 ...
 */
router.post('/api-keys', validate(createApiKeySchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const authReq = req;
    ...
    if (plugin_installation_id) {
        const installCheck = await req.dbClient.query('SELECT id FROM plugin_installations WHERE id = $1 AND tenant_id = $2', [plugin_installation_id, tenantId]);
...
//# sourceMappingURL=marketplace-developer.js.map