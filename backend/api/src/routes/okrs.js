...
import { createOkrSchema, updateOkrSchema, updateOkrProgressSchema, createKeyResultSchema, updateKeyResultSchema, updateKeyResultProgressSchema, createOkrCheckinSchema, } from '../schemas/okrs.js';
...
router.get('/', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { owner_id, status, okr_type, department, period_type, limit = '100', offset = '0', } = req.query;
    let query = `
      SELECT o.*,
    ...
    const countResult = await req.dbClient.query('SELECT COUNT(*) FROM okrs WHERE tenant_id = $1', [tenantId]);
    res.json({
        success: true,
        data: result.rows,
        meta: {
 ...
 * POST /okrs
 */
router.post('/', validate(createOkrSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { objective, okr_type = 'individual', department, period_type = 'quarterly', period_start, period_end, status = 'draft', overall_progress = 0, confidence_level, owner_id, created_by, } = req.body;
    if (!objective) {
 ...
 * PATCH /okrs/:id
 */
router.patch('/:id', validate(updateOkrSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'];
    const existing = await req.dbClient.query('SELECT id FROM okrs WHERE id = $1 AND tenant_id = $2', [id, tenantId]);
    ...
    const result = await req.dbClient.query(`UPDATE okrs SET ${updates.join(', ')} WHERE id = $${paramIndex} AND tenant_id = $${paramIndex + 1} RETURNING *`, [...values, id, tenantId]);
    res.json({ success: true, data: result.rows[0] || null, message: 'OKR updated' });
}));
/**
 * PATCH /okrs/:id/progress
 */
router.patch('/:id/progress', validate(updateOkrProgressSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'];
...
router.delete('/:id', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'];
    const result = await req.dbClient.query('DELETE FROM okrs WHERE id = $1 AND tenant_id = $2 RETURNING id', [id, tenantId]);
    if (result.rows.length === 0) {
        throw Errors.notFound('OKR');
    }
 ...
 */
router.get('/:id/key-results', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const okrId = req.params['id'];
    const result = await req.dbClient.query(`
      SELECT kr.*,
 ...
 */
router.post('/:id/key-results', validate(createKeyResultSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const okrId = req.params['id'];
    const { title, description, metric_type = 'percentage', unit, start_value = 0, target_value, current_value = 0, weight = 1, owner_id, due_date, } = req.body;
    if (!title) {
    ...
    // Verify OKR exists
    const okrCheck = await req.dbClient.query('SELECT id FROM okrs WHERE id = $1 AND tenant_id = $2', [okrId, tenantId]);
 ...
 */
router.get('/:id/key-results/:krId', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
 ...
 * Update a key result
 */
router.patch('/:id/key-results/:krId', validate(updateKeyResultSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const krId = req.params['krId'];
    const allowedFields = [
        'title',
    ...
    updates.push('updated_at = NOW()');
    const result = await req.dbClient.query(`UPDATE key_results SET ${updates.join(', ')} WHERE id = $${paramIndex} AND tenant_id = $${paramIndex + 1} RETURNING *`, [...values, krId, tenantId]);
    if (result.rows.length === 0) {
        throw Errors.notFound('Key result');
    }
 ...
 */
router.patch('/:id/key-results/:krId/progress', validate(updateKeyResultProgressSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const krId = req.params['krId'];
    ...
    const keyResult = await performanceManagementService.updateKeyResultProgress(tenantId, krId, current_value, confidence_level);
    if (!keyResult) {
        throw Errors.notFound('Key result');
    }
 ...
 * Delete a key result
 */
router.delete('/:id/key-results/:krId', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { id: okrId, krId } = req.params;
    const result = await req.dbClient.query('DELETE FROM key_results WHERE id = $1 AND okr_id = $2 AND tenant_id = $3 RETURNING id', [krId, okrId, tenantId]);
    if (result.rows.length === 0) {
        throw Errors.notFound('Key result');
    }
    // Recalculate OKR overall progress
...
router.get('/:id/checkins', asyncHandler(async (req, res) => {
    ...
    `, [
        okrId,
        tenantId,
        safeParseInt(limit, { fallback: 20 }),
        safeParseInt(offset, { fallback: 0 }),
    ]);
    const countResult = await req.dbClient.query('SELECT COUNT(*) FROM okr_checkins WHERE okr_id = $1 AND tenant_id = $2', [okrId, tenantId]);
    res.json({
        success: true,
        data: result.rows,
        meta: {
 ...
 */
router.post('/:id/checkins', validate(createOkrCheckinSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const okrId = req.params['id'];
    const { progress_snapshot, confidence_level, blockers, achievements, next_steps, notes, key_result_updates, created_by, } = req.body;
    // Verify OKR exists
    const okrCheck = await req.dbClient.query('SELECT id, overall_progress FROM okrs WHERE id = $1 AND tenant_id = $2', [okrId, tenantId]);
    if (okrCheck.rows.length === 0) {
        throw Errors.notFound('OKR');
    }
 ...
 */
router.get('/:id/progress-history', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const okrId = req.params['id'];
    const result = await req.dbClient.query(`
      SELECT
