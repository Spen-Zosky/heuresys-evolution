...
import type { ExportStatus } from '../services/data-export.js';
import { validate } from '../middleware/validate.js';
import {
  createExportConfigSchema,
  updateExportConfigSchema,
  createExportJobSchema,
  quickExportReportSchema,
  quickExportTableSchema,
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const { data_source, search } = req.query as Record<string, string>;

    const configs = await dataExportService.listConfigs(tenantId, {
      data_source: data_source as string,
...
router.post(
  '/configs',
  validate(createExportConfigSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const {
      name,
...
router.patch(
  '/configs/:id',
  validate(updateExportConfigSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'] as string;
...
router.post(
  '/report/:reportId',
  validate(quickExportReportSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
...
router.post(
  '/table/:tableName',
  validate(quickExportTableSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
