...
router.post(
  '/skills',
  validate(createCareerSkillSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const dbClient = req.dbClient!;
    const tenantId = getTenantIdOrThrow(req);
    const userId = getUserId(req);
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const params: (string | number)[] = [userId as string, tenantId];
...
router.post(
  '/goals',
  validate(createCareerGoalSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const dbClient = req.dbClient!;
    const tenantId = getTenantIdOrThrow(req);
    const userId = getUserId(req);
    ...
    if (milestones && Array.isArray(milestones) && milestones.length > 0) {
      ...
      const mTitles = milestones.map((m: any) => m.title);
      const mTypes = milestones.map((m: any) => m.type || 'action');
      const mDueDates = milestones.map((m: any) => m.due_date ?? null);
      const mSequences = milestones.map((_: any, i: number) => i);
...
router.post(
  '/goals/:id/milestones',
  validate(createGoalMilestoneSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const dbClient = req.dbClient!;
    const tenantId = getTenantIdOrThrow(req);
    const { id: goalId } = req.params as Record<string, string>;
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const grouped: Record<string, any[]> = {
      courses: recsResult.rows.filter((r: any) => r.type === 'course'),
      opportunities: recsResult.rows.filter(
        (r: any) => r.type === 'opportunity' || r.type === 'role'
      ),
      skills: recsResult.rows.filter((r: any) => r.type === 'skill'),
      mentors: recsResult.rows.filter((r: any) => r.type === 'mentor'),
    };
    ...
    if (recsResult.rows.length === 0) {
      ...
      const suggestedSkills: Record<string, any>[] = [];
      pathsResult.rows.forEach((path: Record<string, any>) => {
        const skills = path.required_skills || [];
        skills.forEach((skill: string) => {
          if (!suggestedSkills.find((s: Record<string, any>) => s.title === skill)) {
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const result = await dbClient.query(
      ...
      [tenantId, userId, rating, comment || `AI chat feedback: ${rating} for message ${message_id}`]
    );
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const dbClient = req.dbClient!;
    const userId = getUserId(req);
    const { message, employeeId, conversationHistory } = req.body as {
      message: string;
      employeeId?: string;
    ...
    if (empId) {
      ...
      const empResult = await dbClient.query(
        `SELECT e.first_name, e.last_name, e.job_title, d.name as department
      ...
      if (empResult.rows[0]) {
        ...
        contextParts.push(
          `PROFILO: ${emp.first_name} ${emp.last_name}, ${emp.job_title || 'N/A'}, Dipartimento: ${emp.department || 'N/A'}`
        );
      }

      // Current skills
      const skillsResult = await dbClient.query(
        `SELECT COALESCE(s.preferred_label_en, es.custom_skill_name) as skill_name,
                s.skill_type, es.proficiency_level
         FROM employee_skills es
         LEFT JOIN esco_skills s ON s.id = es.esco_skill_id
         WHERE es.employee_id = $1
         ORDER BY es.proficiency_level DESC NULLS LAST LIMIT 20`,
        [empId]
      );
        ...
        const skillList = skillsResult.rows
          .map(
            (s: Record<string, unknown>) =>
              `${s.skill_name} (${s.skill_type || 'skill'}, lv${s.proficiency_level || '?'})`
          )
          .join(', ');
      ...
      try {
        const recResult = await dbClient.query(
          `SELECT * FROM fn_employee_career_recommendations($1::uuid, 5)`,
          [empId]
        );
          ...
          const recs = recResult.rows
            .map(
              (r: Record<string, unknown>) =>
                `${r.occupation_label} (copertura ${Math.round(parseFloat(r.skill_coverage as string) * 100)}%, skill ${r.skills_held}/${r.total_essential})`
            )
            .join('; ');
      ...
      const occMatch = message.match(
        /(?:diventare|passare a|per|verso|come)\s+(?:un[ao]?\s+)?(.+?)(?:\?|$)/i
      );
      if (occMatch) {
        try {
          const occResult = await dbClient.query(
            `SELECT * FROM fn_find_matching_occupations($1, 'en', 3)`,
            [occMatch[1]!.trim()]
          );
          if (occResult.rows[0]) {
            const gapResult = await dbClient.query(
              `SELECT * FROM fn_skill_gap_analysis($1::uuid, $2)`,
              [empId, occResult.rows[0]?.uri]
            );
            if (gapResult.rows.length > 0) {
              const easy = gapResult.rows.filter(
                (r: Record<string, unknown>) => r.gap_difficulty === 'easy'
              );
              const moderate = gapResult.rows.filter(
                (r: Record<string, unknown>) => r.gap_difficulty === 'moderate'
              );
              const hard = gapResult.rows.filter(
                (r: Record<string, unknown>) => r.gap_difficulty === 'hard'
              );
              contextParts.push(
                `GAP ANALYSIS verso "${occResult.rows[0]?.preferred_label}": ` +
                  `Facili (${easy.length}): ${
                    easy
                      .slice(0, 5)
                      .map((r: Record<string, unknown>) => r.missing_skill_label)
                      .join(', ') || '-'
                  }; ` +
    ...
    if (/rischio|critich|concentrazione|vulnerabil/i.test(message)) {
      try {
        const riskResult = await dbClient.query(
          `SELECT COALESCE(s.preferred_label_en, es.custom_skill_name) AS skill_label,
           ...
           ORDER BY count(DISTINCT es.employee_id) ASC LIMIT 10`,
          [tenantId]
        );
        if (riskResult.rows.length > 0) {
          const risks = riskResult.rows
            .map((r: Record<string, unknown>) => `${r.skill_label} (${r.emp_count} persona/e)`)
            .join(', ');
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const enrollments = await req.dbClient!.query(
      `SELECT ce.id, ce.course_id, ce.status, ce.progress_percent,
    ...
    const inProgress = enrollments.rows.filter(
      (r: any) => r.status === 'in_progress' || r.status === 'enrolled'
    );
    const completed = enrollments.rows.filter((r: any) => r.status === 'completed');
