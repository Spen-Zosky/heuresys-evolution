...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const totalRated = ratingDist.rows.reduce((sum: number, r: any) => sum + parseInt(r.count), 0);
    const ratings = ratingDist.rows.map((r: Record<string, any>) => ({
      rating: r.rating,
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (review_type) {
      ...
      params.push(review_type as string);
      paramIndex++;
    }
    ...
    if (reviewer_id) {
      ...
      countParams.push(reviewer_id as string);
      ci++;
    }
    if (status) {
    ...
    if (review_type) {
      ...
      countParams.push(review_type as string);
    }
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const { employee_id, status, include_goals } = req.query as Record<string, string>;
    ...
    `;
    const params: (string | boolean)[] = [tenantId, employee_id as string];
    let paramIndex = 3;
    ...
    if (include_goals === 'true' && result.rows.length > 0) {
      const reviewIds = result.rows.map((r: Record<string, string>) => r.id);
      ...
      const goalsMap = new Map(
        goalsResult.rows.map((r: Record<string, string>) => [r.performance_review_id, r])
      );
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const { manager_id, status, review_cycle_id } = req.query as Record<string, string>;
    ...
    let query = `
      ...
      WHERE pr.tenant_id = $1
        AND e.manager_id = $2
    `;
    const params: (string | boolean)[] = [tenantId, manager_id as string];
    let paramIndex = 3;
    ...
    if (review_cycle_id) {
      ...
      params.push(review_cycle_id as string);
      paramIndex++;
    }
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const { manager_id, review_cycle_id } = req.query as Record<string, string>;
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const { review_cycle_id, org_unit_id } = req.query as Record<string, string>;

    const grid = await performanceManagementService.get9BoxGrid(
      tenantId,
      review_cycle_id as string | undefined
    );

    // If department filter, filter the results
    if (org_unit_id) {
      const filteredGrid: Record<string, Record<string, unknown>[]> = {};
      let total = 0;

      for (const [boxKey, employees] of Object.entries(grid.grid)) {
        const filtered = employees.filter(
          (e: Record<string, unknown>) => e.org_unit_id === org_unit_id
        );
 ...
 */
router.get(
  '/feedback-360/pending',
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const { rater_id } = req.query as Record<string, string>;
...
router.patch(
  ...
  validate(updateFeedback360Schema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    ...
    const feedbackCheck = await req.dbClient!.query(
      'SELECT id, status FROM feedback_360 WHERE id = $1 AND tenant_id = $2',
      [feedbackId, tenantId]
    );
    ...
    if (submit) {
      ...
      if (updates.length > 0) {
        ...
        await req.dbClient!.query(
          `UPDATE feedback_360 SET ${updates.join(', ')} WHERE id = $${paramIndex} AND tenant_id = $${paramIndex + 1} RETURNING *`,
          values
        );
      }

      const result = await req.dbClient!.query(
        `SELECT id, tenant_id, target_employee_id, reviewer_employee_id,
         ...
         FROM feedback_360 WHERE id = $1 AND tenant_id = $2`,
        [feedbackId, tenantId]
      );
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const frameworkResult = await req.dbClient!.query(
      `SELECT id, tenant_id, name, description, version, framework_type,
              is_active, created_at, updated_at
       FROM competency_frameworks WHERE id = $1 AND tenant_id = $2`,
    ...
    const competenciesResult = await req.dbClient!.query(
      'SELECT id, tenant_id, framework_id, name, description, category, behavioral_indicators, weight, sort_order, is_active, created_at, updated_at FROM competencies WHERE framework_id = $1 AND tenant_id = $2 ORDER BY category, sort_order',
      [frameworkId, tenantId]
    );
...
router.patch(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const existing = await req.dbClient!.query(
      'SELECT id FROM performance_reviews WHERE id = $1 AND tenant_id = $2',
      [id, tenantId]
    );
    ...
    const result = await req.dbClient!.query(
      `UPDATE performance_reviews SET ${updates.join(', ')} WHERE id = $${paramIndex} AND tenant_id = $${paramIndex + 1} RETURNING *`,
      [...values, id, tenantId]
    );
...
router.delete(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const result = await req.dbClient!.query(
      'DELETE FROM performance_reviews WHERE id = $1 AND tenant_id = $2 RETURNING id',
      [id, tenantId]
    );
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    // Get review details
    const reviewCheck = await req.dbClient!.query(
      'SELECT employee_id, review_period_start, review_period_end, goals_auto_populated FROM performance_reviews WHERE id = $1 AND tenant_id = $2',
      [reviewId, tenantId]
    );
    ...
    const { employee_id, review_period_start, review_period_end, goals_auto_populated } =
      reviewCheck.rows[0];
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const result = await req.dbClient!.query(
      'SELECT fn_auto_populate_goal_ratings($1, $2) as count',
      [tenantId, reviewId]
    );
...
router.put(
  '/:id/goals/:goalId',
  validate(updateGoalRatingSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const reviewId = req.params['id'] as string;
...
router.put(
  '/:id/self-assessment',
  validate(saveSelfAssessmentSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const reviewId = req.params['id'] as string;
    const {
    ...
    const reviewCheck = await req.dbClient!.query(
      'SELECT id, status, self_assessment_status, employee_id FROM performance_reviews WHERE id = $1 AND tenant_id = $2',
      [reviewId, tenantId]
    );
    ...
    const updateResult = await req.dbClient!.query(
      `UPDATE performance_reviews SET ${updates.join(', ')} WHERE id = $${paramIndex} AND tenant_id = $${paramIndex + 1} RETURNING *`,
      values
    );
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const reviewCheck = await req.dbClient!.query(
      'SELECT employee_id, self_assessment_status FROM performance_reviews WHERE id = $1 AND tenant_id = $2',
      [reviewId, tenantId]
    );
...
router.delete(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const reviewCheck = await req.dbClient!.query(
      'SELECT self_assessment_status FROM performance_reviews WHERE id = $1 AND tenant_id = $2',
      [reviewId, tenantId]
    );
    ...
    const result = await req.dbClient!.query(
      'DELETE FROM self_assessment_evidence WHERE id = $1 AND performance_review_id = $2 AND tenant_id = $3 RETURNING id',
      [evidenceId, reviewId, tenantId]
    );
...
router.put(
  '/:id/competencies',
  validate(updateCompetenciesSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const reviewId = req.params['id'] as string;
    ...
    res.json({
      ...
      data: {
        ...
        comparison: {
          ...
          gap:
            review.self_rating && review.overall_rating
...
router.put(
  '/:id/manager-review',
  validate(saveManagerReviewSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const reviewId = req.params['id'] as string;
    const {
    ...
    // Check review exists
    const reviewCheck = await req.dbClient!.query(
      'SELECT id, employee_id, manager_submitted_at FROM performance_reviews WHERE id = $1 AND tenant_id = $2',
      [reviewId, tenantId]
    );
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    // Get review
    const reviewResult = await req.dbClient!.query(
      `SELECT id, tenant_id, employee_id, reviewer_id, review_type, status,
                overall_rating, goal_achievement_rating, competency_rating,
                review_period_start, review_period_end, review_cycle_id,
                self_assessment_status, goals_count, competencies_count
         FROM performance_reviews WHERE id = $1 AND tenant_id = $2`,
      [reviewId, tenantId]
    );
    ...
    if (goalsCheck.rows[0].total > 0 && goalsCheck.rows[0].rated < goalsCheck.rows[0].total) {
      errors.push(
        `${goalsCheck.rows[0]?.total - goalsCheck.rows[0]?.rated} goals not rated by manager`
      );
    }
    ...
    const updateParams: unknown[] = [reviewId, tenantId];
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const currentReview = await req.dbClient!.query(
      'SELECT employee_id FROM performance_reviews WHERE id = $1 AND tenant_id = $2',
      [reviewId, tenantId]
    );
    ...
    const result = await req.dbClient!.query(
      ...
      LIMIT $4
    `,
      [employeeId, tenantId, reviewId, safeParseInt(limit as string, { fallback: 50 })]
    );
...
router.post(
  '/:id/self-review',
  validate(createSelfReviewSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const reviewId = req.params['id'] as string;
    const {
    ...
    // Verify review exists and get employee_id
    const reviewCheck = await req.dbClient!.query(
      'SELECT id, employee_id FROM performance_reviews WHERE id = $1 AND tenant_id = $2',
      [reviewId, tenantId]
    );
    if (reviewCheck.rows.length === 0) {
      throw Errors.notFound('Performance review');
    ...
    const existingCheck = await req.dbClient!.query(
      'SELECT id FROM self_reviews WHERE performance_review_id = $1',
      [reviewId]
    );
...
router.patch(
  '/:id/self-review',
  validate(updateSelfReviewSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const reviewId = req.params['id'] as string;

    const existingResult = await req.dbClient!.query(
      'SELECT id, status FROM self_reviews WHERE performance_review_id = $1 AND tenant_id = $2',
      [reviewId, tenantId]
    );
    ...
    const result = await req.dbClient!.query(
      `UPDATE self_reviews SET ${updates.join(', ')} WHERE id = $${paramIndex} RETURNING *`,
 ...
 */
router.post(
  '/:id/self-review/submit',
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const reviewId = req.params['id'] as string;

    const existingResult = await req.dbClient!.query(
      'SELECT id FROM self_reviews WHERE performance_review_id = $1 AND tenant_id = $2',
      [reviewId, tenantId]
    );
    if (existingResult.rows.length === 0) {
      throw Errors.notFound('Self-review');
    ...
    const selfReview = await performanceManagementService.submitSelfReview(
      tenantId,
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const reviewId = req.params['id'] as string;
    const { include_details } = req.query as Record<string, string>;

    // Get employee_id from review
    const reviewCheck = await req.dbClient!.query(
      'SELECT employee_id, review_period_start, review_period_end FROM performance_reviews WHERE id = $1 AND tenant_id = $2',
      [reviewId, tenantId]
    );
    if (reviewCheck.rows.length === 0) {
      throw Errors.notFound('Performance review');
    ...
    const summary = await performanceManagementService.get360FeedbackSummary(
      tenantId,
      employee_id,
      undefined // Could filter by review_cycle_id if needed
    );
 ...
 */
router.post(
  '/:id/feedback-360/request',
  validate(request360FeedbackSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const reviewId = req.params['id'] as string;
    ...
    const feedbackRequests = await performanceManagementService.request360Feedback(
      tenantId,
      reviewCheck.rows[0]?.employee_id,
      raters,
      review_cycle_id
 ...
 */
router.post(
  '/feedback-360/:feedbackId/decline',
  validate(decline360FeedbackSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const feedbackId = req.params['feedbackId'] as string;
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const reviewCheck = await req.dbClient!.query(
      'SELECT id, status, employee_id FROM performance_reviews WHERE id = $1 AND tenant_id = $2',
      [reviewId, tenantId]
    );
    if (reviewCheck.rows.length === 0) {
      throw Errors.notFound('Performance review');
    ...
    const selfReviewCheck = await req.dbClient!.query(
      "SELECT id FROM self_reviews WHERE performance_review_id = $1 AND status = 'submitted'",
      [reviewId]
    );
...
router.post(
  '/:id/acknowledge',
  validate(acknowledgeReviewSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const reviewId = req.params['id'] as string;
