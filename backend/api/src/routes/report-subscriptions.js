...
import { createReportSubscriptionSchema, updateReportSubscriptionSchema, } from '../schemas/report-subscriptions.js';
...
router.get('/', asyncHandler(async (req, res) => {
    ...
    const { report_id, user_id, is_active, page = '1', page_size = '20', } = req.query;
    const filters = {
        page: parseInt(page),
    ...
    const { subscriptions, total } = await reportSubscriptionsService.listSubscriptions(tenantId, filters);
    res.json({
        success: true,
        data: subscriptions,
        meta: {
 ...
 */
router.post('/', validate(createReportSubscriptionSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { report_id, name, description, schedule, delivery, parameters, filters, created_by } = req.body;
 ...
 * Update subscription
 */
router.patch('/:id', validate(updateReportSubscriptionSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'];
    const subscription = await reportSubscriptionsService.updateSubscription(tenantId, id, req.body);
    if (!subscription) {
        throw Errors.notFound('Subscription');
    }
    res.json({
...
router.get('/:id/history', asyncHandler(async (req, res) => {
    ...
    const history = await reportSubscriptionsService.getDeliveryHistory(tenantId, id, safeParseInt(limit, { fallback: 50 }));
