...
import { requireTenant, TenantRequest } from '../middleware/tenantContext.js';
...
router.get('/meta/types', (_req: Request, res: Response) => {
  const types = Object.entries(LEAVE_TYPES).map(([key, value]) => {
    const metadata = LEAVE_TYPE_METADATA[value as keyof typeof LEAVE_TYPE_METADATA];
    return {
      value,
...
router.get(
  '/balances',
  asyncHandler(async (req: Request, res: Response) => {
    const tenantReq = req as TenantRequest & AuthenticatedRequest;
    ...
    if (employeeId) {
      ...
      if (!['SUPERUSER', 'TENANT_OWNER', 'HR'].includes(userRole)) {
        ...
        if (employeeId !== authReq.user.employeeId) {
          ...
          const teamCheck = await req.dbClient!.query(
            'SELECT 1 FROM employees WHERE id = $1 AND manager_id = $2 AND tenant_id = $3',
            [employeeId, authReq.user.employeeId, tenantId]
          );
    ...
    const result = await req.dbClient!.query(
      ...
      [targetEmployeeId, tenantId, targetYear]
    );

    // Get employee info
    const employeeResult = await req.dbClient!.query(
      'SELECT first_name, last_name, hire_date FROM employees WHERE id = $1 AND tenant_id = $2',
    ...
    res.json({
      ...
      data: {
        ...
        balances: result.rows.map((row) => ({
          ...row,
          typeLabel:
            LEAVE_TYPE_METADATA[row.leave_type as keyof typeof LEAVE_TYPE_METADATA]?.label ||
            row.leave_type,
...
router.post(
  ...
  validate(initializeLeaveBalancesSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantReq = req as TenantRequest & AuthenticatedRequest;
    const tenantId = tenantReq.tenant.id;
    ...
    // Get CCNL defaults
    const ccnl = (ccnlType as CCNLType) || 'commercio';
    ...
    const results = await withTransaction(async (client) => {
      ...
      for (const lt of leaveTypes) {
        ...
        const upsertResult = await client.query(
          `INSERT INTO employee_time_off_balances
             ...
             RETURNING *`,
          [tenantId, employeeId, lt.type, targetYear, totalDays]
        );
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (['SUPERUSER', 'TENANT_OWNER', 'HR'].includes(userRole)) {
      ...
      const teamResult = await req.dbClient!.query(
        'SELECT id FROM employees WHERE manager_id = $1 AND tenant_id = $2',
        [authReq.user.employeeId, tenantId]
      );
      const teamIds = teamResult.rows.map((r) => r.id);

      if (employeeId) {
        // Specific employee - must be self or team member
        if (employeeId !== authReq.user.employeeId && !teamIds.includes(employeeId as string)) {
          throw createAppError('Access denied', 403, ErrorCodes.FORBIDDEN);
        }
        conditions.push(`r.employee_id = $${paramIndex++}`);
        params.push(employeeId);
    ...
    const countResult = await req.dbClient!.query(
      `SELECT COUNT(*) FROM employee_time_off_requests r WHERE ${whereClause}`,
      params
    );
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const input = req.body as LeaveRequestInput;

    // Validate required fields
    if (!input.leaveType || !Object.values(LEAVE_TYPES).includes(input.leaveType as LeaveType)) {
    ...
    if (input.halfDayStart) daysRequested -= 0.5;
    if (input.halfDayEnd) daysRequested -= 0.5;

    // Check balance for accrued types
    const typeMetadata = LEAVE_TYPE_METADATA[input.leaveType as keyof typeof LEAVE_TYPE_METADATA];
    if (typeMetadata?.accrued) {
      const balanceResult = await req.dbClient!.query(
        `SELECT (total_days + carryover_days + adjustment_days - used_days - pending_days) as available
         FROM employee_time_off_balances
         WHERE employee_id = $1 AND tenant_id = $2 AND leave_type = $3 AND year = $4`,
        [employeeId, tenantId, input.leaveType, startDate.getFullYear()]
      );

      if (balanceResult.rows.length === 0) {
        throw createAppError(
          'No leave balance found for this type',
          400,
      ...
      if (available < daysRequested) {
        throw createAppError(
          `Insufficient balance. Available: ${available} days, Requested: ${daysRequested} days`,
          400,
          ErrorCodes.VALIDATION_ERROR
        );
      }
    ...
    const overlapCheck = await req.dbClient!.query(
      `SELECT id FROM employee_time_off_requests
       WHERE employee_id = $1 AND tenant_id = $2
       AND status NOT IN ('rejected', 'cancelled')
       AND NOT (end_date < $3 OR start_date > $4)`,
      [employeeId, tenantId, input.startDate, input.endDate]
    );
    ...
    const result = await withTransaction(async (client) => {
      ...
      if (typeMetadata?.accrued) {
        await client.query(
          ...
          [daysRequested, employeeId, tenantId, input.leaveType, startDate.getFullYear()]
        );
      }
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    res.json({
      ...
      data: {
        ...(result.rows[0] || {}),
        typeLabel:
          LEAVE_TYPE_METADATA[result.rows[0]?.leave_type as keyof typeof LEAVE_TYPE_METADATA]
            ?.label,
      },
    });
  })
...
router.post(
  '/requests/:id/approve',
  checkPermission(PERMISSIONS.LEAVE_APPROVE),
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const requestResult = await req.dbClient!.query(
      `SELECT id, employee_id, leave_type, start_date, end_date, days_requested,
       ...
       FROM employee_time_off_requests WHERE id = $1 AND tenant_id = $2`,
      [id, tenantId]
    );
    ...
    const result = await withTransaction(async (client) => {
      ...
      const typeMetadata =
        LEAVE_TYPE_METADATA[request.leave_type as keyof typeof LEAVE_TYPE_METADATA];
      if (typeMetadata?.accrued) {
        ...
        await client.query(
          `UPDATE employee_time_off_balances
             SET pending_days = pending_days - $1,
                 ...
                 updated_at = NOW()
             WHERE employee_id = $2 AND tenant_id = $3 AND leave_type = $4 AND year = $5`,
          [request.days_requested, request.employee_id, tenantId, request.leave_type, year]
        );
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (request.status === 'approved') {
      ...
      if (startDate <= new Date()) {
        throw createAppError(
          'Cannot cancel a request that has already started',
          400,
          ErrorCodes.VALIDATION_ERROR
        );
      }
    ...
    const result = await withTransaction(async (client) => {
      ...
      const updateResult = await client.query(
        `UPDATE employee_time_off_requests
         ...
         WHERE id = $3 AND tenant_id = $4
         RETURNING *`,
        [reason || null, employeeId, id, tenantId]
      );
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    const tenantReq = req as TenantRequest & AuthenticatedRequest;
    const tenantId = tenantReq.tenant.id;
    const { startDate, endDate, orgUnitId, teamOnly } = req.query as Record<string, string>;
    ...
    const result = await req.dbClient!.query(
      `SELECT r.id, r.employee_id, r.leave_type, r.start_date, r.end_date, r.days_requested,
    ...
    const holidaysResult = await req.dbClient!.query(
      `SELECT date, name, holiday_type
       ...
       AND is_active = true
       ORDER BY date`,
      [tenantId, start.toISOString().split('T')[0], end.toISOString().split('T')[0]]
    );
    ...
    res.json({
      ...
      data: {
        ...
        leaves: result.rows.map((row) => ({
          ...row,
          typeLabel: LEAVE_TYPE_METADATA[row.leave_type as keyof typeof LEAVE_TYPE_METADATA]?.label,
        })),
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const result = await req.dbClient!.query(
      `SELECT id, tenant_id, name, date, holiday_type, is_recurring,
...
router.get(
  '/statistics',
  checkPermission(PERMISSIONS.LEAVE_VIEW_ALL, PERMISSIONS.LEAVE_VIEW_TEAM),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantReq = req as TenantRequest;
    const tenantId = tenantReq.tenant.id;
    const { year, orgUnitId } = req.query as Record<string, string>;

    const targetYear = year ? parseInt(year as string, 10) : new Date().getFullYear();

    let orgUnitCondition = '';
    const params: unknown[] = [tenantId, targetYear];
    const paramIndex = 3;
    ...
    const [
      ...
      req.dbClient!.query(
        `SELECT COUNT(*) FROM employees e WHERE tenant_id = $1 AND is_active = true ${orgUnitCondition}`,
      ...
      req.dbClient!.query(
        `SELECT COUNT(*)
           FROM employee_time_off_requests r
           JOIN employees e ON r.employee_id = e.id
           WHERE r.tenant_id = $1 AND r.status = 'pending' ${orgUnitCondition}`,
        [tenantId, ...(orgUnitId ? [orgUnitId] : [])]
      ),

      // Usage by type
      req.dbClient!.query(
        `SELECT r.leave_type, SUM(r.days_requested) as total_days, COUNT(*) as request_count
           FROM employee_time_off_requests r
           JOIN employees e ON r.employee_id = e.id
           WHERE r.tenant_id = $1
           AND r.status = 'approved'
      ...
      // Monthly usage
      req.dbClient!.query(
        `SELECT EXTRACT(MONTH FROM r.start_date) as month, SUM(r.days_requested) as total_days
           FROM employee_time_off_requests r
           JOIN employees e ON r.employee_id = e.id
           WHERE r.tenant_id = $1
           AND r.status = 'approved'
