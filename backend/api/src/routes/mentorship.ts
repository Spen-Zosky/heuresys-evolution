...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const [countResult, dataResult] = await Promise.all([
      ...
      req.dbClient!.query(
        `SELECT m.id, m.program_id, m.mentor_id, m.mentee_id, m.status,
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const countResult = await req.dbClient!.query(
      'SELECT COUNT(*) FROM mentorship_programs WHERE tenant_id = $1',
      [tenantId]
    );
...
router.post(
  '/programs',
  validate(createMentorshipProgramSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const {
      name,
      description,
      program_type = 'traditional',
      duration_months,
      max_participants,
      focus_areas,
...
router.patch(
  '/programs/:id',
  validate(updateMentorshipProgramSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'] as string;

    const existing = await req.dbClient!.query(
      'SELECT id FROM mentorship_programs WHERE id = $1 AND tenant_id = $2',
      [id, tenantId]
    );
    ...
    const result = await req.dbClient!.query(
      `UPDATE mentorship_programs SET ${updates.join(', ')}
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (mentor_id) {
      ...
      params.push(mentor_id as string);
    }
    if (mentee_id) {
      query += ` AND m.mentee_id = $${paramIndex++}`;
      params.push(mentee_id as string);
    }
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const programCheck = await req.dbClient!.query(
      'SELECT id FROM mentorship_programs WHERE id = $1 AND tenant_id = $2',
      [program_id, tenantId]
    );
...
router.patch(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const existing = await req.dbClient!.query(
      'SELECT id FROM mentorships WHERE id = $1 AND tenant_id = $2',
      [id, tenantId]
    );
    ...
    const result = await req.dbClient!.query(
      `UPDATE mentorships SET ${updates.join(', ')}
       WHERE id = $${paramIndex++} AND tenant_id = $${paramIndex}
       RETURNING *`,
      [...values, id, tenantId]
    );
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (mentorship_id) {
      ...
      params.push(mentorship_id as string);
    }
    if (status) {
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const mentorshipCheck = await req.dbClient!.query(
      'SELECT id FROM mentorships WHERE id = $1 AND tenant_id = $2',
      [mentorship_id, tenantId]
    );
...
router.patch(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const result = await req.dbClient!.query(
      `UPDATE mentorship_sessions SET ${updates.join(', ')} WHERE id = $${paramIndex} RETURNING *`,
      [...values, id]
    );
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const { org_unit_id, skill_id } = req.query as Record<string, string>;
    ...
    if (skill_id) {
      ...
      params.push(skill_id as string);
    }
