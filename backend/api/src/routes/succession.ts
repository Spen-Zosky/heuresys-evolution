...
import {
  createCriticalRoleSchema,
  updateCriticalRoleSchema,
  createSuccessionCandidateSchema,
  updateSuccessionCandidateSchema,
} from '../schemas/succession.js';
...
router.get(
  '/critical-roles',
  requirePermission('TALENT', 'VIEW'),
  applyScopeFilter('TALENT'),
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (succession_status) {
      ...
      params.push(succession_status as string);
      paramIndex++;
    }
    ...
    const countResult = await req.dbClient!.query(
      `SELECT COUNT(*) FROM critical_roles cr WHERE ${scope.where}`,
      scope.params
    );
...
router.post(
  '/critical-roles',
  requirePermission('TALENT', 'CREATE'),
  validate(createCriticalRoleSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const {
      role_name,
...
router.patch(
  '/critical-roles/:id',
  requirePermission('TALENT', 'EDIT'),
  validate(updateCriticalRoleSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'] as string;

    const existing = await req.dbClient!.query(
      'SELECT id FROM critical_roles WHERE id = $1 AND tenant_id = $2',
      [id, tenantId]
    );
    ...
    const result = await req.dbClient!.query(
      `UPDATE critical_roles SET ${updates.join(', ')} WHERE id = $${paramIndex} AND tenant_id = $${paramIndex + 1} RETURNING *`,
      [...values, id, tenantId]
    );
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const { readiness_level, limit = '100', offset = '0' } = req.query as Record<string, string>;
    ...
    if (readiness_level) {
      ...
      params.push(readiness_level as string);
      paramIndex++;
    }
...
router.post(
  '/candidates',
  requirePermission('TALENT', 'CREATE'),
  validate(createSuccessionCandidateSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const {
      critical_role_id,
      candidate_employee_id,
      readiness_level = 'ready_2_years',
      strengths,
      development_needs,
      development_plan,
      rank_order = 1,
    ...
    const roleCheck = await req.dbClient!.query(
      'SELECT id FROM critical_roles WHERE id = $1 AND tenant_id = $2',
      [critical_role_id, tenantId]
    );
...
router.patch(
  '/candidates/:id',
  requirePermission('TALENT', 'EDIT'),
  validate(updateSuccessionCandidateSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'] as string;
    ...
    const result = await req.dbClient!.query(
      `UPDATE succession_candidates SET ${updates.join(', ')} WHERE id = $${paramIndex} RETURNING *`,
      [...values, id]
    );
...
router.delete(
  '/candidates/:id',
  requirePermission('TALENT', 'DELETE'),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'] as string;
    ...
    await req.dbClient!.query(
      'DELETE FROM succession_candidates WHERE id = $1 AND critical_role_id IN (SELECT id FROM critical_roles WHERE tenant_id = $2)',
      [id, tenantId]
    );
...
router.delete(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const result = await req.dbClient!.query(
      'DELETE FROM critical_roles WHERE id = $1 AND tenant_id = $2 RETURNING id',
      [id, tenantId]
    );
