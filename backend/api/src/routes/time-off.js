...
import { createTimeOffRequestSchema, cancelTimeOffRequestSchema, rejectTimeOffRequestSchema, } from '../schemas/hr-operations-extended.js';
...
router.get('/balances', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { year, leave_type, limit = '100', offset = '0' } = req.query;
 ...
 */
router.get('/requests', requirePermission('TIME_ATTENDANCE', 'VIEW'), applyScopeFilter('TIME_ATTENDANCE'), asyncHandler(async (req, res) => {
    const scope = getScopeCondition(req, 'r');
    const { status, leave_type, year, limit = '50', offset = '0', } = req.query;
    ...
    const countResult = await req.dbClient.query(`SELECT COUNT(*) FROM employee_time_off_requests r WHERE ${countScope.where}`, countScope.params);
    res.json({
        success: true,
        data: result.rows,
        meta: {
 ...
 */
router.get('/by-leave-type', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { year } = req.query;
    const targetYear = year ? parseInt(year, 10) : new Date().getFullYear();
    const result = await req.dbClient.query(`
...
async function getEmployeeId(userId, tenantId, dbClient) {
    const result = await dbClient.query(`SELECT e.id FROM employees e
     JOIN users u ON u.employee_id = e.id
     WHERE u.id = $1 AND e.tenant_id = $2`, [userId, tenantId]);
    return result.rows[0]?.id || null;
}
/**
 * GET /time-off/my/balances - Get current user's leave balances
 */
router.get('/my/balances', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    ...
    const result = await req.dbClient.query(`SELECT b.id, b.employee_id, b.leave_type, b.total_days, b.used_days,
              b.pending_days, b.year, b.created_at, b.updated_at, b.tenant_id,
              b.carryover_days, b.carryover_expires_at, b.accrued_days,
              b.adjustment_days, b.adjustment_reason,
       ...
       ORDER BY b.leave_type`, [employeeId, tenantId, currentYear]);
...
router.get('/my/requests', asyncHandler(async (req, res) => {
    ...
    `;
    const params = [employeeId, tenantId, requestYear];
    if (status) {
 ...
 */
router.post('/my/requests', validate(createTimeOffRequestSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const userId = req.user?.userId;
    if (!userId) {
        throw Errors.unauthorized('Unauthorized');
    ...
    const { leave_type, start_date, end_date, reason, half_day_start = false, half_day_end = false, } = req.body;
    ...
    if (half_day_end)
        days -= 0.5;
    // Check balance
    const balanceResult = await req.dbClient.query(`SELECT id, employee_id, leave_type, total_days, used_days, pending_days, year,
              created_at, updated_at, tenant_id, carryover_days, carryover_expires_at,
              accrued_days, adjustment_days, adjustment_reason
       FROM employee_time_off_balances
       WHERE employee_id = $1 AND tenant_id = $2 AND leave_type = $3 AND year = $4`, [employeeId, tenantId, leave_type, new Date().getFullYear()]);
    if (balanceResult.rows.length > 0) {
        const balance = balanceResult.rows[0];
        const available = parseFloat(balance.total_days || 0) +
    ...
    // Create request
    const result = await req.dbClient.query(`INSERT INTO employee_time_off_requests (
    ...
    await req.dbClient.query(`UPDATE employee_time_off_balances
       SET pending_days = COALESCE(pending_days, 0) + $1, updated_at = NOW()
       WHERE employee_id = $2 AND tenant_id = $3 AND leave_type = $4 AND year = $5`, [days, employeeId, tenantId, leave_type, new Date().getFullYear()]);
    // Create approval step
    if (approverId) {
        await req.dbClient.query(`INSERT INTO leave_approval_steps (
          tenant_id, request_id, step_order, approver_id, approver_type, status
        ) VALUES ($1, $2, 1, $3, 'manager', 'pending')`, [tenantId, result.rows[0]?.id, approverId]);
    }
 ...
 */
router.post('/my/requests/:id/cancel', validate(cancelTimeOffRequestSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const userId = req.user?.userId;
    const id = req.params['id'];
    ...
    const requestResult = await req.dbClient.query(`SELECT id, employee_id, leave_type, start_date, end_date, days_requested,
       ...
       WHERE id = $1 AND employee_id = $2 AND tenant_id = $3`, [id, employeeId, tenantId]);
    ...
    await req.dbClient.query(`UPDATE employee_time_off_requests
       SET status = 'cancelled', cancelled_at = NOW(), cancelled_by = $1,
           cancellation_reason = $2, updated_at = NOW()
       WHERE id = $3 AND tenant_id = $4`, [employeeId, reason, id, tenantId]);
    // Return pending days to balance
    await req.dbClient.query(`UPDATE employee_time_off_balances
       SET pending_days = GREATEST(0, COALESCE(pending_days, 0) - $1), updated_at = NOW()
       WHERE employee_id = $2 AND tenant_id = $3 AND leave_type = $4 AND year = $5`, [request.days_requested, employeeId, tenantId, request.leave_type, new Date().getFullYear()]);
...
router.get('/leave-types', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const result = await req.dbClient.query(`SELECT leave_type, name, description, allow_carryover, max_carryover_days
...
router.get('/my/calendar', asyncHandler(async (req, res) => {
    ...
    const empResult = await req.dbClient.query(`SELECT org_unit_id FROM employees WHERE id = $1 AND tenant_id = $2`, [employeeId, tenantId]);
    const orgUnitId = empResult.rows[0]?.org_unit_id;
    // Get team time-off for the month
    const result = await req.dbClient.query(`SELECT r.id, r.leave_type, r.start_date, r.end_date, r.status,
       ...
       ORDER BY r.start_date`, [tenantId, orgUnitId, targetMonth, targetYear]);
    res.json({
        success: true,
        data: {
            month: targetMonth,
 ...
 */
router.get('/team-requests', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const userId = req.user?.userId;
    if (!userId) {
        throw Errors.unauthorized('Unauthorized');
    ...
    const result = await req.dbClient.query(`SELECT r.id, r.employee_id, r.leave_type, r.start_date, r.end_date,
       ...
       ORDER BY r.created_at ASC`, [tenantId, employeeId, status]);
    res.json({ success: true, data: result.rows });
}));
/**
...
router.post('/requests/:id/approve', asyncHandler(async (req, res) => {
    ...
    const requestResult = await req.dbClient.query(`SELECT id, employee_id, leave_type, start_date, end_date, days_requested,
              ...
              cancellation_requested, cancellation_reason, cancelled_at, cancelled_by
       FROM employee_time_off_requests
       WHERE id = $1 AND approver_id = $2 AND tenant_id = $3 AND status = 'pending'`, [id, employeeId, tenantId]);
    if (requestResult.rows.length === 0) {
        throw Errors.notFound('Request');
    }
    const request = requestResult.rows[0];
    ...
    await req.dbClient.query(`UPDATE leave_approval_steps
       SET status = 'approved', decision_at = NOW()
       WHERE request_id = $1 AND approver_id = $2 AND tenant_id = $3`, [id, employeeId, tenantId]);
    res.json({ success: true });
}));
/**
 * POST /time-off/requests/:id/reject - Reject request (manager)
 */
router.post('/requests/:id/reject', validate(rejectTimeOffRequestSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const userId = req.user?.userId;
    const id = req.params['id'];
    const { reason } = req.body;
    ...
    await req.dbClient.query(`UPDATE employee_time_off_requests
       ...
       WHERE id = $2 AND tenant_id = $3`, [reason, id, tenantId]);
    // Return pending days to balance
    await req.dbClient.query(`UPDATE employee_time_off_balances
       SET pending_days = GREATEST(0, COALESCE(pending_days, 0) - $1), updated_at = NOW()
       WHERE employee_id = $2 AND tenant_id = $3 AND leave_type = $4 AND year = $5`, [
    ...
    await req.dbClient.query(`UPDATE leave_approval_steps
       ...
       WHERE request_id = $2 AND approver_id = $3 AND tenant_id = $4`, [reason, id, employeeId, tenantId]);
    res.json({ success: true });
}));
