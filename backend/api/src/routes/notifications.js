...
import { createNotificationSchema, markAllReadSchema, updateNotificationPreferencesSchema, } from '../schemas/engagement.js';
...
router.get('/', asyncHandler(async (req, res) => {
    ...
    const [countResult, dataResult] = await Promise.all([
        req.dbClient.query(`SELECT COUNT(*) as total FROM notifications n WHERE n.tenant_id = $1 ${whereClause}`, [tenantId]),
        req.dbClient.query(`SELECT n.id, n.type, n.title, n.message, n.priority, n.read, n.read_at,
...
router.get('/', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { user_id, type, priority, read, limit = '100', offset = '0', } = req.query;
    let query = `
      SELECT n.id, n.type, n.title, n.message, n.priority, n.read, n.read_at,
             n.user_id, n.tenant_id, n.action_url, n.action_label, n.metadata,
             n.created_at, n.expires_at
    ...
    const result = await req.dbClient.query(query, params);
    const countResult = await req.dbClient.query('SELECT COUNT(*) FROM notifications WHERE tenant_id = $1', [tenantId]);
    res.json({
        success: true,
        data: result.rows,
        meta: {
 ...
 */
router.post('/mark-all-read', validate(markAllReadSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
...
router.get('/my', asyncHandler(async (req, res) => {
    ...
    params.push(safeParseInt(limit, { fallback: 20 }), safeParseInt(offset, { fallback: 0 }));
    const result = await req.dbClient.query(query, params);
    // Get unread count
    const countResult = await req.dbClient.query(`SELECT COUNT(*) as count FROM notifications
 ...
 */
router.get('/my/unread-count', asyncHandler(async (req, res) => {
    const userId = req.user?.userId;
    if (!userId) {
        throw Errors.unauthorized('Unauthorized');
    }
    const result = await req.dbClient.query(`SELECT COUNT(*) as count FROM notifications
       WHERE user_id = $1 AND read = FALSE
       AND (expires_at IS NULL OR expires_at > NOW())`, [userId]);
    res.json({
        success: true,
 ...
 */
router.get('/my/preferences', asyncHandler(async (req, res) => {
    const userId = req.user?.userId;
    if (!userId) {
        throw Errors.unauthorized('Unauthorized');
    }
    const result = await req.dbClient.query(`SELECT id, user_id, email_enabled, in_app_enabled, goal_reminders,
 ...
 */
router.put('/my/preferences', validate(updateNotificationPreferencesSchema), asyncHandler(async (req, res) => {
    const userId = req.user?.userId;
    if (!userId) {
        throw Errors.unauthorized('Unauthorized');
    }
    const { email_enabled, in_app_enabled, goal_reminders, review_reminders, flight_risk_alerts, checkin_reminders, survey_notifications, recognition_notifications, system_notifications, quiet_hours_start, quiet_hours_end, mentorship_notifications, mobility_notifications, wellbeing_notifications, } = req.body;
    const result = await req.dbClient.query(`
 ...
 */
router.post('/my/:id/read', asyncHandler(async (req, res) => {
    const userId = req.user?.userId;
    const id = req.params['id'];
    if (!userId) {
        throw Errors.unauthorized('Unauthorized');
    }
    await req.dbClient.query(`UPDATE notifications SET read = TRUE, read_at = NOW()
 ...
 */
router.post('/my/mark-all-read', asyncHandler(async (req, res) => {
    const userId = req.user?.userId;
    if (!userId) {
        throw Errors.unauthorized('Unauthorized');
    }
    const result = await req.dbClient.query(`UPDATE notifications SET read = TRUE, read_at = NOW()
 ...
 * POST /notifications
 */
router.post('/', validate(createNotificationSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { type, title, message, priority = 'normal', user_id, action_url, action_label, metadata, expires_at, } = req.body;
...
router.delete('/:id', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'];
    const result = await req.dbClient.query('DELETE FROM notifications WHERE id = $1 AND tenant_id = $2 RETURNING id', [id, tenantId]);
    if (result.rows.length === 0) {
        throw Errors.notFound('Notification');
    }
 ...
 */
router.put('/preferences/:userId', validate(updateNotificationPreferencesSchema), asyncHandler(async (req, res) => {
    const userId = req.params['userId'];
    const { email_enabled, in_app_enabled, goal_reminders, review_reminders, flight_risk_alerts, checkin_reminders, survey_notifications, recognition_notifications, system_notifications, quiet_hours_start, quiet_hours_end, mentorship_notifications, mobility_notifications, wellbeing_notifications, } = req.body;
    // Upsert preferences
    const result = await req.dbClient.query(`
