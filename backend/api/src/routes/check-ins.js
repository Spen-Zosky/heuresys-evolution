...
import { createCheckInSchema, updateCheckInSchema, completeCheckInSchema, } from '../schemas/performance.js';
...
router.get('/upcoming', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { employee_id, manager_id, days = '7' } = req.query;
    let query = `
      SELECT ci.*,
...
router.get('/', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { employee_id, manager_id, status, meeting_type, limit = '100', offset = '0', } = req.query;
    let query = `
      SELECT ci.*,
        e.first_name || ' ' || e.last_name as employee_name,
        m.first_name || ' ' || m.last_name as manager_name
...
router.post('/', validate(createCheckInSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { employee_id, manager_id, scheduled_date, duration_minutes = 30, meeting_type = 'one_on_one', agenda, employee_notes, manager_notes, action_items, employee_mood, status = 'scheduled', } = req.body;
 ...
 * PATCH /check-ins/:id
 */
router.patch('/:id', validateUUID(), validate(updateCheckInSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'];
    const existing = await req.dbClient.query('SELECT id FROM check_ins WHERE id = $1 AND tenant_id = $2', [id, tenantId]);
    ...
    const result = await req.dbClient.query(`UPDATE check_ins SET ${updates.join(', ')} WHERE id = $${paramIndex} AND tenant_id = $${paramIndex + 1} RETURNING *`, [...values, id, tenantId]);
    res.json({ success: true, data: result.rows[0] || null, message: 'Check-in updated' });
}));
/**
 * POST /check-ins/:id/complete
 */
router.post('/:id/complete', validateUUID(), validate(completeCheckInSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'];
...
router.delete('/:id', validateUUID(), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'];
    const result = await req.dbClient.query('DELETE FROM check_ins WHERE id = $1 AND tenant_id = $2 RETURNING id', [id, tenantId]);
    if (result.rows.length === 0) {
        throw Errors.notFound('Check-in');
    }
...
//# sourceMappingURL=check-ins.js.map