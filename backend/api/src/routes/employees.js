...
import { createEmployeeSchema, updateEmployeeSchema, archiveEmployeeSchema, updateSelfEmployeeSchema, } from '../schemas/employees.js';
import { asyncHandler } from '../errors/middleware.js';
import { safeParseInt } from '../utils/query-helpers.js';
import { applyFieldPolicy, applyFieldPolicyAllDb, loadPolicyForRole, } from '../utils/field-policy.js';
const router = Router();
 ...
 */
router.get('/analytics-stats', checkPermission(PERMISSIONS.EMPLOYEES_VIEW_ALL), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    // Run all queries in parallel for performance
    const [headcountResult, tenureResult, costCenterResult, departmentProductivity, turnoverResult,] = await Promise.all([
        // Basic headcount
        req.dbClient.query('SELECT COUNT(*) as total FROM employees WHERE tenant_id = $1 AND is_active = true', [tenantId]),
        // Average tenure
        req.dbClient.query(`
    ...
    const totalBudget = costCenterResult.rows.reduce((sum, cc) => sum + (parseFloat(cc.amount) || 0), 0);
    const costAnalysis = costCenterResult.rows.map((cc, idx) => ({
    ...
    const productivityMetrics = departmentProductivity.rows.map((d) => ({
 ...
 * NOTE: Must be before /:id routes
 */
router.get('/stats', checkPermission(PERMISSIONS.EMPLOYEES_VIEW_ALL), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const [totalResult, activeResult, departmentStats, locationStats, recentHires, ageStats, genderStats, nationalityStats,] = await Promise.all([
        req.dbClient.query('SELECT COUNT(*) FROM employees WHERE tenant_id = $1', [tenantId]),
        req.dbClient.query('SELECT COUNT(*) FROM employees WHERE tenant_id = $1 AND is_active = true', [tenantId]),
        req.dbClient.query(`
    ...
    const byAge = ageStats.rows.map((row) => ({
        range: row.range,
        count: parseInt(row.count),
        percentage: total > 0 ? Math.round((parseInt(row.count) / total) * 100) : 0,
    }));
    const byGender = genderStats.rows.map((row) => ({
        gender: row.gender,
        count: parseInt(row.count),
        percentage: total > 0 ? Math.round((parseInt(row.count) / total) * 100) : 0,
    }));
    const byNationality = nationalityStats.rows.map((row) => ({
        country: row.country,
        count: parseInt(row.count),
    }));
    res.json({
 ...
 */
router.get('/', requirePermission('CORE_HR', 'VIEW'), applyScopeFilter('CORE_HR'), asyncHandler(async (req, res) => {
    getTenantIdOrThrow(req);
    // Pagination
    const page = Math.max(1, safeParseInt(req.query['page'], { fallback: 1 }));
    const limit = Math.min(500, Math.max(1, safeParseInt(req.query['limit'], { fallback: 20 })));
    ...
    // Get total count
    const countResult = await req.dbClient.query(`SELECT COUNT(*) as total FROM employees e ${whereClause}`, params);
    const total = parseInt(countResult.rows[0]?.total);
    // Get employees with resolved names for departments, locations, cost centers
    const employeesResult = await req.dbClient.query(`SELECT
    ...
    const tenantIdForPolicy = req.user?.tenantId || null;
    const filteredEmployees = await applyFieldPolicyAllDb(employeesResult.rows, 'employees', policyMap, tenantIdForPolicy);
    res.json({
        success: true,
        data: {
...
router.get('/me', asyncHandler(async (req, res) => {
    ...
    const leaveBalances = await req.dbClient.query(`SELECT leave_type, balance AS balance_remaining, (balance + used_days) AS balance_total
       FROM leave_balances
       WHERE employee_id = $1 AND year = EXTRACT(YEAR FROM CURRENT_DATE)`, [employeeId]);
    res.json({
        success: true,
        data: {
 ...
 */
router.patch('/me', validate(updateSelfEmployeeSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const authReq = req;
    const employeeId = authReq.user.employeeId;
    if (!employeeId) {
    ...
    if (updates.length === 0) {
        throw createAppError('No valid fields to update. Only personal contact information can be self-updated.', 400, ErrorCodes.VALIDATION_ERROR);
    }
    ...
    const result = await req.dbClient.query(`UPDATE employees
       ...
       RETURNING id, first_name, last_name, phone_mobile, phone_home, personal_email,
                 ...
                 iban, bank_name`, [...values, employeeId, tenantId]);
    if (result.rows.length === 0) {
        throw createAppError('Employee profile not found', 404, ErrorCodes.NOT_FOUND);
    }
    res.json({
...
router.get('/me/documents', asyncHandler(async (req, res) => {
    ...
    const tableCheck = await req.dbClient.query(`SELECT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'employee_documents')`);
    ...
    const result = await req.dbClient.query(`SELECT id, document_type, filename, file_size, upload_date, description
       FROM employee_documents
       WHERE employee_id = $1 AND tenant_id = $2
       ORDER BY upload_date DESC`, [employeeId, tenantId]);
    res.json({
        success: true,
        data: result.rows,
    });
 ...
 */
router.get('/me/team', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const authReq = req;
    const employeeId = authReq.user.employeeId;
    if (!employeeId) {
    ...
    const result = await req.dbClient.query(`SELECT
       ...
       ORDER BY last_name, first_name`, [employeeId, tenantId]);
    res.json({
        success: true,
        data: {
 ...
 */
router.get('/me/contracts', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const authReq = req;
    const employeeId = authReq.user.employeeId;
    if (!employeeId) {
    ...
    const result = await req.dbClient.query(`SELECT c.id, c.employee_id, c.contract_type, c.contract_code,
 ...
 */
router.get('/meta/termination-reasons', (_req, res) => {
    const reasons = [
 ...
 */
router.get('/meta/employment-statuses', (_req, res) => {
    const statuses = [
 ...
 */
router.get('/:id', checkPermission(PERMISSIONS.EMPLOYEES_VIEW_ALL), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
 ...
 * Create new employee
 */
router.post('/', requirePermission('CORE_HR', 'CREATE'), validate(createEmployeeSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { first_name, last_name, email, job_title, org_unit_id, location_id, manager_id, hire_date, } = req.body;
    // Validation
    if (!first_name || !last_name || !email) {
        throw createAppError('Missing required fields: first_name, last_name, email', 400, ErrorCodes.VALIDATION_ERROR);
    }
    // Check email uniqueness within tenant
    const existingEmail = await req.dbClient.query('SELECT id FROM employees WHERE email = $1 AND tenant_id = $2', [email, tenantId]);
    if (existingEmail.rows.length > 0) {
        throw createAppError('Email already exists', 409, ErrorCodes.ALREADY_EXISTS);
    }
    const result = await req.dbClient.query(`INSERT INTO employees (
 ...
 * Update employee
 */
router.patch('/:id', requirePermission('CORE_HR', 'EDIT'), validate(updateEmployeeSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'];
    if (!id || !isValidUUID(id)) {
        throw createAppError('Invalid employee ID format', 400, ErrorCodes.INVALID_INPUT);
    ...
    const existing = await req.dbClient.query('SELECT id FROM employees WHERE id = $1 AND tenant_id = $2', [id, tenantId]);
 ...
 */
router.delete('/:id', requirePermission('CORE_HR', 'DELETE'), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'];
    ...
    if (hardDelete) {
        ...
        const result = await req.dbClient.query('DELETE FROM employees WHERE id = $1 AND tenant_id = $2 RETURNING id', [id, tenantId]);
        if (result.rows.length === 0) {
            throw createAppError('Employee not found', 404, ErrorCodes.NOT_FOUND);
        }
        res.json({
 ...
 */
router.get('/:id/direct-reports', checkPermission(PERMISSIONS.EMPLOYEES_VIEW_ALL), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'];
    if (!id || !isValidUUID(id)) {
        throw createAppError('Invalid employee ID format', 400, ErrorCodes.INVALID_INPUT);
 ...
 */
router.post('/:id/reactivate', requirePermission('CORE_HR', 'EDIT'), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'];
    if (!id || !isValidUUID(id)) {
        throw createAppError('Invalid employee ID format', 400, ErrorCodes.INVALID_INPUT);
    ...
    const existing = await req.dbClient.query('SELECT id, is_active, first_name, last_name FROM employees WHERE id = $1 AND tenant_id = $2', [id, tenantId]);
    if (existing.rows.length === 0) {
        throw createAppError('Employee not found', 404, ErrorCodes.NOT_FOUND);
    }
    ...
    await withTransaction(async (client) => {
        ...
        await client.query('UPDATE tenants SET employee_count = (SELECT COUNT(*) FROM employees WHERE tenant_id = $1 AND is_active = true) WHERE id = $1', [tenantId]);
    }, tenantId);
    res.json({
        success: true,
 ...
 */
router.post('/:id/archive', checkPermission(PERMISSIONS.EMPLOYEES_UPDATE), validate(archiveEmployeeSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'];
    ...
    // Check employee exists
    const existing = await req.dbClient.query('SELECT id, first_name, last_name, is_active FROM employees WHERE id = $1 AND tenant_id = $2', [id, tenantId]);
    if (existing.rows.length === 0) {
        throw createAppError('Employee not found', 404, ErrorCodes.NOT_FOUND);
    }
    if (!existing.rows[0].is_active) {
    ...
    await withTransaction(async (client) => {
        await client.query(`UPDATE employees SET
           ...
           updated_at = NOW()
         WHERE id = $1 AND tenant_id = $2`, [id, tenantId, termination_date, `${reason}${notes ? ' - ' + notes : ''}`]);
        // Update tenant employee count
        await client.query('UPDATE tenants SET employee_count = (SELECT COUNT(*) FROM employees WHERE tenant_id = $1 AND is_active = true) WHERE id = $1', [tenantId]);
    }, tenantId);
    res.json({
 ...
 */
router.get('/:id/org-chart', checkPermission(PERMISSIONS.EMPLOYEES_VIEW_ALL), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'];
    const depth = Math.min(5, safeParseInt(req.query['depth'], { fallback: 2 }));
    if (!id || !isValidUUID(id)) {
        throw createAppError('Invalid employee ID format', 400, ErrorCodes.INVALID_INPUT);
    }
    const employeeResult = await req.dbClient.query('SELECT id, tenant_id, first_name, last_name, job_title, manager_id FROM employees WHERE id = $1 AND tenant_id = $2', [id, tenantId]);
    if (employeeResult.rows.length === 0) {
        throw createAppError('Employee not found', 404, ErrorCodes.NOT_FOUND);
    }
    const employee = employeeResult.rows[0];
    // Get manager chain (upward) — single CTE instead of N+1 loop
    const managerChainResult = await req.dbClient.query(`WITH RECURSIVE manager_chain AS (
      ...
      FROM manager_chain
      ORDER BY chain_depth DESC`, [id, tenantId, employee.manager_id]);
    ...
    const reportsResult = await req.dbClient.query(`WITH RECURSIVE report_tree AS (
      ...
      FROM report_tree
      ORDER BY tree_depth, last_name, first_name`, [id, tenantId, depth]);
