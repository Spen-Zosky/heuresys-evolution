...
let onetService: ONetImportService | null = null;

function getONetService(): ONetImportService {
  if (!onetService) {
 ...
 */
router.post(
  '/import/jobs/:jobId/execute',
  validate(onetImportExecuteSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const jobId = req.params.jobId as string;
    ...
    switch (job.import_type) {
      ...
      default:
        throw Errors.badRequest(
          `Import type ${job.import_type} not supported for direct execution`
        );
    }
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const offset = safeParseInt(req.query.offset as string, { fallback: 0, min: 0 });
    const jobZone = req.query.job_zone ? parseInt(req.query.job_zone as string) : undefined;
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const result = await dbClient.query(
      `SELECT id, onet_soc_code, title, description, job_zone,
              related_experience, education_required, on_job_training,
              source_version, created_at, updated_at
       FROM onet_occupations WHERE id::text = $1 OR onet_soc_code = $1`,
      [id]
    );
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const occResult = await dbClient.query(
      `SELECT id FROM onet_occupations WHERE id::text = $1 OR onet_soc_code = $1`,
      [id]
    );
 ...
 */
router.get(
  '/skills/:id/occupations',
  asyncHandler(async (req: Request, res: Response) => {
    const dbClient = req.dbClient!;
    const id = req.params.id as string;
    const minImportance = safeParseInt(req.query.min_importance as string, { fallback: 50 });

    // Get skill ID if element_id was passed
    const skillResult = await dbClient.query(
      `SELECT id FROM onet_skills WHERE id::text = $1 OR element_id = $1`,
      [id]
    );
...
router.get(
  '/unified-skills',
  asyncHandler(async (req: Request, res: Response) => {
    const limit = safeParseInt(req.query.limit as string, { fallback: 100, max: 500 });
    const offset = safeParseInt(req.query.offset as string, { fallback: 0, min: 0 });
    const sources = (req.query.sources as string)?.split(',') || [
      'esco',
      'onet_skill',
      'onet_ability',
      'onet_knowledge',
