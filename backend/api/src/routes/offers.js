...
import { createOfferSchema, updateOfferSchema, approveOfferSchema, declineOfferSchema, } from '../schemas/compensation-extended.js';
...
router.get('/', requirePermission('RECRUITMENT', 'VIEW'), applyScopeFilter('RECRUITMENT'), asyncHandler(async (req, res) => {
    ...
    const { candidate_id, requisition_id, status, limit = '100', offset = '0', } = req.query;
  ...
  `;
    ...
    const countResult = await req.dbClient.query(`SELECT COUNT(*) FROM recruiting_offers o WHERE ${scope.where}`, scope.params);
    res.json({
        success: true,
        data: result.rows,
        meta: {
 ...
 * POST /offers
 */
router.post('/', requirePermission('RECRUITMENT', 'CREATE'), validate(createOfferSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { candidate_id, requisition_id, salary_offered, currency = 'EUR', bonus_offered, equity_offered, start_date, expiry_date, job_title, department, location, employment_type, benefits, notes, } = req.body;
    ...
    const candidateCheck = await req.dbClient.query('SELECT id FROM recruiting_candidates WHERE id = $1 AND tenant_id = $2', [candidate_id, tenantId]);
 ...
 * PATCH /offers/:id
 */
router.patch('/:id', requirePermission('RECRUITMENT', 'EDIT'), validate(updateOfferSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'];
    const existing = await req.dbClient.query('SELECT id, status FROM recruiting_offers WHERE id = $1 AND tenant_id = $2', [id, tenantId]);
    ...
    const result = await req.dbClient.query(`UPDATE recruiting_offers SET ${updates.join(', ')} WHERE id = $${paramIndex} AND tenant_id = $${paramIndex + 1} RETURNING *`, [...values, id, tenantId]);
    res.json({ success: true, data: result.rows[0] || null, message: 'Offer updated' });
}));
/**
 * POST /offers/:id/approve
 */
router.post('/:id/approve', requirePermission('RECRUITMENT', 'APPROVE'), validate(approveOfferSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'];
 ...
 * POST /offers/:id/send
 */
router.post('/:id/send', requirePermission('RECRUITMENT', 'EDIT'), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'];
    ...
    `, [id, tenantId]);
        ...
        await client.query("UPDATE recruiting_candidates SET stage = 'offer', last_activity = NOW() WHERE id = $1", [updateResult.rows[0]?.candidate_id]);
 ...
 */
router.post('/:id/accept', requirePermission('RECRUITMENT', 'APPROVE'), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'];
    ...
    `, [id, tenantId]);
        ...
        await client.query("UPDATE recruiting_candidates SET stage = 'hired', last_activity = NOW() WHERE id = $1", [updateResult.rows[0]?.candidate_id]);
 ...
 */
router.post('/:id/decline', requirePermission('RECRUITMENT', 'APPROVE'), validate(declineOfferSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'];
...
router.delete('/:id', requirePermission('RECRUITMENT', 'DELETE'), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'];
    const result = await req.dbClient.query("DELETE FROM recruiting_offers WHERE id = $1 AND tenant_id = $2 AND status = 'draft' RETURNING id", [id, tenantId]);
    if (result.rows.length === 0) {
