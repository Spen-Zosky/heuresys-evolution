...
import { createRequisitionSchema, updateRequisitionSchema, } from '../schemas/compensation-extended.js';
...
router.get('/', requirePermission('RECRUITMENT', 'VIEW'), applyScopeFilter('RECRUITMENT'), asyncHandler(async (req, res) => {
    ...
    const { status, priority, department, hiring_manager_id, limit = '100', offset = '0', } = req.query;
    let query = `
    SELECT r.*,
  ...
  `;
    ...
    const countResult = await req.dbClient.query(`SELECT COUNT(*) FROM recruiting_requisitions r WHERE ${countScope.where}`, countScope.params);
    res.json({
        success: true,
        data: result.rows,
        meta: {
 ...
 * POST /requisitions
 */
router.post('/', requirePermission('RECRUITMENT', 'CREATE'), validate(createRequisitionSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { title, department, location, employment_type, priority = 'normal', description, requirements, salary_min, salary_max, currency = 'EUR', hiring_manager_id, recruiter_id, target_hire_date, headcount = 1, } = req.body;
    if (!title) {
 ...
 */
router.patch('/:id', requirePermission('RECRUITMENT', 'EDIT'), validate(updateRequisitionSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'];
    const existing = await req.dbClient.query('SELECT id FROM recruiting_requisitions WHERE id = $1 AND tenant_id = $2', [id, tenantId]);
    ...
    const result = await req.dbClient.query(`UPDATE recruiting_requisitions SET ${updates.join(', ')} WHERE id = $${paramIndex} AND tenant_id = $${paramIndex + 1} RETURNING *`, [...values, id, tenantId]);
...
router.delete('/:id', requirePermission('RECRUITMENT', 'DELETE'), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'];
    const result = await req.dbClient.query(`UPDATE recruiting_requisitions SET status = 'cancelled', closed_at = NOW(), updated_at = NOW() WHERE id = $1 AND tenant_id = $2 RETURNING id`, [id, tenantId]);
    if (result.rows.length === 0) {
        throw Errors.notFound('Requisition', id);
    }
