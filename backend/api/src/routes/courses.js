 ...
 */
router.get('/talent-stats', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    ...
    const topGapSkills = (gapMetrics.top_gap_skills || []).map((skill, idx) => ({
        skill,
...
router.get('/categories', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const result = await req.dbClient.query('SELECT DISTINCT category FROM courses WHERE tenant_id = $1 AND category IS NOT NULL ORDER BY category LIMIT 100', [tenantId]);
...
router.get('/', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { status, category, skill_level, is_mandatory, search, limit = '100', offset = '0', } = req.query;
    ...
    const result = await req.dbClient.query(query, params);
    const countResult = await req.dbClient.query(`SELECT COUNT(*) FROM courses ${whereClause}`, filterParams);
...
router.post('/', validate(createCourseSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { code, title, title_en, description, description_en, category, duration_hours, skill_level, provider, is_mandatory = false, is_certification = false, provider_url, thumbnail_url, language = 'it', tags, prerequisites, created_by, } = req.body;
    if (!title) {
 ...
 * PATCH /courses/:id
 */
router.patch('/:id', validateUUID(), validate(updateCourseSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'];
    const existing = await req.dbClient.query('SELECT id FROM courses WHERE id = $1 AND tenant_id = $2', [id, tenantId]);
    ...
    const result = await req.dbClient.query(`UPDATE courses SET ${updates.join(', ')} WHERE id = $${paramIndex} AND tenant_id = $${paramIndex + 1} RETURNING *`, [...values, id, tenantId]);
...
router.delete('/:id', validateUUID(), asyncHandler(async (req, res) => {
    ...
    const enrollmentCount = await req.dbClient.query(`SELECT COUNT(*) FROM course_enrollments ce
       JOIN employees e ON ce.employee_id = e.id
       WHERE ce.course_id = $1 AND e.tenant_id = $2
       AND ce.status NOT IN ('completed', 'cancelled')`, [id, tenantId]);
    if (parseInt(enrollmentCount.rows[0].count) > 0) {
        throw Errors.badRequest('Cannot delete course with active enrollments');
    }
    const result = await req.dbClient.query("UPDATE courses SET status = 'archived', updated_at = NOW() WHERE id = $1 AND tenant_id = $2 RETURNING id", [id, tenantId]);
    if (result.rows.length === 0) {
        throw Errors.notFound('Course');
    }
