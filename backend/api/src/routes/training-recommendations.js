...
import { employeeIdParamSchema, courseIdParamSchema, skillIdParamSchema, pathIdParamSchema, trainingRecommendationsQuerySchema, courseSearchQuerySchema, learningPathsQuerySchema, gapBasedQuerySchema, } from '../schemas/analytics-extended.js';
 ...
 */
router.get('/employees/:employeeId', validate(employeeIdParamSchema, 'params'), validate(trainingRecommendationsQuerySchema, 'query'), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const employeeId = req.params['employeeId'];
    const preferredLang = req.query['language'];
    const maxDuration = safeParseInt(req.query['max_duration'], { fallback: 0 });
    const options = {
        max_courses: safeParseInt(req.query['max_courses'], { fallback: 10 }),
        max_learning_paths: safeParseInt(req.query['max_learning_paths'], { fallback: 3 }),
        preferred_delivery_methods: req.query['delivery_methods']
            ? req.query['delivery_methods'].split(',')
            : [],
        ...
        target_skills: req.query['skills'] ? req.query['skills'].split(',') : [],
    };
    const recommendations = await trainingService.getRecommendationsForEmployee(tenantId, employeeId, options);
    res.json({
        success: true,
 ...
 */
router.get('/employees/:employeeId/preferences', validate(employeeIdParamSchema, 'params'), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const employeeId = req.params['employeeId'];
 ...
 */
router.get('/courses/search', validate(courseSearchQuerySchema, 'query'), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const skillQuery = req.query['skill'];
    const limit = safeParseInt(req.query['limit'], { fallback: 10 });
    if (!skillQuery) {
        throw Errors.badRequest('skill query parameter is required');
    }
    const courses = await trainingService.searchCoursesBySkillEmbedding(tenantId, skillQuery, limit);
    res.json({
        success: true,
        data: courses,
 ...
 */
router.get('/courses/:courseId/stats', validate(courseIdParamSchema, 'params'), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
 ...
 */
router.get('/skills/:skillId/courses', validate(skillIdParamSchema, 'params'), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
 ...
 */
router.get('/learning-paths', validate(learningPathsQuerySchema, 'query'), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
 ...
 */
router.get('/learning-paths/:pathId', validate(pathIdParamSchema, 'params'), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
 ...
 */
router.get('/gap-based', validate(gapBasedQuerySchema, 'query'), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const orgUnitId = req.query['org_unit_id'];
    const limit = safeParseInt(req.query['limit'], { fallback: 20 });
    let query = `
...
//# sourceMappingURL=training-recommendations.js.map