...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const sortCol = validateIdentifier(
      allowedSorts.includes(sort as string) ? (sort as string) : 'created_at',
      'marketplace-reviews.list-sort'
    );
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const existingReview = await req.dbClient!.query(
...
router.put(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const existing = await req.dbClient!.query(
      'SELECT id, plugin_id FROM plugin_reviews WHERE id = $1 AND tenant_id = $2',
      [id, tenantId]
    );
