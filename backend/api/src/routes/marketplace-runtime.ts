...
import {
  executeHookSchema,
  registerRuntimeHookSchema,
  registerRuntimeUiSlotSchema,
 ...
 */
router.post(
  '/hooks/:hookName/execute',
  validate(executeHookSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const hookName = req.params['hookName'] as string;
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const slotName = req.params['slotName'] as string;
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const countResult = await req.dbClient!.query(
      'SELECT COUNT(*) FROM plugin_hook_executions WHERE tenant_id = $1',
      [tenantId]
    );
 ...
 */
router.post(
  '/plugins/:pluginId/hooks',
  validate(registerRuntimeHookSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    ...
    const installCheck = await req.dbClient!.query(
      "SELECT id FROM plugin_installations WHERE plugin_id = $1 AND tenant_id = $2 AND status = 'active'",
      [pluginId, tenantId]
    );
 ...
 */
router.post(
  '/plugins/:pluginId/ui-slots',
  validate(registerRuntimeUiSlotSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const pluginId = req.params['pluginId'] as string;
 ...
 */
router.delete(
  '/plugins/:pluginId/hooks/:hookId',
  asyncHandler(async (req: Request, res: Response) => {
    const pluginId = req.params['pluginId'] as string;
    const hookId = req.params['hookId'] as string;

    const result = await req.dbClient!.query(
      'DELETE FROM plugin_hooks WHERE id = $1 AND plugin_id = $2 RETURNING id',
      [hookId, pluginId]
    );
 ...
 */
router.delete(
  '/plugins/:pluginId/ui-slots/:slotId',
  asyncHandler(async (req: Request, res: Response) => {
    const pluginId = req.params['pluginId'] as string;
    const slotId = req.params['slotId'] as string;

    const result = await req.dbClient!.query(
      'DELETE FROM plugin_ui_slots WHERE id = $1 AND plugin_id = $2 RETURNING id',
      [slotId, pluginId]
    );
