...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const countResult = await req.dbClient!.query(
      'SELECT COUNT(*) FROM rag_documents WHERE tenant_id = $1',
      [tenantId]
    );
...
router.post(
  '/',
  validate(createRagDocumentSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const {
      filename,
...
router.patch(
  '/:id',
  validate(updateRagDocumentSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'] as string;
    ...
    const result = await req.dbClient!.query(
      `UPDATE rag_documents SET ${updates.join(', ')} WHERE id = $${paramIndex} AND tenant_id = $${paramIndex + 1} RETURNING *`,
      [...values, id, tenantId]
    );
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const existing = await req.dbClient!.query(
      'SELECT id, status FROM rag_documents WHERE id = $1 AND tenant_id = $2',
      [id, tenantId]
    );
...
router.delete(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const result = await req.dbClient!.query(
      'DELETE FROM rag_documents WHERE id = $1 AND tenant_id = $2 RETURNING id',
      [id, tenantId]
    );
