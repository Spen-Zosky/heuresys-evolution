...
import {
  overtimeStatsQuerySchema,
  overtimeByTypeQuerySchema,
...
router.get(
  '/stats',
  validate(overtimeStatsQuerySchema, 'query'),
...
router.get(
  '/by-type',
  validate(overtimeByTypeQuerySchema, 'query'),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
...
router.get(
  '/',
  validate(overtimeListQuerySchema, 'query'),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const {
      employee_id,
    ...
    if (overtime_type) {
      ...
      params.push(overtime_type as string);
    }
