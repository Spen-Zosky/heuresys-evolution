...
router.get('/', authMiddleware, requirePermission('PLATFORM', 'VIEW'), asyncHandler(async (req, res) => {
 ...
 */
router.get('/meta/plans', (_req, res) => {
...
router.get('/current', authMiddleware, asyncHandler(async (req, res) => {
    ...
    const result = await pool.query(`SELECT id, code, name, description, status, subscription_plan,
...
router.get('/:identifier', authMiddleware, requirePermission('PLATFORM', 'VIEW'), asyncHandler(async (req, res) => {
    const identifier = req.params['identifier'];
    // Check if identifier is UUID or code
    const isUUID = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(identifier);
    const result = await pool.query(`SELECT id, code, name, description, nace_code, region, status,
              employee_count, subscription_plan, industry_type,
              annual_revenue_eur, sap_company_code, verified_website, tax_id,
              created_at, updated_at
       FROM tenants
       WHERE ${isUUID ? 'id' : 'code'} = $1`, [identifier]);
    if (result.rows.length === 0) {
...
router.get('/:identifier/stats', authMiddleware, requirePermission('PLATFORM', 'VIEW'), asyncHandler(async (req, res) => {
    const identifier = req.params['identifier'];
    // First get the tenant
    const isUUID = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(identifier);
    const tenantResult = await pool.query(`SELECT id, code, name FROM tenants WHERE ${isUUID ? 'id' : 'code'} = $1`, [identifier]);
    if (tenantResult.rows.length === 0) {
        throw createAppError(`Tenant '${identifier}' not found`, 404, ErrorCodes.TENANT_NOT_FOUND);
    }
    const tenantId = tenantResult.rows[0]?.id;
    // Get statistics
    const [employeeCount, orgUnitCount, locationCount, activeGoals, pendingReviews] = await Promise.all([
        ...
        pool.query("SELECT COUNT(*) FROM performance_reviews WHERE tenant_id = $1 AND status = 'pending'", [tenantId]),
    ]);
    res.json({
        success: true,
        data: {
...
router.post('/', authMiddleware, requirePermission('PLATFORM', 'CREATE'), validate(createTenantSchema), asyncHandler(async (req, res) => {
    const authReq = req;
    const { code, name, description, nace_code, region, status = 'pending', subscription_plan = 'free', industry_type, sap_company_code, annual_revenue_eur, } = req.body;
    ...
    if (!/^[a-z0-9-]+$/.test(code)) {
        throw createAppError('Code must be lowercase alphanumeric with hyphens only', 400, ErrorCodes.VALIDATION_ERROR);
    }
    ...
    // Validate status
    if (status && !VALID_STATUSES.includes(status)) {
        throw createAppError(`Invalid status. Must be one of: ${VALID_STATUSES.join(', ')}`, 400, ErrorCodes.VALIDATION_ERROR);
    }
    // Validate subscription plan
    if (subscription_plan && !VALID_PLANS.includes(subscription_plan)) {
        throw createAppError(`Invalid subscription plan. Must be one of: ${VALID_PLANS.join(', ')}`, 400, ErrorCodes.VALIDATION_ERROR);
    }
...
router.patch('/:identifier', authMiddleware, validate(updateTenantSchema), asyncHandler(async (req, res) => {
    const authReq = req;
    const identifier = req.params['identifier'];
    // Check if identifier is UUID or code
    const isUUID = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(identifier);
    // Get existing tenant
    const tenantResult = await pool.query(`SELECT id, code, name, description, nace_code, region, status,
       ...
       FROM tenants WHERE ${isUUID ? 'id' : 'code'} = $1`, [identifier]);
    if (tenantResult.rows.length === 0) {
        throw createAppError(`Tenant '${identifier}' not found`, 404, ErrorCodes.TENANT_NOT_FOUND);
    }
    ...
    const { name, description, nace_code, region, status, subscription_plan, industry_type, sap_company_code, annual_revenue_eur, verified_website, tax_id, } = req.body;
    // Non-SUPERUSER cannot change status or subscription_plan
    if (!isSuperuser && (status !== undefined || subscription_plan !== undefined)) {
        throw createAppError('Only SUPERUSER can change status or subscription plan', 403, ErrorCodes.FORBIDDEN);
    }
    ...
    const result = await pool.query(`UPDATE tenants SET ${updates.join(', ')} WHERE id = $${paramIndex}
         RETURNING id, code, name, description, nace_code, region, status,
                   subscription_plan, industry_type, sap_company_code,
...
router.delete('/:identifier', authMiddleware, requirePermission('PLATFORM', 'DELETE'), asyncHandler(async (req, res) => {
    const authReq = req;
    const identifier = req.params['identifier'];
    ...
    const isUUID = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(identifier);
    // Get existing tenant
    const tenantResult = await pool.query(`SELECT id, code, name, status FROM tenants WHERE ${isUUID ? 'id' : 'code'} = $1`, [identifier]);
    if (tenantResult.rows.length === 0) {
        throw createAppError(`Tenant '${identifier}' not found`, 404, ErrorCodes.TENANT_NOT_FOUND);
    }
    const tenant = tenantResult.rows[0];
    ...
    if (permanent === 'true') {
        ...
        const employeeCount = await pool.query('SELECT COUNT(*) FROM employees WHERE tenant_id = $1', [tenant.id]);
        if (parseInt(employeeCount.rows[0].count, 10) > 0) {
            throw createAppError('Cannot permanently delete tenant with existing employees. Deactivate instead.', 400, ErrorCodes.VALIDATION_ERROR);
        }
...
router.post('/:identifier/activate', authMiddleware, requirePermission('PLATFORM', 'EDIT'), asyncHandler(async (req, res) => {
    const authReq = req;
    const identifier = req.params['identifier'];
    const isUUID = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(identifier);
    const tenantResult = await pool.query(`SELECT id, code, name, status FROM tenants WHERE ${isUUID ? 'id' : 'code'} = $1`, [identifier]);
