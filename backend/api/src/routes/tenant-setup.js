...
import { ErrorCodes, CCNL_TYPES, CCNL_LEAVE_DEFAULTS } from '@heuresys/shared';
import { authMiddleware } from '../middleware/auth.js';
import { requirePermission } from '../middleware/rbpMiddleware.js';
import { validate } from '../middleware/validate.js';
import { tenantSetupStep1Schema, tenantSetupStep2Schema, tenantSetupStep3Schema, tenantUpdateSettingsSchema, } from '../schemas/hr-operations-extended.js';
...
router.get('/status', asyncHandler(async (req, res) => {
    ...
    const result = await dbClient.query(`SELECT id, code, name, setup_completed, setup_step, settings,
...
// =============================================================================
router.get('/ccnl-types', (_req, res) => {
    const ccnlOptions = Object.entries(CCNL_TYPES).map(([key, value]) => {
        const defaults = CCNL_LEAVE_DEFAULTS[value];
        return {
            value,
...
// =============================================================================
router.post('/step/1', validate(tenantSetupStep1Schema), asyncHandler(async (req, res) => {
    const dbClient = req.dbClient;
    const authReq = req;
    const tenantId = authReq.user.tenantId;
    if (!tenantId) {
    ...
    const { name, code, taxId, addressStreet, addressCity, addressPostalCode, addressCountry, contactEmail, contactPhone, description, industryType, naceCode, region, } = req.body;
    // Validate required fields
    if (!name || name.trim().length < 2) {
        throw createAppError('Company name is required (min 2 characters)', 400, ErrorCodes.VALIDATION_ERROR);
    }
    // Validate Italian tax ID format (Partita IVA) if provided
    if (taxId) {
        const taxIdRegex = /^[0-9]{11}$/;
        if (!taxIdRegex.test(taxId)) {
            throw createAppError('Invalid Italian Partita IVA format (11 digits required)', 400, ErrorCodes.VALIDATION_ERROR);
        }
    }
...
// =============================================================================
router.post('/step/2', validate(tenantSetupStep2Schema), asyncHandler(async (req, res) => {
    const dbClient = req.dbClient;
    const authReq = req;
    const tenantId = authReq.user.tenantId;
    if (!tenantId) {
    ...
    const defaults = CCNL_LEAVE_DEFAULTS[ccnlType];
    // Use custom rules if provided, otherwise use defaults
    const finalLeaveRules = customRules && leaveRules
        ? {
    ...
    const result = await dbClient.query(`UPDATE tenants SET
        ...
        updated_at = NOW()
       WHERE id = $1
       RETURNING setup_step, settings`, [tenantId, JSON.stringify(newSettings)]);
    if (result.rows.length === 0) {
        throw createAppError('Tenant not found', 404, ErrorCodes.TENANT_NOT_FOUND);
    }
    res.json({
...
// =============================================================================
router.post('/step/3', validate(tenantSetupStep3Schema), asyncHandler(async (req, res) => {
    const dbClient = req.dbClient;
    const authReq = req;
    const tenantId = authReq.user.tenantId;
    if (!tenantId) {
    ...
    const { startMonth, payPeriod, holidayCalendar, customHolidays } = req.body;
    ...
    const newSettings = {
        ...
        fiscalYear: {
            ...
            payPeriod: validPayPeriod,
            holidayCalendar: validCalendar,
        },
        holidays: {
            calendar: validCalendar,
            customHolidays: validCalendar === 'custom' && Array.isArray(customHolidays) ? customHolidays : [],
        },
    };
    ...
    const result = await dbClient.query(`UPDATE tenants SET
        ...
        updated_at = NOW()
       WHERE id = $1
       RETURNING *`, [tenantId, JSON.stringify(newSettings)]);
    if (result.rows.length === 0) {
        throw createAppError('Tenant not found', 404, ErrorCodes.TENANT_NOT_FOUND);
    }
    res.json({
...
// =============================================================================
router.put('/settings', validate(tenantUpdateSettingsSchema), asyncHandler(async (req, res) => {
    const dbClient = req.dbClient;
    const authReq = req;
    const tenantId = authReq.user.tenantId;
    if (!tenantId) {
    ...
    const result = await dbClient.query(`UPDATE tenants SET
        ...
        updated_at = NOW()
       WHERE id = $1
       RETURNING settings`, [tenantId, JSON.stringify(mergedSettings)]);
    if (result.rows.length === 0) {
        throw createAppError('Tenant not found', 404, ErrorCodes.TENANT_NOT_FOUND);
    }
    res.json({
...
//# sourceMappingURL=tenant-setup.js.map