...
import { createKnowledgeBaseSchema, ingestCCNLSchema, ingestPolicySchema, processPendingSchema, } from '../schemas/platform.js';
 ...
 */
router.post('/', checkPermission(PERMISSIONS.TENANT_CONFIGURE), validate(createKnowledgeBaseSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    ...
    const existing = await req.dbClient.query('SELECT id FROM rag_knowledge_bases WHERE code = $1 AND tenant_id = $2', [code, tenantId]);
 ...
 */
router.get('/ccnl/contracts', asyncHandler(async (req, res) => {
    const result = await req.dbClient.query(`
 ...
 */
router.get('/ccnl/contracts/:code', asyncHandler(async (req, res) => {
 ...
 */
router.post('/ccnl/ingest', checkPermission(PERMISSIONS.TENANT_CONFIGURE), validate(ingestCCNLSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
 ...
 */
router.post('/policies/ingest', checkPermission(PERMISSIONS.TENANT_CONFIGURE), validate(ingestPolicySchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
 ...
 */
router.post('/documents/:id/reprocess', checkPermission(PERMISSIONS.TENANT_CONFIGURE), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const docId = req.params['id'];
    // Verify document exists
    const docResult = await req.dbClient.query('SELECT id FROM rag_documents WHERE id = $1 AND tenant_id = $2', [docId, tenantId]);
    if (docResult.rows.length === 0) {
 ...
 */
router.post('/process-pending', checkPermission(PERMISSIONS.TENANT_CONFIGURE), validate(processPendingSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
