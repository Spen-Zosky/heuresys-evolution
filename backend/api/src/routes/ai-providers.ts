...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    const { provider, days } = req.query as Record<string, string>;
    const daysNum = safeParseInt(days as string, { fallback: 30 });
    const since = new Date(Date.now() - daysNum * 24 * 60 * 60 * 1000);

    const factory = getProviderFactory();
    const costs = await factory.loadCostSummary(provider as ProviderName | undefined, since);
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const { provider, days } = req.query as Record<string, string>;
    const daysNum = safeParseInt(days as string, { fallback: 7 });
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    const dbClient = req.dbClient!;
    const { provider, months } = req.query as Record<string, string>;
    const monthsNum = safeParseInt(months as string, { fallback: 3 });
  ...
  `;
    const params: unknown[] = [monthsNum];
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const result = await dbClient.query(
      `SELECT tenant_code, tenant_name, provider, request_count, total_tokens, total_cost FROM v_ai_tenant_costs`
    );
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    const dbClient = req.dbClient!;
    const { provider, operation, limit, offset } = req.query as Record<string, string>;
    const limitNum = safeParseInt(limit as string, { fallback: 50, max: 500 });
...
router.put(
  '/config/:provider',
  validate(updateProviderConfigSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const dbClient = req.dbClient!;
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const factory = getProviderFactory();
    const result = await factory.generateEmbedding(
      text,
