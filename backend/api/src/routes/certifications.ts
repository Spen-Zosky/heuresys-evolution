  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const data = hasFilters
      ...
      : await cachedForTenant(
          tenantId,
          `certifications:list:${limitNum}:${offsetNum}`,
          fetchData,
          CACHE_TTL.MODERATE
        );
...
router.patch(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const existing = await req.dbClient!.query(
      'SELECT id FROM certifications WHERE id = $1 AND tenant_id = $2',
      [id, tenantId]
    );
    ...
    const result = await req.dbClient!.query(
      `UPDATE certifications SET ${updates.join(', ')} WHERE id = $${paramIndex} AND tenant_id = $${paramIndex + 1} RETURNING *`,
      [...values, id, tenantId]
    );
...
router.delete(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const result = await req.dbClient!.query(
      'UPDATE certifications SET is_active = false WHERE id = $1 AND tenant_id = $2 RETURNING id',
      [id, tenantId]
    );
