...
import type { EventCategory, AggregationPeriod } from '../services/analytics-pipeline.js';
import {
  generateHRDashboardPDF,
  generateCompensationPDF,
  generateWorkforcePlanningPDF,
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const { event_type, category, entity_type, entity_id, user_id, session_id, data, metrics } =
      req.body;
...
router.post(
  '/events/batch',
  validate(trackEventBatchSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const limitNum = safeParseInt(limit as string, { fallback: 100, min: 1, max: 5000 });
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const { metric_name, entity_type, period, start_date, end_date } = req.query as Record<
      string,
      string
    >;
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const { metric_name, entity_type, period } = req.query as Record<string, string>;

    if (!metric_name || !entity_type || !period) {
      throw Errors.badRequest('metric_name, entity_type, and period are required');
    }

    const metric = await analyticsPipelineService.getCurrentMetric(
      tenantId,
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const counts = await analyticsPipelineService.getEventCountsByCategory(
      tenantId,
      startDate,
      endDate
    );
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const hrObj = hrData as Record<string, unknown>;
    const perfObj = perfData as Record<string, unknown>;
    const recruitObj = recruitData as Record<string, unknown>;
    const learnObj = learnData as Record<string, unknown>;
...
router.post(
  ...
  validate(computeAggregationsSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    ...
    await analyticsPipelineService.computeAggregations(
      tenantId,
      period,
    ...
    if (format === 'pdf') {
      ...
      switch (dashboard) {
        ...
        case 'performance':
          ...
          break;
        default:
          pdfBuffer = await generateGenericPDF(
            data,
            `${dashboard.charAt(0).toUpperCase() + dashboard.slice(1)} Analytics`
          );
      }
      content = pdfBuffer;
...
function flattenDataForExport(data: any): any[] {
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const r = await (req as any).dbClient.query(
      `SELECT total_headcount, active_employees, new_hires_30d, new_hires_90d,
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const r = await (req as any).dbClient.query(
      `SELECT turnover_rate_annual, terminations_30d, terminations_90d,
