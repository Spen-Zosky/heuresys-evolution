...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (plugin_installation_id) {
      ...
      params.push(plugin_installation_id as string);
      paramIndex++;
    }
...
router.delete(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const result = await req.dbClient!.query(
      'DELETE FROM plugin_webhooks WHERE id = $1 AND tenant_id = $2 RETURNING id',
      [id, tenantId]
    );
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const webhookCheck = await req.dbClient!.query(
      'SELECT id FROM plugin_webhooks WHERE id = $1 AND tenant_id = $2',
      [id, tenantId]
    );
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const webhookCheck = await req.dbClient!.query(
      'SELECT id, url, events FROM plugin_webhooks WHERE id = $1 AND tenant_id = $2',
      [id, tenantId]
    );
