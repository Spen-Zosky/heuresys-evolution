...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const [countResult, dataResult] = await Promise.all([
      ...
      req.dbClient!.query(
        `SELECT r.id, r.from_employee_id, r.to_employee_id, r.message, r.category,
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const { period = '30', limit = '10' } = req.query as Record<string, string>;
    ...
    const result = await req.dbClient!.query(
      ...
      LIMIT $3
    `,
      [tenantId, parseInt(period as string), safeParseInt(limit as string, { fallback: 50 })]
    );
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const result = await req.dbClient!.query(
      ...
      LIMIT $2
    `,
      [tenantId, safeParseInt(limit as string, { fallback: 50 })]
    );
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (badge_type) {
      ...
      params.push(badge_type as string);
      paramIndex++;
    }
    ...
    const countResult = await req.dbClient!.query(
      'SELECT COUNT(*) FROM recognition WHERE tenant_id = $1',
      [tenantId]
    );
...
router.post(
  '/',
  validate(createRecognitionSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const {
      from_employee_id,
    ...
    const receiverCheck = await req.dbClient!.query(
      'SELECT id FROM employees WHERE id = $1 AND tenant_id = $2',
      [to_employee_id, tenantId]
    );
...
router.patch(
  '/:id',
  validate(updateRecognitionSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'] as string;

    const existing = await req.dbClient!.query(
      'SELECT id FROM recognition WHERE id = $1 AND tenant_id = $2',
      [id, tenantId]
    );
    ...
    const result = await req.dbClient!.query(
      `UPDATE recognition SET ${updates.join(', ')} WHERE id = $${paramIndex} AND tenant_id = $${paramIndex + 1} RETURNING *`,
      [...values, id, tenantId]
    );
...
router.delete(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const result = await req.dbClient!.query(
      'DELETE FROM recognition WHERE id = $1 AND tenant_id = $2 RETURNING id',
      [id, tenantId]
    );
