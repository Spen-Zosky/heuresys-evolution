...
import { createOrgUnitSchema, updateOrgUnitSchema, moveOrgUnitSchema, } from '../schemas/employees.js';
...
router.get('/', asyncHandler(async (req, res) => {
    ...
    const { is_active, org_type, parent_id, search, limit = '100', offset = '0', } = req.query;
    ...
    const fetchData = async () => {
      ...
      `;
        ...
        const countResult = await req.dbClient.query('SELECT COUNT(*) FROM org_units WHERE tenant_id = $1', [tenantId]);
        return {
            rows: result.rows,
    ...
    const data = hasFilters
        ? await fetchData()
        : await cachedForTenant(tenantId, `org-units:list:${limitNum}:${offsetNum}`, fetchData, CACHE_TTL.REFERENCE);
    res.json({
        success: true,
        data: data.rows,
        meta: {
 ...
 */
router.get(['/tree', '/hierarchy'], asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
...
router.get('/types', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const result = await req.dbClient.query('SELECT DISTINCT org_type FROM org_units WHERE tenant_id = $1 AND org_type IS NOT NULL ORDER BY org_type LIMIT 100', [tenantId]);
...
router.get('/:id', validateUUID(), asyncHandler(async (req, res) => {
    ...
    const result = await req.dbClient.query(`SELECT o.*, d.name as department_name, l.name as location_name,
        m.first_name || ' ' || m.last_name as manager_name,
 ...
 * POST /org-units
 */
router.post('/', checkPermission(PERMISSIONS.TENANT_CONFIGURE), validate(createOrgUnitSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { code, name, name_en, parent_id, org_level, org_type, default_location_id, manager_id, deputy_manager_id, headcount_budget, valid_from, sort_order, } = req.body;
    if (!code || !name) {
        throw Errors.badRequest('Code and name are required');
    }
    const existing = await req.dbClient.query('SELECT id FROM org_units WHERE code = $1 AND tenant_id = $2', [code, tenantId]);
    if (existing.rows.length > 0) {
        throw Errors.conflict('Org unit code already exists');
    }
    const result = await req.dbClient.query(`INSERT INTO org_units (tenant_id, code, name, name_en, parent_id, org_level, org_type,
 ...
 * PATCH /org-units/:id
 */
router.patch('/:id', validateUUID(), checkPermission(PERMISSIONS.TENANT_CONFIGURE), validate(updateOrgUnitSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'];
    const existing = await req.dbClient.query('SELECT id FROM org_units WHERE id = $1 AND tenant_id = $2', [id, tenantId]);
    if (existing.rows.length === 0) {
        throw Errors.notFound('Org unit');
    }
    const allowedFields = [
    ...
    const result = await req.dbClient.query(`UPDATE org_units SET ${updates.join(', ')} WHERE id = $${paramIndex} AND tenant_id = $${paramIndex + 1} RETURNING *`, [...values, id, tenantId]);
    await invalidateCachePattern(`t:${tenantId}:org-units:*`);
 ...
 * DELETE /org-units/:id
 */
router.delete('/:id', validateUUID(), checkPermission(PERMISSIONS.TENANT_CONFIGURE), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'];
    const existing = await req.dbClient.query('SELECT id, name FROM org_units WHERE id = $1 AND tenant_id = $2', [id, tenantId]);
    if (existing.rows.length === 0) {
        throw Errors.notFound('Org unit');
    }
    const childCount = await req.dbClient.query('SELECT COUNT(*) FROM org_units WHERE parent_id = $1 AND tenant_id = $2', [id, tenantId]);
    if (parseInt(childCount.rows[0].count) > 0) {
        throw Errors.badRequest('Cannot delete org unit with children');
    }
    const employeeCount = await req.dbClient.query('SELECT COUNT(*) FROM employees WHERE org_unit_id = $1 AND tenant_id = $2', [id, tenantId]);
    ...
    await req.dbClient.query('UPDATE org_units SET is_active = false, valid_to = CURRENT_DATE, updated_at = NOW() WHERE id = $1 AND tenant_id = $2', [id, tenantId]);
    await invalidateCachePattern(`t:${tenantId}:org-units:*`);
...
router.get('/:id/children', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'];
    const result = await req.dbClient.query(`SELECT o.id, o.code, o.name, o.org_level, o.org_type, o.is_active,
        (SELECT COUNT(*) FROM employees e WHERE e.org_unit_id = o.id) as employee_count
       FROM org_units o WHERE o.parent_id = $1 AND o.tenant_id = $2 ORDER BY o.sort_order, o.name`, [id, tenantId]);
...
router.get('/:id/path', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'];
    const result = await req.dbClient.query(`WITH RECURSIVE org_path AS (
      ...
      SELECT id, code, name, org_level FROM org_path ORDER BY depth DESC LIMIT 200`, [id, tenantId]);
    res.json({
        success: true,
        data: result.rows,
 ...
 */
router.post('/:id/move', checkPermission(PERMISSIONS.TENANT_CONFIGURE), validate(moveOrgUnitSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'];
    const { newParentId } = req.body;
    // Verify org unit exists
    const existing = await req.dbClient.query('SELECT id, name, parent_id FROM org_units WHERE id = $1 AND tenant_id = $2', [id, tenantId]);
    if (existing.rows.length === 0) {
        throw Errors.notFound('Org unit');
    }
    // If newParentId is provided, verify it exists and is not a descendant
    if (newParentId) {
        const parentCheck = await req.dbClient.query('SELECT id FROM org_units WHERE id = $1 AND tenant_id = $2', [newParentId, tenantId]);
        if (parentCheck.rows.length === 0) {
            throw Errors.badRequest('New parent org unit not found');
        }
        // Check for circular reference
        const circularCheck = await req.dbClient.query(`WITH RECURSIVE descendants AS (
          SELECT id FROM org_units WHERE id = $1
          UNION ALL
          SELECT o.id FROM org_units o
          INNER JOIN descendants d ON o.parent_id = d.id
        )
        SELECT 1 FROM descendants WHERE id = $2`, [id, newParentId]);
    ...
    // Update the parent
    const result = await req.dbClient.query(`UPDATE org_units SET parent_id = $1, updated_at = NOW()
       WHERE id = $2 AND tenant_id = $3 RETURNING *`, [newParentId || null, id, tenantId]);
    res.json({
        success: true,
 ...
 */
router.get('/meta/statistics', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const [totalResult, byTypeResult, byLevelResult, maxDepthResult] = await Promise.all([
        req.dbClient.query('SELECT COUNT(*) FROM org_units WHERE tenant_id = $1 AND is_active = true', [tenantId]),
        req.dbClient.query(`SELECT org_type, COUNT(*) as count
         FROM org_units WHERE tenant_id = $1 AND is_active = true
         GROUP BY org_type ORDER BY count DESC`, [tenantId]),
        req.dbClient.query(`SELECT org_level, COUNT(*) as count
         FROM org_units WHERE tenant_id = $1 AND is_active = true
         GROUP BY org_level ORDER BY org_level`, [tenantId]),
        req.dbClient.query(`SELECT MAX(org_level) as max_depth FROM org_units WHERE tenant_id = $1 AND is_active = true`, [tenantId]),
    ]);
    res.json({
        success: true,
        data: {
