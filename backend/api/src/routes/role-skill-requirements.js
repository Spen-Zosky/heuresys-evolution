...
import { replaceSkillRequirementsSchema, createSkillRequirementSchema, seedFromEscoSchema, copyFromRoleSchema, } from '../schemas/skills-assessment.js';
...
// =============================================================================
router.get('/:id/skill-requirements', asyncHandler(async (req, res) => {
    const dbClient = req.dbClient;
    const { id: roleId } = req.params;
    const { importance, includeSkillDetails = 'true' } = req.query;
    // Verify role exists
...
// =============================================================================
router.put('/:id/skill-requirements', validate(replaceSkillRequirementsSchema), asyncHandler(async (req, res) => {
    const dbClient = req.dbClient;
    const { id: roleId } = req.params;
    ...
    for (const req of requirements) {
        try {
            const { skillId, knowledge, skill, ability, behavior, attitude, minCompositeScore, importance = 'important', weight = 1.0, isPrimary = false, notes, } = req;
            if (!skillId) {
...
// =============================================================================
router.post('/:id/skill-requirements', validate(createSkillRequirementSchema), asyncHandler(async (req, res) => {
    const dbClient = req.dbClient;
    const { id: roleId } = req.params;
    const { skillId, knowledge, skill, ability, behavior, attitude, minCompositeScore, importance = 'important', weight = 1.0, isPrimary = false, notes, tenantId, } = req.body;
    if (!skillId) {
...
// =============================================================================
router.delete('/:id/skill-requirements/:skillId', asyncHandler(async (req, res) => {
    const dbClient = req.dbClient;
    const { id: roleId, skillId } = req.params;
    const { tenantId } = req.query;
    let query = 'DELETE FROM role_skill_requirements WHERE role_id = $1 AND skill_id = $2';
    const params = [roleId, skillId];
    if (tenantId) {
...
// =============================================================================
router.post('/:id/skill-requirements/seed-from-esco', validate(seedFromEscoSchema), asyncHandler(async (req, res) => {
    const dbClient = req.dbClient;
    const { id: roleId } = req.params;
...
// =============================================================================
router.post('/:id/skill-requirements/copy-from/:sourceRoleId', validate(copyFromRoleSchema), asyncHandler(async (req, res) => {
    const dbClient = req.dbClient;
    const { id: targetRoleId, sourceRoleId } = req.params;
...
// =============================================================================
router.get('/:id/succession', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const roleId = req.params['id'];
    const minSkillMatch = req.query['min_skill_match']
        ? parseFloat(req.query['min_skill_match'])
        : undefined;
    const maxCandidates = safeParseInt(req.query['max_candidates'], { fallback: 0 });
...
//# sourceMappingURL=role-skill-requirements.js.map