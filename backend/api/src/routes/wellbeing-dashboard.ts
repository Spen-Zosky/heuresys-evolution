...
import {
  ...
  createBurnoutAssessmentSchema,
  createWellbeingGoalSchema,
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const { employee_id, from_date, to_date, limit = '50' } = req.query as Record<string, string>;
...
router.post(
  '/checkins',
  validate(createWellbeingCheckinSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const {
      employee_id,
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const { employee_id, risk_level } = req.query as Record<string, string>;
...
router.post(
  '/burnout-risk/assess',
  validate(createBurnoutAssessmentSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const {
      employee_id,
      exhaustion_score, // 1-10
      cynicism_score, // 1-10
    ...
    if (avgScore >= 7) overall_risk = 'critical';
    else if (avgScore >= 5) overall_risk = 'high';
    else if (avgScore >= 3) overall_risk = 'moderate';
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const { employee_id, status } = req.query as Record<string, string>;
...
router.post(
  '/goals',
  validate(createWellbeingGoalSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const { employee_id, goal_type, title, description, target_value, unit, target_date } =
      req.body;
