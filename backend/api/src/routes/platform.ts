    ...
    const data = await cached(
      ...
      async () => {
        const [
          ...
          dbClient.query(
            `SELECT COUNT(*) as total_tenants, COUNT(*) FILTER (WHERE status = 'active') as active_tenants FROM tenants`
          ),
          dbClient.query(`SELECT COUNT(*) as total_users FROM users`),
          dbClient.query(`SELECT COUNT(*) as total_employees FROM employees`),
          dbClient.query(
            `SELECT t.id, t.name, t.code, t.status, (SELECT COUNT(*) FROM employees e WHERE e.tenant_id = t.id) as employee_count, (SELECT COUNT(*) FROM users u JOIN employees e2 ON u.employee_id = e2.id WHERE e2.tenant_id = t.id) as user_count FROM tenants t ORDER BY t.name`
          ),
          dbClient.query(
          ...
          dbClient.query(
            `SELECT COUNT(*) as requests_24h FROM audit_logs WHERE timestamp >= NOW() - INTERVAL '24 hours'`
          ),
        ]);
        ...
        return {
          ...
          tenants: tenantBreakdown.rows.map((t: Record<string, string>) => ({
            id: t.id,
            name: t.name,
            code: t.code,
            status: t.status,
...
router.get(
  '/health',
  asyncHandler(async (_req: Request, res: Response) => {
    const health: Record<string, { status: string; latency?: number; message?: string }> = {
...
router.get(
  '/alerts',
  validate(platformAlertsQuerySchema, 'query'),
...
router.get(
  ...
  asyncHandler(async (_req: Request, res: Response) => {
    ...
    const row = firstRowOrNull(summary) || ({} as Record<string, unknown>);
...
router.get(
  ...
  asyncHandler(async (_req: Request, res: Response) => {
    ...
    const conn = firstRowOrNull(connResult) || ({ active: 0, max: 100 } as Record<string, unknown>);
    const errStats =
      firstRowOrNull(errorResult) || ({ errors_1h: 0, total_1h: 0 } as Record<string, unknown>);
    ...
    res.json({
      ...
      data: {
        ...
        error_rate_1h:
          errStats.total_1h > 0 ? Math.round((errStats.errors_1h / errStats.total_1h) * 100) : 0,
...
router.get(
  ...
  asyncHandler(async (_req: Request, res: Response) => {
    ...
    let lastBackupStatus: 'OK' | 'FAILED' | 'UNKNOWN' = 'UNKNOWN';
    const frequency: string | null = 'Ogni giorno alle 02:00';
    const retentionDays: number | null = 7;
    let dailyCount = 0;
    let weeklyCount = 0;
    let monthlyCount = 0;
    ...
    try {
      ...
      if (fs.existsSync(weeklyDir)) {
        weeklyCount = fs.readdirSync(weeklyDir).filter((f: string) => f.endsWith('.sql.gz')).length;
      }
      if (fs.existsSync(monthlyDir)) {
        monthlyCount = fs
          .readdirSync(monthlyDir)
          .filter((f: string) => f.endsWith('.sql.gz')).length;
      }
...
router.get(
  ...
  asyncHandler(async (_req: Request, res: Response) => {
    const [failedLogins, activeSessions, rlsTables, lastSecurityEvent] = await Promise.all([
      ...
      pool.query(
        `SELECT COUNT(DISTINCT tablename) as rls_tables
         FROM pg_policies
         WHERE schemaname = 'public'`
      ),
      pool.query(
        `SELECT timestamp as last_event FROM audit_logs
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const tenantFilter: string | null = req.tenantId || null;
