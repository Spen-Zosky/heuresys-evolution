...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (work_type) {
      ...
      params.push(work_type as string);
    }
    if (job_level) {
    ...
    const countResult = await req.dbClient!.query(
      'SELECT COUNT(*) FROM internal_job_postings WHERE tenant_id = $1',
      [tenantId]
    );
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    await req.dbClient!.query(
      'UPDATE internal_job_postings SET views_count = views_count + 1 WHERE id = $1',
      [id]
    );
...
router.patch(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const existing = await req.dbClient!.query(
      'SELECT id FROM internal_job_postings WHERE id = $1 AND tenant_id = $2',
      [id, tenantId]
    );
    ...
    const result = await req.dbClient!.query(
      `UPDATE internal_job_postings SET ${updates.join(', ')}
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (job_posting_id) {
      ...
      params.push(job_posting_id as string);
    }
    if (employee_id) {
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const jobCheck = await req.dbClient!.query(
      'SELECT id, required_skills FROM internal_job_postings WHERE id = $1 AND tenant_id = $2 AND status = $3',
      [job_posting_id, tenantId, 'open']
    );
    ...
    const existingApp = await req.dbClient!.query(
      'SELECT id FROM internal_applications WHERE job_posting_id = $1 AND employee_id = $2',
      [job_posting_id, employee_id]
    );
    ...
    const empSkills = await req.dbClient!.query(
    ...
    const matchedSkills = reqSkills.filter((s: string) => empSkillIds.includes(s));
    const matchScore =
      reqSkills.length > 0 ? Math.round((matchedSkills.length / reqSkills.length) * 100) : null;
    ...
    await req.dbClient!.query(
      'UPDATE internal_job_postings SET applications_count = applications_count + 1 WHERE id = $1',
      [job_posting_id]
    );
...
router.patch(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const result = await req.dbClient!.query(
      `UPDATE internal_applications SET ${updates.join(', ')}
