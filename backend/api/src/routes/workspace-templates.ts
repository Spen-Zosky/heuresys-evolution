...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const result = await pool.query(
      `SELECT wt.id, wt.target_role_id, wt.tenant_id, wt.name,
        ...
        ORDER BY r.hierarchy_level, wt.tenant_id NULLS FIRST`,
      [tenantId]
    );
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const result = await pool.query(
      `SELECT wt.id, wt.target_role_id, wt.tenant_id, wt.name,
         ...
         JOIN rbp_roles r ON r.id = wt.target_role_id
        WHERE wt.id = $1
          AND (wt.tenant_id IS NULL OR wt.tenant_id = $2)`,
      [id, tenantId]
    );
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    const authReq = req as AuthenticatedRequest;
    const tenantId = authReq.tenantId ?? authReq.user.tenantId;
    const { name, target_role_id, widget_config, layout_config } = req.body as {
      name?: string;
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const result = await pool.query(
      `UPDATE workspace_templates
       ...
       RETURNING id, target_role_id, tenant_id, name, widget_config, layout_config, is_active, updated_at`,
      params
    );
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const tenantId = authReq.tenantId ?? authReq.user.tenantId;
    const { id } = req.params;
    const { name: overrideName } = (req.body ?? {}) as { name?: string };

    // Fetch source template
    const source = await pool.query(
      `SELECT target_role_id, name, widget_config, layout_config
