...
import { createSalaryBandSchema, updateSalaryBandSchema, } from '../schemas/compensation-extended.js';
...
router.get('/', asyncHandler(async (req, res) => {
    ...
    const [countResult, dataResult] = await Promise.all([
        req.dbClient.query(`SELECT COUNT(*) as total FROM salary_bands sb WHERE sb.tenant_id = $1 ${whereClause}`, [tenantId]),
        req.dbClient.query(`SELECT sb.id, sb.band_code, sb.band_name, sb.description, sb.job_level,
...
router.get('/', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { job_level, job_family, is_active, geo_region, limit = '100', offset = '0', } = req.query;
    ...
    const fetchData = async () => {
      ...
      `;
        ...
        if (geo_region) {
            ...
            params.push(geo_region);
            paramIndex++;
        }
        query += ` ORDER BY sb.job_level, sb.job_family, sb.band_name LIMIT $${paramIndex} OFFSET $${paramIndex + 1}`;
        params.push(limitNum, offsetNum);
        const result = await req.dbClient.query(query, params);
        const countResult = await req.dbClient.query('SELECT COUNT(*) FROM salary_bands WHERE tenant_id = $1', [tenantId]);
        return {
            rows: result.rows,
    ...
    const data = hasFilters
        ? await fetchData()
        : await cachedForTenant(tenantId, `salary-bands:list:${limitNum}:${offsetNum}`, fetchData, CACHE_TTL.MODERATE);
    res.json({
        success: true,
        data: data.rows,
        meta: {
 ...
 * POST /salary-bands
 */
router.post('/', requirePermission('COMPENSATION', 'CREATE'), validate(createSalaryBandSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { band_code, band_name, description, job_level, job_family, currency = 'EUR', min_salary, mid_salary, max_salary, range_spread_percent, geo_region, geo_adjustment_percent, effective_from, effective_to, is_active = true, created_by, } = req.body;
 ...
 */
router.patch('/:id', requirePermission('COMPENSATION', 'EDIT'), validate(updateSalaryBandSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'];
    const existing = await req.dbClient.query('SELECT id FROM salary_bands WHERE id = $1 AND tenant_id = $2', [id, tenantId]);
    ...
    const result = await req.dbClient.query(`UPDATE salary_bands SET ${updates.join(', ')} WHERE id = $${paramIndex} AND tenant_id = $${paramIndex + 1} RETURNING *`, [...values, id, tenantId]);
...
router.delete('/:id', requirePermission('COMPENSATION', 'DELETE'), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'];
    const result = await req.dbClient.query('UPDATE salary_bands SET is_active = false, updated_at = NOW() WHERE id = $1 AND tenant_id = $2 RETURNING id', [id, tenantId]);
    if (result.rows.length === 0) {
        throw Errors.notFound('Salary band');
    }
...
//# sourceMappingURL=salary-bands.js.map