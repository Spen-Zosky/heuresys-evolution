...
import { createOntologyEmbeddingService, } from '../services/ontology-embedding.js';
import { createCrossEntityService, } from '../services/cross-entity-embedding.js';
...
import { ontologySkillSearchSchema, ontologyEmbeddingJobSchema, ontologyInferRelationsSchema, ontologyCrossSearchSchema, ontologyCrossEmbeddingsGenerateSchema, ontologyOrganizationAdviceSchema, createTenantSkillSchema, updateTenantSkillSchema, createSkillDimensionSchema, updateSkillDimensionSchema, createSkillRelationSchema, createCategorySchema, updateCategorySchema, recordSkillUsageSchema, } from '../schemas/semantic.js';
...
async function getCrossEntityService() {
...
router.get('/skills', asyncHandler(async (req, res) => {
    ...
    const { skill_type, is_digital, is_green, is_transversal, has_embedding, search, lang = 'en', } = req.query;
    const params = [];
    ...
    // Count query
    const countResult = await dbClient.query(`SELECT COUNT(*) FROM esco_skills s ${whereClause}`, params);
 ...
 */
router.get('/skills/popular-pairs', asyncHandler(async (req, res) => {
    const dbClient = req.dbClient;
    const { tenantId, contextType, limit } = req.query;
 ...
 */
router.get('/skills/:id/dimensions', asyncHandler(async (req, res) => {
    const dbClient = req.dbClient;
    const { id } = req.params;
    // Verify skill exists
 ...
 */
router.get('/skills/:id/relations', asyncHandler(async (req, res) => {
    const dbClient = req.dbClient;
    const id = req.params.id;
    const { direction = 'both', relation_type } = req.query;
    if (!id) {
 ...
 */
router.post('/skills/search', validate(ontologySkillSearchSchema), asyncHandler(async (req, res) => {
    const dbClient = req.dbClient;
    const { query, language = 'en', limit = 20, similarity_threshold = 0.7, skill_type, is_digital, is_green, is_transversal, use_vector = true, // Try vector search by default
    include_custom_skills = false, } = req.body;
    ...
    const embeddingCheck = await dbClient.query('SELECT COUNT(*) FROM esco_skills WHERE embedding_en IS NOT NULL LIMIT 1');
    ...
    if (!searchResults) {
        const searchQuery = `%${escapeILIKE(query)}%`;
      ...
      `, params);
        searchResults = result.rows.map((row) => ({
            id: row.id,
            preferredLabel: lang === 'it' ? row.preferred_label_it || row.preferred_label_en : row.preferred_label_en,
            altLabels: [],
    ...
    res.json({
        ...
        data: {
            coverage: {
                ...
                coverage_percentage: totalSkills > 0 ? Math.round((withEmbedding / totalSkills) * 100 * 100) / 100 : 0,
            },
            jobs: {
 ...
 */
router.get('/embeddings/jobs', asyncHandler(async (req, res) => {
    const dbClient = req.dbClient;
 ...
 */
router.post('/embeddings/jobs', validate(ontologyEmbeddingJobSchema), asyncHandler(async (req, res) => {
    const { target = 'esco_skills', job_type = 'incremental' } = req.body;
    const tenantId = req.tenantId;
    if (target !== 'esco_skills' &&
        target !== 'tenant_custom_skills' &&
        target !== 'industry_classifications') {
        throw Errors.badRequest('Invalid target. Must be "esco_skills", "tenant_custom_skills", or "industry_classifications"');
    }
 ...
 * Get job status
 */
router.get('/embeddings/jobs/:id', asyncHandler(async (req, res) => {
    const id = req.params.id;
    if (!id) {
 ...
 */
router.post('/embeddings/jobs/:id/start', validate(ontologyEmbeddingJobSchema), asyncHandler(async (req, res) => {
    const id = req.params.id;
    if (!id) {
        throw Errors.badRequest('Job ID required');
    }
 ...
 */
router.post('/embeddings/infer-relations', validate(ontologyInferRelationsSchema), asyncHandler(async (req, res) => {
    const dbClient = req.dbClient;
    const { similarity_threshold = 0.85, limit = 1000 } = req.body;
    // Check if we have embeddings
    const embeddingCheck = await dbClient.query('SELECT COUNT(*) FROM esco_skills WHERE embedding_en IS NOT NULL');
 ...
 */
router.get('/tenant-skills', asyncHandler(async (req, res) => {
    const dbClient = req.dbClient;
    const tenantId = req.tenantId;
    if (!tenantId) {
    ...
    const countResult = await dbClient.query('SELECT COUNT(*) FROM tenant_custom_skills WHERE tenant_id = $1 AND is_active = true', [tenantId]);
    res.json({
        success: true,
        data: result.rows,
 ...
 */
router.get('/cross-search', asyncHandler(async (req, res) => {
    const query = req.query.q;
    const language = req.query.language || 'en';
    const entityTypesParam = req.query.entityTypes;
    const limit = safeParseInt(req.query.limit, { fallback: 10, max: 50 });
    const similarityThreshold = parseFloat(req.query.similarityThreshold) || 0.5;
    const tenantId = req.tenantId;
    ...
    let requestedTypes = ['skills', 'occupations', 'jobs', 'courses'];
    if (entityTypesParam) {
        requestedTypes = entityTypesParam
            .split(',')
            .filter((t) => validEntityTypes.includes(t));
    }
 ...
 */
router.post('/cross-search', validate(ontologyCrossSearchSchema), asyncHandler(async (req, res) => {
    const { query, language = 'en', entityTypes = ['skills', 'occupations', 'jobs', 'courses'], limit = 10, similarityThreshold = 0.5, tenantId, } = req.body;
    ...
    const requestedTypes = entityTypes.filter((t) => validEntityTypes.includes(t));
    if (requestedTypes.length === 0) {
        throw Errors.badRequest('At least one valid entity type is required');
    }
    const service = await getCrossEntityService();
 ...
 */
router.get('/cross-embeddings/status', asyncHandler(async (_req, res) => {
    const service = await getCrossEntityService();
 ...
 */
router.post('/cross-embeddings/generate', validate(ontologyCrossEmbeddingsGenerateSchema), asyncHandler(async (req, res) => {
 ...
 */
router.post('/organization/advice', validate(ontologyOrganizationAdviceSchema), asyncHandler(async (req, res) => {
    const { industry, companySize, language = 'it', additionalContext } = req.body;
    if (!industry || typeof industry !== 'string') {
        throw Errors.badRequest('industry is required (e.g., "lavorazioni meccaniche", "software development")');
    }
...
router.get('/industries', asyncHandler(async (req, res) => {
    ...
    const language = req.query.language === 'en' ? 'en' : 'it';
    ...
    const params = [numericLevel, limit];
    let searchClause = '';
    if (search) {
        searchClause = `AND (${nameCol} ILIKE $3 OR code ILIKE $3)`;
        params.push('%' + search + '%');
    }
    const result = await dbClient.query(`SELECT code, name_it, name_en, ${nameCol} AS name, ${descCol} AS description, level
 ...
 */
router.get('/industries/:code/occupations', asyncHandler(async (req, res) => {
    const dbClient = req.dbClient;
    const code = req.params.code || '';
    const language = req.query.language === 'en' ? 'en' : 'it';
    const limit = safeParseInt(req.query.limit, { fallback: 20, max: 50 });
    const embCol = validateEmbeddingColumn(language === 'it' ? 'embedding_it' : 'embedding_en', 'ontology.industries-occupations');
    const labelCol = validateEmbeddingColumn(language === 'it' ? 'preferred_label_it' : 'preferred_label_en', 'ontology.industries-occupations');
    // Determine NACE level based on code format
    const naceLevel = code.length === 1 ? 1 : code.length <= 2 ? 2 : 3;
    const tableName = validateTableName('industry_classifications', 'ontology.industries-occupations');
 ...
 */
router.post('/tenant-skills', validate(createTenantSkillSchema), asyncHandler(async (req, res) => {
    const dbClient = req.dbClient;
    const { tenant_id, code, name_en, name_it, description_en, description_it, skill_type, base_esco_skill_id, category_id, } = req.body;
    if (!tenant_id || !code || !name_en) {
        throw Errors.badRequest('tenant_id, code, and name_en are required');
    }
    const result = await dbClient.query(`INSERT INTO tenant_custom_skills
 ...
 */
router.put('/tenant-skills/:id', validate(updateTenantSkillSchema), asyncHandler(async (req, res) => {
    const dbClient = req.dbClient;
    const id = req.params.id;
    const { name_en, name_it, description_en, description_it, skill_type, base_esco_skill_id, category_id, is_active, } = req.body;
 ...
 */
router.delete('/tenant-skills/:id', asyncHandler(async (req, res) => {
    const dbClient = req.dbClient;
    const id = req.params.id;
    const hardDelete = req.query.hard === 'true';
    const tenantId = req.tenantId;
    if (hardDelete) {
        const result = await dbClient.query(tenantId
            ? `DELETE FROM tenant_custom_skills WHERE id = $1 AND tenant_id = $2 RETURNING id`
            : `DELETE FROM tenant_custom_skills WHERE id = $1 RETURNING id`, tenantId ? [id, tenantId] : [id]);
        if (result.rows.length === 0) {
            throw Errors.notFound('Tenant custom skill', id);
        }
    }
        ...
        const result = await dbClient.query(tenantId
            ...
            : `UPDATE tenant_custom_skills SET is_active = false, updated_at = NOW() WHERE id = $1 RETURNING id`, tenantId ? [id, tenantId] : [id]);
        if (result.rows.length === 0) {
            throw Errors.notFound('Tenant custom skill', id);
        }
    }
 ...
 */
router.post('/skills/:id/dimensions', validate(createSkillDimensionSchema), asyncHandler(async (req, res) => {
    const dbClient = req.dbClient;
    const escoSkillId = req.params.id;
    const { dimension_type, description_en, description_it, level_scale, min_level, max_level, is_primary, } = req.body;
    ...
    const result = await dbClient.query(`INSERT INTO ontology_skill_dimensions
 ...
 */
router.put('/skills/:id/dimensions/:dimensionId', validate(updateSkillDimensionSchema), asyncHandler(async (req, res) => {
    const dbClient = req.dbClient;
    const escoSkillId = req.params.id;
    const dimensionId = req.params.dimensionId;
    const { description_en, description_it, level_scale, min_level, max_level, is_primary } = req.body;
    const updates = [];
    const params = [];
    let paramIndex = 1;
    if (description_en !== undefined) {
    ...
    const result = await dbClient.query(`UPDATE ontology_skill_dimensions SET ${updates.join(', ')}
 ...
 */
router.delete('/skills/:id/dimensions/:dimensionId', asyncHandler(async (req, res) => {
    const dbClient = req.dbClient;
    const escoSkillId = req.params.id;
    const dimensionId = req.params.dimensionId;
    const result = await dbClient.query(`DELETE FROM ontology_skill_dimensions WHERE id = $1 AND esco_skill_id = $2 RETURNING id`, [dimensionId, escoSkillId]);
    if (result.rows.length === 0) {
        throw Errors.notFound('Skill dimension', dimensionId);
    }
    res.json({
 ...
 */
router.post('/skills/:id/relations', validate(createSkillRelationSchema), asyncHandler(async (req, res) => {
    const dbClient = req.dbClient;
    const sourceSkillId = req.params.id;
    ...
    // Insert the relation
    const result = await dbClient.query(`INSERT INTO ontology_skill_relations
       ...
       ON CONFLICT (source_skill_id, target_skill_id, relation_type) DO UPDATE SET
         ...
         updated_at = NOW()
       RETURNING *`, [sourceSkillId, target_skill_id, relation_type, strength || 1.0]);
    // If bidirectional, create reverse relation
    if (bidirectional) {
        const reverseType = relation_type === 'broader'
            ? 'narrower'
            : relation_type === 'narrower'
                ? 'broader'
                : relation_type;
        await dbClient.query(`INSERT INTO ontology_skill_relations
         (source_skill_id, target_skill_id, relation_type, strength, source, is_inferred)
         VALUES ($1, $2, $3, $4, 'manual', false)
         ON CONFLICT (source_skill_id, target_skill_id, relation_type) DO UPDATE SET
           strength = EXCLUDED.strength,
           updated_at = NOW()`, [target_skill_id, sourceSkillId, reverseType, strength || 1.0]);
    }
    res.status(201).json({
        success: true,
        data: result.rows[0] || null,
 ...
 */
router.delete('/skills/:id/relations/:relationId', asyncHandler(async (req, res) => {
    const dbClient = req.dbClient;
    ...
    if (deleteBidirectional) {
        const relResult = await dbClient.query(`SELECT target_skill_id, relation_type FROM ontology_skill_relations WHERE id = $1`, [relationId]);
        if (relResult.rows.length > 0) {
            reverseRelation = relResult.rows[0];
        }
    }
    const result = await dbClient.query(`DELETE FROM ontology_skill_relations WHERE id = $1 AND source_skill_id = $2 RETURNING id`, [relationId, skillId]);
    if (result.rows.length === 0) {
    ...
    if (deleteBidirectional && reverseRelation) {
        const reverseType = reverseRelation.relation_type === 'broader'
            ? 'narrower'
            : reverseRelation.relation_type === 'narrower'
                ? 'broader'
                : reverseRelation.relation_type;
        await dbClient.query(`DELETE FROM ontology_skill_relations
         WHERE source_skill_id = $1 AND target_skill_id = $2 AND relation_type = $3`, [reverseRelation.target_skill_id, skillId, reverseType]);
    }
    res.json({
        success: true,
 ...
 */
router.post('/categories', validate(createCategorySchema), asyncHandler(async (req, res) => {
    const dbClient = req.dbClient;
    const { code, name_en, name_it, description_en, description_it, parent_id, level, esco_pillar, isced_field, } = req.body;
    if (!code || !name_en) {
    ...
    if (categoryLevel === undefined && parent_id) {
        const parentResult = await dbClient.query('SELECT level FROM ontology_categories WHERE id = $1', [parent_id]);
        if (parentResult.rows.length > 0) {
            categoryLevel = parentResult.rows[0]?.level + 1;
        }
    }
    const result = await dbClient.query(`INSERT INTO ontology_categories
 ...
 */
router.put('/categories/:id', validate(updateCategorySchema), asyncHandler(async (req, res) => {
    const dbClient = req.dbClient;
    const id = req.params.id;
    const { name_en, name_it, description_en, description_it, parent_id, level, esco_pillar, isced_field, is_active, } = req.body;
    const updates = [];
    const params = [];
    let paramIndex = 1;
    if (name_en !== undefined) {
    ...
    updates.push('updated_at = NOW()');
    params.push(id);
    const result = await dbClient.query(`UPDATE ontology_categories SET ${updates.join(', ')} WHERE id = $${paramIndex} RETURNING *`, params);
    if (result.rows.length === 0) {
        throw Errors.notFound('Category', id);
    }
...
router.delete('/categories/:id', asyncHandler(async (req, res) => {
    ...
    const childCheck = await dbClient.query(`SELECT COUNT(*) FROM ontology_categories WHERE parent_id = $1`, [id]);
    if (parseInt(childCheck.rows[0].count) > 0) {
        throw Errors.badRequest('Cannot delete category with child categories. Delete children first.');
    }
    const result = await dbClient.query(`DELETE FROM ontology_categories WHERE id = $1 RETURNING id`, [id]);
    if (result.rows.length === 0) {
        throw Errors.notFound('Category', id);
    }
 ...
 */
router.get('/skills/:id/suggestions', asyncHandler(async (req, res) => {
    const dbClient = req.dbClient;
    const skillId = req.params.id;
    const tenantId = req.query.tenantId;
    const { limit, includeRelations, includeUsage } = req.query;
    const limitNum = safeParseInt(limit, { fallback: 10, max: 50 });
 ...
 */
router.post('/skills/record-usage', validate(recordSkillUsageSchema), asyncHandler(async (req, res) => {
    const dbClient = req.dbClient;
    const { skillIds, tenantId, contextType } = req.body;
