...
import { gapAnalysisSchema, employeeRoleGapSchema, teamRoleGapSchema, compareGapSchema, gapRecommendationsSchema, cachedRecommendationsSchema, createDevelopmentActionSchema, } from '../schemas/talent.js';
...
// =============================================================================
router.post('/', validate(gapAnalysisSchema), asyncHandler(async (req, res) => {
    const { analysisType, employeeId, employeeIds, roleId, 
    // projectId,  // Reserved for future team_project analysis
    tenantId, aggregation = 'average', } = req.body;
    if (!analysisType) {
    ...
    switch (analysisType) {
        ...
        case 'team_role':
            ...
            result = await gapAnalysisService.analyzeTeamVsRole(employeeIds, roleId, aggregation, tenantId);
            break;
        case 'team_project':
            res.status(501).json({
                success: false,
...
// =============================================================================
router.post('/employee-role', validate(employeeRoleGapSchema), asyncHandler(async (req, res) => {
...
// =============================================================================
router.post('/team-role', validate(teamRoleGapSchema), asyncHandler(async (req, res) => {
    ...
    const result = await gapAnalysisService.analyzeTeamVsRole(employeeIds, roleId, aggregation, tenantId);
    res.json({
        success: true,
        data: result,
    });
...
router.get('/summary/:employeeId', asyncHandler(async (req, res) => {
    ...
    const { roleIds, limit = '5' } = req.query;
    // Get employee's current role if not specified
    let targetRoleIds = [];
    if (roleIds) {
        targetRoleIds = roleIds.split(',');
    }
    else {
    ...
    const analyses = await Promise.all(targetRoleIds
        .slice(0, safeParseInt(limit, { fallback: 50 }))
        .map(async (roleId) => {
        try {
...
// =============================================================================
router.post('/compare', validate(compareGapSchema), asyncHandler(async (req, res) => {
    ...
    const analyses = await Promise.all(employeeIds.map(async (empId) => {
        try {
            return await gapAnalysisService.analyzeEmployeeVsRole(empId, roleId, tenantId);
        }
        catch (err) {
            return { error: err.message, employeeId: empId };
        }
    }));
    // Filter valid results and sort by fit score
    const validResults = analyses.filter((a) => !('error' in a));
...
// =============================================================================
router.post('/recommendations', validate(gapRecommendationsSchema), asyncHandler(async (req, res) => {
    ...
    const gapAnalysis = await gapAnalysisService.analyzeEmployeeVsRole(employeeId, roleId, tenantId);
...
// =============================================================================
router.post('/:analysisId/recommendations', validate(cachedRecommendationsSchema), asyncHandler(async (req, res) => {
    const { employeeId, roleId, tenantId, options = {} } = req.body;
    if (!employeeId || !roleId) {
...
// =============================================================================
router.get('/development-plan/:employeeId', asyncHandler(async (req, res) => {
    const employeeId = req.params.employeeId;
    ...
    // Run analysis
    const gapAnalysis = await gapAnalysisService.analyzeEmployeeVsRole(employeeId, roleId);
...
// =============================================================================
router.post('/:employeeId/development-actions', validate(createDevelopmentActionSchema), asyncHandler(async (req, res) => {
    const { employeeId } = req.params;
    const { recommendationId, title, type, priority, skillCoverage, deadline, status, notes, estimatedImpact, duration, } = req.body;
    if (!title || !type) {
