...
router.get('/me', asyncHandler(async (req, res) => {
    ...
    const result = await req.dbClient.query(`
      ...
      LIMIT $4
    `, [employeeId, tenantId, yearFilter, safeParseInt(limit, { fallback: 50 })]);
    res.json({
        success: true,
        data: result.rows,
 ...
 */
router.get('/me/latest', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const authReq = req;
    const employeeId = authReq.user?.employeeId;
    if (!employeeId) {
...
router.get('/', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { employee_id, period, status, year, limit = '50', offset = '0', } = req.query;
    let query = `
      SELECT
        ps.id,
        ps.employee_id,
 ...
 */
router.post('/', validate(createPayStubSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { employee_id, period, period_start, period_end, gross_pay, net_pay, deductions, payment_date, status = 'draft', } = req.body;
 ...
 * Update pay stub
 */
router.patch('/:id', validate(updatePayStubSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'];
    ...
    const result = await req.dbClient.query(`UPDATE employee_pay_stubs SET ${updates.join(', ')} WHERE id = $${paramIndex} RETURNING *`, [...values, id]);
...
//# sourceMappingURL=pay-stubs.js.map