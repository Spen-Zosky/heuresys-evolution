...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const [countResult, dataResult] = await Promise.all([
      ...
      client.query(
        `SELECT al.id, al.timestamp, al.user_id, al.user_email, al.user_role,
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const { resourceType, resourceId } = req.params as Record<string, string>;
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const result = await req.dbClient!.query(
      ...
      LIMIT $3
    `,
      [tenantId, userId, safeParseInt(limit as string, { fallback: 50 })]
    );
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const result = await req.dbClient!.query(
      `SELECT settings->'auditLog' as audit_config FROM tenants WHERE id = $1`,
      [tenantId]
    );
 ...
 */
router.get('/meta/actions', (_req: Request, res: Response) => {
  const actions = [
