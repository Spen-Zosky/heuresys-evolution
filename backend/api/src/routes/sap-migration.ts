...
import {
  ...
  SAPExportParser,
  MigrationJobConfig,
} from '../services/sap-migration.js';
import { validate } from '../middleware/validate.js';
import {
  sapParseTestSchema,
  createSapJobSchema,
  sapParseSchema,
  sapParsePreviewSchema,
  createSapMappingSchema,
  updateSapMappingSchema,
  sapExecuteJobSchema,
  sapRollbackJobSchema,
  sapDeltaSyncCheckSchema,
  sapDeltaSyncExecuteSchema,
  sapEmployeeMappingSchema,
...
router.post(
  '/parse-preview',
  checkPermission(PERMISSIONS.TENANT_CONFIGURE),
  validate(sapParsePreviewSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const { fileContent, format, limit = 10 } = req.body;
 ...
 */
router.get(
  '/jobs/:id/validation-errors',
  checkPermission(PERMISSIONS.TENANT_CONFIGURE),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const jobId = req.params['id'] as string;
    const { severity, limit = '100', offset = '0' } = req.query as Record<string, string>;
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const { infotype } = req.query as Record<string, string>;

    const service = new SAPMigrationService(tenantId);
    const mappings = await service.getMappings(infotype as string | undefined);
...
router.post(
  '/mappings',
  checkPermission(PERMISSIONS.TENANT_CONFIGURE),
  validate(createSapMappingSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const {
      infotype,
...
router.put(
  '/mappings/:id',
  checkPermission(PERMISSIONS.TENANT_CONFIGURE),
  validate(updateSapMappingSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
...
router.post(
  '/delta-sync/check',
  checkPermission(PERMISSIONS.TENANT_CONFIGURE),
  validate(sapDeltaSyncCheckSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
...
router.post(
  '/delta-sync/execute',
  checkPermission(PERMISSIONS.TENANT_CONFIGURE),
  validate(sapDeltaSyncExecuteSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const authReq = req as AuthenticatedRequest;
...
router.post(
  '/employee-mappings',
  checkPermission(PERMISSIONS.TENANT_CONFIGURE),
  validate(sapEmployeeMappingSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
