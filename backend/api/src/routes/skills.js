...
router.get('/stats', asyncHandler(async (req, res) => {
    ...
    const data = await cached('skills:stats', async () => {
...
router.get('/types', asyncHandler(async (req, res) => {
    const dbClient = req.dbClient || pool;
    const data = await cached('skills:types', async () => {
        const result = await dbClient.query(`
...
router.get('/digital', asyncHandler(async (req, res) => {
    ...
    const countResult = await dbClient.query('SELECT COUNT(*) FROM esco_skills WHERE is_digital = true');
    res.json({
        success: true,
        data: result.rows,
        meta: {
 ...
 */
router.get('/green', asyncHandler(async (req, res) => {
    const dbClient = req.dbClient || pool;
    const { limit = '100', offset = '0' } = req.query;
    const result = await dbClient.query(`
      SELECT id, uri, preferred_label_en, description_en, skill_type,
    ...
    const countResult = await dbClient.query('SELECT COUNT(*) FROM esco_skills WHERE is_green = true');
    res.json({
        success: true,
        data: result.rows,
        meta: {
...
router.get('/search', asyncHandler(async (req, res) => {
    const dbClient = req.dbClient || pool;
    const { q, skill_type, reuse_level, is_digital, is_green, limit = '50', } = req.query;
    if (!q) {
...
router.get('/', asyncHandler(async (req, res) => {
    const dbClient = req.dbClient || pool;
    const { skill_type, reuse_level, is_digital, is_green, limit = '100', offset = '0', } = req.query;
...
router.get('/:id', asyncHandler(async (req, res) => {
    ...
    const result = await dbClient.query(`SELECT id, uri, preferred_label, description, skill_type, reuse_level,
 ...
 */
router.post('/', validate(createSkillSchema), asyncHandler(async (req, res) => {
    const dbClient = req.dbClient || pool;
    const { uri, preferred_label_en, description_en, skill_type, reuse_level, is_digital = false, is_green = false, } = req.body;
 ...
 * Update a skill
 */
router.patch('/:id', validate(updateSkillSchema), asyncHandler(async (req, res) => {
    const dbClient = req.dbClient || pool;
    const id = req.params['id'];
    ...
    const result = await dbClient.query(`UPDATE esco_skills SET ${updates.join(', ')} WHERE id = $${paramIndex} RETURNING *`, [...values, id]);
