...
import {
  ...
  linkIdParamSchema,
  performanceSkillLinksQuerySchema,
  mentorMatchesQuerySchema,
...
// Valid rating levels
const VALID_RATING_LEVELS = ['low', 'medium', 'high'] as const;
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const result = await dbClient.query(
      `SELECT * FROM fn_trigger_gap_analysis_for_low_ratings($1, $2)`,
      [tenantId, employeeId]
    );
...
// ============================================================================
router.get(
  '/mentor-matches/:employeeId',
  validate(employeeIdParamSchema, 'params'),
  validate(mentorMatchesQuerySchema, 'query'),
  asyncHandler(async (req: Request, res: Response) => {
    const dbClient = req.dbClient!;
    const { employeeId } = req.params as Record<string, string>;
    const { skillId, limit = '10' } = req.query as Record<string, string>;
    const tenantId = getTenantIdOrThrow(req);
    ...
    if (skillId && !isValidUUID(skillId as string)) {
...
// ============================================================================
router.get(
  '/learning-recommendations/:employeeId',
  validate(employeeIdParamSchema, 'params'),
  asyncHandler(async (req: Request, res: Response) => {
    const dbClient = req.dbClient!;
    const { employeeId } = req.params as Record<string, string>;
    ...
    const result = await dbClient.query(
      `SELECT * FROM v_learning_recommendations
       WHERE tenant_id = $1 AND employee_id = $2
       ORDER BY competency_rating ASC`,
      [tenantId, employeeId]
    );

    // Group by competency
    const byCompetency = result.rows.reduce(
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const result = await dbClient.query(
      `SELECT tenant_id, employee_id, employee_name, job_title, department_name,
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const result = await dbClient.query(
      `SELECT tenant_id, employee_id, employee_name, job_title, department_name,
       ...
       FROM v_performance_skill_summary
       WHERE tenant_id = $1
       ORDER BY low_rated DESC, employee_name`,
      [tenantId]
    );
...
router.get(
  '/links/:employeeId',
  validate(employeeIdParamSchema, 'params'),
  validate(performanceSkillLinksQuerySchema, 'query'),
  asyncHandler(async (req: Request, res: Response) => {
    const dbClient = req.dbClient!;
    const { employeeId } = req.params as Record<string, string>;
    const { ratingLevel } = req.query as Record<string, string>;
    const tenantId = getTenantIdOrThrow(req);
    ...
    if (ratingLevel && !(VALID_RATING_LEVELS as readonly string[]).includes(ratingLevel)) {
    ...
    `;
    const params: (string | number)[] = [tenantId, employeeId];

    if (ratingLevel) {
      query += ` AND psl.rating_level = $3`;
      params.push(ratingLevel as string);
    }
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const pendingReviews = await dbClient.query(
      `SELECT pr.id
    ...
    for (const review of pendingReviews.rows) {
      try {
        const linkResult = await dbClient.query(
          `SELECT * FROM fn_link_performance_to_skills($1, $2)`,
          [tenantId, review.id]
        );
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const competencies = await dbClient.query(
      `SELECT
        psl.competency_name,
       ...
       ORDER BY psl.competency_rating ASC`,
      [tenantId, employeeId]
    );
    ...
    const gapAnalysis = await dbClient.query(
      `SELECT id, tenant_id, analysis_name, analysis_type, target_entity_type,
