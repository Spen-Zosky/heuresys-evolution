...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const totalRes = await req.dbClient!.query(
    ...
    const genderDistribution = genderRes.rows.map((r: Record<string, any>) => ({
      gender: r.gender,
      count: parseInt(r.count, 10),
      percentage:
        totalEmployees > 0 ? Math.round((parseInt(r.count, 10) / totalEmployees) * 1000) / 10 : 0,
    }));
    ...
    const ageDistribution = ageRes.rows.map((r: Record<string, any>) => ({
      range: r.age_range,
      count: parseInt(r.count, 10),
      percentage:
        totalEmployees > 0 ? Math.round((parseInt(r.count, 10) / totalEmployees) * 1000) / 10 : 0,
    ...
    const contractTypeDistribution = contractRes.rows.map((r: Record<string, any>) => ({
      type: r.type,
      count: parseInt(r.count, 10),
      percentage:
        totalEmployees > 0 ? Math.round((parseInt(r.count, 10) / totalEmployees) * 1000) / 10 : 0,
    ...
    const seniorityDistribution = seniorityRes.rows.map((r: Record<string, any>) => ({
