...
import { createReviewCycleSchema, updateReviewCycleSchema, addCycleParticipantsSchema, updateCycleParticipantSchema, addCyclePhasesSchema, updateCyclePhaseSchema, closeCycleSchema, createTemplateSchema, updateTemplateSchema, autoAssignParticipantsSchema, } from '../schemas/performance.js';
...
router.get('/', asyncHandler(async (req, res) => {
    ...
    const { status, cycle_type, limit = '100', offset = '0' } = req.query;
    let query = `
    ...
    const countResult = await req.dbClient.query('SELECT COUNT(*) FROM review_cycles WHERE tenant_id = $1', [tenantId]);
    res.json({
        success: true,
        data: result.rows,
        meta: {
...
router.get('/active', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const data = await cachedForTenant(tenantId, 'review-cycles:active', async () => {
 ...
 * POST /review-cycles
 */
router.post('/', requirePermission('PERFORMANCE', 'CREATE'), validate(createReviewCycleSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
 ...
 */
router.patch('/:id', validateUUID(), requirePermission('PERFORMANCE', 'EDIT'), validate(updateReviewCycleSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'];
    const existing = await req.dbClient.query('SELECT id FROM review_cycles WHERE id = $1 AND tenant_id = $2', [id, tenantId]);
    ...
    const result = await req.dbClient.query(`UPDATE review_cycles SET ${updates.join(', ')} WHERE id = $${paramIndex} AND tenant_id = $${paramIndex + 1} RETURNING *`, [...values, id, tenantId]);
    await invalidateCachePattern(`t:${tenantId}:review-cycles:*`);
...
router.delete('/:id', validateUUID(), requirePermission('PERFORMANCE', 'DELETE'), asyncHandler(async (req, res) => {
    ...
    const feedbackCount = await req.dbClient.query('SELECT COUNT(*) FROM feedback_360 WHERE review_cycle_id = $1', [id]);
    ...
    const result = await req.dbClient.query('DELETE FROM review_cycles WHERE id = $1 AND tenant_id = $2 RETURNING id', [id, tenantId]);
    if (result.rows.length === 0) {
        throw Errors.notFound('Review cycle');
    }
    await invalidateCachePattern(`t:${tenantId}:review-cycles:*`);
...
router.get('/:id/participants', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const cycleId = req.params['id'];
    const { status, manager_id, org_unit_id, limit = '100', offset = '0', } = req.query;
    const participants = await performanceManagementService.getReviewCycleParticipants(tenantId, cycleId, {
        status: status,
 ...
 */
router.post('/:id/participants', validate(addCycleParticipantsSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const cycleId = req.params['id'];
    const { employee_ids, org_unit_id, include_all } = req.body;
    // Verify cycle exists and is not completed
    const cycleCheck = await req.dbClient.query('SELECT id, status FROM review_cycles WHERE id = $1 AND tenant_id = $2', [cycleId, tenantId]);
    ...
    if (include_all) {
        ...
        const employees = await req.dbClient.query('SELECT id FROM employees WHERE tenant_id = $1 AND status = $2', [tenantId, 'active']);
        participantIds = employees.rows.map((e) => e.id);
    }
    else if (org_unit_id) {
        // Add all employees from a department
        const employees = await req.dbClient.query('SELECT id FROM employees WHERE tenant_id = $1 AND org_unit_id = $2 AND status = $3', [tenantId, org_unit_id, 'active']);
        participantIds = employees.rows.map((e) => e.id);
    }
    ...
    const addedCount = await performanceManagementService.addReviewCycleParticipants(tenantId, cycleId, participantIds);
    await invalidateCachePattern(`t:${tenantId}:review-cycles:*`);
    res.status(201).json({
        success: true,
 ...
 */
router.get('/:id/participants/:participantId', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
 ...
 */
router.patch('/:id/participants/:participantId', validate(updateCycleParticipantSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const participantId = req.params['participantId'];
    const { status, notes, self_review_completed, manager_review_completed, calibration_completed, } = req.body;
    const additionalUpdates = {};
    if (notes !== undefined)
        additionalUpdates.notes = notes;
    ...
    const participant = await performanceManagementService.updateParticipantStatus(tenantId, participantId, status, Object.keys(additionalUpdates).length > 0 ? additionalUpdates : undefined);
    if (!participant) {
        throw Errors.notFound('Participant');
    }
    await invalidateCachePattern(`t:${tenantId}:review-cycles:*`);
 ...
 */
router.delete('/:id/participants/:participantId', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { id: cycleId, participantId } = req.params;
    const result = await req.dbClient.query('DELETE FROM review_cycle_participants WHERE id = $1 AND review_cycle_id = $2 AND tenant_id = $3 RETURNING id', [participantId, cycleId, tenantId]);
    if (result.rows.length === 0) {
        throw Errors.notFound('Participant');
    }
    await invalidateCachePattern(`t:${tenantId}:review-cycles:*`);
...
router.post('/:id/launch', asyncHandler(async (req, res) => {
    ...
    const cycleCheck = await req.dbClient.query('SELECT id, status, name FROM review_cycles WHERE id = $1 AND tenant_id = $2', [cycleId, tenantId]);
    if (cycleCheck.rows.length === 0) {
        throw Errors.notFound('Review cycle');
    }
    ...
    const participantCount = await req.dbClient.query('SELECT COUNT(*) FROM review_cycle_participants WHERE review_cycle_id = $1', [cycleId]);
 ...
 */
router.post('/:id/close', validate(closeCycleSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const cycleId = req.params['id'];
    ...
    if (!force) {
        const incompleteCount = await req.dbClient.query("SELECT COUNT(*) FROM review_cycle_participants WHERE review_cycle_id = $1 AND status != 'completed'", [cycleId]);
 ...
 */
router.post('/:id/phases', validate(addCyclePhasesSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const cycleId = req.params['id'];
 ...
 */
router.patch('/:id/phases/:phaseId', validate(updateCyclePhaseSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { id: cycleId, phaseId } = req.params;
 ...
 */
router.get('/config/templates', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { is_active, template_type } = req.query;
    const cacheKey = `review-cycles:templates:${is_active ?? 'all'}:${template_type ?? 'all'}`;
    const data = await cachedForTenant(tenantId, cacheKey, async () => {
 ...
 */
router.post('/config/templates', validate(createTemplateSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { name, description, template_type, rating_scale_type, rating_scale_config, sections, competencies, include_goals, include_development_plan, is_default, } = req.body;
    if (!name || !sections) {
        throw Errors.badRequest('name and sections are required');
    }
    // If setting as default, unset other defaults
    if (is_default) {
        await req.dbClient.query('UPDATE performance_review_templates SET is_default = false WHERE tenant_id = $1', [tenantId]);
    }
    const result = await req.dbClient.query(`
 ...
 */
router.get('/config/templates/:templateId', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const templateId = req.params['templateId'];
    const result = await req.dbClient.query(`SELECT id, tenant_id, name, description, template_type,
        rating_scale_type, rating_scale_config, sections, competencies,
        include_goals, include_development_plan, is_default, is_active,
        created_at, updated_at
      FROM performance_review_templates WHERE id = $1 AND tenant_id = $2`, [templateId, tenantId]);
    if (result.rows.length === 0) {
 ...
 * Update a template
 */
router.patch('/config/templates/:templateId', validate(updateTemplateSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const templateId = req.params['templateId'];
    const allowedFields = [
        'name',
    ...
    for (const field of allowedFields) {
        if (req.body[field] !== undefined) {
            ...
            if (['sections', 'competencies', 'rating_scale_config'].includes(field) &&
                typeof value === 'object') {
    ...
    if (req.body.is_default === true) {
        await req.dbClient.query('UPDATE performance_review_templates SET is_default = false WHERE tenant_id = $1 AND id != $2', [tenantId, templateId]);
    }
    values.push(templateId, tenantId);
    const result = await req.dbClient.query(`UPDATE performance_review_templates SET ${updates.join(', ')} WHERE id = $${paramIndex++} AND tenant_id = $${paramIndex} RETURNING *`, values);
    if (result.rows.length === 0) {
        throw Errors.notFound('Template');
    }
    await invalidateCachePattern(`t:${tenantId}:review-cycles:*`);
 ...
 */
router.post('/:id/auto-assign', validate(autoAssignParticipantsSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const cycleId = req.params['id'];
    ...
    const result = await req.dbClient.query('SELECT fn_auto_assign_review_participants($1, $2, $3, $4) as count', [tenantId, cycleId, org_unit_ids || null, employee_status]);
 ...
 */
router.get('/config/rating-scales', asyncHandler(async (_req, res) => {
    const scales = [
        {
            type: '1-5',
            config: {
...
router.get('/:id/summary', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const cycleId = req.params['id'];
    const result = await req.dbClient.query('SELECT * FROM v_review_cycle_summary WHERE id = $1 AND tenant_id = $2', [cycleId, tenantId]);
    if (result.rows.length === 0) {
        throw Errors.notFound('Review cycle');
    }
    // Get phases
    const phases = await req.dbClient.query(`SELECT id, tenant_id, review_cycle_id, phase_name, phase_order,
        start_date, end_date, status, instructions, reminder_days_before,
        escalation_days_after, is_required, created_at, updated_at
      FROM review_cycle_phases WHERE review_cycle_id = $1 ORDER BY phase_order LIMIT 50`, [cycleId]);
    res.json({
        success: true,
        data: {
            ...(result.rows[0] || {}),
...
//# sourceMappingURL=review-cycles.js.map