...
import { createSkillAssessmentSchema, updateSkillAssessmentSchema, } from '../schemas/skills-assessment.js';
...
router.get('/', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { employee_id, skill_name, method, has_gap, limit = '50', offset = '0', } = req.query;
    let query = `
    SELECT
      esa.id,
      esa.employee_id,
...
router.get('/employee/:employeeId', asyncHandler(async (req, res) => {
    ...
    const empCheck = await req.dbClient.query('SELECT id, first_name, last_name, job_title FROM employees WHERE id = $1 AND tenant_id = $2', [employeeId, tenantId]);
...
router.get('/gaps', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { min_gap = '1', limit = '50' } = req.query;
    const result = await req.dbClient.query(`
    SELECT
      esa.id,
      esa.employee_id,
  ...
  `, [
        tenantId,
        safeParseInt(min_gap, { fallback: 0 }),
 ...
 */
router.post('/', validate(createSkillAssessmentSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { employee_id, skill_name, esco_skill_uri, assessed_level, required_level, assessment_date, assessment_method, assessed_by, evidence_notes, certification_url, } = req.body;
    if (!employee_id || !skill_name || !assessed_level) {
        throw Errors.badRequest('employee_id, skill_name, and assessed_level are required');
    }
    // Verify employee belongs to tenant
    const empCheck = await req.dbClient.query('SELECT id FROM employees WHERE id = $1 AND tenant_id = $2', [employee_id, tenantId]);
    if (empCheck.rows.length === 0) {
 ...
 * Update assessment
 */
router.patch('/:id', validate(updateSkillAssessmentSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'];
  ...
  `, [id, tenantId]);
    ...
    const result = await req.dbClient.query(`UPDATE employee_skill_assessments SET ${updates.join(', ')} WHERE id = $${paramIndex} RETURNING *`, [...values, id]);
 ...
 */
router.get('/skills/summary', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const result = await req.dbClient.query(`
    SELECT
      esa.skill_name,
...
//# sourceMappingURL=skill-assessments.js.map