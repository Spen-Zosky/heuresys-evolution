 ...
 */
const LOCKOUT_DURATIONS: readonly number[] = [0, 0, 0, 0, 0, 60, 300, 900, 3600, 86400];
...
async function checkAccountLock(
  ...
  const result = await pool.query(
    'SELECT attempt_count, locked_until FROM login_attempts WHERE username = $1',
    [username]
  );
  ...
  const row = result.rows[0] as { attempt_count: number; locked_until: string | null };
...
async function recordFailedAttempt(username: string): Promise<void> {
  const result = await pool.query(
    `INSERT INTO login_attempts (username, attempt_count, last_failed_at, updated_at)
     VALUES ($1, 1, NOW(), NOW())
  ...
  const attemptCount = (result.rows[0] as { attempt_count: number }).attempt_count;
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const lockStatus = await checkAccountLock(username as string);
    ...
    const userResult = await pool.query(
      `SELECT u.id, u.username, u.password_hash, u.role, u.permissions,
    ...
    if (userResult.rows.length === 0) {
      ...
      await recordFailedAttempt(username as string);
    ...
    await resetLoginAttempts(username as string);
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (jti && exp) {
      ...
      if (remainingTTL > 0) {
        ...
        if (!blacklisted) {
          logger.warn(
            `[Auth] Failed to blacklist token on logout (Redis unavailable) - jti: ${jti}`
          );
        }
      }
    }
    ...
    if (refreshToken) {
      try {
        ...
        const refreshPayload = jwt.decode(refreshToken) as JWTPayload | null;
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (result.rows.length === 0) {
      res.json({
        success: true,
        message: `No lockout record found for user '${username as string}'. Account is not locked.`,
      });
      return;
    }
    ...
    res.json({
      success: true,
      message: `Account '${(result.rows[0] as { username: string }).username}' has been unlocked successfully`,
    });
  })
);
