...
import {
  createMarketplaceHookSchema,
  updateMarketplaceHookSchema,
  createMarketplaceUiSlotSchema,
  updateMarketplaceUiSlotSchema,
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (plugin_id) {
      ...
      params.push(plugin_id as string);
      paramIndex++;
    }

    if (hook_name) {
      query += ` AND h.hook_name = $${paramIndex}`;
      params.push(hook_name as string);
      paramIndex++;
    }
...
router.post(
  ...
  validate(createMarketplaceHookSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const dbClient = req.dbClient!;
    try {
      const {
...
router.put(
  ...
  validate(updateMarketplaceHookSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const dbClient = req.dbClient!;
    const id = req.params['id'] as string;
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (slot_name) {
      ...
      params.push(slot_name as string);
      paramIndex++;
    }
...
router.post(
  ...
  authMiddleware,
  requirePermission('MARKETPLACE', 'CREATE'),
  validate(createMarketplaceUiSlotSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const dbClient = req.dbClient!;
    try {
...
router.put(
  ...
  authMiddleware,
  requirePermission('MARKETPLACE', 'EDIT'),
  validate(updateMarketplaceUiSlotSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const dbClient = req.dbClient!;
    const id = req.params['id'] as string;
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const { hook_id, status, limit = '50', offset = '0' } = req.query as Record<string, string>;
