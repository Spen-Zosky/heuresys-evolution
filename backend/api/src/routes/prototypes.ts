...
import { CompanySize } from '../services/industry-prototype.js';
import { JobSkillsEnrichmentService } from '../services/job-skills-enrichment.js';
import { validate } from '../middleware/validate.js';
import {
  prototypeResearchSchema,
  createProcessSchema,
  createStaffingRulesSchema,
  calculateStaffingSchema,
  generatePrototypeSchema,
  previewPrototypeSchema,
  validateStaffingSchema,
  addJobSkillSchema,
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (nace_section) {
      ...
      params.push((nace_section as string) + '%');
      paramIndex++;
    }

    if (size_class) {
      query += ` AND ip.company_size_code = $${paramIndex}`;
      params.push((size_class as string).toUpperCase());
      paramIndex++;
    }
...
router.post(
  '/research',
  validate(prototypeResearchSchema),
  asyncHandler(async (req: Request, res: Response) => {
...
router.get(
  '/:id/processes',
  asyncHandler(async (req: Request, res: Response) => {
    const prototypeId = req.params['id'] as string;
    const { process_category } = req.query as Record<string, string>;
    ...
    `;
    const params: string[] = [prototypeId];
    const paramIndex = 2;

    if (process_category) {
      query += ` AND bp.process_category = $${paramIndex}`;
      params.push(process_category as string);
    }
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!process_code || !process_name || !process_category || !value_chain_position) {
      throw Errors.badRequest(
        'process_code, process_name, process_category, and value_chain_position are required'
      );
    }
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    const prototypeId = req.params['id'] as string;
    const { company_size } = req.query as Record<string, string>;

    const service = new StaffingRulesGeneratorService('system');
    const rules = await service.getStaffingRules(
      prototypeId,
      company_size ? (company_size as CompanySize) : undefined
    );
...
router.post(
  '/:id/staffing-rules',
  validate(createStaffingRulesSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const prototypeId = req.params['id'] as string;
 ...
 */
router.post(
  '/:id/staffing-rules/calculate',
  validate(calculateStaffingSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const prototypeId = req.params['id'] as string;
    const { company_size = 'medium' } = req.body;

    const service = new StaffingRulesGeneratorService('system');
    const result = await service.calculateOptimalStaffing(prototypeId, company_size as CompanySize);
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const protoResult = await req.dbClient!.query(
      'SELECT nace_class_code FROM industry_profiles WHERE id = $1',
      [prototypeId]
    );
 ...
 */
router.post(
  '/tenants/:tenantId/generate/preview',
  requireTenant,
  validate(previewPrototypeSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = req.params['tenantId'] as string;
    ...
    // Get prototype details
    const protoResult = await req.dbClient!.query(
      `SELECT id, code, name, description, nace_class_code,
       ...
       FROM industry_profiles WHERE id = $1`,
      [effectiveProfileId]
    );
    ...
    const processesResult = await req.dbClient!.query(
      `SELECT id, profile_id, process_code, process_name, process_category,
              value_chain_position, description, typical_inputs, typical_outputs,
              created_at, updated_at
       FROM business_processes WHERE profile_id = $1 ORDER BY value_chain_position`,
      [effectiveProfileId]
    );

    // Get staffing rules
    const staffingService = new StaffingRulesGeneratorService(tenantId);
    const staffingResult = await staffingService.calculateOptimalStaffing(
      effectiveProfileId,
 ...
 */
router.post(
  '/tenants/:tenantId/validate-staffing',
  requireTenant,
  validate(validateStaffingSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = req.params['tenantId'] as string;
 ...
 */
router.get(
  '/tenants/:tenantId/org-chart/excalidraw',
  requireTenant,
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = req.params['tenantId'] as string;
 ...
 */
router.get(
  '/tenants/:tenantId/prototype-report',
  requireTenant,
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = req.params['tenantId'] as string;
    ...
    const tenantResult = await req.dbClient!.query(
      `SELECT t.id, t.code, t.name, t.description, t.region, t.status,
 ...
 */
router.post(
  '/job-templates/:jobTemplateId/enrich-skills',
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const jobTemplateId = req.params['jobTemplateId'] as string;
 ...
 */
router.get(
  '/job-templates/:jobTemplateId/skill-distribution',
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const jobTemplateId = req.params['jobTemplateId'] as string;
 ...
 */
router.get(
  '/job-templates/:jobTemplateId/skill-balance',
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const jobTemplateId = req.params['jobTemplateId'] as string;
 ...
 */
router.get(
  '/job-templates/:jobTemplateId/suggest-skills',
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const jobTemplateId = req.params['jobTemplateId'] as string;
    const {
    ...
    const suggestions = await service.suggestSkillsForJobTemplate(jobTemplateId, {
      max_suggestions: parseInt(max_suggestions as string),
      ...
      min_transferability_score: min_transferability
        ? parseFloat(min_transferability as string)
        : undefined,
    });
 ...
 */
router.post(
  '/job-templates/:jobTemplateId/add-skill',
  validate(addJobSkillSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const jobTemplateId = req.params['jobTemplateId'] as string;
 ...
 */
router.get(
  '/clusters/:clusterId/suggest-skills',
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const clusterId = req.params['clusterId'] as string;
    const { exclude_ids, limit = '10' } = req.query as Record<string, string>;

    if (!clusterId) {
      throw Errors.badRequest('Cluster ID is required');
    }

    const excludeIds = exclude_ids ? (exclude_ids as string).split(',') : [];

    const service = new JobSkillsEnrichmentService(tenantId);
    const suggestions = await service.suggestSkillsByCluster(
      clusterId,
      excludeIds,
      safeParseInt(limit as string, { fallback: 50 })
    );
