...
import {
  ...
  updateInstallationConfigSchema,
  disableInstallationSchema,
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const pluginCheck = await req.dbClient!.query(
      'SELECT id FROM plugins WHERE id = $1 AND status = $2',
      [plugin_id, 'published']
    );
    ...
    if (depsResult.rows.length > 0) {
      const installedDeps = await req.dbClient!.query(
        `SELECT plugin_id FROM plugin_installations
         WHERE tenant_id = $1 AND status IN ('active', 'disabled') AND plugin_id = ANY($2)`,
        [
          tenantId,
          depsResult.rows.map((r: { depends_on_plugin_id: string }) => r.depends_on_plugin_id),
        ]
      );
      const installedSet = new Set(
        installedDeps.rows.map((r: { plugin_id: string }) => r.plugin_id)
      );
      const missing = depsResult.rows.filter(
        (d: { depends_on_plugin_id: string; name: string }) =>
          !installedSet.has(d.depends_on_plugin_id)
      );
      if (missing.length > 0) {
        throw Errors.badRequest('Missing required dependencies', {
          code: 'MISSING_DEPENDENCIES',
          missing: missing.map((d: { depends_on_plugin_id: string; name: string }) => ({
    ...
    // Get latest version
    const versionCheck = await req.dbClient!.query(
      'SELECT id FROM plugin_versions WHERE plugin_id = $1 AND is_latest = true',
      [plugin_id]
    );
    ...
    const existingInstall = await req.dbClient!.query(
      `SELECT id FROM plugin_installations
       WHERE plugin_id = $1 AND tenant_id = $2 AND status IN ('active', 'disabled')`,
      [plugin_id, tenantId]
    );
    ...
    await req.dbClient!.query(
      'UPDATE plugins SET total_installations = total_installations + 1 WHERE id = $1',
      [plugin_id]
    );
...
router.put(
  '/:id/configuration',
  validate(updateInstallationConfigSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'] as string;
...
router.patch(
  '/:id/disable',
  validate(disableInstallationSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'] as string;
...
router.delete(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const installResult = await req.dbClient!.query(
      `SELECT id, plugin_id FROM plugin_installations
       WHERE id = $1 AND tenant_id = $2 AND status IN ('active', 'disabled')`,
      [id, tenantId]
    );
    ...
    if (dependentCheck.rows.length > 0) {
      throw Errors.badRequest('Cannot uninstall: other installed plugins depend on this plugin', {
        code: 'HAS_DEPENDENTS',
        dependents: dependentCheck.rows.map((r: { id: string; name: string }) => ({
          id: r.id,
          name: r.name,
        })),
      });
    ...
    await req.dbClient!.query(
      'DELETE FROM plugin_configurations WHERE plugin_installation_id = $1 AND tenant_id = $2',
      [id, tenantId]
    );
    ...
    await req.dbClient!.query(
      'UPDATE plugins SET total_installations = GREATEST(total_installations - 1, 0) WHERE id = $1',
      [pluginId]
    );
