...
import { createCareerPathSchema, updateCareerPathSchema, addLevelSkillSchema, simulateCareerPathSchema, enrollCareerPathSchema, updateCareerProgressSchema, } from '../schemas/talent.js';
...
router.get('/', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { department, path_type, is_active, search, limit = '100', offset = '0', } = req.query;
    let query = `
      SELECT cp.*,
    ...
    const countResult = await req.dbClient.query('SELECT COUNT(*) FROM career_paths WHERE tenant_id = $1', [tenantId]);
    res.json({
        success: true,
        data: result.rows,
        meta: {
 ...
 * POST /career-paths
 */
router.post('/', validate(createCareerPathSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { name, description, department, path_type = 'linear', is_active = true, created_by_employee_id, } = req.body;
    if (!name) {
 ...
 */
router.patch('/:id', validate(updateCareerPathSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'];
    const existing = await req.dbClient.query('SELECT id FROM career_paths WHERE id = $1 AND tenant_id = $2', [id, tenantId]);
    ...
    const result = await req.dbClient.query(`UPDATE career_paths SET ${updates.join(', ')} WHERE id = $${paramIndex} AND tenant_id = $${paramIndex + 1} RETURNING *`, [...values, id, tenantId]);
...
router.delete('/:id', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'];
    const result = await req.dbClient.query('DELETE FROM career_paths WHERE id = $1 AND tenant_id = $2 RETURNING id', [id, tenantId]);
    if (result.rows.length === 0) {
        throw Errors.notFound('Career path');
    }
 ...
 */
router.get('/:id/levels', asyncHandler(async (req, res) => {
 ...
 */
router.post('/:id/levels/:levelId/skills', validate(addLevelSkillSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { levelId } = req.params;
    const { skill_id, importance = 'important', is_mandatory = false, weight = 1.0, required_knowledge_level, required_skill_level, required_ability_level, required_behavior_level, required_attitude_level, min_composite_score, notes, } = req.body;
    if (!skill_id) {
 ...
 */
router.post('/simulate', validate(simulateCareerPathSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { employee_id, target_path_id, target_level_id, target_job_id, simulation_name } = req.body;
    if (!employee_id) {
 ...
 */
router.get('/simulations/:id', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'];
    const result = await req.dbClient.query(`
      SELECT
...
router.get('/recommendations/:employeeId', asyncHandler(async (req, res) => {
    ...
    const recommendations = await careerPathService.getRecommendationsForEmployee(tenantId, employeeId, refresh === 'true');
    res.json({
        success: true,
 ...
 */
router.get('/reachable-roles/:employeeId', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const employeeId = req.params['employeeId'];
    const { max_gap_threshold } = req.query;
    const roles = await careerPathService.getReachableRoles(tenantId, employeeId, max_gap_threshold ? parseFloat(max_gap_threshold) : undefined);
    res.json({
        success: true,
        data: roles,
        count: roles.length,
 ...
 */
router.get('/progress/:employeeId', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
 ...
 */
router.post('/:pathId/enroll/:employeeId', validate(enrollCareerPathSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { employeeId, pathId } = req.params;
 ...
 */
router.put('/progress/:employeeId/:levelId', validate(updateCareerProgressSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const employeeId = req.params['employeeId'];
    const levelId = req.params['levelId'];
    const { status } = req.body;
    const updateFields = [];
    const values = [employeeId, levelId, tenantId];
    let paramIndex = 4;
    if (status) {
 ...
 */
router.get('/fit-score/:employeeId/:pathId', asyncHandler(async (req, res) => {
    getTenantIdOrThrow(req); // Verify tenant access
    const { employeeId, pathId } = req.params;
    // Use DB function
    const result = await req.dbClient.query(`
...
//# sourceMappingURL=career-paths.js.map