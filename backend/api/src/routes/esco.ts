...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    const dbClient = req.dbClient!;
    const { level, parent_uri } = req.query as Record<string, string>;
    ...
    if (parent_uri) {
      params.push(parent_uri as string);
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const validLang = (lang as string) === 'it' ? 'it' : 'en';
    const labelCol = validateEmbeddingColumn(
      validLang === 'it' ? 'preferred_label_it' : 'preferred_label_en',
      'esco.occupations-sort'
    );
    ...
    if (isco_code) {
      params.push(isco_code as string);
    ...
    params.push(safeParseInt(offset as string, { fallback: 0 }));
    ...
    if (search) {
      countParams.push(`%${escapeILIKE(search as string)}%`);
      countQuery += ` AND (
    ...
    if (isco_code) {
      countParams.push(isco_code as string);
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const validSkillLang = (lang as string) === 'it' ? 'it' : 'en';
    const skillLabelCol = validateEmbeddingColumn(
      validSkillLang === 'it' ? 'preferred_label_it' : 'preferred_label_en',
      'esco.skills-sort'
    );
    ...
    if (skill_type) {
      countParams.push(skill_type as string);
      countQuery += ` AND s.skill_type = $${countParams.length}`;
    }
    if (is_digital === 'true') countQuery += ` AND s.is_digital = true`;
    if (is_green === 'true') countQuery += ` AND s.is_green = true`;
    if (is_transversal === 'true') countQuery += ` AND s.is_transversal = true`;
    if (reuse_level) {
      countParams.push(reuse_level as string);
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    const dbClient = req.dbClient!;
    const { parent_uri } = req.query as Record<string, string>;
 ...
 */
router.get(
  '/occupation-skills/:occupationId',
  asyncHandler(async (req: Request, res: Response) => {
    const dbClient = req.dbClient!;
    const occupationId = req.params.occupationId as string;
    const { relation_type } = req.query as Record<string, string>;
    ...
    `;
    const params: string[] = [occupationId];

    if (relation_type) {
      params.push(relation_type as string);
