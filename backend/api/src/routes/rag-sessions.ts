...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const countResult = await req.dbClient!.query(
      'SELECT COUNT(*) FROM rag_sessions WHERE tenant_id = $1',
      [tenantId]
    );
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const session = await req.dbClient!.query(
      'SELECT id FROM rag_sessions WHERE id = $1 AND tenant_id = $2',
      [id, tenantId]
    );
    ...
    const result = await req.dbClient!.query(
      ...
      LIMIT $2
    `,
      [id, safeParseInt(limit as string, { fallback: 50 })]
    );
...
router.post(
  '/',
  validate(createRagSessionSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const { user_id, user_id_employee_id, provider, model, title, system_prompt, sources_enabled } =
      req.body;
...
router.patch(
  '/:id',
  validate(updateRagSessionSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'] as string;
    ...
    const result = await req.dbClient!.query(
      `UPDATE rag_sessions SET ${updates.join(', ')} WHERE id = $${paramIndex} AND tenant_id = $${paramIndex + 1} RETURNING *`,
      [...values, id, tenantId]
    );
...
router.delete(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const result = await req.dbClient!.query(
      'DELETE FROM rag_sessions WHERE id = $1 AND tenant_id = $2 RETURNING id',
      [id, tenantId]
    );
