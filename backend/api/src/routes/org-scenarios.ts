...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const baseline = scenarios.find((s: any) => s.isBaseline) || null;
...
router.post(
  '/',
  validate(CreateOrgScenarioSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const {
      name,
    ...
    const userId = (req as Request & { user?: Record<string, any> }).user?.userId || null;
...
router.put(
  '/:id',
  validate(UpdateOrgScenarioSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const { id } = req.params;
    const {
