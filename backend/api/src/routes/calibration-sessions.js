...
import { createCalibrationSessionSchema, updateCalibrationSessionSchema, addCalibrationParticipantsSchema, flagOutlierSchema, createAdjustmentSchema, updateAdjustmentSchema, updateAdjustmentNotesSchema, completeCalibrationSessionSchema, cancelCalibrationSessionSchema, } from '../schemas/performance.js';
...
router.get('/', requirePermission('PERFORMANCE', 'VIEW'), applyScopeFilter('PERFORMANCE'), asyncHandler(async (req, res) => {
    ...
    const { status, review_cycle_id, org_unit_id, limit = '100', offset = '0', } = req.query;
    ...
    const countResult = await req.dbClient.query(`SELECT COUNT(*) FROM calibration_sessions cs WHERE ${scope.where}`, scope.params);
    res.json({
        success: true,
        data: result.rows,
        meta: {
 ...
 */
router.get('/:id/outliers', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
 ...
 */
router.post('/:id/flag-outlier', requirePermission('PERFORMANCE', 'CREATE'), validate(flagOutlierSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const sessionId = req.params['id'];
 ...
 */
router.get('/:id/9box', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const sessionId = req.params['id'];
    const result = await req.dbClient.query(`
    ...
    const summary = {
        ...
        development_needed: (boxes[4] || []).length + (boxes[2] || []).length + (boxes[1] || []).length,
    };
    res.json({
        success: true,
        data: {
 ...
 */
router.get('/:id/bell-curve', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const sessionId = req.params['id'];
    const result = await req.dbClient.query(`
 ...
 */
router.get('/:id/audit-log', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const sessionId = req.params['id'];
 ...
 */
router.patch('/:id/adjustments/:adjustmentId/notes', requirePermission('PERFORMANCE', 'EDIT'), validate(updateAdjustmentNotesSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { id: sessionId, adjustmentId } = req.params;
    ...
    const oldValue = await req.dbClient.query('SELECT discussion_notes FROM calibration_adjustments WHERE id = $1 AND calibration_session_id = $2 AND tenant_id = $3', [adjustmentId, sessionId, tenantId]);
...
router.get('/:id/details', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const sessionId = req.params['id'];
    const details = await performanceManagementService.getCalibrationSessionDetails(tenantId, sessionId);
    if (!details.session) {
        throw Errors.notFound('Calibration session');
    }
 ...
 */
router.post('/', requirePermission('PERFORMANCE', 'CREATE'), validate(createCalibrationSessionSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { name, description, review_cycle_id, org_unit_id, scheduled_date, scheduled_end_date, location, meeting_link, facilitator_id, facilitator_ids, status = 'scheduled', } = req.body;
    if (!name) {
 ...
 */
router.patch('/:id', requirePermission('PERFORMANCE', 'EDIT'), validate(updateCalibrationSessionSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const sessionId = req.params['id'];
    const existing = await req.dbClient.query('SELECT id, status FROM calibration_sessions WHERE id = $1 AND tenant_id = $2', [sessionId, tenantId]);
    ...
    const result = await req.dbClient.query(`UPDATE calibration_sessions SET ${updates.join(', ')} WHERE id = $${paramIndex} AND tenant_id = $${paramIndex + 1} RETURNING *`, [...values, sessionId, tenantId]);
    res.json({
        success: true,
        data: result.rows[0] || null,
 ...
 */
router.delete('/:id', requirePermission('PERFORMANCE', 'DELETE'), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const sessionId = req.params['id'];
    // Check session status
    const sessionCheck = await req.dbClient.query('SELECT status FROM calibration_sessions WHERE id = $1 AND tenant_id = $2', [sessionId, tenantId]);
    ...
    await withTransaction(async (txClient) => {
        ...
        await txClient.query('DELETE FROM calibration_sessions WHERE id = $1 AND tenant_id = $2 RETURNING id', [sessionId, tenantId]);
    }, tenantId);
 ...
 */
router.post('/:id/participants', requirePermission('PERFORMANCE', 'CREATE'), validate(addCalibrationParticipantsSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const sessionId = req.params['id'];
    ...
    const addedCount = await performanceManagementService.addCalibrationParticipants(tenantId, sessionId, participants);
    res.status(201).json({
        success: true,
 ...
 */
router.delete('/:id/participants/:participantId', requirePermission('PERFORMANCE', 'DELETE'), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { id: sessionId, participantId } = req.params;
    const result = await req.dbClient.query('DELETE FROM calibration_participants WHERE id = $1 AND session_id = $2 AND tenant_id = $3 RETURNING id', [participantId, sessionId, tenantId]);
    if (result.rows.length === 0) {
 ...
 */
router.post('/:id/start', requirePermission('PERFORMANCE', 'EDIT'), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const sessionId = req.params['id'];
    const sessionCheck = await req.dbClient.query('SELECT status FROM calibration_sessions WHERE id = $1 AND tenant_id = $2', [sessionId, tenantId]);
    if (sessionCheck.rows.length === 0) {
    ...
    const subjectCount = await req.dbClient.query("SELECT COUNT(*) FROM calibration_participants WHERE session_id = $1 AND role = 'subject'", [sessionId]);
 ...
 */
router.post('/:id/complete', requirePermission('PERFORMANCE', 'EDIT'), validate(completeCalibrationSessionSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const sessionId = req.params['id'];
    const { notes, decisions } = req.body;
    const session = await performanceManagementService.completeCalibrationSession(tenantId, sessionId, notes, decisions);
    if (!session) {
 ...
 */
router.post('/:id/cancel', requirePermission('PERFORMANCE', 'EDIT'), validate(cancelCalibrationSessionSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const sessionId = req.params['id'];
 ...
 */
router.post('/:id/adjustments', requirePermission('PERFORMANCE', 'CREATE'), validate(createAdjustmentSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const sessionId = req.params['id'];
    const { employee_id, performance_review_id, original_rating, adjusted_rating, adjustment_reason, competency_adjustments, adjusted_by, } = req.body;
    if (!employee_id) {
    ...
    const adjustment = await performanceManagementService.recordCalibrationAdjustment(tenantId, sessionId, {
        employee_id,
 ...
 */
router.patch('/:id/adjustments/:adjustmentId', requirePermission('PERFORMANCE', 'EDIT'), validate(updateAdjustmentSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    ...
    updates.push('updated_at = NOW()');
    const result = await req.dbClient.query(`UPDATE calibration_adjustments SET ${updates.join(', ')} WHERE id = $${paramIndex} AND tenant_id = $${paramIndex + 1} RETURNING *`, [...values, adjustmentId, tenantId]);
    if (result.rows.length === 0) {
        throw Errors.notFound('Adjustment');
    }
 ...
 */
router.delete('/:id/adjustments/:adjustmentId', requirePermission('PERFORMANCE', 'DELETE'), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { id: sessionId, adjustmentId } = req.params;
    const result = await req.dbClient.query('DELETE FROM calibration_adjustments WHERE id = $1 AND session_id = $2 AND tenant_id = $3 RETURNING id', [adjustmentId, sessionId, tenantId]);
    if (result.rows.length === 0) {
        throw Errors.notFound('Adjustment');
    }
 ...
 */
router.get('/:id/distribution', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const sessionId = req.params['id'];
 ...
 */
router.post('/:id/apply-adjustments', requirePermission('PERFORMANCE', 'EDIT'), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const sessionId = req.params['id'];
    ...
    const adjIds = adjustments.rows.map((a) => a.id);
...
//# sourceMappingURL=calibration-sessions.js.map