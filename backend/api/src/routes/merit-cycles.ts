...
import {
  createMeritCycleSchema,
  updateMeritCycleSchema,
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const countResult = await req.dbClient!.query(
      `SELECT COUNT(*) FROM merit_cycles mc WHERE ${scope.where}`,
      scope.params
    );
...
router.post(
  ...
  validate(createMeritCycleSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const {
      name,
...
router.patch(
  ...
  validate(updateMeritCycleSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'] as string;

    const existing = await req.dbClient!.query(
      'SELECT id FROM merit_cycles WHERE id = $1 AND tenant_id = $2',
      [id, tenantId]
    );
    ...
    const result = await req.dbClient!.query(
      `UPDATE merit_cycles SET ${updates.join(', ')} WHERE id = $${paramIndex} AND tenant_id = $${paramIndex + 1} RETURNING *`,
      [...values, id, tenantId]
    );
...
router.delete(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const result = await req.dbClient!.query(
      "UPDATE merit_cycles SET status = 'cancelled', updated_at = NOW() WHERE id = $1 AND tenant_id = $2 RETURNING id",
      [id, tenantId]
    );
