...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    let payload: { userId: string; requires_2fa: boolean };
    try {
    ...
    const userResult = await pool.query(
      `SELECT u.id, u.username, u.role, u.permissions, u.is_active,
    ...
    if (!verified && user.totp_recovery_codes?.length > 0) {
      for (let i = 0; i < user.totp_recovery_codes.length; i++) {
        ...
        const match =
          (await bcrypt.compare(cleanCode, hashedCode)) ||
    ...
    if (!verified) {
      ...
      if (attempts >= MAX_TOTP_ATTEMPTS) {
        ...
        await pool.query(
          'UPDATE users SET totp_failed_attempts = $1, totp_lockout_until = $2 WHERE id = $3',
      ...
      throw Errors.unauthorized(
        `Invalid code. ${MAX_TOTP_ATTEMPTS - attempts} attempts remaining.`
      );
    }

    // Success
    await pool.query(
      'UPDATE users SET totp_failed_attempts = 0, totp_lockout_until = NULL, last_login = NOW() WHERE id = $1',
      [user.id]
    );
