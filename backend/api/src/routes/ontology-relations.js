...
router.get('/skill-clusters', asyncHandler(async (req, res) => {
    const limit = safeParseInt(req.query['limit'], { fallback: 20, min: 1, max: 100 });
    const offset = safeParseInt(req.query['offset'], { fallback: 0, min: 0 });
    const rawType = req.query['skillType'];
 ...
 */
router.get('/career-pathways/:occupationId', asyncHandler(async (req, res) => {
 ...
 */
router.get('/process-skill-impact', asyncHandler(async (req, res) => {
    const dbClient = req.dbClient;
    const service = new OntologyRelationsService(dbClient ?? pool);
