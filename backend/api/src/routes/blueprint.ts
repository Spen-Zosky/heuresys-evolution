...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!tenantId) throw Errors.badRequest('Tenant context required');
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const countResult = await req.dbClient!.query(
      `SELECT COUNT(*) AS total FROM blueprint_runs WHERE tenant_id = $1`,
      [tenantId]
    );
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const runResult = await req.dbClient!.query(
      `SELECT * FROM blueprint_runs WHERE id = $1 AND tenant_id = $2`,
      [runId, tenantId]
    );
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const countResult = await req.dbClient!.query(
      `SELECT COUNT(*) AS total FROM blueprint_results WHERE ${whereClause}`,
      params
    );
...
router.post(
  '/runs/:runId/results/:resultId/apply',
  requirePermission('ORGANIZATION', 'EDIT'),
  asyncHandler(async (req: Request, res: Response) => {
    const { runId, resultId } = req.params as { runId: string; resultId: string };
    assertUUID(runId, 'run ID');
    assertUUID(resultId, 'result ID');

    const result = await req.dbClient!.query(
      `UPDATE blueprint_results
       SET is_applied = true, applied_at = NOW(), applied_by = $1
       WHERE id = $2 AND run_id = $3
       RETURNING *`,
      [(req as AuthenticatedRequest).user?.userId ?? null, resultId, runId]
    );
