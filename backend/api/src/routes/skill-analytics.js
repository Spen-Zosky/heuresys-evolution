...
import { skillAnalyticsHeatmapQuerySchema, skillAnalyticsTrendsQuerySchema, skillAnalyticsShortagesQuerySchema, skillAnalyticsKsabaQuerySchema, skillAnalyticsEmergingQuerySchema, skillAnalyticsDepartmentsQuerySchema, orgUnitIdParamSchema, } from '../schemas/analytics-extended.js';
 ...
 */
router.get('/heatmap', validate(skillAnalyticsHeatmapQuerySchema, 'query'), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const orgUnitIds = req.query['org_unit_ids']
        ? req.query['org_unit_ids'].split(',')
        : undefined;
 ...
 */
router.get('/trends', validate(skillAnalyticsTrendsQuerySchema, 'query'), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const skillIds = req.query['skill_ids']
        ? req.query['skill_ids'].split(',')
        : undefined;
    const period = req.query['period'] || 'monthly';
    const lookbackPeriods = safeParseInt(req.query['lookback_periods'], { fallback: 6 });
 ...
 */
router.get('/shortages', validate(skillAnalyticsShortagesQuerySchema, 'query'), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const orgUnitId = req.query['org_unit_id'];
    const minShortage = safeParseInt(req.query['min_shortage'], { fallback: 0 });
 ...
 */
router.get('/ksaba', validate(skillAnalyticsKsabaQuerySchema, 'query'), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const orgUnitId = req.query['org_unit_id'];
 ...
 */
router.get('/emerging', validate(skillAnalyticsEmergingQuerySchema, 'query'), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const lookbackDays = safeParseInt(req.query['lookback_days'], { fallback: 90 });
    const minOccurrences = safeParseInt(req.query['min_occurrences'], { fallback: 3 });
 ...
 */
router.get('/org-units', validate(skillAnalyticsDepartmentsQuerySchema, 'query'), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const orgUnitIds = req.query['org_unit_ids']
        ? req.query['org_unit_ids'].split(',')
        : undefined;
 ...
 */
router.get('/org-units/:orgUnitId', validate(orgUnitIdParamSchema, 'params'), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
...
//# sourceMappingURL=skill-analytics.js.map