...
import {
  createWellbeingCheckinBasicSchema,
  updateWellbeingCheckinSchema,
} from '../schemas/wellbeing.js';
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const scale = (v: number) => Math.round(v * 10);

    const positiveAvg = [mood, energy, wlb, sleep].filter((v) => v > 0);
    const overallRaw =
      positiveAvg.length > 0 ? positiveAvg.reduce((a, b) => a + b, 0) / positiveAvg.length : 0;
    // Subtract stress contribution (higher stress = lower wellbeing)
    const stressAdjustment = stress > 0 ? 10 - stress : 0;
    const overall =
      positiveAvg.length > 0
        ? Math.round(
            ((overallRaw + stressAdjustment) / (positiveAvg.length + (stress > 0 ? 1 : 0))) * 10
          )
        : 0;

    // Resources count
    const resourcesResult = await req.dbClient!.query(
      'SELECT COUNT(*) AS cnt FROM wellbeing_resources WHERE tenant_id = $1 AND is_active = true',
      [tenantId]
    );

    // Active goals count
    const goalsResult = await req.dbClient!.query(
      "SELECT COUNT(*) AS cnt FROM wellbeing_goals WHERE tenant_id = $1 AND status = 'active'",
      [tenantId]
    );
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const countResult = await req.dbClient!.query(
      'SELECT COUNT(*) FROM wellbeing_resources WHERE tenant_id = $1 AND is_active = true AND deleted_at IS NULL',
      [tenantId]
    );
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const result = await req.dbClient!.query(query, params);
    const countResult = await req.dbClient!.query(
      'SELECT COUNT(*) FROM wellbeing_checkins WHERE tenant_id = $1',
      [tenantId]
    );
...
router.post(
  '/checkins',
  validate(createWellbeingCheckinBasicSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const {
      employee_id,
...
router.patch(
  '/checkins/:id',
  validate(updateWellbeingCheckinSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'] as string;

    const existing = await req.dbClient!.query(
      'SELECT id FROM wellbeing_checkins WHERE id = $1 AND tenant_id = $2',
      [id, tenantId]
    );
    if (existing.rows.length === 0) {
      throw Errors.notFound('Check-in');
    ...
    const result = await req.dbClient!.query(
      `UPDATE wellbeing_checkins SET ${updates.join(', ')} WHERE id = $${paramIndex} AND tenant_id = $${paramIndex + 1} RETURNING *`,
      [...values, id, tenantId]
    );
...
router.delete(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const result = await req.dbClient!.query(
      'DELETE FROM wellbeing_checkins WHERE id = $1 AND tenant_id = $2 RETURNING id',
      [id, tenantId]
    );
