...
async function resolveDashboardId(dashboardCode: unknown): Promise<number | null> {
  if (!dashboardCode || typeof dashboardCode !== 'string') return null;
  const row = await pool.query<{ id: number }>(
    `SELECT id FROM rbp_dashboards WHERE code = $1 LIMIT 1`,
    [dashboardCode]
  );
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const userWs = await pool.query(
      `SELECT uw.id, uw.name, uw.layout_config, uw.dashboard_id,
              COALESCE(
                json_agg(
                  json_build_object(
                    'code', wc.code,
    ...
    const roleRow = await pool.query(
      `SELECT id FROM rbp_roles WHERE hierarchy_level = $1 LIMIT 1`,
      [roleLevel]
    );
    ...
    if (roleId) {
      const tmpl = await pool.query(
        `SELECT id, name, name_it, name_en, layout_config, widget_config
      ...
      if (tmpl.rows.length > 0) {
        ...
        const codes = rawWidgets
          .map((w: Record<string, unknown>) => w.widget_code || w.code)
          .filter(Boolean);
        ...
        const cacheRows =
          codes.length > 0
            ? (
                await pool.query<{ code: string; cache_ttl_seconds: number; swr_seconds: number }>(
                  `SELECT code, cache_ttl_seconds, swr_seconds FROM widget_catalog WHERE code = ANY($1)`,
                  [codes]
                )
              ).rows
            : [];
        const cacheMap = new Map(cacheRows.map((r) => [r.code, r]));
        const widgets = rawWidgets.map((w: Record<string, unknown>) => {
          const code = (w.widget_code || w.code) as string;
...
async function ensureDashboardWorkspace(
  ...
  if (dashboardId) {
    const dashRow = await pool.query<{ name: string }>(
      `SELECT name FROM rbp_dashboards WHERE id = $1 LIMIT 1`,
      [dashboardId]
    );
  ...
  const created = await pool.query(
    `INSERT INTO user_workspaces
     ...
     RETURNING id`,
    [userId, tenantId, dashboardId, resolvedName]
  );
...
router.get(
  ...
  asyncHandler(async (_req: Request, res: Response) => {
    const result = await pool.query(
      `SELECT id, code, name, name_it, name_en, icon
         FROM rbp_dashboards
        WHERE is_active = true
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const cat = await pool.query<{ id: string }>(
      `SELECT id FROM widget_catalog WHERE code = $1 AND is_active = true LIMIT 1`,
      [code]
    );
    ...
    const existing = await pool.query(
      `SELECT id FROM workspace_widgets
        WHERE workspace_id = $1 AND widget_catalog_id = $2
        LIMIT 1`,
      [workspaceId, widgetCatalogId]
    );
    ...
    const maxSort = await pool.query<{ max_sort: number | null }>(
    ...
    const inserted = await pool.query(
      `INSERT INTO workspace_widgets
       ...
       RETURNING id`,
      [workspaceId, widgetCatalogId, sortOrder]
    );
...
router.delete(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const result = await pool.query(
      `DELETE FROM workspace_widgets ww
        ...
        WHERE ww.workspace_id = uw.id
          ...
          AND wc.code = $4
       RETURNING ww.id`,
      [userId, tenantId, dashboardId, code]
    );
...
router.patch(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const sets: string[] = [];
    const params: unknown[] = [userId, tenantId, dashboardId, code];
    ...
    const result = await pool.query(
      `UPDATE workspace_widgets ww
        ...
        WHERE ww.workspace_id = uw.id
          ...
          AND uw.dashboard_id IS NOT DISTINCT FROM $3::int
          AND wc.code = $4
       RETURNING ww.id, ww.position_x, ww.position_y, ww.width, ww.height, ww.is_visible, ww.sort_order`,
      params
    );
...
router.patch(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const tenantId = authReq.tenantId ?? authReq.user.tenantId;
    const dashboardId = await resolveDashboardId(req.query.dashboard);
    const layoutConfig = (req.body as { layout_config?: unknown })?.layout_config;
    ...
    await pool.query(
      `UPDATE user_workspaces
        ...
        WHERE id = $1`,
      [workspaceId, JSON.stringify(layoutConfig)]
    );
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    try {
      ...
      const sourceWs = await client.query(
        `SELECT id, name, layout_config, dashboard_id
      ...
      const newWs = await client.query(
        `INSERT INTO user_workspaces
         ...
         RETURNING id, name`,
        [userId, tenantId, dashboardId, `${src.name} (copy)`, JSON.stringify(src.layout_config)]
      );
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const widgetsResult = await pool.query(
      `SELECT wc.code, wc.name, wc.name_it, wc.name_en, wc.widget_type,
...
async function fetchHeadcountKpi(ctx: WidgetContext): Promise<Record<string, unknown>> {
  if (!ctx.tenantId) return { label: 'Headcount', value: null, sublabel: null };
  try {
    const result = await pool.query(
...
async function fetchOpenPositionsKpi(ctx: WidgetContext): Promise<Record<string, unknown>> {
  if (!ctx.tenantId) return { label: 'Posizioni aperte', value: null };
  try {
    const result = await pool.query(
      `SELECT COUNT(*)::int AS total,
...
async function fetchEngagementScore(ctx: WidgetContext): Promise<Record<string, unknown>> {
  if (!ctx.tenantId) return { label: 'Engagement', value: null };
  try {
    const result = await pool.query(
      `SELECT engagement_score, respondents
...
async function fetchPerformanceScores(ctx: WidgetContext): Promise<Record<string, unknown>> {
  if (!ctx.tenantId) return { label: 'Performance', value: null };
  try {
    const result = await pool.query(
      `SELECT
        ...
        WHERE tenant_id = $1
          ...
          AND status IN ('completed', 'finalized', 'submitted', 'shared')`,
      [ctx.tenantId]
    );
    const row = result.rows[0];
...
async function fetchLeaveBalance(ctx: WidgetContext): Promise<Record<string, unknown>> {
  if (!ctx.employeeId || !ctx.tenantId) return { year: null, lines: [] };
  try {
    const currentYear = new Date().getFullYear();
    const result = await pool.query(
      `SELECT leave_type, balance, used_days, pending_days, year
         FROM leave_balances
        WHERE tenant_id = $1 AND employee_id = $2
          AND year = $3
        ORDER BY leave_type`,
      [ctx.tenantId, ctx.employeeId, currentYear]
    );
...
async function fetchPendingApprovals(ctx: WidgetContext): Promise<Record<string, unknown>> {
  if (!ctx.userId || !ctx.tenantId) return { total: 0, items: [] };
  try {
    const result = await pool.query(
 ...
 */
async function fetchPlaceholderMetadata(
  code: string,
  ctx: WidgetContext
): Promise<Record<string, unknown>> {
  if (!ctx.tenantId) return { placeholder: true, code, metadata: null };
  try {
    const result = await pool.query(
      `SELECT code, name, description, name_it, name_en, description_it, description_en,
...
async function fetchMyCard(ctx: WidgetContext): Promise<Record<string, unknown>> {
  if (!ctx.employeeId || !ctx.tenantId) return { employee: null };
  try {
    const result = await pool.query(
      `SELECT e.id, e.first_name, e.last_name, e.email, e.job_title, e.hire_date,
              e.location AS location_text,
              ou.name AS department_name,
              loc.name AS work_location, loc.city AS work_city,
              ec.contract_type, ec.annual_salary AS base_salary, ec.currency,
              ec.level, ec.ccnl_code, ec.end_date AS contract_end_date,
              ec.monthly_salary, ec.num_monthly_payments, ec.salary_override_reason,
              cl.monthly_salary AS ccnl_min_monthly,
              mgr.first_name AS manager_first_name, mgr.last_name AS manager_last_name,
              mgr.job_title AS manager_job_title,
              (
                SELECT COALESCE(SUM(total_days + accrued_days + carryover_days + adjustment_days - used_days - pending_days), 0)::float
                FROM employee_time_off_balances
                WHERE employee_id = e.id AND tenant_id = e.tenant_id
                  AND leave_type = 'vacation'
                  AND year = EXTRACT(year FROM CURRENT_DATE)::int
              ) AS vacation_days_remaining,
              (
                SELECT MIN(review_period_end)
                FROM performance_reviews
                WHERE employee_id = e.id AND tenant_id = e.tenant_id
                  AND status IN ('scheduled', 'in_progress', 'draft')
                  AND review_period_end >= CURRENT_DATE
              ) AS next_review_date,
              (
                SELECT COUNT(*)::int
                FROM employees
                WHERE manager_id = e.id AND tenant_id = e.tenant_id
       ...
       LEFT JOIN employees mgr ON mgr.id = e.manager_id AND mgr.tenant_id = e.tenant_id
       LEFT JOIN employee_contracts ec ON ec.employee_id = e.id AND ec.tenant_id = e.tenant_id AND ec.is_current = true
       LEFT JOIN ccnl_levels cl ON cl.ccnl_code = ec.ccnl_code AND cl.level_code = ec.level AND cl.is_current = true
...
async function fetchNotifications(ctx: WidgetContext): Promise<Record<string, unknown>> {
  if (!ctx.userId || !ctx.tenantId) return { items: [] };
  try {
    const result = await pool.query(
      `SELECT id, type, title, message, priority, read, action_url, created_at
       FROM notifications
...
async function fetchMyTasks(): Promise<Record<string, unknown>> {
  // tasks table does not exist — return empty gracefully
  return { items: [] };
}

async function fetchMyDocuments(ctx: WidgetContext): Promise<Record<string, unknown>> {
  if (!ctx.employeeId || !ctx.tenantId) return { items: [] };
  try {
    const result = await pool.query(
      `SELECT id, title, document_type, category, status, created_at
...
async function fetchLearningProgress(ctx: WidgetContext): Promise<Record<string, unknown>> {
  if (!ctx.employeeId) return { total: 0, completed: 0, in_progress: 0 };
  try {
    const result = await pool.query(
      `SELECT
         COUNT(*) AS total,
...
async function fetchCareerPath(ctx: WidgetContext): Promise<Record<string, unknown>> {
  if (!ctx.employeeId || !ctx.tenantId) return { job_title: null, career_paths: [] };
  try {
    const empResult = await pool.query(
      `SELECT job_title FROM employees WHERE id = $1 AND tenant_id = $2 LIMIT 1`,
      [ctx.employeeId, ctx.tenantId]
    );
    const jobTitle = empResult.rows[0]?.job_title ?? null;

    const pathResult = await pool.query(
      `SELECT cp.id, cp.name, cp.path_type
...
async function fetchQuickLinks(): Promise<Record<string, unknown>> {
  return {
    links: [
