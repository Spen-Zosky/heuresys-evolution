...
import { createMarketplaceDependencySchema, updateMarketplaceDependencySchema, } from '../schemas/marketplace-extended.js';
 ...
 */
router.get('/dependents/:pluginId', asyncHandler(async (req, res) => {
    const dbClient = req.dbClient || pool;
    const pluginId = req.params['pluginId'];
    const result = await dbClient.query(`
    SELECT
 ...
 */
router.post('/', authMiddleware, requirePermission('MARKETPLACE', 'CREATE'), validate(createMarketplaceDependencySchema), asyncHandler(async (req, res) => {
    const dbClient = req.dbClient || pool;
    const { plugin_id, depends_on_plugin_id, min_version, max_version, is_optional = false, } = req.body;
 ...
 */
router.put('/:id', authMiddleware, requirePermission('MARKETPLACE', 'EDIT'), validate(updateMarketplaceDependencySchema), asyncHandler(async (req, res) => {
    const dbClient = req.dbClient || pool;
...
router.delete('/:id', authMiddleware, requirePermission('MARKETPLACE', 'DELETE'), asyncHandler(async (req, res) => {
    const dbClient = req.dbClient || pool;
    const id = req.params['id'];
    const result = await dbClient.query('DELETE FROM plugin_dependencies WHERE id = $1 RETURNING id', [id]);
    if (result.rows.length === 0) {
        throw Errors.notFound('Dependency', id);
    }
 ...
 */
router.get('/check/:pluginId', asyncHandler(async (req, res) => {
    const dbClient = req.dbClient || pool;
    const pluginId = req.params['pluginId'];
...
//# sourceMappingURL=marketplace-dependencies.js.map