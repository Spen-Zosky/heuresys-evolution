...
import {
  createContinuousFeedbackSchema,
  createQuickFeedbackSchema,
  createFeedback360Schema,
  updateFeedback360Schema,
  completeFeedback360Schema,
  createQuestionnaireSchema,
  requestFeedback360Schema,
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const { limit = '100', offset = '0', employee_id } = req.query as Record<string, string>;
    ...
    const continuousParams: (string | number)[] = [tenantId];
    ...
    const f360Params: (string | number)[] = [tenantId];
    let f360Filter = '';
    if (employee_id) {
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (from_employee_id) {
      ...
      params.push(from_employee_id as string);
      paramIndex++;
    }

    if (to_employee_id) {
      query += ` AND cf.to_employee_id = $${paramIndex}`;
      params.push(to_employee_id as string);
      paramIndex++;
    }
    ...
    const countResult = await req.dbClient!.query(
      'SELECT COUNT(*) FROM continuous_feedback WHERE tenant_id = $1',
      [tenantId]
    );
...
router.post(
  '/continuous',
  validate(createContinuousFeedbackSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const {
      from_employee_id,
...
router.delete(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const result = await req.dbClient!.query(
      'DELETE FROM continuous_feedback WHERE id = $1 AND tenant_id = $2 RETURNING id',
      [id, tenantId]
    );
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const { limit = '50', offset = '0', category } = req.query as Record<string, string>;
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const { feedback_type, limit = '100', offset = '0' } = req.query as Record<string, string>;
...
router.post(
  '/continuous/quick',
  validate(createQuickFeedbackSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const {
      from_employee_id,
 ...
 */
router.get(
  '/360/suggested-peers/:employeeId',
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const employeeId = req.params['employeeId'] as string;
    const { review_cycle_id, limit = '10' } = req.query as Record<string, string>;
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const stats = {
      ...
      response_rate:
        result.rows.length > 0
          ? Math.round(
              (result.rows.filter((r) => r.status === 'completed').length / result.rows.length) *
                100
            )
          : 0,
    };
 ...
 */
router.post(
  '/360/complete/:requestId',
  validate(completeFeedback360Schema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (target_employee_id) {
      ...
      params.push(target_employee_id as string);
      paramIndex++;
    }

    if (reviewer_employee_id) {
      query += ` AND f.reviewer_employee_id = $${paramIndex}`;
      params.push(reviewer_employee_id as string);
      paramIndex++;
    }
    ...
    const result = await req.dbClient!.query(query, params);
    const countResult = await req.dbClient!.query(
      'SELECT COUNT(*) FROM feedback_360 WHERE tenant_id = $1',
      [tenantId]
    );
...
router.post(
  '/360',
  validate(createFeedback360Schema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const {
      target_employee_id,
...
router.patch(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const existing = await req.dbClient!.query(
      'SELECT id FROM feedback_360 WHERE id = $1 AND tenant_id = $2',
      [id, tenantId]
    );
 ...
 */
router.post(
  '/360/:reviewId/request',
  validate(requestFeedback360Schema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const reviewId = req.params['reviewId'] as string;
    const {
