...
import { getUserPermissions, getDashboardNavItems, invalidateRbpCache, isRbpEnabled, } from '../middleware/rbpMiddleware.js';
 ...
 */
router.get('/dashboard/:slug/nav-items', async (req, res) => {
    try {
        const authReq = req;
        const roleCode = authReq.user?.role;
        ...
        const accessCheck = await pool.query(`SELECT 1 FROM rbp_role_dashboards rd
       JOIN rbp_roles r ON r.id = rd.role_id
       JOIN rbp_dashboards d ON d.id = rd.dashboard_id
       WHERE r.code = $1 AND d.code = $2`, [roleCode, slug]);
...
router.get('/roles', async (req, res) => {
    try {
        ...
        const result = await pool.query(`SELECT code, name, description, description_it, description_en,
...
router.get('/dashboards', async (req, res) => {
    try {
        ...
        const result = await pool.query(`SELECT d.code, d.name, d.description, d.name_it, d.name_en,
       ...
       ORDER BY rd.is_default DESC, d.sort_order`, [roleCode]);
        return res.json(result.rows);
    }
    catch (err) {
...
router.get('/navigation', async (req, res) => {
    try {
        ...
        const result = await pool.query(`SELECT d.code AS dashboard_code, d.name AS dashboard_name,
       ...
       WHERE r.code = $1 AND d.is_active = true
       ORDER BY rd.is_default DESC, d.sort_order, ni.sort_order`, [roleCode]);
 ...
 */
router.post('/cache/invalidate', validate(InvalidateCacheSchema), async (req, res) => {
    try {
        const authReq = req;
        const roleCode = authReq.user?.role;
        if (!['SUPERUSER', 'TENANT_OWNER'].includes(roleCode)) {
...
router.get('/teams', async (req, res) => {
    try {
        ...
        const result = await (req.dbClient ?? pool).query(`SELECT t.id, t.code, t.name, t.description, t.purpose, t.is_active, t.created_at,
...
router.post('/teams', validate(CreateTeamSchema), async (req, res) => {
    try {
        ...
        const result = await (req.dbClient ?? pool).query(`INSERT INTO rbp_teams (tenant_id, code, name, description, purpose, created_by)
       VALUES ($1, $2, $3, $4, $5, $6)
       RETURNING *`, [tenantId, code, name, description || null, purpose || null, userId]);
 ...
 */
router.post('/teams/:teamId/members', validate(TeamMemberSchema), async (req, res) => {
    try {
        const teamId = parseInt(req.params.teamId);
        const { employeeId } = req.body;
        // Cat A: use appPool (via req.dbClient) to enforce RLS on rbp_team_members INSERT
        const result = await (req.dbClient ?? pool).query(`INSERT INTO rbp_team_members (team_id, employee_id)
       VALUES ($1, $2)
       ON CONFLICT (team_id, employee_id) DO NOTHING
       RETURNING *`, [teamId, employeeId]);
 ...
 */
router.post('/teams/:teamId/leaders', validate(TeamMemberSchema), async (req, res) => {
    try {
        const teamId = parseInt(req.params.teamId);
        const { employeeId } = req.body;
        // Cat A: use appPool (via req.dbClient) to enforce RLS on rbp_team_leaders INSERT
        const result = await (req.dbClient ?? pool).query(`INSERT INTO rbp_team_leaders (team_id, employee_id)
       VALUES ($1, $2)
       ON CONFLICT (team_id, employee_id) DO NOTHING
       RETURNING *`, [teamId, employeeId]);
        return res.status(201).json({
 ...
 */
router.post('/break-glass', validate(BreakGlassSchema), async (req, res) => {
    try {
        const authReq = req;
        const userId = authReq.user?.userId;
        const roleCode = authReq.user?.role;
        ...
        await (req.dbClient ?? pool).query(`INSERT INTO audit_logs (tenant_id, user_id, action, resource_type, description, metadata, ip_address)
       VALUES ($1, $2, 'BREAK_GLASS_REQUEST', 'rbp_role_elevation', $3, $4, $5)`, [
        ...
        console.warn(`[RBP] BREAK-GLASS: User ${userId} (${roleCode}) requested elevation to ${targetRole}. Reason: ${reason}`);
        return res.json({
            status: 'PENDING',
...
router.get('/break-glass/pending', async (req, res) => {
    try {
        ...
        const result = await (req.dbClient ?? pool).query(`SELECT id, user_id, metadata, ip_address, created_at
       FROM audit_logs
 ...
 */
router.post('/break-glass/:requestId/approve', async (req, res) => {
    try {
        const authReq = req;
        ...
        const requestResult = await (req.dbClient ?? pool).query(`SELECT id, user_id, metadata FROM audit_logs
       WHERE id = $1 AND action = 'BREAK_GLASS_REQUEST'
         AND (metadata->>'status') = 'PENDING'`, [requestId]);
        ...
        await (req.dbClient ?? pool).query(`UPDATE audit_logs SET metadata = metadata || $1
       WHERE id = $2`, [
            JSON.stringify({
                status: 'APPROVED',
        ...
        // Log approval
        await (req.dbClient ?? pool).query(`INSERT INTO audit_logs (tenant_id, user_id, action, resource_type, metadata, ip_address)
       VALUES ($1, $2, 'BREAK_GLASS_APPROVED', 'rbp_role_elevation', $3, $4)`, [
        ...
        console.warn(`[RBP] BREAK-GLASS APPROVED: Request ${requestId} approved by ${approverId}. Expires: ${expiresAt.toISOString()}`);
        return res.json({
            status: 'APPROVED',
            requestId,
            targetRole: details.targetRole,
 ...
 */
router.post('/break-glass/:requestId/deny', validate(DenyBreakGlassSchema), async (req, res) => {
    try {
        const authReq = req;
        const approverRole = authReq.user?.role;
        const approverId = authReq.user?.userId;
