...
import {
  compaRatioQuerySchema,
  bonusDistributionQuerySchema,
} from '../schemas/workforce-analytics.js';
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    res.json({
      ...
      data: {
        ...
        by_gender: genderEquity.rows.map((r: Record<string, any>) => ({
          gender: r.gender,
        ...
        by_org_unit_gender: departmentEquity.rows.map((r: Record<string, any>) => ({
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    res.json({
      ...
      data: {
        ...
        bands: bands.rows.map((b: Record<string, any>) => ({
          id: b.id,
          code: b.band_code,
          name: b.band_name,
        ...
        band_distribution: bandAssignments.rows.map((b: Record<string, any>) => ({
...
router.get(
  '/compa-ratio',
  validate(compaRatioQuerySchema, 'query'),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const dbClient = req.dbClient!;
    const { org_unit_id } = req.query as Record<string, string>;
    ...
    compaRatios.rows.forEach((r) => {
      if (r.compa_ratio === null) {
        ...
        if (ratio < 80) distribution.below_80++;
        else if (ratio < 90) distribution['80_to_90']++;
        else if (ratio < 100) distribution['90_to_100']++;
        else if (ratio < 110) distribution['100_to_110']++;
        else if (ratio < 120) distribution['110_to_120']++;
    ...
    res.json({
      ...
      data: {
        ...
        distribution,
        by_org_unit: byDepartment.rows.map((r: Record<string, any>) => ({
          department: r.department_name,
          employee_count: parseInt(r.employee_count),
        ...
        employees: compaRatios.rows.slice(0, 50).map((r: Record<string, any>) => ({
          id: r.id,
...
router.get(
  '/bonus-distribution',
  validate(bonusDistributionQuerySchema, 'query'),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const dbClient = req.dbClient!;
    ...
    res.json({
      ...
      data: {
        ...
        by_performance: byPerformance.rows.map((r: Record<string, any>) => ({
        ...
        plans: plansSummary.rows.map((r: Record<string, any>) => ({
          name: r.plan_name,
          type: r.plan_type,
