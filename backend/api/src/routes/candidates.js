...
import { createCandidateSchema, updateCandidateSchema, advanceCandidateSchema, rejectCandidateSchema, } from '../schemas/recruitment.js';
...
router.get('/', requirePermission('RECRUITMENT', 'VIEW'), applyScopeFilter('RECRUITMENT'), asyncHandler(async (req, res) => {
    ...
    const [countResult, dataResult] = await Promise.all([
        req.dbClient.query(`SELECT COUNT(*) as total FROM recruiting_candidates rc ${whereClause}`, params),
        req.dbClient.query(`SELECT rc.id, rc.first_name, rc.last_name, rc.email, rc.phone,
...
router.get('/', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { requisition_id, stage, source, search, limit = '100', offset = '0', } = req.query;
    let query = `
    SELECT c.*,
  ...
  `;
    ...
    const countResult = await req.dbClient.query('SELECT COUNT(*) FROM recruiting_candidates WHERE tenant_id = $1', [tenantId]);
    res.json({
        success: true,
        data: result.rows,
        meta: {
 ...
 * POST /candidates
 */
router.post('/', requirePermission('RECRUITMENT', 'CREATE'), validate(createCandidateSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { requisition_id, first_name, last_name, email, phone, current_company, job_title, experience_years, source = 'direct', resume_url, linkedin_url, portfolio_url, notes, skills, } = req.body;
    if (!first_name || !last_name || !email) {
        throw Errors.badRequest('first_name, last_name, and email are required');
    }
    // Check for duplicate email for same requisition
    if (requisition_id) {
        const duplicate = await req.dbClient.query('SELECT id FROM recruiting_candidates WHERE email = $1 AND requisition_id = $2 AND tenant_id = $3', [email, requisition_id, tenantId]);
 ...
 * PATCH /candidates/:id
 */
router.patch('/:id', requirePermission('RECRUITMENT', 'EDIT'), validate(updateCandidateSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'];
    const existing = await req.dbClient.query('SELECT id FROM recruiting_candidates WHERE id = $1 AND tenant_id = $2', [id, tenantId]);
    ...
    const result = await req.dbClient.query(`UPDATE recruiting_candidates SET ${updates.join(', ')} WHERE id = $${paramIndex} AND tenant_id = $${paramIndex + 1} RETURNING *`, [...values, id, tenantId]);
    res.json({ success: true, data: result.rows[0] || null, message: 'Candidate updated' });
}));
/**
 * POST /candidates/:id/advance
 */
router.post('/:id/advance', requirePermission('RECRUITMENT', 'EDIT'), validate(advanceCandidateSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'];
    const { next_stage, notes } = req.body;
    const stageOrder = ['new', 'screening', 'interview', 'offer', 'hired'];
    const current = await req.dbClient.query('SELECT stage FROM recruiting_candidates WHERE id = $1 AND tenant_id = $2', [id, tenantId]);
 ...
 */
router.post('/:id/reject', requirePermission('RECRUITMENT', 'EDIT'), validate(rejectCandidateSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'];
...
router.delete('/:id', requirePermission('RECRUITMENT', 'DELETE'), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'];
    const result = await req.dbClient.query('DELETE FROM recruiting_candidates WHERE id = $1 AND tenant_id = $2 RETURNING id', [id, tenantId]);
    if (result.rows.length === 0) {
        throw Errors.notFound('Candidate', id);
    }
