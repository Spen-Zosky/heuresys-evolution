...
import { testAzureConfigSchema, testGoogleConfigSchema, updateAzureConfigSchema, updateGoogleConfigSchema, } from '../schemas/admin.js';
const router = Router();
// =============================================================================
...
router.get('/providers', asyncHandler(async (req, res) => {
    ...
    const result = await dbClient.query(`SELECT settings->'sso' as sso_config FROM tenants WHERE code = $1`, [tenantCode]);
 ...
 */
router.get('/azure/login', asyncHandler(async (req, res) => {
    const dbClient = req.dbClient;
    const tenantCode = req.query['tenant'];
    ...
    const result = await dbClient.query(`SELECT id, settings->'sso'->'azureAd' as azure_config FROM tenants WHERE code = $1`, [tenantCode]);
    if (result.rows.length === 0) {
        throw Errors.notFound('Tenant', tenantCode);
    }
    ...
    const redirectUri = azureConfig.redirectUri || `${config.baseUrl}/api/v1/auth/sso/azure/callback`;
 ...
 */
router.get('/azure/callback', asyncHandler(async (req, res) => {
    const dbClient = req.dbClient;
    ...
    // Get Azure AD config
    const tenantResult = await dbClient.query(`SELECT id, code, settings->'sso'->'azureAd' as azure_config FROM tenants WHERE id = $1`, [stateData.tenantId]);
    ...
    if (!tokenResponse.ok) {
        ...
        throw Errors.badRequest(`Token exchange failed: ${errorData.error_description || 'Unknown error'}`);
    }
    ...
    const user = await provisionSSOUser(dbClient, stateData.tenantId, userInfo.email, userInfo.given_name || userInfo.name.split(' ')[0] || 'User', userInfo.family_name || userInfo.name.split(' ').slice(1).join(' ') || '', 'azure_ad', userInfo.sub);
    // Generate JWT tokens
    const tokenPayload = {
        userId: user.id,
    ...
    const redirectUrl = new URL(stateData.redirect || '/dashboard', config.frontendUrl || config.baseUrl);
 ...
 */
router.post('/azure/test', validate(testAzureConfigSchema), asyncHandler(async (req, res) => {
    ...
    if (!response.ok) {
        ...
        throw Errors.badRequest(`Connection failed: ${errorData.error_description || 'Invalid credentials'}`);
    }
    res.json({
        success: true,
        data: {
...
router.get('/google/login', asyncHandler(async (req, res) => {
    ...
    const result = await dbClient.query(`SELECT id, settings->'sso'->'google' as google_config FROM tenants WHERE code = $1`, [tenantCode]);
    if (result.rows.length === 0) {
        throw Errors.notFound('Tenant', tenantCode);
    }
    ...
    const redirectUri = googleConfig.redirectUri || `${config.baseUrl}/api/v1/auth/sso/google/callback`;
    const scope = 'openid profile email';
    const state = Buffer.from(JSON.stringify({
        tenantCode,
        tenantId: result.rows[0]?.id,
...
router.get('/google/callback', asyncHandler(async (req, res) => {
    ...
    // Get Google config
    const tenantResult = await dbClient.query(`SELECT id, code, settings->'sso'->'google' as google_config FROM tenants WHERE id = $1`, [stateData.tenantId]);
    if (tenantResult.rows.length === 0) {
        throw Errors.notFound('Tenant', stateData.tenantId);
    }
    ...
    // Provision or update user via JIT provisioning
    const user = await provisionSSOUser(dbClient, stateData.tenantId, userInfo.email, userInfo.given_name || userInfo.name.split(' ')[0] || 'User', userInfo.family_name || userInfo.name.split(' ').slice(1).join(' ') || '', 'google', userInfo.sub);
    // Generate JWT tokens
 ...
 */
router.post('/google/test', validate(testGoogleConfigSchema), asyncHandler(async (req, res) => {
 ...
 */
router.put('/azure/config', validate(updateAzureConfigSchema), asyncHandler(async (req, res) => {
    const dbClient = req.dbClient;
 ...
 */
router.put('/google/config', validate(updateGoogleConfigSchema), asyncHandler(async (req, res) => {
    const dbClient = req.dbClient;
 ...
 */
async function provisionSSOUser(dbClient, tenantId, email, firstName, lastName, _ssoProvider, _ssoId) {
    // Check if user exists by email in this tenant
    const existingUser = await dbClient.query(`SELECT u.id, u.username, u.role, u.permissions, u.employee_id
     FROM users u
     JOIN employees e ON u.employee_id = e.id
     WHERE e.tenant_id = $1 AND e.email = $2`, [tenantId, email]);
    ...
    // Create employee
    const employeeResult = await dbClient.query(`INSERT INTO employees (tenant_id, first_name, last_name, email, employment_status, auth_role)
     VALUES ($1, $2, $3, $4, 'active', 'USER')
     RETURNING id`, [tenantId, firstName, lastName, email]);
    const employeeId = employeeResult.rows[0]?.id;
    // Generate username from tenant code and name
    const username = `${tenantCode}.${firstName.toLowerCase()}.${lastName.toLowerCase()}`.replace(/[^a-z0-9.]/g, '');
    // Create user (no password for SSO users)
    const userResult = await dbClient.query(`INSERT INTO users (username, role, permissions, is_active, employee_id, last_login)
     VALUES ($1, 'USER', ARRAY['employees:view:own', 'leave:view:own', 'leave:request'], true, $2, NOW())
     RETURNING id, username, role, permissions, employee_id`, [username, employeeId]);
