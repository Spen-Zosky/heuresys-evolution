...
import { createOrgChartGeneratorService, } from '../services/org-chart-generator.js';
import { validate } from '../middleware/validate.js';
import { createOrgChartSessionSchema, generateOrgChartSchema, generatePrototypeSchema, exportTenantOrgUnitsSchema, createSnapshotSchema, } from '../schemas/employees.js';
 ...
 */
router.post('/sessions', validate(createOrgChartSessionSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { sessionName, config } = req.body;
    if (!sessionName) {
        throw createAppError('Session name is required', 400, ErrorCodes.VALIDATION_ERROR);
    }
    const validMethods = ['web_search', 'template', 'nace_esco', 'combined'];
...
router.get('/sessions', asyncHandler(async (req, res) => {
    ...
    // Get total count
    const countResult = await req.dbClient.query(`SELECT COUNT(*) FROM org_chart_generation_sessions WHERE tenant_id = $1`, [tenantId]);
    res.json({
        success: true,
        data: {
            sessions: result.rows,
            meta: buildMeta(parseInt(countResult.rows[0]?.count), safeParseInt(limit, { fallback: 50 }), safeParseInt(offset, { fallback: 0 })),
        },
    });
}));
/**
 ...
 */
router.post('/sessions/:id/generate', validate(generateOrgChartSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { id } = req.params;
 ...
 */
router.post('/generate-prototype', validate(generatePrototypeSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    ...
    const generatorService = createOrgChartGeneratorService(tenantId, config.aiProvider);
    // Create session
    const session = await generatorService.createSession(sessionName || `Generation ${new Date().toISOString()}`, config);
    // Generate org chart
 ...
 */
router.post('/sessions/:id/assign-employees', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { id } = req.params;
    if (!id || !isValidUUID(id)) {
        throw createAppError('Invalid session ID', 400, ErrorCodes.VALIDATION_ERROR);
    ...
    const sessionResult = await req.dbClient.query(`SELECT generated_structure FROM org_chart_generation_sessions
    ...
    if (!generatedStructure) {
        throw createAppError('No generated structure found. Generate org chart first.', 400, ErrorCodes.VALIDATION_ERROR);
    }
 ...
 */
router.get('/staging/:sessionId/compare', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    ...
    if (format === 'detailed-csv') {
        ...
        res.setHeader('Content-Type', 'text/csv');
        res.setHeader('Content-Disposition', `attachment; filename="comparison_detailed_${sessionId}.csv"`);
        res.send(csv);
        return;
    }
 ...
 */
router.get('/staging/:sessionId/employees', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { sessionId } = req.params;
    const { level, changeType, limit = 100, offset = 0 } = req.query;
    if (!sessionId || !isValidUUID(sessionId)) {
        throw createAppError('Invalid session ID', 400, ErrorCodes.VALIDATION_ERROR);
    }
    let query = `
 ...
 */
router.post('/staging/:sessionId/approve', checkPermission('employees:update'), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { sessionId } = req.params;
 ...
 */
router.post('/staging/:sessionId/apply', checkPermission('employees:update'), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { sessionId } = req.params;
    if (!sessionId || !isValidUUID(sessionId)) {
        throw createAppError('Invalid session ID', 400, ErrorCodes.VALIDATION_ERROR);
 ...
 */
router.get('/sessions/:id/export/json', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { id } = req.params;
    if (!id || !isValidUUID(id)) {
        throw createAppError('Invalid session ID', 400, ErrorCodes.VALIDATION_ERROR);
    }
    const result = await req.dbClient.query(`SELECT generated_structure FROM org_chart_generation_sessions
       WHERE id = $1 AND tenant_id = $2`, [id, tenantId]);
 ...
 */
router.get('/sessions/:id/export/excalidraw', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { id } = req.params;
    ...
    const excalidrawFile = await excalidrawService.exportToExcalidraw(id, includeEmployees !== 'false');
    res.setHeader('Content-Type', 'application/json');
 ...
 */
router.get('/staging/:sessionId/export/excalidraw', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { sessionId } = req.params;
    if (!sessionId || !isValidUUID(sessionId)) {
        throw createAppError('Invalid session ID', 400, ErrorCodes.VALIDATION_ERROR);
    ...
    res.setHeader('Content-Type', 'application/json');
    res.setHeader('Content-Disposition', `attachment; filename="staging_org_chart_${sessionId}.excalidraw"`);
    res.send(JSON.stringify(excalidrawFile, null, 2));
}));
/**
 ...
 */
router.post('/sessions/:id/export/tenant-org-units', checkPermission('employees:update'), validate(exportTenantOrgUnitsSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { id } = req.params;
    ...
    const sessionResult = await req.dbClient.query(`SELECT session_name, generated_structure FROM org_chart_generation_sessions
         WHERE id = $1 AND tenant_id = $2`, [id, tenantId]);
    if (sessionResult.rows.length === 0) {
        throw createAppError('Session not found', 404, ErrorCodes.NOT_FOUND);
    }
    ...
    for (const unit of sortedUnits) {
        ...
        if (unit.parentCode) {
            ...
            if (parentId) {
                const parentResult = await req.dbClient.query(`SELECT path FROM tenant_org_units WHERE id = $1`, [parentId]);
    ...
    if (setAsActive) {
        await req.dbClient.query(`UPDATE tenant_org_charts SET is_active = false WHERE tenant_id = $1 AND id != $2`, [tenantId, chartId]);
        await req.dbClient.query(`UPDATE tenant_org_charts SET is_active = true, status = 'active' WHERE id = $1`, [chartId]);
    }
    res.json({
        success: true,
        data: {
 ...
 */
router.get('/tenant-units', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    // Get active chart
    const chartResult = await req.dbClient.query(`SELECT id, name, status, effective_date FROM tenant_org_charts
 ...
 */
router.get('/industry-prototype', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const prototypeService = createIndustryPrototypeService(tenantId);
    const prototype = await prototypeService.getPrototypeForTenant();
    res.json({
...
router.get('/snapshots', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { sessionId, type, limit = 20 } = req.query;
    let query = `
 ...
 */
router.post('/sessions/:id/snapshots', validate(createSnapshotSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { id } = req.params;
    ...
    const snapshotId = await excalidrawService.saveSnapshot(id, snapshotName || `Snapshot ${new Date().toISOString()}`, snapshotType, excalidrawFile);
    res.status(201).json({
        success: true,
        data: {
            snapshotId,
...
//# sourceMappingURL=org-charts.js.map