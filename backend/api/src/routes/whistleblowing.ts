...
import {
  createWhistleblowingReportSchema,
  updateWhistleblowingStatusSchema,
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const countResult = await req.dbClient!.query(
      'SELECT COUNT(*) FROM whistleblowing_reports WHERE tenant_id = $1',
      [tenantId]
    );
...
router.post(
  '/reports',
  validate(createWhistleblowingReportSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const {
      category,
    ...
    const anonymousToken =
      reporter_type === 'anonymous' ? crypto.randomBytes(32).toString('hex') : null;
...
router.patch(
  ...
  validate(updateWhistleblowingStatusSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'] as string;
    ...
    const existing = await req.dbClient!.query(
      'SELECT id FROM whistleblowing_reports WHERE id = $1 AND tenant_id = $2',
      [id, tenantId]
    );
    ...
    const result = await req.dbClient!.query(
      `UPDATE whistleblowing_reports SET ${updates.join(', ')}
