...
import {
  gapRiskSchema,
  hiringRecommendationsSchema,
  trainingInvestmentsSchema,
  createWorkforcePlanSchema,
  updateWorkforcePlanStatusSchema,
  simulateWorkforceSchema,
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (orgUnitId) params.push(orgUnitId);
...
router.post(
  '/hiring-recommendations',
  validate(hiringRecommendationsSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    ...
    const recommendations = await planningService.generateHiringRecommendations(
      tenantId,
      gap_assessments
    );
...
router.post(
  ...
  validate(trainingInvestmentsSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const { gap_assessments } = req.body;
    ...
    const investments = await planningService.generateTrainingInvestments(
      tenantId,
      gap_assessments
    );
      ...
      summary: {
        ...
        avg_roi:
          investments.length > 0
            ? Math.round(
                (investments.reduce((sum, i) => sum + i.roi_estimate, 0) / investments.length) * 100
              ) / 100
            : 0,
      },
    });
...
router.post(
  '/plans',
  validate(createWorkforcePlanSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
...
router.patch(
  '/plans/:planId/status',
  validate(updateWorkforcePlanStatusSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const planId = req.params['planId'] as string;
...
router.post(
  '/simulate',
  validate(simulateWorkforceSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const { requirements } = req.body;
    ...
    const hiringRecommendations = await planningService.generateHiringRecommendations(
      tenantId,
      gapAnalysis
    );
    const trainingInvestments = await planningService.generateTrainingInvestments(
      tenantId,
      gapAnalysis
    );
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const lastHeadcount =
      historicalResult.rows.length > 0
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const companyAttritionRate =
      companyData.total_headcount > 0
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const orgUnitData = deptResult.rows.map((row) => {
      ...
      return {
        ...
        status:
          current >= target ? 'optimal' : current >= target * 0.9 ? 'near_target' : 'understaffed',
      };
    });
    ...
    res.json({
      ...
      data: {
        ...
        summary: {
          ...
          overall_utilization:
            totalTarget > 0 ? Math.round((totalCurrent / totalTarget) * 100) : 100,
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = getTenantIdOrThrow(req);
    const orgUnitId = req.params['id'] as string;
