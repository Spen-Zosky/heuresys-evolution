...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const existingEnrollment = await req.dbClient!.query(
      "SELECT id FROM course_enrollments WHERE employee_id = $1 AND course_id = $2 AND status NOT IN ('cancelled', 'completed')",
...
router.patch(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const result = await req.dbClient!.query(
      `UPDATE course_enrollments SET ${updates.join(', ')} WHERE id = $${paramIndex} RETURNING *`,
      [...values, id]
    );
