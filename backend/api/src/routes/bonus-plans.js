...
router.get('/', requirePermission('COMPENSATION', 'VIEW'), applyScopeFilter('COMPENSATION'), asyncHandler(async (req, res) => {
    ...
    const { status, bonus_type, limit = '100', offset = '0' } = req.query;
    ...
    const countResult = await req.dbClient.query(`SELECT COUNT(*) FROM bonus_plans bp WHERE ${scope.where}`, scope.params);
    res.json({
        success: true,
        data: result.rows,
        meta: {
 ...
 * POST /bonus-plans
 */
router.post('/', requirePermission('COMPENSATION', 'CREATE'), validate(createBonusPlanSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { name, description, bonus_type, period_start, period_end, payout_date, total_budget, calculation_method, eligibility_rules, performance_multipliers, created_by, } = req.body;
    if (!name || !bonus_type) {
 ...
 */
router.patch('/:id', requirePermission('COMPENSATION', 'EDIT'), validate(updateBonusPlanSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'];
    const existing = await req.dbClient.query('SELECT id, status FROM bonus_plans WHERE id = $1 AND tenant_id = $2', [id, tenantId]);
    ...
    const result = await req.dbClient.query(`UPDATE bonus_plans SET ${updates.join(', ')} WHERE id = $${paramIndex} AND tenant_id = $${paramIndex + 1} RETURNING *`, [...values, id, tenantId]);
...
router.delete('/:id', requirePermission('COMPENSATION', 'DELETE'), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'];
    const result = await req.dbClient.query("UPDATE bonus_plans SET status = 'cancelled', updated_at = NOW() WHERE id = $1 AND tenant_id = $2 RETURNING id", [id, tenantId]);
    if (result.rows.length === 0) {
        throw Errors.notFound('Bonus plan');
    }
...
//# sourceMappingURL=bonus-plans.js.map