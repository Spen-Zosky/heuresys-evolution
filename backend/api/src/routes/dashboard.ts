 ...
 */
function getClient(req: Request, tenantId: string | null) {
...
// ============================================================================
type TimeRange = 'today' | '7d' | '30d' | '90d' | 'ytd' | '12m' | 'all';

function getDateRange(range: TimeRange | string): { dateFrom: Date; dateTo: Date } {
  const now = new Date();
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const employeeStats = await client.query(
      ...
      FROM employees WHERE ${TF}`,
      [tenantId]
    );

    // Goals stats
    const goalsStats = await client.query(
      `SELECT
    ...
    const learningStats = await client.query(
      ...
      FROM courses WHERE ${TF}`,
      [tenantId]
    );

    // Recognition stats
    const recognitionStats = await client.query(
      `SELECT
        COUNT(*)::int as total_recognitions,
        COALESCE(SUM(points_awarded), 0)::int as total_points
      FROM recognition WHERE ${TF}`,
      [tenantId]
    );
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const headcountByDept = await client.query(
      `SELECT d.name as department, COUNT(e.id)::int as count
      FROM employees e
    ...
    // Recruiting pipeline
    const recruitingPipeline = await client.query(
    ...
    // Open requisitions
    const openRequisitions = await client.query(
      `SELECT COUNT(*)::int as count
      FROM recruiting_requisitions
      WHERE ${TF} AND status = 'open'`,
      [tenantId]
    );

    // Time off requests pending
    const pendingTimeOff = await client.query(
      `SELECT COUNT(*)::int as count
      FROM employee_time_off_requests etor
      JOIN employees e ON etor.employee_id = e.id
      WHERE ($1::uuid IS NULL OR e.tenant_id = $1) AND etor.status = 'pending'`,
      [tenantId]
    );
