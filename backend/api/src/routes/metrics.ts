...
function isMetricsAuthorized(req: Request): boolean {
  ...
  if (METRICS_TOKEN) {
    ...
    if (authHeader === `Bearer ${METRICS_TOKEN}`) return true;
  }

  // Check IP allowlist
  const clientIp = req.ip || req.socket.remoteAddress || '';
  if (METRICS_ALLOWED_IPS.includes(clientIp)) return true;

  // In development without METRICS_TOKEN, allow all (backwards compat)
  if (!METRICS_TOKEN && process.env.NODE_ENV !== 'production') return true;
