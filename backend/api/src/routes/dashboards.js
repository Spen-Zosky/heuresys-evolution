...
import { createWidgetTemplateSchema, createDashboardSchema, updateDashboardSchema, duplicateDashboardSchema, addWidgetSchema, updateWidgetSchema, updateWidgetPositionsSchema, } from '../schemas/platform.js';
...
router.get('/stats', asyncHandler(async (req, res) => {
    ...
    const stats = await dashboardWidgetService.getDashboardStats(tenantId, user_id);
 ...
 */
router.get('/popular-widgets', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { limit = '10' } = req.query;
    const widgets = await dashboardWidgetService.getPopularWidgets(tenantId, safeParseInt(limit, { fallback: 50 }));
...
router.get('/templates', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { category, type, include_system = 'true' } = req.query;
 ...
 */
router.post('/templates', validate(createWidgetTemplateSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
...
router.get('/', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { user_id, include_public = 'true', search, page = '1', page_size = '20', } = req.query;
    if (!user_id) {
        throw Errors.badRequest('user_id is required');
    }
    const { dashboards, total } = await dashboardWidgetService.listDashboards(tenantId, user_id, {
...
router.get('/:id', asyncHandler(async (req, res) => {
    ...
    const { user_id } = req.query;
    const dashboard = await dashboardWidgetService.getDashboard(tenantId, id, user_id);
    if (!dashboard) {
 ...
 */
router.post('/', validate(createDashboardSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const { name, description, layout_type, is_default, is_shared, shared_with, created_by } = req.body;
    if (!name || !created_by) {
 ...
 * Update dashboard
 */
router.patch('/:id', validate(updateDashboardSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'];
...
router.delete('/:id', asyncHandler(async (req, res) => {
    ...
    const deleted = await dashboardWidgetService.deleteDashboard(tenantId, id, user_id);
    if (!deleted) {
        throw Errors.notFound('Dashboard');
    }
 ...
 */
router.post('/:id/duplicate', validate(duplicateDashboardSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'];
 ...
 */
router.get('/:id/widgets', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'];
 ...
 */
router.post('/:id/widgets', validate(addWidgetSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'];
 ...
 * Update widget
 */
router.patch('/:id/widgets/:widgetId', validate(updateWidgetSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
 ...
 */
router.put('/:id/widgets/positions', validate(updateWidgetPositionsSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'];
 ...
 */
router.get('/:id/widgets/:widgetId/data', asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const id = req.params['id'];
    const widgetId = req.params['widgetId'];
    const widgets = await dashboardWidgetService.getWidgets(tenantId, id);
