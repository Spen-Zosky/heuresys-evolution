...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const { id: employeeId } = req.params as Record<string, string>;
    const { includeHistory = 'false', includeSkillDetails = 'true' } = req.query as Record<
      string,
      string
    >;
    ...
    const employeeResult = await dbClient.query(
      `SELECT id, first_name, last_name, job_title, org_unit_id, tenant_id FROM employees WHERE id = $1 AND tenant_id = $2`,
      [employeeId, tenantId]
    );
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (validSkills.length > 0) {
      ...
      try {
        ...
        updated = result.rows.map((r: { skill_id: string }) => r.skill_id);
      } catch (err) {
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const skillResult = await dbClient.query(
      'SELECT id, preferred_label_en FROM esco_skills WHERE id = $1',
      [skillId]
    );
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    const dbClient = req.dbClient!;
    const { id: employeeId } = req.params as Record<string, string>;
    const { skillId, limit = '50', offset = '0' } = req.query as Record<string, string>;
    const tenantId = getTenantIdOrThrow(req);
    ...
    const params: unknown[] = [employeeId, tenantId];
    let paramIndex = 3;
