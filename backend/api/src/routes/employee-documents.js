...
async function getEmployeeId(dbClient, userId, tenantId) {
...
router.get('/', asyncHandler(async (req, res) => {
    ...
    const { category, document_type, status = 'active', limit = '50', offset = '0', } = req.query;
    let query = `
    SELECT d.*,
  ...
  `;
    ...
    const statsResult = await dbClient.query(`SELECT
     ...
     GROUP BY category`, [employeeId, tenantId]);
    res.json({
        success: true,
        data: {
...
router.get('/categories', asyncHandler(async (req, res) => {
    ...
    const result = await dbClient.query(`SELECT
     ...
     WHERE employee_id = $1 AND tenant_id = $2 AND status = 'active' AND is_latest = TRUE
     GROUP BY category
     ORDER BY category`, [employeeId, tenantId]);
    const categories = [
        {
            id: 'employment',
            name: 'Employment',
...
router.get('/:id', validateUUID(), asyncHandler(async (req, res) => {
    ...
    const result = await dbClient.query(`SELECT d.*,
            e_up.first_name || ' ' || e_up.last_name as uploaded_by_name,
     ...
     WHERE d.id = $1 AND d.employee_id = $2 AND d.tenant_id = $3`, [id, employeeId, tenantId]);
    if (result.rows.length === 0) {
        throw Errors.notFound('Document', id);
    }
    // Get version history
    const versionsResult = await dbClient.query(`SELECT id, version, filename, file_size, created_at
     FROM employee_documents
     WHERE (parent_document_id = $1 OR id = $1)
     ORDER BY version DESC`, [result.rows[0]?.parent_document_id || id]);
    res.json({
        success: true,
        data: {
            ...(result.rows[0] || {}),
...
router.get('/expiring/list', asyncHandler(async (req, res) => {
    const dbClient = req.dbClient;
    const userId = req.user?.userId;
    const tenantId = getTenantIdOrThrow(req);
    if (!userId || !tenantId) {
    ...
    const result = await dbClient.query(`SELECT id, title, document_type, category, expiry_date,
     ...
     ORDER BY expiry_date ASC`, [employeeId, tenantId, parseInt(days)]);
...
router.post('/', validate(createDocumentSchema), asyncHandler(async (req, res) => {
    ...
    const { title, description, document_type, category, filename, original_name, mime_type, file_size, file_path, document_date, expiry_date, reference_number, } = req.body;
    if (!title || !document_type || !filename || !original_name || !mime_type || !file_path) {
        throw Errors.badRequest('title, document_type, filename, original_name, mime_type, and file_path are required');
    }
    // Path traversal protection
    if (file_path.includes('..') || file_path.startsWith('/')) {
        throw Errors.badRequest('Invalid file path');
    }
    const result = await dbClient.query(`INSERT INTO employee_documents (
...
router.delete('/:id', validateUUID(), asyncHandler(async (req, res) => {
    ...
    await dbClient.query(`UPDATE employee_documents SET status = 'archived', updated_at = CURRENT_TIMESTAMP
...
router.post('/requests', validate(createDocumentRequestSchema), asyncHandler(async (req, res) => {
    const dbClient = req.dbClient;
    const userId = req.user?.userId;
    const tenantId = getTenantIdOrThrow(req);
    if (!userId || !tenantId) {
    ...
    const result = await dbClient.query(`INSERT INTO document_requests (
        tenant_id, employee_id, document_type, purpose, additional_notes, priority
      ) VALUES ($1, $2, $3, $4, $5, $6)
      RETURNING *`, [tenantId, employeeId, document_type, purpose, additional_notes, priority]);
    res.status(201).json({ success: true, data: result.rows[0] || null });
}));
// GET /employee-documents/request-types - Get available request types
router.get('/request-types/list', async (_req, res) => {
    const requestTypes = [
        {
