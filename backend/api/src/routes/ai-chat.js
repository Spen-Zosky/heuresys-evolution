...
import { createAIOrchestrator } from '../services/ai-orchestrator.js';
import { validate } from '../middleware/validate.js';
import { createSessionSchema, sendMessageSchema, messageFeedbackSchema, respondEscalationSchema, assignEscalationSchema, } from '../schemas/ai-chat.js';
...
router.get('/sessions', asyncHandler(async (req, res) => {
    ...
    const { limit = '20', offset = '0', includeArchived = 'false', } = req.query;
    let query = `
      SELECT rs.*,
...
router.post('/sessions', validate(createSessionSchema), asyncHandler(async (req, res) => {
    ...
    let employeeId = null;
    if (userId) {
        const empResult = await req.dbClient.query('SELECT id FROM employees WHERE user_id = $1 AND tenant_id = $2', [userId, tenantId]);
 ...
 */
router.post('/sessions/:sessionId/messages', validate(sendMessageSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    ...
    const sessionResult = await req.dbClient.query(`SELECT id, user_id, tenant_id, provider, model, title, system_prompt,
              sources_enabled, created_at, updated_at, is_archived
       FROM rag_sessions WHERE id = $1 AND tenant_id = $2`, [sessionId, tenantId]);
 ...
 */
router.post('/messages/:messageId/feedback', validate(messageFeedbackSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
...
router.get('/escalations', asyncHandler(async (req, res) => {
    ...
    params.push(safeParseInt(limit, { fallback: 50 }), safeParseInt(offset, { fallback: 0 }));
    const result = await req.dbClient.query(query, params);
    const countResult = await req.dbClient.query('SELECT COUNT(*) FROM ai_escalation_queue WHERE tenant_id = $1 AND status = $2', [tenantId, status || 'pending']);
    res.json({
        success: true,
        data: result.rows,
        meta: {
 ...
 */
router.post('/escalations/:id/respond', validate(respondEscalationSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
 ...
 */
router.post('/escalations/:id/assign', validate(assignEscalationSchema), asyncHandler(async (req, res) => {
    const tenantId = getTenantIdOrThrow(req);
    const escalationId = req.params['id'];
