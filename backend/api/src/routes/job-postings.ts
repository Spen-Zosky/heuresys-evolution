...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const [countResult, dataResult] = await Promise.all([
      req.dbClient!.query(
        `SELECT COUNT(*) as total FROM internal_job_postings ijp ${whereClause}`,
        params
      ),
      req.dbClient!.query(
        `SELECT ijp.id, ijp.title, ijp.status, ijp.department, ijp.location,
...
router.get(
  ...
  `,
    ...
    await req.dbClient!.query(
      'UPDATE internal_job_postings SET views_count = COALESCE(views_count, 0) + 1 WHERE id = $1',
      [id]
    );
...
router.patch(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const result = await req.dbClient!.query(
      `UPDATE internal_job_postings SET ${updates.join(', ')} WHERE id = $${paramIndex} AND tenant_id = $${paramIndex + 1} RETURNING *`,
      [...values, id, tenantId]
    );
...
router.delete(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const result = await req.dbClient!.query(
      "UPDATE internal_job_postings SET status = 'archived', updated_at = NOW() WHERE id = $1 AND tenant_id = $2 RETURNING id",
      [id, tenantId]
    );
