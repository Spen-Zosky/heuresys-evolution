...
import { createUserSchema, updateUserSchema, resetPasswordSchema, bulkCreateUsersSchema, } from '../schemas/users.js';
 ...
 */
async function sendWelcomeEmail(email, username, _temporaryPassword, // Prefixed: used only by email service, not logged for security
tenantName) {
 ...
 */
router.get('/meta/roles', async (_req, res) => {
 ...
 */
router.get('/', requirePermission('SECURITY', 'VIEW'), asyncHandler(async (req, res) => {
    const authReq = req;
    ...
    // Get total count
    const countResult = await dbClient.query(`SELECT COUNT(*) as total
     FROM users u
    ...
    const sortOrderParam = req.query['sort_order'];
    ...
    // Get users
    const usersResult = await dbClient.query(`SELECT
 ...
 */
router.get('/:id', requirePermission('SECURITY', 'VIEW'), asyncHandler(async (req, res) => {
    const authReq = req;
    const dbClient = req.dbClient || pool;
 ...
 */
router.post('/', requirePermission('SECURITY', 'CREATE'), validate(createUserSchema), asyncHandler(async (req, res) => {
    const authReq = req;
    const dbClient = req.dbClient || pool;
    const { username, password, role, permissions, employee_id, is_active = true, generate_password = false, send_welcome_email = false, } = req.body;
    // Validation
    if (!username) {
    ...
    if (generate_password) {
        ...
        throw createAppError('Password is required (or use generate_password: true)', 400, ErrorCodes.VALIDATION_ERROR);
    }
    else {
        const validation = validatePassword(password);
        if (!validation.valid) {
            throw createAppError(`Password does not meet policy requirements: ${validation.errors.join('; ')}`, 400, ErrorCodes.VALIDATION_ERROR);
        }
    }
    // Validate role
    ...
    if (!validRoles.includes(userRole)) {
        throw createAppError(`Invalid role. Must be one of: ${validRoles.join(', ')}`, 400, ErrorCodes.VALIDATION_ERROR);
    }
    ...
    if (employee_id) {
        ...
        const employeeCheck = await dbClient.query('SELECT id, tenant_id FROM employees WHERE id = $1', [employee_id]);
        ...
        if (authReq.user.role !== 'SUPERUSER' &&
            ...
            throw createAppError('Cannot link user to employee from another tenant', 403, ErrorCodes.FORBIDDEN);
        }
        // Check if employee already has a user
        const existingUserForEmployee = await dbClient.query('SELECT id FROM users WHERE employee_id = $1', [employee_id]);
    ...
    if (!['SUPERUSER', 'TENANT_OWNER', 'DEMO'].includes(userRole) && !employee_id) {
        throw createAppError('Non-system users must be linked to an employee', 400, ErrorCodes.VALIDATION_ERROR);
    }
    // Hash password
    const passwordHash = await bcrypt.hash(userPassword, 12);
    // Create user
    const result = await dbClient.query(`INSERT INTO users (username, password_hash, role, permissions, employee_id, is_active)
     VALUES ($1, $2, $3, $4, $5, $6)
     RETURNING id, username, role, permissions, is_active, employee_id, created_at`, [username, passwordHash, userRole, permissions || [], employee_id || null, is_active]);
    const newUser = result.rows[0];
    // Send welcome email if requested and employee has email
    let welcomeEmailSent = false;
    if (send_welcome_email && employee_id) {
        const employeeInfo = await dbClient.query(`SELECT e.email, e.first_name, t.name as tenant_name
       FROM employees e
       JOIN tenants t ON e.tenant_id = t.id
       WHERE e.id = $1`, [employee_id]);
        if (employeeInfo.rows[0]?.email) {
            await sendWelcomeEmail(employeeInfo.rows[0]?.email, username, userPassword, employeeInfo.rows[0]?.tenant_name || 'Heuresys Platform');
 ...
 */
router.patch('/:id', requirePermission('SECURITY', 'EDIT'), validate(updateUserSchema), asyncHandler(async (req, res) => {
    const authReq = req;
    const dbClient = req.dbClient || pool;
    const id = req.params['id'];
    if (!id || !isValidUUID(id)) {
    ...
    const existingResult = await dbClient.query(`SELECT u.*, e.tenant_id FROM users u
     LEFT JOIN employees e ON u.employee_id = e.id
     WHERE u.id = $1`, [id]);
    ...
    if (ROLES[existingUser.role] < ROLES[authReq.user.role]) {
        ...
        if (req.body[field] !== undefined) {
            ...
            if (field === 'role') {
                ...
                if (newRole === 'SUPERUSER' && authReq.user.role !== 'SUPERUSER') {
                    throw createAppError('Only SUPERUSER can promote to SUPERUSER', 403, ErrorCodes.FORBIDDEN);
                }
                if (ROLES[newRole] < ROLES[authReq.user.role]) {
                    throw createAppError('Cannot set role with higher privileges', 403, ErrorCodes.FORBIDDEN);
                }
            }
    ...
    const result = await dbClient.query(`UPDATE users
     SET ${updates.join(', ')}, updated_at = NOW()
     WHERE id = $${paramIndex}
     RETURNING id, username, role, permissions, is_active, employee_id, updated_at`, [...values, id]);
 ...
 */
router.delete('/:id', requirePermission('SECURITY', 'DELETE'), asyncHandler(async (req, res) => {
    const authReq = req;
    const dbClient = req.dbClient || pool;
    const id = req.params['id'];
 ...
 */
router.post('/:id/reset-password', requirePermission('SECURITY', 'EDIT'), validate(resetPasswordSchema), asyncHandler(async (req, res) => {
    const authReq = req;
    const dbClient = req.dbClient || pool;
    const id = req.params['id'];
 ...
 */
router.post('/bulk', requirePermission('SECURITY', 'CREATE'), validate(bulkCreateUsersSchema), asyncHandler(async (req, res) => {
    const authReq = req;
    const dbClient = req.dbClient || pool;
    ...
    if (employee_ids.length > 100) {
        throw createAppError('Maximum 100 users per bulk operation', 400, ErrorCodes.VALIDATION_ERROR);
    }
    // Validate role
    const validRoles = Object.keys(ROLES);
    const userRole = role;
    ...
    // Get employee data
    const employeesResult = await dbClient.query(`SELECT id, first_name, last_name, email, tenant_id
     FROM employees
     WHERE id = ANY($1)`, [employee_ids]);
    const results = { created: [], skipped: [], failed: [] };
    for (const employeeId of employee_ids) {
        try {
            const employee = employeesResult.rows.find((e) => e.id === employeeId);
            if (!employee) {
            ...
            // Generate username
            const baseUsername = `${tenantCode}.${employee.first_name.toLowerCase()}.${employee.last_name.toLowerCase()}`.replace(/[^a-z0-9.]/g, '');
            ...
            // Create user
            await dbClient.query(`INSERT INTO users (username, password_hash, role, permissions, employee_id, is_active)
         VALUES ($1, $2, $3, $4, $5, true)`, [username, passwordHash, userRole, [], employeeId]);
 ...
 */
router.get('/permissions/available', requirePermission('SECURITY', 'VIEW'), async (_req, res) => {
