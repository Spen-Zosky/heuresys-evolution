...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const [countResult, dataResult] = await Promise.all([
      req.dbClient!.query(
        `SELECT COUNT(*) as total FROM employee_contracts ec ${whereClause}`,
        params
      ),
      req.dbClient!.query(
        `SELECT ec.id, ec.employee_id, ec.contract_type, ec.start_date, ec.end_date,
...
// =============================================================================
router.get('/meta/ccnl', (_req: Request, res: Response) => {
...
// =============================================================================
router.get('/meta/amendment-types', (_req: Request, res: Response) => {
...
router.get(
  '/',
  checkPermission(PERMISSIONS.EMPLOYEES_VIEW_ALL, PERMISSIONS.EMPLOYEES_VIEW_TEAM),
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const sortColumn = validateIdentifier(
      validSortColumns.includes(sortBy as string) ? (sortBy as string) : 'start_date',
      'contracts.list-sort'
    );
    ...
    const countResult = await req.dbClient!.query(
      `SELECT COUNT(*) FROM contracts c WHERE ${whereClause}`,
      params
    );
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const daysNum = Math.max(1, Math.min(365, parseInt(days as string, 10)));
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const amendmentsResult = await req.dbClient!.query(
      `SELECT id, contract_id, tenant_id, amendment_type, effective_date,
...
router.post(
  '/',
  checkPermission(PERMISSIONS.EMPLOYEES_CREATE, PERMISSIONS.EMPLOYEES_UPDATE),
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const contractMeta = CONTRACT_TYPES[input.contractType as keyof typeof CONTRACT_TYPES];
    if (contractMeta.requiresEndDate && !input.endDate) {
      throw createAppError(
        `End date is required for ${contractMeta.label} contracts`,
        400,
    ...
    const overlapCheck = await req.dbClient!.query(
      `SELECT id FROM contracts
         ...
         AND start_date <= COALESCE($4, '9999-12-31')`,
      [input.employeeId, tenantId, input.startDate, input.endDate || null]
    );
...
router.put(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const input = req.body as Partial<ContractInput>;

    // Check if contract exists
    const existingResult = await req.dbClient!.query(
      `SELECT id, tenant_id, employee_id, contract_type, contract_code,
    ...
    for (const field of significantFields) {
      ...
      if (inputKey && inputKey in input) {
        const newValue = (input as Record<string, unknown>)[inputKey];
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const existingResult = await req.dbClient!.query(
      `SELECT id, tenant_id, employee_id, contract_type, status,
    ...
    const result = await req.dbClient!.query(
      `UPDATE contracts SET
         ...
         RETURNING *`,
      [terminationDate, terminationReason || null, userId, id, tenantId]
    );
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const input = req.body as AmendmentInput;
    ...
    // Check if contract exists
    const contractCheck = await req.dbClient!.query(
...
router.delete(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (existingResult.rows[0].status !== 'draft') {
      throw createAppError(
        'Only draft contracts can be deleted. Use termination for active contracts.',
        400,
        ErrorCodes.VALIDATION_ERROR
      );
    }
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const [totalResult, byTypeResult, byStatusResult, expiringResult, avgSalaryResult] =
      await Promise.all([
        // Total contracts
        ...
        req.dbClient!.query(
          `SELECT contract_type, COUNT(*) as count
           FROM contracts WHERE tenant_id = $1 GROUP BY contract_type`,
          [tenantId]
        ),
        ...
        req.dbClient!.query(
          `SELECT status, COUNT(*) as count
           FROM contracts WHERE tenant_id = $1 GROUP BY status`,
          [tenantId]
        ),
        ...
        // Average salary
        req.dbClient!.query(
          `SELECT AVG(gross_annual_salary) as avg_salary
           FROM contracts
           WHERE tenant_id = $1 AND status = 'active' AND gross_annual_salary IS NOT NULL`,
          [tenantId]
        ),
      ]);
    ...
    res.json({
      ...
      data: {
        ...
        byType: byTypeResult.rows.reduce(
          (acc, row) => {
        ...
        byStatus: byStatusResult.rows.reduce(
          (acc, row) => {
