...
import { createMigrationJobSchema, startMigrationJobSchema, approveUnknownSkillSchema, rejectUnknownSkillSchema, matchSkillSchema, } from '../schemas/semantic.js';
 ...
 */
router.get('/stats/:tenantId', asyncHandler(async (req, res) => {
 ...
 */
router.post('/jobs', validate(createMigrationJobSchema), asyncHandler(async (req, res) => {
 ...
 */
router.post('/jobs/:jobId/start', validate(startMigrationJobSchema), asyncHandler(async (req, res) => {
    const jobId = req.params.jobId;
 ...
 */
router.get('/unmapped', asyncHandler(async (req, res) => {
    const tenantId = req.query.tenant_id;
    const jobType = req.query.job_type || 'all';
    if (!tenantId) {
        throw Errors.badRequest('tenant_id query parameter is required');
    }
    const service = getMigrationService();
    const counts = await service.countUnmappedRecords(tenantId, jobType);
    res.json({
        success: true,
        data: {
            counts,
 ...
 */
router.get('/unknown-skills', asyncHandler(async (req, res) => {
    const dbClient = req.dbClient;
    const tenantId = req.query.tenant_id;
  ...
  `;
    ...
    // Get total count
    const countResult = await dbClient.query(query.replace(/SELECT[\s\S]*?FROM/, 'SELECT COUNT(*) FROM'), params);
 ...
 */
router.post('/unknown-skills/:skillId/approve', validate(approveUnknownSkillSchema), asyncHandler(async (req, res) => {
 ...
 */
router.post('/unknown-skills/:skillId/reject', validate(rejectUnknownSkillSchema), asyncHandler(async (req, res) => {
    const skillId = req.params.skillId;
 ...
 */
router.get('/jobs/:jobId/log', asyncHandler(async (req, res) => {
    const dbClient = req.dbClient;
    const jobId = req.params.jobId;
 ...
 */
router.post('/match', validate(matchSkillSchema), asyncHandler(async (req, res) => {
...
//# sourceMappingURL=skill-migration.js.map