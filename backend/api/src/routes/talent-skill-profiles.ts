...
router.get(
  '/',
  validate(talentSkillProfilesListQuerySchema, 'query'),
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (min_score) {
      ...
      countParams.push(parseFloat(min_score as string));
    }
    if (max_score) {
      countQuery += ` AND COALESCE(sp.avg_composite_score, 0) <= $${countParamIndex++}`;
      countParams.push(parseFloat(max_score as string));
    }
    if (search) {
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (skillsResult.rows.length > 0) {
      ...
      const calc = (field: string) => ({
        avg: parseFloat(
          (skills.reduce((sum, s) => sum + (parseFloat(s[field]) || 0), 0) / skills.length).toFixed(
            2
          )
        ),
      ...
      data: {
        ...
        summary: {
          ...
          avg_composite_score:
            skills.length > 0
              ? parseFloat(
                  (skills.reduce((sum, s) => sum + s.composite_score, 0) / skills.length).toFixed(2)
                )
              : 0,
        },
