...
router.get(
  ...
  `;
    ...
    if (pricing_model) {
      ...
      params.push(pricing_model as string);
      paramIndex++;
    }
    ...
    const validSortKey = Object.hasOwn(allowedSorts, sort as string)
      ? (sort as string)
      : 'total_installations';
  ...
  `;
    ...
    if (pricing_model) {
      ...
      countParams.push(pricing_model as string);
      countIdx++;
    }
    if (featured === 'true') {
...
router.delete(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const installCheck = await dbClient.query(
      `SELECT COUNT(*) FROM plugin_installations
     WHERE plugin_id = $1 AND status IN ('active', 'disabled')`,
      [id]
    );
...
router.post(
  ...
  authMiddleware,
  requirePermission('MARKETPLACE', 'CREATE'),
  validate(createPluginDependencySchema),
  asyncHandler(async (req: Request, res: Response) => {
    const dbClient = req.dbClient!;
    const id = req.params['id'] as string;
