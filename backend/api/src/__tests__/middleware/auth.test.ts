...
// ---------------------------------------------------------------------------
const mockIsTokenBlacklisted = jest.fn<(jti: string) => Promise<boolean>>();
mockIsTokenBlacklisted.mockResolvedValue(false);

// Use absolute path to avoid resolver issues with jest.unstable_mockModule
const redisModulePath = new URL('../../config/redis.js', import.meta.url).pathname.replace(
  /\.js$/,
  '.ts'
);
...
function makePayload(overrides: Record<string, unknown> = {}) {
  return buildTokenPayload({
    userId: 'user-001',
    ...overrides,
  } as Partial<ReturnType<typeof buildTokenPayload>>);
}
...
describe('Auth Middleware', () => {
  ...
  describe('verifyToken', () => {
    ...
    it('should throw on expired token', () => {
      const expired = jwt.sign(
        { userId: 'u-exp', username: 'expired', role: 'EMPLOYEE', permissions: [] },
        TEST_JWT_SECRET,
        { expiresIn: '-1s', issuer: 'heuresys-platform' }
      );
    ...
    it('should throw on token signed with wrong secret', () => {
      const wrongSecret = jwt.sign(
        { userId: 'u-wrong', username: 'wrong', role: 'EMPLOYEE', permissions: [] },
        'wrong-secret-key',
        { expiresIn: '1h', issuer: 'heuresys-platform' }
      );
    ...
    it('should throw on token with wrong issuer', () => {
      const wrongIssuer = jwt.sign(
        { userId: 'u-iss', username: 'issuer', role: 'EMPLOYEE', permissions: [] },
        TEST_JWT_SECRET,
  ...
  describe('authMiddleware', () => {
    ...
    it('should attach user payload to req.user', async () => {
      ...
      const user = (mockReq as unknown as { user: Record<string, unknown> }).user;
    ...
    it('should reject request with no Authorization header (401)', async () => {
      ...
      expect(mockNext).toHaveBeenCalledTimes(1);
      const err = mockNext.mock.calls[0]?.[0] as
    ...
    it('should reject request with non-Bearer authorization scheme', async () => {
      ...
      const err = mockNext.mock.calls[0]?.[0] as { httpStatus?: number } | undefined;
      expect(err?.httpStatus).toBe(401);
    });
    ...
    it('should reject expired token (401)', async () => {
      const expired = jwt.sign(
        { userId: 'u-exp', username: 'exp', role: 'EMPLOYEE', permissions: [] },
        TEST_JWT_SECRET,
        { expiresIn: '-1s', issuer: 'heuresys-platform' }
      );
  ...
  describe('requireRole', () => {
    ...
    it('should return 401 when no user is attached to request', async () => {
      const mw = requireRole('EMPLOYEE');
      await mw({} as Request, mockRes as Response, mockNext);
      const err = mockNext.mock.calls[0]?.[0] as { httpStatus?: number } | undefined;
      expect(err?.httpStatus).toBe(401);
    });
    ...
    it('should treat legacy ADMIN as TENANT_ADMIN (level 1)', async () => {
      const mw = requireRole('TENANT_ADMIN');
      await mw(authReq('ADMIN'), mockRes as Response, mockNext);
      expect(mockNext.mock.calls[0]?.[0]).toBeUndefined();
    });

    it('should treat legacy HR as HR_MANAGER (level 4)', async () => {
      const mw = requireRole('HR_MANAGER');
      await mw(authReq('HR'), mockRes as Response, mockNext);
      expect(mockNext.mock.calls[0]?.[0]).toBeUndefined();
    });

    it('should treat legacy USER as EMPLOYEE (level 7)', async () => {
      const mw = requireRole('EMPLOYEE');
      await mw(authReq('USER'), mockRes as Response, mockNext);
      expect(mockNext.mock.calls[0]?.[0]).toBeUndefined();
    });

    it('should treat legacy DEMO as EMPLOYEE (level 7)', async () => {
      const mw = requireRole('EMPLOYEE');
      await mw(authReq('DEMO'), mockRes as Response, mockNext);
      expect(mockNext.mock.calls[0]?.[0]).toBeUndefined();
    });
  });
  ...
  describe('requirePermission', () => {
    ...
    it('should return 401 when no user is on the request', () => {
      ...
      mw({} as Request, mockRes as Response, mockNext);
      const err = mockNext.mock.calls[0]?.[0] as { httpStatus?: number } | undefined;
      expect(err?.httpStatus).toBe(401);
    });
  });
  ...
  describe('getUserOrThrow', () => {
    ...
    it('should throw with message containing "Authentication required" when no user', () => {
      expect(() => getUserOrThrow({} as Request)).toThrow();
      try {
        getUserOrThrow({} as Request);
      } catch (e) {
