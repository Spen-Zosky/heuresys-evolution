...
beforeAll(async () => {
    try {
        ...
        rateLimitModule = (await import('../../middleware/rateLimit.js'));
    }
    catch (err) {
...
describe('Rate Limit Middleware', () => {
    describe('Module availability', () => {
        it('should be loadable or fail due to missing rate-limit-redis (Docker-only dep)', () => {
            if (loadError) {
                ...
                console.warn('rate-limit-redis not installed in host environment; ' +
    ...
    describe('apiRateLimiter', () => {
        ...
        it('should have correct arity (req, res, next)', () => {
            if (!rateLimitModule)
                return;
            expect(rateLimitModule.apiRateLimiter.length).toBe(3);
        });
    });
    describe('authRateLimiter', () => {
        ...
        it('should be a valid Express middleware', () => {
            if (!rateLimitModule)
                return;
            expect(rateLimitModule.authRateLimiter.length).toBe(3);
        });
    });
    describe('aiRateLimiter', () => {
        ...
        it('should be a valid Express middleware', () => {
            if (!rateLimitModule)
                return;
            expect(rateLimitModule.aiRateLimiter.length).toBe(3);
        });
    });
    describe('exportRateLimiter', () => {
        ...
        it('should be a valid Express middleware', () => {
            if (!rateLimitModule)
                return;
            expect(rateLimitModule.exportRateLimiter.length).toBe(3);
        });
    });
    describe('Rate Limiter Types', () => {
        ...
        it('all rate limiters should be unique instances', () => {
            if (!rateLimitModule)
                return;
            const { apiRateLimiter, authRateLimiter, aiRateLimiter, exportRateLimiter, heavyComputeRateLimiter, } = rateLimitModule;
