 ...
 * and user metadata.
 */
async function login(username: string, password: string): Promise<LoginResponse['data'] | null> {
  try {
 ...
 */
function expectAccessDenied(status: number, body: Record<string, unknown>, _context: string): void {
  expect(status).toBeGreaterThanOrEqual(400);
  const data = body as unknown as SingleResourceResponse;
  expect(data.success).toBe(false);
}

describe('Multi-Tenant Isolation Integration', () => {
  let tenantAAuth: NonNullable<LoginResponse['data']>;
  let tenantBAuth: NonNullable<LoginResponse['data']>;
  ...
  let tenantADepartmentId: string;
  let tenantBDepartmentId: string;
  ...
  beforeAll(async () => {
    ...
    if (!authA || !authB) {
      console.error(
        'Failed to authenticate test users. Ensure the API is running ' +
    ...
    if (!canAccessEmployees) {
      console.warn(
        `Tenant A user (${TENANT_A.username}, role=${tenantAAuth.user.role}) cannot access /employees (status=${empProbe.status}). ` +
  ...
  describe('Employee List Isolation', () => {
    it('should return only Tenant A employees when using Tenant A token', async () => {
      if (!setupSucceeded || !canAccessEmployees) return;

      const { status, body } = await authGet(
        '/api/v1/employees?limit=100',
        tenantAAuth.accessToken,
        tenantAAuth.user.tenantId
      );

      expect(status).toBe(200);

      const data = body as unknown as EmployeeListResponse;
    ...
    it('should return only Tenant B employees when using Tenant B token', async () => {
      ...
      const { status, body } = await authGet(
        '/api/v1/employees?limit=100',
        tenantBAuth.accessToken,
        tenantBAuth.user.tenantId
      );
    ...
    it('should return different employee sets for different tenants', async () => {
      ...
      const dataA = responseA.body as unknown as EmployeeListResponse;
      const dataB = responseB.body as unknown as EmployeeListResponse;
  ...
  describe('OrgUnit List Isolation', () => {
    it('should return only Tenant A departments when using Tenant A token', async () => {
      if (!setupSucceeded || !canAccessOrgUnits) return;
      ...
      const data = body as unknown as DepartmentListResponse;
      expect(data.success).toBe(true);
    ...
    it('should return different department sets for different tenants', async () => {
      ...
      const dataA = responseA.body as unknown as DepartmentListResponse;
      const dataB = responseB.body as unknown as DepartmentListResponse;
  ...
  describe('Cross-Tenant Read Access Prevention', () => {
    it('should deny when Tenant A tries to GET a Tenant B employee', async () => {
      if (!setupSucceeded || !canAccessEmployees || !tenantBEmployeeId) return;
      ...
      const { status, body } = await authGet(
        `/api/v1/employees/${tenantBEmployeeId}`,
        tenantAAuth.accessToken,
        tenantAAuth.user.tenantId
      );
    ...
    it('should deny when Tenant B tries to GET a Tenant A employee', async () => {
      if (!setupSucceeded || !canAccessEmployees || !tenantAEmployeeId) return;

      const { status, body } = await authGet(
        `/api/v1/employees/${tenantAEmployeeId}`,
        tenantBAuth.accessToken,
        tenantBAuth.user.tenantId
      );
    ...
    it('should deny when Tenant A tries to GET a Tenant B department', async () => {
      if (!setupSucceeded || !canAccessOrgUnits || !tenantBDepartmentId) return;

      const { status, body } = await authGet(
        `/api/v1/org-units/${tenantBDepartmentId}`,
        tenantAAuth.accessToken,
        tenantAAuth.user.tenantId
      );
    ...
    it('should deny when Tenant B tries to GET a Tenant A department', async () => {
      if (!setupSucceeded || !canAccessOrgUnits || !tenantADepartmentId) return;

      const { status, body } = await authGet(
        `/api/v1/org-units/${tenantADepartmentId}`,
        tenantBAuth.accessToken,
        tenantBAuth.user.tenantId
      );
  ...
  describe('Cross-Tenant Mutation Prevention', () => {
    it('should deny when Tenant A tries to PATCH a Tenant B employee', async () => {
      ...
      const { status, body } = await authPatch(
        `/api/v1/employees/${tenantBEmployeeId}`,
        tenantAAuth.accessToken,
        tenantAAuth.user.tenantId,
        { job_title: 'HACKED' }
      ...
      const { body: verifyBody } = await authGet(
        `/api/v1/employees/${tenantBEmployeeId}`,
        tenantBAuth.accessToken,
        tenantBAuth.user.tenantId
      );
      const verifyData = verifyBody as unknown as SingleResourceResponse;
    ...
    it('should deny when Tenant A tries to DELETE a Tenant B employee', async () => {
      ...
      const { status: verifyStatus, body: verifyBody } = await authGet(
        `/api/v1/employees/${tenantBEmployeeId}`,
        tenantBAuth.accessToken,
        tenantBAuth.user.tenantId
      );
  ...
  describe('Data Integrity Verification', () => {
    it('should return consistent employee counts within API limit', async () => {
      ...
      const { body: page1 } = await authGet(
        '/api/v1/employees?limit=1&page=1',
        tenantAAuth.accessToken,
        tenantAAuth.user.tenantId
      );

      const data1 = page1 as unknown as EmployeeListResponse;
      ...
      const { body: allRecords } = await authGet(
        `/api/v1/employees?limit=${requestLimit}&page=1`,
        tenantAAuth.accessToken,
        tenantAAuth.user.tenantId
      );

      const allData = allRecords as unknown as EmployeeListResponse;
