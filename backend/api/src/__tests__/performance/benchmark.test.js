...
describe('Performance Benchmarks', () => {
    ...
    describe('Tenant Endpoints', () => {
        ...
        it('should get single tenant within acceptable time', async () => {
            const result = await runBenchmark('GET /api/v1/tenants/rtl-bank', `${API_BASE_URL}/api/v1/tenants/rtl-bank`, { headers: authHeaders() });
            console.log(`GET /tenants/:id: avg=${result.avg}ms, p95=${result.p95}ms, p99=${result.p99}ms`);
            expect(result.avg).toBeLessThan(MAX_RESPONSE_TIME_MS);
        });
    });
    describe('Auth Endpoints', () => {
        it('should respond to login within acceptable time', async () => {
            const result = await runBenchmark('POST /api/v1/auth/login', `${API_BASE_URL}/api/v1/auth/login`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
            ...
            console.log(`POST /auth/login: avg=${result.avg}ms, p95=${result.p95}ms, p99=${result.p99}ms`);
