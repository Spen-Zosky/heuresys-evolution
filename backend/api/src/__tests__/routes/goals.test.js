...
// Test data builders
// ---------------------------------------------------------------------------
function buildGoalRow(overrides = {}) {
    return {
        id: GOAL_ID,
        tenant_id: TENANT_ID,
...
describe('Goals Routes - Behavioral Tests', () => {
    ...
    describe('GET /goals', () => {
        it('should return paginated list of goals', async () => {
            ...
            // List query
            mockQuery.mockResolvedValueOnce({ rows: [goal1, goal2], rowCount: 2 });
            // Count query
        ...
        it('should filter by employee_id', async () => {
            const token = createSysadminToken();
            mockQuery.mockResolvedValueOnce({ rows: [buildGoalRow()], rowCount: 1 });
            ...
            const res = await supertest(app)
                ...
                .set('X-Tenant-ID', TENANT_ID);
            expect(res.status).toBe(200);
            const listQueryParams = mockQuery.mock.calls[0][1];
    ...
    describe('GET /goals/:id', () => {
        it('should return goal detail with related data', async () => {
            ...
            mockQuery.mockResolvedValueOnce({ rows: [goal], rowCount: 1 });
            const res = await supertest(app)
    ...
    describe('POST /goals', () => {
        it('should create goal successfully with valid data', async () => {
            ...
            // INSERT RETURNING *
            mockQuery.mockResolvedValueOnce({ rows: [newGoal], rowCount: 1 });
            const res = await supertest(app)
                .post('/api/v1/goals')
                .set('Authorization', `Bearer ${token}`)
                .set('X-Tenant-ID', TENANT_ID)
    ...
    describe('PUT /goals/:id', () => {
        it('should update goal successfully', async () => {
            ...
            // UPDATE RETURNING *
            mockQuery.mockResolvedValueOnce({ rows: [updatedGoal], rowCount: 1 });
            const res = await supertest(app)
    ...
    describe('DELETE /goals/:id', () => {
        it('should delete goal successfully when no children', async () => {
            ...
            mockQuery.mockResolvedValueOnce({ rows: [{ count: '0' }], rowCount: 1 });
            // DELETE RETURNING id
            mockQuery.mockResolvedValueOnce({ rows: [{ id: GOAL_ID }], rowCount: 1 });
            const res = await supertest(app)
                .delete(`/api/v1/goals/${GOAL_ID}`)
                .set('Authorization', `Bearer ${token}`)
                .set('X-Tenant-ID', TENANT_ID);
    ...
    describe('GET /goals/templates', () => {
        it('should return list of goal templates', async () => {
            ...
            mockQuery.mockResolvedValueOnce({ rows: templates, rowCount: 2 });
            const res = await supertest(app)
    ...
    describe('POST /goals/templates', () => {
        it('should create goal template', async () => {
            ...
            mockQuery.mockResolvedValueOnce({ rows: [newTemplate], rowCount: 1 });
            const res = await supertest(app)
    ...
    describe('POST /goals/from-template', () => {
        it('should create goal from template', async () => {
            ...
            // Get template
            mockQuery.mockResolvedValueOnce({ rows: [template], rowCount: 1 });
            // INSERT goal
            mockQuery.mockResolvedValueOnce({ rows: [newGoal], rowCount: 1 });
            // Update usage count
    ...
    describe('GET /goals/team', () => {
        it('should return team goals with summary', async () => {
            ...
            // Team goals query
            mockQuery.mockResolvedValueOnce({ rows: teamGoals, rowCount: 1 });
            // Summary query
            mockQuery.mockResolvedValueOnce({ rows: [summaryRow], rowCount: 1 });
            const res = await supertest(app)
    ...
    describe('POST /goals/:id/check-in', () => {
        it('should record goal check-in and update progress', async () => {
            ...
            // INSERT check-in
            mockQuery.mockResolvedValueOnce({ rows: [checkInResult], rowCount: 1 });
