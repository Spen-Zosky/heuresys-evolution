...
describe('Analytics Routes', () => {
  describe('Router Configuration', () => {
    ...
    it('should have GET /dashboard/hr route', () => {
      const hrDashboardRoute = analyticsRouter.stack.find(
        (layer: { route?: { path: string; methods: { get?: boolean } } }) =>
          layer.route?.path === '/dashboard/hr' && layer.route?.methods?.get
      )
      expect(hrDashboardRoute).toBeDefined()
    })

    it('should have GET /headcount-trend route', () => {
      const headcountRoute = analyticsRouter.stack.find(
        (layer: { route?: { path: string; methods: { get?: boolean } } }) =>
          layer.route?.path === '/headcount-trend' && layer.route?.methods?.get
      )
      expect(headcountRoute).toBeDefined()
    })

    it('should have GET /turnover-summary route', () => {
      const turnoverRoute = analyticsRouter.stack.find(
        (layer: { route?: { path: string; methods: { get?: boolean } } }) =>
          layer.route?.path === '/turnover-summary' && layer.route?.methods?.get
      )
