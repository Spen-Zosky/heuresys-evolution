...
describe('Courses Routes', () => {
    describe('Router Configuration', () => {
        ...
        it('should have GET / route for listing courses', () => {
            const getRoute = coursesRouter.stack.find((layer) => layer.route?.path === '/' && layer.route?.methods?.get);
            expect(getRoute).toBeDefined();
        });
        it('should have GET /:id route for getting course by ID', () => {
            const getByIdRoute = coursesRouter.stack.find((layer) => layer.route?.path === '/:id' && layer.route?.methods?.get);
            expect(getByIdRoute).toBeDefined();
        });
        it('should have POST / route for creating course', () => {
            const postRoute = coursesRouter.stack.find((layer) => layer.route?.path === '/' && layer.route?.methods?.post);
