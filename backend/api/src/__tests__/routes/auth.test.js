...
describe('Auth Routes - Behavioral Tests', () => {
    ...
    describe('POST /api/v1/auth/login', () => {
        ...
        it('should return 401 when user does not exist', async () => {
            // No account lock
            mockQuery.mockResolvedValueOnce({ rows: [], rowCount: 0 }); // checkAccountLock
            // User lookup returns empty
            mockQuery.mockResolvedValueOnce({ rows: [], rowCount: 0 }); // SELECT user
            // recordFailedAttempt - INSERT on conflict
            mockQuery.mockResolvedValueOnce({ rows: [{ attempt_count: 1 }], rowCount: 1 });
            const res = await supertest(app)
                .post('/api/v1/auth/login')
