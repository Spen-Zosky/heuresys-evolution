...
describe('skill-migration Routes', () => {
    ...
    describe('POST /unknown-skills/:skillId/approve', () => {
        ...
        it('should approve an unknown skill mapping', async () => {
            ...
            expect(mockApproveUnknownSkillMapping).toHaveBeenCalledWith(SKILL_ID, USER_UUID, true, undefined);
        });
        it('should approve with custom esco_skill_id', async () => {
            ...
            expect(res.status).toBe(200);
            expect(res.body.success).toBe(true);
            expect(mockApproveUnknownSkillMapping).toHaveBeenCalledWith(SKILL_ID, USER_UUID, false, escoId);
        });
        it('should return 500 when service throws', async () => {
...
//# sourceMappingURL=skill-migration.test.js.map