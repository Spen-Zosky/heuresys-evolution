...
import { resetFactories, DEFAULT_IDS } from '../factories/index.js';
...
describe('SSO Routes', () => {
    ...
    describe('GET /auth/sso/azure/callback', () => {
        it('should return 400 when error query param present', async () => {
            const res = await supertest(app).get('/api/v1/auth/sso/azure/callback?error=access_denied&error_description=User+denied');
            expect(res.status).toBe(400);
        });
        ...
        it('should return 400 for invalid state parameter', async () => {
            const res = await supertest(app).get('/api/v1/auth/sso/azure/callback?code=abc&state=invalid-not-base64!');
            expect(res.status).toBe(400);
        });
    });
