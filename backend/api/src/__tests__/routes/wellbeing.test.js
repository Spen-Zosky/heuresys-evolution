...
// Test data builders
// ---------------------------------------------------------------------------
function buildCheckin(overrides = {}) {
    return {
        id: CHECKIN_ID,
        tenant_id: TENANT_ID,
...
describe('Wellbeing Routes - Behavioral Tests', () => {
    ...
    describe('GET /wellbeing/stats', () => {
        it('should return aggregated wellbeing statistics', async () => {
            ...
            mockQuery.mockResolvedValueOnce({ rows: [trendsRow], rowCount: 1 });
            const res = await supertest(app)
        ...
        it('should return zeroed stats when no check-ins exist', async () => {
            ...
            mockQuery.mockResolvedValueOnce({ rows: [emptyTrends], rowCount: 1 });
            const res = await supertest(app)
                .get('/api/v1/wellbeing/stats')
                .set('Authorization', `Bearer ${token}`)
                .set('X-Tenant-ID', TENANT_ID);
    ...
    describe('GET /wellbeing/trends', () => {
        it('should return daily trends with default 30 days', async () => {
            ...
            mockQuery.mockResolvedValueOnce({ rows: trendRows, rowCount: 2 });
            const res = await supertest(app)
    ...
    describe('GET /wellbeing/checkins', () => {
        it('should return paginated list of check-ins', async () => {
            ...
            // List query
            mockQuery.mockResolvedValueOnce({ rows: [checkin1, checkin2], rowCount: 2 });
            // Count query
            mockQuery.mockResolvedValueOnce({ rows: [{ count: '12' }], rowCount: 1 });
            const res = await supertest(app)
                .get('/api/v1/wellbeing/checkins')
                .set('Authorization', `Bearer ${token}`)
                .set('X-Tenant-ID', TENANT_ID);
        ...
        it('should filter by employee_id', async () => {
            ...
            mockQuery.mockResolvedValueOnce({ rows: [checkin], rowCount: 1 });
        ...
        it('should filter by is_anonymous', async () => {
            ...
            mockQuery.mockResolvedValueOnce({ rows: [anonCheckin], rowCount: 1 });
            mockQuery.mockResolvedValueOnce({ rows: [{ count: '1' }], rowCount: 1 });
            const res = await supertest(app)
        ...
        it('should respect custom limit and offset', async () => {
            const token = createSysadminToken();
            mockQuery.mockResolvedValueOnce({ rows: [buildCheckin()], rowCount: 1 });
    ...
    describe('GET /wellbeing/employee/:employeeId', () => {
        it('should return employee wellbeing history with averages', async () => {
            ...
            mockQuery.mockResolvedValueOnce({ rows: checkins, rowCount: 3 });
            // Query 2: averages
            mockQuery.mockResolvedValueOnce({ rows: [averagesRow], rowCount: 1 });
            const res = await supertest(app)
        ...
        it('should return empty history for employee with no check-ins', async () => {
            ...
            mockQuery.mockResolvedValueOnce({ rows: [], rowCount: 0 });
            mockQuery.mockResolvedValueOnce({ rows: [emptyAverages], rowCount: 1 });
            const res = await supertest(app)
                .get(`/api/v1/wellbeing/employee/${EMPLOYEE_ID}`)
                .set('Authorization', `Bearer ${token}`)
                .set('X-Tenant-ID', TENANT_ID);
    ...
    describe('POST /wellbeing/checkins', () => {
        it('should create check-in successfully with valid data', async () => {
            ...
            mockQuery.mockResolvedValueOnce({ rows: [], rowCount: 0 });
            // INSERT RETURNING *
            mockQuery.mockResolvedValueOnce({ rows: [newCheckin], rowCount: 1 });
            const res = await supertest(app)
                .post('/api/v1/wellbeing/checkins')
                .set('Authorization', `Bearer ${token}`)
                .set('X-Tenant-ID', TENANT_ID)
        ...
        it('should create check-in with only required fields', async () => {
            ...
            mockQuery.mockResolvedValueOnce({ rows: [], rowCount: 0 });
            mockQuery.mockResolvedValueOnce({ rows: [minimalCheckin], rowCount: 1 });
            const res = await supertest(app)
                .post('/api/v1/wellbeing/checkins')
                .set('Authorization', `Bearer ${token}`)
                .set('X-Tenant-ID', TENANT_ID)
    ...
    describe('PATCH /wellbeing/checkins/:id', () => {
        it('should update check-in successfully', async () => {
            ...
            // Existence check
            mockQuery.mockResolvedValueOnce({ rows: [{ id: CHECKIN_ID }], rowCount: 1 });
            // UPDATE RETURNING *
...
//# sourceMappingURL=wellbeing.test.js.map