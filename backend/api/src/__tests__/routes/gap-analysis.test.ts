...
jest.unstable_mockModule(
  resolve('../../services/gap-analysis/gap-recommendations.service.js'),
  () => ({
...
describe('Gap Analysis Routes', () => {
  ...
  it('POST /compare should return 200 with comparison results', async () => {
    mockAnalyzeEmployeeVsRole.mockResolvedValueOnce(
      buildGapResult({ targetId: EMPLOYEE_ID, overallFitScore: 72.5 })
    );
    mockAnalyzeEmployeeVsRole.mockResolvedValueOnce(
      buildGapResult({
