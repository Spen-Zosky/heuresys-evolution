...
describe('prototypes Routes', () => {
  ...
  describe('GET /nace-sections', () => {
    it('should return 200 with enriched NACE sections', async () => {
      ...
      const sectionK = res.body.data.find((s: Record<string, unknown>) => s.code === 'K');
  ...
  describe('GET /job-templates/:jobTemplateId/suggest-skills', () => {
    ...
    it('should pass query params to service', async () => {
      ...
      await supertest(app)
        .get(
          `/api/v1/prototypes/job-templates/${VALID_UUID}/suggest-skills?max_suggestions=5&include_hard=true&include_soft=false&min_transferability=0.5`
        )
        .set('Authorization', `Bearer ${token}`);

      expect(mockSuggestSkillsForJobTemplate).toHaveBeenCalledWith(
        VALID_UUID,
