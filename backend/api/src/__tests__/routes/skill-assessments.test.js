...
describe('Skill Assessments Routes', () => {
    ...
    describe('GET /skill-assessments', () => {
        ...
        it('should filter by employee_id', async () => {
            ...
            const res = await supertest(app)
                ...
                .set('Authorization', `Bearer ${token}`);
            expect(res.status).toBe(200);
            expect(mockQuery).toHaveBeenCalledWith(expect.stringContaining('esa.employee_id = $'), expect.arrayContaining([TENANT_ID, EMP_ID]));
        });
        it('should filter by skill_name with ILIKE', async () => {
            ...
            const res = await supertest(app)
                ...
                .set('Authorization', `Bearer ${token}`);
            expect(res.status).toBe(200);
            expect(mockQuery).toHaveBeenCalledWith(expect.stringContaining('esa.skill_name ILIKE'), expect.arrayContaining([TENANT_ID]));
        });
        it('should filter by method', async () => {
            ...
            const res = await supertest(app)
                ...
                .set('Authorization', `Bearer ${token}`);
            expect(res.status).toBe(200);
            expect(mockQuery).toHaveBeenCalledWith(expect.stringContaining('assessment_method = $'), expect.arrayContaining(['self']));
        });
        it('should filter by has_gap', async () => {
            ...
            const res = await supertest(app)
                ...
                .set('Authorization', `Bearer ${token}`);
            expect(res.status).toBe(200);
            expect(mockQuery).toHaveBeenCalledWith(expect.stringContaining('esa.gap > 0'), expect.any(Array));
        });
    ...
    describe('GET /skill-assessments/gaps', () => {
        ...
        it('should pass min_gap and limit params', async () => {
            ...
            expect(mockQuery).toHaveBeenCalledWith(expect.stringContaining('esa.gap >='), expect.arrayContaining([TENANT_ID, 2, 10]));
        });
    });
    // ===========================================================================
    // SINGLE ASSESSMENT
...
//# sourceMappingURL=skill-assessments.test.js.map