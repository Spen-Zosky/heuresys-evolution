...
function findGetHandler(path) {
    return healthRouter.stack.find((layer) => layer.route?.path === path && layer.route?.methods?.get)?.route?.stack[0]?.handle;
}
...
describe('Health Routes', () => {
    describe('GET /health', () => {
        it('should return success status', async () => {
            ...
            if (healthHandler) {
                ...
                await healthHandler({}, mockRes, jest.fn());
    ...
    describe('router configuration', () => {
        it('should have /health route registered', () => {
            const hasHealthRoute = healthRouter.stack.some((layer) => layer.route?.path === '/health');
            expect(hasHealthRoute).toBe(true);
        });
        it('should have /db-health route registered', () => {
            const hasDbHealthRoute = healthRouter.stack.some((layer) => layer.route?.path === '/db-health');
