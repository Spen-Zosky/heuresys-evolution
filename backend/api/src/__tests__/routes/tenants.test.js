...
import { buildTenant, buildSysadminTokenPayload, buildSuperuserTokenPayload, buildAdminTokenPayload, buildEmployeeTokenPayload, resetFactories, DEFAULT_IDS, } from '../factories/index.js';
// ---------------------------------------------------------------------------
...
describe('Tenants Routes - Behavioral Tests', () => {
    ...
    describe('GET /api/v1/tenants/meta/statuses', () => {
        it('should return list of valid statuses', async () => {
            ...
            const values = res.body.data.map((s) => s.value);
    ...
    describe('GET /api/v1/tenants/meta/plans', () => {
        it('should return list of subscription plans', async () => {
            ...
            expect(res.body.success).toBe(true);
            expect(res.body.data).toBeInstanceOf(Array);
            const values = res.body.data.map((p) => p.value);
    ...
    describe('GET /api/v1/tenants/:identifier/stats', () => {
        it('should return tenant statistics', async () => {
            ...
            mockQuery.mockResolvedValueOnce({ rows: [{ count: '200' }], rowCount: 1 }); // employees
            mockQuery.mockResolvedValueOnce({ rows: [{ count: '10' }], rowCount: 1 }); // departments
            mockQuery.mockResolvedValueOnce({ rows: [{ count: '5' }], rowCount: 1 }); // locations
            mockQuery.mockResolvedValueOnce({ rows: [{ count: '50' }], rowCount: 1 }); // goals
            mockQuery.mockResolvedValueOnce({ rows: [{ count: '15' }], rowCount: 1 }); // reviews
            const res = await supertest(app)
