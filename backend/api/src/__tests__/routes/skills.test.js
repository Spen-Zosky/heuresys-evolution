...
describe('Skills Routes', () => {
    describe('Router Configuration', () => {
        ...
        it('should have GET / route for listing skills', () => {
            const getRoute = skillsRouter.stack.find((layer) => layer.route?.path === '/' && layer.route?.methods?.get);
            expect(getRoute).toBeDefined();
        });
        it('should have GET /:id route for getting skill by ID', () => {
            const getByIdRoute = skillsRouter.stack.find((layer) => layer.route?.path === '/:id' && layer.route?.methods?.get);
            expect(getByIdRoute).toBeDefined();
        });
        it('should have POST / route for creating skill', () => {
            const postRoute = skillsRouter.stack.find((layer) => layer.route?.path === '/' && layer.route?.methods?.post);
