...
describe('Skill Analytics Routes', () => {
    ...
    describe('GET /skill-analytics/heatmap', () => {
        ...
        it('should pass department and skill filters', async () => {
            ...
            const res = await supertest(app)
                .get(`/api/v1/skill-analytics/heatmap?org_unit_ids=${DEPT_ID}&skill_ids=s1,s2&min_proficiency=3`)
                .set('Authorization', `Bearer ${token}`);
            expect(res.status).toBe(200);
            expect(mockGetSkillCoverageHeatmap).toHaveBeenCalledWith(TENANT_ID, expect.objectContaining({
    ...
    describe('GET /skill-analytics/trends', () => {
        ...
        it('should pass period and lookback params', async () => {
            ...
            const res = await supertest(app)
                ...
                .set('Authorization', `Bearer ${token}`);
            expect(res.status).toBe(200);
            expect(mockGetSkillTrends).toHaveBeenCalledWith(TENANT_ID, expect.objectContaining({
                period: 'quarterly',
                lookback_periods: 4,
                skill_ids: ['s1', 's2'],
            }));
    ...
    describe('GET /skill-analytics/shortages', () => {
        ...
        it('should pass department and min_shortage filters', async () => {
            ...
            await supertest(app)
                ...
                .set('Authorization', `Bearer ${token}`);
            expect(mockGetCriticalShortages).toHaveBeenCalledWith(TENANT_ID, expect.objectContaining({
                org_unit_id: DEPT_ID,
                min_shortage: 3,
            }));
        });
    ...
    describe('GET /skill-analytics/ksaba', () => {
        ...
        it('should pass org_unit_id and employee_id filters', async () => {
            ...
            await supertest(app)
                .get(`/api/v1/skill-analytics/ksaba?org_unit_id=${DEPT_ID}&employee_id=${DEFAULT_IDS.EMPLOYEE_ID}`)
                .set('Authorization', `Bearer ${token}`);
            expect(mockGetKSABADistribution).toHaveBeenCalledWith(TENANT_ID, expect.objectContaining({
                org_unit_id: DEPT_ID,
    ...
    describe('GET /skill-analytics/emerging', () => {
        ...
        it('should pass lookback, occurrences, and limit params', async () => {
            ...
            await supertest(app)
                ...
                .set('Authorization', `Bearer ${token}`);
            expect(mockGetEmergingSkills).toHaveBeenCalledWith(TENANT_ID, expect.objectContaining({
                lookback_days: 30,
                min_occurrences: 5,
                limit: 10,
            }));
    ...
    describe('GET /skill-analytics/org-units', () => {
        ...
        it('should pass org_unit_ids filter', async () => {
            ...
            await supertest(app)
                ...
                .set('Authorization', `Bearer ${token}`);
            expect(mockGetDepartmentComparison).toHaveBeenCalledWith(TENANT_ID, expect.objectContaining({
...
//# sourceMappingURL=skill-analytics.test.js.map