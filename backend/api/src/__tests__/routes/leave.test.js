...
describe('Leave Routes', () => {
    describe('Router Configuration', () => {
        ...
        it('should have GET /requests route for listing leave requests', () => {
            const getRoute = leaveRouter.stack.find((layer) => layer.route?.path === '/requests' && layer.route?.methods?.get);
            expect(getRoute).toBeDefined();
        });
        it('should have POST /requests route for creating leave request', () => {
            const postRoute = leaveRouter.stack.find((layer) => layer.route?.path === '/requests' && layer.route?.methods?.post);
            expect(postRoute).toBeDefined();
        });
        it('should have GET /balances route for leave balances', () => {
            const balancesRoute = leaveRouter.stack.find((layer) => layer.route?.path === '/balances' && layer.route?.methods?.get);
...
describe('Leave API Response Format', () => {
    ...
    it('should define expected leave balance structure', () => {
        ...
        expect(expectedBalance.remaining).toBe(expectedBalance.entitled - expectedBalance.used);
    });
