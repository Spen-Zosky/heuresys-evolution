...
describe('Users Routes', () => {
    describe('Router Configuration', () => {
        ...
        it('should have GET /meta/roles route', () => {
            const metaRolesRoute = usersRouter.stack.find((layer) => layer.route?.path === '/meta/roles' && layer.route?.methods?.get);
            expect(metaRolesRoute).toBeDefined();
        });
        it('should have GET / route for listing users', () => {
            const getRoute = usersRouter.stack.find((layer) => layer.route?.path === '/' && layer.route?.methods?.get);
            expect(getRoute).toBeDefined();
        });
        it('should have GET /:id route for getting user by ID', () => {
            const getByIdRoute = usersRouter.stack.find((layer) => layer.route?.path === '/:id' && layer.route?.methods?.get);
            expect(getByIdRoute).toBeDefined();
        });
        it('should have POST / route for creating user', () => {
            const postRoute = usersRouter.stack.find((layer) => layer.route?.path === '/' && layer.route?.methods?.post);
            expect(postRoute).toBeDefined();
        });
        it('should have PATCH /:id route for updating user', () => {
            const patchRoute = usersRouter.stack.find((layer) => layer.route?.path === '/:id' && layer.route?.methods?.patch);
            expect(patchRoute).toBeDefined();
        });
        it('should have DELETE /:id route for deleting user', () => {
            const deleteRoute = usersRouter.stack.find((layer) => layer.route?.path === '/:id' && layer.route?.methods?.delete);
    ...
    describe('Protected Routes', () => {
        it('should require authentication for all routes', () => {
            ...
            const hasAuthMiddleware = usersRouter.stack.some((layer) => layer.name === 'authMiddleware' || layer.handle?.name === 'authMiddleware');
