...
describe('Internal Mobility Routes', () => {
    ...
    describe('GET /jobs', () => {
        ...
        it('should apply filter params (status, department, work_type, job_level)', async () => {
            ...
            const res = await supertest(app)
                .get('/api/v1/internal-mobility/jobs?status=open&department=IT&work_type=remote&job_level=senior&limit=10&offset=5')
                .set('Authorization', `Bearer ${token}`);
            expect(res.status).toBe(200);
    ...
    describe('GET /applications', () => {
        ...
        it('should filter by job_posting_id, employee_id, status', async () => {
            mockQuery.mockResolvedValueOnce({ rows: [], rowCount: 0 });
            const res = await supertest(app)
                .get(`/api/v1/internal-mobility/applications?job_posting_id=${JOB_ID}&employee_id=${EMP_ID}&status=submitted`)
                .set('Authorization', `Bearer ${token}`);
            expect(res.status).toBe(200);
        });
    });
...
//# sourceMappingURL=internal-mobility.test.js.map