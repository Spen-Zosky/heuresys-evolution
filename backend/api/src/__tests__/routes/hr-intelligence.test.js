...
describe('HR Intelligence Routes', () => {
    ...
    describe('GET /skills/search (public)', () => {
        ...
        it('should pass custom query parameters', async () => {
            mockSearchSkills.mockResolvedValueOnce({ skills: [], total: 0 });
            await supertest(app).get('/api/v1/hr-intelligence/skills/search?q=python&skillType=knowledge&limit=10&offset=5');
    ...
    describe('GET /skills/:uri/hierarchy (public)', () => {
        ...
        it('should decode URI parameter', async () => {
            ...
            await supertest(app).get('/api/v1/hr-intelligence/skills/http%3A%2F%2Fesco.eu%2Fskill%2F1/hierarchy');
    ...
    describe('GET /occupations/search (public)', () => {
        it('should return 200 with occupation results', async () => {
            ...
            const res = await supertest(app).get('/api/v1/hr-intelligence/occupations/search?q=developer');
            expect(res.status).toBe(200);
            expect(res.body.success).toBe(true);
        ...
        it('should pass custom limit/offset', async () => {
            ...
            await supertest(app).get('/api/v1/hr-intelligence/occupations/search?q=test&limit=5&offset=10');
    ...
    describe('GET /occupations/:uri/skills (public)', () => {
        ...
        it('should decode URI parameter', async () => {
            ...
            await supertest(app).get('/api/v1/hr-intelligence/occupations/http%3A%2F%2Fesco.eu%2Focc%2F1/skills');
    ...
    describe('GET /employees/:employeeId/skills', () => {
        it('should return 401 without auth token', async () => {
            const res = await supertest(authApp).get(`/api/v1/hr-intelligence/employees/${EMP_ID}/skills`);
            expect(res.status).toBe(401);
        });
    ...
    describe('POST /employees/:employeeId/skills/:skillId/verify', () => {
        const SKILL_ID = DEFAULT_IDS.GOAL_ID;
        it('should return 401 without auth token', async () => {
            const res = await supertest(authApp).post(`/api/v1/hr-intelligence/employees/${EMP_ID}/skills/${SKILL_ID}/verify`);
            expect(res.status).toBe(401);
        });
    ...
    describe('DELETE /employees/:employeeId/skills/:skillId', () => {
        const SKILL_ID = DEFAULT_IDS.GOAL_ID;
        it('should return 401 without auth token', async () => {
            const res = await supertest(authApp).delete(`/api/v1/hr-intelligence/employees/${EMP_ID}/skills/${SKILL_ID}`);
            expect(res.status).toBe(401);
        });
    ...
    describe('GET /job-market/postings', () => {
        ...
        it('should pass all query params to service', async () => {
            ...
            await supertest(authApp)
                .get('/api/v1/hr-intelligence/job-market/postings?q=dev&skills=python,java&location=Rome&countryCode=IT&industry=tech&experienceLevel=senior&salaryMin=50000&salaryMax=80000&employmentType=full-time&locationType=remote&limit=10&offset=5')
                .set('Authorization', `Bearer ${token}`);
            expect(mockSearchJobPostings).toHaveBeenCalledWith(expect.objectContaining({
                query: 'dev',
                skills: ['python', 'java'],
                location: 'Rome',
                countryCode: 'IT',
    ...
    describe('GET /job-market/statistics', () => {
        it('should return 200 with statistics', async () => {
            ...
            const res = await supertest(authApp)
                .get('/api/v1/hr-intelligence/job-market/statistics?countryCode=IT&industry=tech&period=monthly&days=60')
                .set('Authorization', `Bearer ${token}`);
            expect(res.status).toBe(200);
            expect(res.body.success).toBe(true);
        ...
        it('should use default days=30 when not specified', async () => {
            ...
            await supertest(authApp)
                ...
                .set('Authorization', `Bearer ${token}`);
            expect(mockGetJobMarketStatistics).toHaveBeenCalledWith(expect.objectContaining({ days: 30 }));
        });
    });
    describe('GET /job-market/trending-skills', () => {
        ...
        it('should use default days=30 and limit=20', async () => {
            ...
            await supertest(authApp)
                ...
                .set('Authorization', `Bearer ${token}`);
            expect(mockGetTrendingSkills).toHaveBeenCalledWith(expect.objectContaining({ days: 30, limit: 20 }));
        });
    });
    // ===========================================================================
    ...
    describe('GET /skill-gap-analyses', () => {
        ...
        it('should use default limit/offset', async () => {
            ...
            await supertest(authApp)
                ...
                .set('Authorization', `Bearer ${token}`);
            expect(mockListSkillGapAnalyses).toHaveBeenCalledWith(expect.objectContaining({ limit: 20, offset: 0 }));
        });
    });
    ...
    describe('GET /benchmarks/reports', () => {
        ...
        it('should pass reportType and pagination params', async () => {
            ...
            await supertest(authApp)
                .get('/api/v1/hr-intelligence/benchmarks/reports?reportType=salary_benchmark&limit=5&offset=10')
                .set('Authorization', `Bearer ${token}`);
...
//# sourceMappingURL=hr-intelligence.test.js.map