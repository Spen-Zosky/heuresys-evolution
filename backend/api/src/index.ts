...
app.use(
  ...
  tenantContextMiddleware,
  marketplaceInstallationsRoutes
);
...
app.use(
  createErrorMiddleware({
    environment: config.nodeEnv as 'development' | 'production' | 'test',
...
async function startServer(): Promise<void> {
  ...
  if (!appDbConnected) {
    ...
    logger.warn(
      '[DB:app] Ensure heuresys_app role exists (run migration 084_configure_app_role.sql)'
    );
  ...
  try {
    const { createEmbeddingQueueProcessor } =
      await import('./services/embedding-queue-processor.js');
    ...
    logger.warn(
      { err: err as Error },
      '[Embeddings] Queue processor not started: %s',
  ...
  try {
    ...
    logger.info(
      `[RBP] Cache initialized: ${stats.roles} roles, ${stats.areas} areas, ${stats.permissions} permissions`
    );
  } catch (err) {
    logger.error({ err: err as Error }, '[RBP] Cache init failed — will retry on first access');
  }
