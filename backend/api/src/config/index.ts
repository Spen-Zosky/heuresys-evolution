...
function resolveJwtSecret(): string {
  ...
  if (isWeak && isProduction) {
    throw new Error(
      'JWT_SECRET matches a known placeholder pattern — set a strong secret for production'
    );
  }

  if (isWeak && isDevelopment) {
    logger.warn(
      '[CONFIG] JWT_SECRET appears weak — acceptable in development, must be changed for production'
    );
  }
...
function resolveRefreshSecret(): string {
  ...
  if (!secret) {
    if (isDevelopment) {
      const devSecret = crypto.randomBytes(32).toString('hex');
      logger.warn(
        '[CONFIG] REFRESH_TOKEN_SECRET not set — using random ephemeral secret (development only)'
      );
      return devSecret;
    }
  ...
  if (isWeak && isProduction) {
    throw new Error(
      'REFRESH_TOKEN_SECRET matches a known placeholder pattern — set a strong secret for production'
    );
  }

  if (isWeak && isDevelopment) {
    logger.warn(
      '[CONFIG] REFRESH_TOKEN_SECRET appears weak — acceptable in development, must be changed for production'
    );
  }
