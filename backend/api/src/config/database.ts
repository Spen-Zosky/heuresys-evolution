...
// =============================================================================
const appPoolConfig: PoolConfig = {
  host: process.env.PLATFORM_DB_HOST || 'localhost',
  port: parseInt(process.env.PLATFORM_DB_PORT || '5433', 10),
...
export async function testAppConnection(): Promise<boolean> {
  try {
    ...
    const row = result.rows[0] as { current_user: string; is_super: string };
    client.release();

    if (row.is_super === 'on') {
      logger.error(
        '[DB:app] SECURITY WARNING: appPool connected as superuser! RLS will NOT be enforced.'
      );
      return false;
    }
