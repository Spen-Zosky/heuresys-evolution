...
import { createAIOrchestrator, AIProvider } from './ai-orchestrator.js';
...
export class DocumentProcessor {
  ...
  constructor(tenantId: string, provider: AIProvider = 'openai') {
  ...
  private async getDocumentContent(doc: {
    filePath: string | null;
    mimeType: string;
    metadata: Record<string, unknown>;
  }): Promise<string> {
  ...
  private async storeChunks(
    ...
    const storedChunks: Array<{ id: string; content: string }> = [];
    ...
    if (knowledgeBaseId) {
      await pool.query(
        'UPDATE rag_documents SET knowledge_base_id = $1 WHERE id = $2 AND tenant_id = $3',
        [knowledgeBaseId, documentId, this.tenantId]
      );
    }
  ...
  async ingestCCNL(params: {
    ...
    version?: string;
    metadata?: Record<string, unknown>;
  }): Promise<{ documentId: string; ccnlId: string }> {
  ...
  async ingestCompanyPolicy(params: {
    ...
    orgUnitId?: string;
    metadata?: Record<string, unknown>;
  }): Promise<{ documentId: string; knowledgeBaseId: string }> {
  ...
  async processPendingDocuments(limit: number = 10): Promise<ProcessingResult[]> {
    const result = await pool.query(
      `
