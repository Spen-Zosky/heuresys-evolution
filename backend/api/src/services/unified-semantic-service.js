...
export class UnifiedSemanticService {
     ...
     */
    async loadApiKeyFromDb() {
        try {
            const result = await this.pool.query(`SELECT config_value FROM system_config WHERE config_key = 'openai_api_key' LIMIT 1`);
    ...
    `;
        ...
        for (const row of result.rows) {
            ...
            if (!resultsByType[row.entity_type]) {
                resultsByType[row.entity_type] = [];
            }
            resultsByType[row.entity_type].push(entity);
        }
        const searchDurationMs = Date.now() - startTime;
        // Log search for analytics
        await this.logSearch(tenantId, query, embeddingStr, entityTypes || [], allResults.length, allResults.slice(0, 5), searchDurationMs);
        return {
            query,
     ...
     */
    async directSearch(options) {
        const startTime = Date.now();
        const { query, tenantId, entityTypes = ['employee', 'department', 'performance_review', 'skill_gap_analysis'], limit = 10, similarityThreshold = 0.5, } = options;
        // Generate query embedding
        const queryEmbedding = await this.generateEmbedding(query);
        if (!queryEmbedding) {
            throw new Error('Failed to generate query embedding');
        ...
        for (const entityType of entityTypes) {
            const results = await this.searchEntityType(entityType, embeddingStr, tenantId, limit, similarityThreshold);
     ...
     */
    async searchEntityType(entityType, embeddingStr, tenantId, limit, threshold) {
        ...
        for (let i = 0; i < records.length; i += batchSize) {
            ...
            for (const record of batch) {
                try {
                    ...
                    if (embedding) {
                        await generator.updateEmbedding(this.pool, String(record.id), embedding, textHash, this.embeddingModel);
                        // Also add to unified index
                        await this.upsertSemanticIndex(String(record.tenant_id), entityType, String(record.id), generator.getName(record), generator.getContext(record), embedding, generator.getMetadata(record));
                        embedded++;
                    }
                    else {
                        errors++;
     ...
     */
    async generateAllEmbeddings(tenantId) {
        const entityTypes = [
            'employee',
            'department',
            'org_unit',
        ...
        for (const entityType of entityTypes) {
            ...
            results.push(result);
            logger.info(`Completed: ${result.embedded} embedded, ${result.skipped} skipped, ${result.errors} errors`);
        }
        return results;
    }
    /**
     ...
     */
    getEmbeddingGenerator(entityType) {
        const generators = {
            employee: {
          ...
          `;
                    if (!force)
                        query += ` AND profile_embedding IS NULL`;
                    if (tenantId)
                ...
                updateEmbedding: async (pool, id, embedding, hash, model) => {
                    await pool.query(`UPDATE employees SET profile_embedding = $1, embedding_text_hash = $2,
             embedding_model = $3, embedding_generated_at = NOW() WHERE id = $4`, [`[${embedding.join(',')}]`, hash, model, id]);
                },
            },
            department: {
                getRecords: async (pool, tenantId, force) => {
                ...
                getName: (r) => r.name,
                getContext: (r) => r.description || '',
                getMetadata: () => ({}),
                updateEmbedding: async (pool, id, embedding, hash, model) => {
                    await pool.query(`UPDATE org_units SET embedding = $1, embedding_text_hash = $2,
             embedding_model = $3, embedding_generated_at = NOW() WHERE id = $4`, [`[${embedding.join(',')}]`, hash, model, id]);
                },
            },
            org_unit: {
            ...
            location: {
                ...
                buildText: (r) => `${r.name}. ${r.location_type || ''}. ${r.address || ''}, ${r.city || ''}, ${r.province || ''}, ${r.country || ''}`,
                getName: (r) => r.name,
                getContext: (r) => `${r.city || ''}, ${r.country || ''}`,
                getMetadata: (r) => ({ type: r.location_type, city: r.city, country: r.country }),
                updateEmbedding: async (pool, id, embedding, hash, model) => {
                    await pool.query(`UPDATE locations SET embedding = $1, embedding_text_hash = $2,
             embedding_model = $3, embedding_generated_at = NOW() WHERE id = $4`, [`[${embedding.join(',')}]`, hash, model, id]);
                },
            },
            performance_review: {
                getRecords: async (pool, tenantId, force) => {
                    ...
                    if (!force)
                        query += ` WHERE content_embedding IS NULL`;
                    else
                        query += ` WHERE 1=1`;
                    if (tenantId)
                        query += ` AND tenant_id = '${tenantId}'`;
                ...
                getContext: (r) => r.review_type || '',
                getMetadata: (r) => ({ employee_id: r.employee_id, review_type: r.review_type }),
                updateEmbedding: async (pool, id, embedding, hash, model) => {
                    await pool.query(`UPDATE performance_reviews SET content_embedding = $1, embedding_text_hash = $2,
             embedding_model = $3, embedding_generated_at = NOW() WHERE id = $4`, [`[${embedding.join(',')}]`, hash, model, id]);
                },
            },
            check_in: {
                ...
                getContext: (r) => r.meeting_type || '',
                getMetadata: (r) => ({ employee_id: r.employee_id, meeting_type: r.meeting_type }),
                updateEmbedding: async (pool, id, embedding, hash, model) => {
                    await pool.query(`UPDATE check_ins SET content_embedding = $1, embedding_text_hash = $2,
             embedding_model = $3, embedding_generated_at = NOW() WHERE id = $4`, [`[${embedding.join(',')}]`, hash, model, id]);
                },
            },
            feedback_360: {
                ...
                getContext: (r) => r.relationship_type || '',
                getMetadata: (r) => ({
                    target_employee_id: r.target_employee_id,
                    relationship: r.relationship_type,
                }),
                updateEmbedding: async (pool, id, embedding, hash, model) => {
                    await pool.query(`UPDATE feedback_360 SET content_embedding = $1, embedding_text_hash = $2,
             embedding_model = $3, embedding_generated_at = NOW() WHERE id = $4`, [`[${embedding.join(',')}]`, hash, model, id]);
                },
            },
            skill_gap_analysis: {
                getRecords: async (pool, tenantId, force) => {
                    ...
                    if (!force)
                        query += ` WHERE analysis_embedding IS NULL`;
                    else
                        query += ` WHERE 1=1`;
                    if (tenantId)
                        query += ` AND tenant_id = '${tenantId}'`;
                ...
                getName: (r) => r.analysis_name || 'Skill Gap Analysis',
                getContext: (r) => r.analysis_type || '',
                getMetadata: (r) => ({ analysis_type: r.analysis_type }),
                updateEmbedding: async (pool, id, embedding, hash, model) => {
                    await pool.query(`UPDATE skill_gap_analyses SET analysis_embedding = $1, embedding_text_hash = $2,
             embedding_model = $3, embedding_generated_at = NOW() WHERE id = $4`, [`[${embedding.join(',')}]`, hash, model, id]);
                },
            },
            career_path: {
                ...
                updateEmbedding: async (pool, id, embedding, hash, model) => {
                    await pool.query(`UPDATE career_paths SET embedding = $1, embedding_text_hash = $2,
             embedding_model = $3, embedding_generated_at = NOW() WHERE id = $4`, [`[${embedding.join(',')}]`, hash, model, id]);
                },
            },
            learning_path: {
                ...
                buildText: (r) => `${r.title}. ${r.title_en || ''}. ${r.description || ''}. Target: ${r.target_role || ''}`,
                getName: (r) => r.title,
                getContext: (r) => r.target_role || '',
                getMetadata: (r) => ({ target_role: r.target_role }),
                updateEmbedding: async (pool, id, embedding, hash, model) => {
                    await pool.query(`UPDATE learning_paths SET embedding = $1, embedding_text_hash = $2,
             embedding_model = $3, embedding_generated_at = NOW() WHERE id = $4`, [`[${embedding.join(',')}]`, hash, model, id]);
                },
            },
            candidate: {
                getRecords: async (pool, tenantId, force) => {
                    ...
                    if (!force)
                        query += ` WHERE profile_embedding IS NULL`;
                    else
                        query += ` WHERE 1=1`;
                    if (tenantId)
                        query += ` AND tenant_id = '${tenantId}'`;
                ...
                updateEmbedding: async (pool, id, embedding, hash, model) => {
                    await pool.query(`UPDATE recruiting_candidates SET profile_embedding = $1, embedding_text_hash = $2,
             embedding_model = $3, embedding_generated_at = NOW() WHERE id = $4`, [`[${embedding.join(',')}]`, hash, model, id]);
                },
            },
        };
     ...
     */
    async upsertSemanticIndex(tenantId, entityType, entityId, entityName, entityContext, embedding, metadata) {
        const embeddingStr = `[${embedding.join(',')}]`;
        await this.pool.query(`INSERT INTO semantic_entity_index
     ...
     * Log search for analytics
     */
    async logSearch(tenantId, queryText, queryEmbedding, entityTypes, resultsCount, topResults, durationMs) {
        try {
            await this.pool.query(`INSERT INTO semantic_search_log
...
export function createUnifiedSemanticService(pool) {
    if (!semanticServiceInstance) {
        semanticServiceInstance = new UnifiedSemanticService(pool);
    }
    return semanticServiceInstance;
}
export function getUnifiedSemanticService() {
    return semanticServiceInstance;
}
//# sourceMappingURL=unified-semantic-service.js.map