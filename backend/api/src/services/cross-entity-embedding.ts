...
export interface OrgAdvisoryRequest {
  ...
  language: 'en' | 'it';
  additionalContext?: string; // "lavorazioni meccaniche", "startup tech", etc.
}
...
export interface OrgAdvisoryResponse {
  ...
  skillsProfile: Array<{
    skillId: string;
    skillLabel: string;
    frequency: number; // How many roles require this skill
    importance: string;
  }>;
...
export class CrossEntityEmbeddingService extends OntologyEmbeddingService {
  ...
  async generateEntityEmbeddings(config: EmbeddingJobConfig): Promise<{
    processed: number;
    failed: number;
    tokens: number;
  }> {
  ...
  private async processOccupationsEmbeddings(jobType: string): Promise<{
    processed: number;
    failed: number;
    tokens: number;
  }> {
    ...
    const whereClause =
      jobType === 'incremental' ? 'WHERE embedding_en IS NULL OR embedding_it IS NULL' : '';
    ...
    while (true) {
      ...
      const textsEn: string[] = [];
      const textsIt: string[] = [];
      const ids: string[] = [];
  ...
  private async processJobTemplatesEmbeddings(jobType: string): Promise<{
    processed: number;
    failed: number;
    tokens: number;
  }> {
  ...
  private async processCoursesEmbeddings(jobType: string): Promise<{
    processed: number;
    failed: number;
    tokens: number;
  }> {
  ...
  private async processGoalsEmbeddings(
    jobType: string,
    tenantId?: string
  ): Promise<{
    processed: number;
    ...
    const params: (string | number)[] = [batchSize, 0];
  ...
  private async processNaceEmbeddings(
    entityType:
      | 'industry_classifications_l1'
      | 'industry_classifications_l2'
      | 'industry_classifications_l3',
    jobType: string
  ): Promise<{ processed: number; failed: number; tokens: number }> {
    const batchSize = 50;
    let offset = 0;
    let processed = 0;
    let failed = 0;
    ...
    const tableName = validateTableName(
      ...
      'CrossEntityEmbedding.processNaceEmbeddings'
    );
    const level =
      entityType === 'industry_classifications_l1'
        ? 1
        : entityType === 'industry_classifications_l2'
          ? 2
          : 3;
    const whereClause =
      jobType === 'incremental' ? 'AND (embedding_en IS NULL OR embedding_it IS NULL)' : '';
  ...
  async crossEntitySearch(options: CrossEntitySearchOptions): Promise<CrossEntitySearchResponse> {
    ...
    const resultsByType: Record<EntityType, EntitySearchResult[]> = {
      skills: [],
      occupations: [],
      jobs: [],
      courses: [],
    ...
    const searchPromises = options.entityTypes.map(async (entityType) => {
      const results = await this.searchEntity(
        entityType,
        vectorStr,
        options.language,
        options.limit,
    ...
    await this.logCrossEntitySearch(
      options.query,
      vectorStr,
      options.entityTypes,
      totalResults,
  ...
  private async searchEntity(
    ...
    const embCol = validateEmbeddingColumn(
      ...
      'CrossEntityEmbedding.searchEntity.embCol'
    );
    ...
    switch (entityType) {
      case 'skills':
        const labelCol = validateEmbeddingColumn(
          ...
          'CrossEntityEmbedding.searchEntity.skills.labelCol'
        );
        const descCol = validateEmbeddingColumn(
          language === 'it' ? 'description_it' : 'description_en',
          'CrossEntityEmbedding.searchEntity.skills.descCol'
        );
        query = `
      ...
      case 'occupations':
        const occLabelCol = validateEmbeddingColumn(
          language === 'it' ? 'preferred_label_it' : 'preferred_label_en',
          'CrossEntityEmbedding.searchEntity.occupations.labelCol'
        );
        const occDescCol = validateEmbeddingColumn(
          language === 'it' ? 'description_it' : 'description_en',
          'CrossEntityEmbedding.searchEntity.occupations.descCol'
        );
        query = `
      ...
      case 'jobs':
        const jobLabelCol = validateEmbeddingColumn(
          language === 'it' ? 'title_it' : 'title_en',
          'CrossEntityEmbedding.searchEntity.jobs.labelCol'
        );
        query = `
      ...
      case 'courses':
        const courseLabelCol = validateEmbeddingColumn(
          language === 'it' ? 'title' : 'title_en',
          'CrossEntityEmbedding.searchEntity.courses.labelCol'
        );
        const courseDescCol = validateEmbeddingColumn(
          language === 'it' ? 'description' : 'description_en',
          'CrossEntityEmbedding.searchEntity.courses.descCol'
        );
        query = `
      ...
      case 'industry_classifications_l1':
        const secLabelCol = validateEmbeddingColumn(
          language === 'it' ? 'name_it' : 'name_en',
          'CrossEntityEmbedding.searchEntity.industry_classifications_l1.labelCol'
        );
        query = `
      ...
      case 'industry_classifications_l2':
        const divLabelCol = validateEmbeddingColumn(
          language === 'it' ? 'name_it' : 'name_en',
          'CrossEntityEmbedding.searchEntity.industry_classifications_l2.labelCol'
        );
        query = `
      ...
      case 'industry_classifications_l3':
        const grpLabelCol = validateEmbeddingColumn(
          language === 'it' ? 'name_it' : 'name_en',
          'CrossEntityEmbedding.searchEntity.industry_classifications_l3.labelCol'
        );
        query = `
  ...
  private async logCrossEntitySearch(
    queryText: string,
    queryEmbedding: string,
    entityTypes: EntityType[],
    resultsCount: number,
    topResults: Record<EntityType, EntitySearchResult[]>,
    durationMs: number,
    tenantId?: string
  ): Promise<void> {
    try {
  ...
  async getOrganizationAdvice(request: OrgAdvisoryRequest): Promise<OrgAdvisoryResponse> {
    // Step 1: Find matching industry via semantic search
    const industryMatch = await this.findMatchingIndustry(
      request.industry,
    ...
    const occupations = await this.findIndustryOccupations(
      industryMatch.code,
      industryMatch.level,
      request.companySize,
      request.language
    ...
    const skillsProfile = await this.buildSkillsProfile(
      occupations.map((o) => o.occupationUri),
      request.language
    );
  ...
  private async findMatchingIndustry(
    industry: string,
    ...
    const embCol = validateEmbeddingColumn(
      language === 'it' ? 'embedding_it' : 'embedding_en',
      'CrossEntityEmbedding.findMatchingIndustry.embCol'
    );
  ...
  private async findIndustryOccupations(
    naceCode: string,
    level: 'section' | 'division' | 'group',
    companySize: number,
    language: 'en' | 'it'
    ...
    const embCol = validateEmbeddingColumn(
      language === 'it' ? 'embedding_it' : 'embedding_en',
      'CrossEntityEmbedding.findIndustryOccupations.embCol'
    );
    const tableName = validateTableName(
      'industry_classifications',
      'CrossEntityEmbedding.findIndustryOccupations'
    );
    ...
    const labelCol = validateEmbeddingColumn(
      language === 'it' ? 'preferred_label_it' : 'preferred_label_en',
      'CrossEntityEmbedding.findIndustryOccupations.labelCol'
    );
    const descCol = validateEmbeddingColumn(
      language === 'it' ? 'description_it' : 'description_en',
      'CrossEntityEmbedding.findIndustryOccupations.descCol'
    );
  ...
  private getDefaultOccupations(companySize: number, language: 'en' | 'it'): OrgRole[] {
  ...
  private assignRolesToCompany(
    ...
    const skilled = occupations.filter(
      (o) => o.isco_code?.startsWith('7') || o.isco_code?.startsWith('8')
    );
    ...
    const coreOccupations =
      skilled.length > 0 ? skilled : technicians.length > 0 ? technicians : professionals;
  ...
  private buildOrgStructure(
    ...
    const deptMap = new Map<string, OrgRole[]>();
  ...
  private async buildSkillsProfile(
  ...
  > {
    ...
    const labelCol = validateEmbeddingColumn(
      language === 'it' ? 'preferred_label_it' : 'preferred_label_en',
      'CrossEntityEmbedding.buildSkillsProfile.labelCol'
    );
  ...
  private generateRecommendations(
    industry: { code: string; nameEn: string; nameIt: string; level: string },
    companySize: number,
    structure: { org_units: Array<{ name: string; headcount: number }> },
    language: 'en' | 'it'
  ): string[] {
    ...
    if (language === 'it') {
      recommendations.push(
        `La tua azienda rientra nel settore "${industry.nameIt}" (NACE ${industry.code}).`
      );

      if (companySize <= 10) {
        recommendations.push(
          'Con meno di 10 dipendenti, consigliamo una struttura piatta con ruoli multifunzionali.',
          "Considera l'outsourcing per funzioni specialistiche come contabilità e IT."
        );
      } else if (companySize <= 50) {
        recommendations.push(
          'Con questa dimensione, è importante definire chiaramente i ruoli e le responsabilità.',
          'Considera di introdurre un livello di middle management per la supervisione operativa.'
        );
      }

      if (structure.org_units.length > 3) {
        recommendations.push(
          'Con più reparti, stabilisci canali di comunicazione chiari tra i team.'
        );
      }
    } else {
      recommendations.push(
        `Your company falls within the "${industry.nameEn}" sector (NACE ${industry.code}).`
      );

      if (companySize <= 10) {
        recommendations.push(
          'With fewer than 10 employees, we recommend a flat structure with multi-functional roles.',
          'Consider outsourcing specialized functions like accounting and IT.'
        );
      } else if (companySize <= 50) {
        recommendations.push(
          'At this size, clearly defining roles and responsibilities is important.',
          'Consider introducing middle management for operational oversight.'
        );
      }

      if (structure.org_units.length > 3) {
        recommendations.push(
          'With multiple departments, establish clear communication channels between teams.'
        );
      }
    }
  ...
  > {
    ...
    const queries: Record<EntityType, { table: string; embCol: string; levelFilter?: string }> = {
    ...
    for (const entity of entities) {
      ...
      const validatedTable = validateTableName(
        table,
        `CrossEntityEmbedding.getEmbeddingStatus(${entity})`
      );
      const validatedEmbCol = validateEmbeddingColumn(
        embCol,
        `CrossEntityEmbedding.getEmbeddingStatus(${entity})`
      );
      const result = await pool.query(`
    ...
    return status as Record<EntityType, { total: number; embedded: number; percentage: number }>;
  }
}
...
export function createCrossEntityService(
