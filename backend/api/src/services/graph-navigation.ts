...
export class GraphNavigationService {
  ...
  async getProcessDeep(processId: string): Promise<ProcessDeepResult | null> {
    const [processRes, phasesRes, rolesRes, skillsRes, kpisRes] = await Promise.all([
      this.dbClient.query(
        `SELECT id, process_code, process_name, process_category,
                value_chain_position, description, typical_inputs, typical_outputs
         FROM business_processes WHERE id = $1`,
        [processId]
      ),
      this.dbClient.query(
        `SELECT id, phase_code, phase_name, phase_order, description,
                estimated_duration_days, is_optional
         FROM process_phases WHERE process_id = $1 ORDER BY phase_order`,
        [processId]
      ),
      this.dbClient.query(
        `SELECT pr.id, pr.role_name, pr.role_type, pr.phase_id,
      ...
      this.dbClient.query(
        `SELECT psr.id, psr.phase_id, psr.proficiency_level, psr.is_mandatory,
                psr.description,
      ...
      this.dbClient.query(
        `SELECT id, kpi_code, kpi_name, phase_id, measurement_unit,
                target_direction, benchmark_value, benchmark_min, benchmark_max,
                description
         FROM process_kpis WHERE process_id = $1`,
        [processId]
      ),
    ]);

    if (processRes.rows.length === 0) return null;
    ...
    return {
      ...
      phases: phasesRes.rows.map((r) => ({
        ...
        estimatedDurationDays:
          r.estimated_duration_days != null ? parseFloat(r.estimated_duration_days) : null,
  ...
  async getProcessSkills(processId: string, tenantId: string): Promise<ProcessSkillRow[]> {
    const result = await this.dbClient.query(
      `SELECT psr.id, psr.phase_id, psr.proficiency_level, psr.is_mandatory,
       ...
       WHERE psr.process_id = $1
       ORDER BY pp.phase_order NULLS FIRST, es.preferred_label`,
      [processId, tenantId]
    );
  ...
  async getSkillProcesses(skillId: string): Promise<SkillProcessRow[]> {
    const result = await this.dbClient.query(
      `SELECT bp.id AS process_id, bp.process_code, bp.process_name,
              bp.process_category,
       ...
       ORDER BY bp.process_code, pp.phase_order NULLS FIRST`,
      [skillId]
    );
  ...
  async getEmployeeProcessQualification(
    employeeId: string
    ...
    const [employeeSkillsRes, processReqsRes] = await Promise.all([
      ...
      this.dbClient.query(
        `SELECT esk.esco_skill_id, COALESCE(esk.proficiency_level, 0) AS proficiency_level
         FROM employee_skills esk
    ...
    const employeeSkillMap = new Map<string, number>();
  ...
  async getOrgUnitCoverage(orgUnitId: string, tenantId: string): Promise<OrgUnitCoverageResult> {
    const [employeesRes, skillProfileRes, processesRes] = await Promise.all([
      ...
      this.dbClient.query(
        `SELECT es.id AS skill_id, es.preferred_label, es.skill_type,
  ...
  async getOrgUnitSkillGaps(orgUnitId: string, tenantId: string): Promise<OrgUnitSkillGap[]> {
    const result = await this.dbClient.query(
  ...
  async getIndustryProcessMap(industryCode: string): Promise<IndustryProcessMapResult> {
    const [occupationsRes, processesRes, skillsRes] = await Promise.all([
      // Q6a: Occupations linked to industry code
      this.dbClient.query(
        `SELECT eo.id, eo.preferred_label, eo.isco_code, eo.uri,
                oic.relevance_score
         ...
         ORDER BY oic.relevance_score DESC`,
        [industryCode]
      ),
      // Q6b: Processes with roles that have those occupations
      this.dbClient.query(
        `SELECT DISTINCT bp.id AS process_id, bp.process_code, bp.process_name,
                bp.process_category
         FROM process_roles pr
      ...
      this.dbClient.query(
        `SELECT es.id, es.preferred_label, es.skill_type,
  ...
  async getProcessKpiCascade(
    processId: string,
    tenantId: string
  ): Promise<KpiCascadeResult | null> {
    const [processRes, kpisRes, rolesRes, orgUnitsRes] = await Promise.all([
      ...
      this.dbClient.query(
        `SELECT pk.id, pk.kpi_code, pk.kpi_name, pk.phase_id,
         ...
         ORDER BY pp.phase_order NULLS FIRST, pk.kpi_code`,
        [processId]
      ),
      this.dbClient.query(
        `SELECT pr.id, pr.role_name, pr.role_type, pr.phase_id,
      ...
      this.dbClient.query(
        `SELECT out2.id AS template_id,
         ...
         GROUP BY out2.id, out2.name_it, out2.code, opm.responsibility_level`,
        [processId, tenantId]
      ),
    ]);
    ...
    const allRoles: KpiCascadeRole[] = rolesRes.rows.map((r) => ({
      id: r.id,
      roleName: r.role_name,
      roleType: r.role_type,
      phaseId: r.phase_id,
    ...
    const orgUnits: KpiCascadeOrgUnit[] = orgUnitsRes.rows.map((r) => ({
    ...
    const kpis: KpiCascadeItem[] = kpisRes.rows.map((r) => {
      const hasBenchmark =
        r.benchmark_value != null || (r.benchmark_min != null && r.benchmark_max != null);
      ...
      let alignmentStatus: 'aligned' | 'partial' | 'unaligned';
