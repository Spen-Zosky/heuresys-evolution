...
export class WorkforcePlanningService {
    ...
    // --------------------------------------------------------------------------
    async getSkillInventory(tenantId, options = {}) {
        let query = `
      WITH skill_data AS (
        SELECT
          esp.skill_id,
    ...
    // --------------------------------------------------------------------------
    async computeGapRisk(tenantId, requirements) {
        const assessments = [];
      ...
      `;
            ...
            const proficiencyGap = req.required_proficiency - (parseFloat(inventory.current_avg_proficiency) || 0);
            // Calculate time to target
            const targetDate = new Date(req.target_date);
            const now = new Date();
            const timeToTarget = Math.max(0, Math.ceil((targetDate.getTime() - now.getTime()) / (1000 * 60 * 60 * 24)));
            // Determine risk factors
            const riskFactors = [];
            if (gapCount > 0)
                riskFactors.push(`Need ${gapCount} more qualified employees`);
            if (proficiencyGap > 1)
                riskFactors.push(`Significant proficiency gap of ${proficiencyGap.toFixed(1)} levels`);
            if (timeToTarget < 90 && gapCount > 0)
                riskFactors.push('Tight timeline (<90 days)');
            if (inventory.current_count === 0)
                riskFactors.push('No employees with this skill');
            // Determine risk level
            let riskLevel;
            if (gapCount > req.required_count * 0.5 ||
    ...
    // --------------------------------------------------------------------------
    async generateHiringRecommendations(_tenantId, gapAssessments) {
        ...
        for (const gap of gapAssessments) {
            ...
            const multiplier = proficiencyMultiplier[profLevel] || 1;
            ...
            // Generate rationale
            const rationale = gap.current_count === 0
    ...
    // --------------------------------------------------------------------------
    async generateTrainingInvestments(tenantId, gapAssessments) {
        const investments = [];
        for (const gap of gapAssessments) {
            // Only recommend training if there's a proficiency gap and we have employees
            if (gap.proficiency_gap <= 0 || gap.current_count === 0)
                continue;
      ...
      `;
            ...
            const avgCourseHours = courses.length > 0
                ? courses.reduce((sum, c) => sum + c.duration_hours, 0) / courses.length
                : 20;
            const avgCourseCost = courses.length > 0
    ...
    // --------------------------------------------------------------------------
    async createWorkforcePlan(tenantId, planData) {
    ...
    // Get Workforce Plans
    // --------------------------------------------------------------------------
    async getWorkforcePlans(tenantId, options = {}) {
        let query = `
      SELECT
    ...
    // --------------------------------------------------------------------------
    async getWorkforcePlanById(tenantId, planId) {
    ...
    // --------------------------------------------------------------------------
    async updatePlanStatus(tenantId, planId, status) {
        const result = await this.pool.query(`
...
//# sourceMappingURL=workforce-planning.service.js.map