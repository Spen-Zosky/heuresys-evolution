...
export class EmbeddingPipelineService {
    ...
    async ensureApiKey() {
        ...
        if (!this.apiKey) {
            throw new Error('OpenAI API key not configured. Set OPENAI_API_KEY env var or system_config row.');
        }
        return this.apiKey;
    }
    // ===========================================================================
    ...
    async queueEntity(params) {
        ...
        // Insert into queue
        const result = await this.pool.query(`INSERT INTO embedding_queue (entity_type, entity_id, tenant_id, operation, priority, status)
       ...
       RETURNING id`, [entityType, entityId, tenantId]);
    ...
    async processQueue(batchSize = 50) {
        ...
        const batchResult = await this.pool.query(`UPDATE embedding_queue
       ...
       RETURNING id, entity_type, entity_id, tenant_id, operation, status, attempts, max_attempts, error_message`, [effectiveBatchSize]);
        ...
        for (const item of deleteItems) {
            try {
                await this.pool.query(`DELETE FROM semantic_entity_index WHERE entity_type = $1 AND entity_id = $2`, [item.entity_type, item.entity_id]);
        ...
        result.durationMs = Date.now() - startTime;
        logger.info(`[EmbeddingPipeline] Processed ${result.processed} items: ` +
     ...
     */
    async processUpsertBatch(items, result) {
        // Fetch text content for all items from semantic_entity_index
        const entityKeys = items.map((i) => `(${this.escapeLiteral(i.entity_type)}, ${this.escapeLiteral(i.entity_id)})`);
        const textResult = await this.pool.query(`SELECT entity_type, entity_id, tenant_id, entity_name, entity_context, metadata
     ...
     */
    async fetchWithBackoff(apiKey, input) {
        let lastError = null;
        ...
        for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
            try {
                ...
                if (response.status === 429 || response.status >= 500) {
                    ...
                    logger.warn(`[EmbeddingPipeline] Rate limited (${response.status}), ` +
                ...
                logger.warn(`[EmbeddingPipeline] Network error, retrying in ${backoff}ms ` +
     ...
     */
    async searchSimilar(queryVector, tenantId, limit = 10, entityType) {
        if (queryVector.length !== EMBEDDING_DIMENSIONS) {
            throw new Error(`Query vector must have ${EMBEDDING_DIMENSIONS} dimensions, got ${queryVector.length}`);
        }
     ...
     */
    async reindexEntity(entityType, entityId, tenantId) {
        // Fetch current text content
        const existingResult = await this.pool.query(`SELECT entity_context FROM semantic_entity_index
       WHERE tenant_id = $1 AND entity_type = $2 AND entity_id = $3`, [tenantId, entityType, entityId]);
        ...
        const text = existingResult.rows[0].entity_context;
        if (!text || !text.trim()) {
        ...
        await this.pool.query(`UPDATE semantic_entity_index
       SET embedding = $1,
           embedding_model = $2,
           embedding_generated_at = NOW(),
           updated_at = NOW()
       WHERE tenant_id = $3 AND entity_type = $4 AND entity_id = $5`, [`[${embedding.join(',')}]`, EMBEDDING_MODEL, tenantId, entityType, entityId]);
    }
    // ===========================================================================
    // STATS
    // ===========================================================================
    ...
    async markCompleted(queueId) {
        await this.pool.query(`UPDATE embedding_queue SET status = 'completed', processed_at = NOW() WHERE id = $1`, [queueId]);
    }
    async markFailed(queueId, errorMessage) {
       ...
       SET status = CASE WHEN attempts + 1 >= max_attempts THEN 'failed' ELSE 'pending' END,
           ...
           error_message = $2,
           processed_at = NOW()
       WHERE id = $1`, [queueId, errorMessage.slice(0, 1000)]);
    }
    /**
...
// =============================================================================
let instance = null;
export function createEmbeddingPipeline(pool) {
    if (!instance) {
