...
export class AnalyticsPipelineService {
   ...
   */
  async trackEvents(events: AnalyticsEvent[]): Promise<string[]> {
    const ids: string[] = [];
  ...
  private async updateRealtimeAggregations(event: AnalyticsEvent): Promise<void> {
    const now = new Date();
    const periods: AggregationPeriod[] = ['hourly', 'daily'];
   ...
   */
  async getTimeSeries(tenantId: string, query: MetricQuery): Promise<TimeSeriesPoint[]> {
   ...
   */
  async getHRDashboardMetrics(tenantId: string): Promise<Record<string, any>> {
    // Headcount
   ...
   */
  async getPerformanceDashboardMetrics(tenantId: string): Promise<Record<string, any>> {
    // Goals overview
   ...
   */
  async getRecruitmentDashboardMetrics(tenantId: string): Promise<Record<string, any>> {
    // Pipeline overview
   ...
   */
  async getLearningDashboardMetrics(tenantId: string): Promise<Record<string, any>> {
    // Enrollment overview
   ...
   */
  async getAggregationStats(tenantId: string): Promise<Record<string, any>> {
    const result = await pool.query(
      `
      SELECT
        period_type,
   ...
   */
  async getHeadcountTrend(tenantId: string): Promise<Record<string, any>[]> {
    const result = await pool.query(
      `
      WITH months AS (
   ...
   */
  async getSkillGapSummary(tenantId: string): Promise<Record<string, any>[]> {
    const result = await pool.query(
      `
   ...
   */
  async getTurnoverSummary(tenantId: string): Promise<{
    monthlyData: Array<{
      month: string;
      voluntary: number;
      involuntary: number;
