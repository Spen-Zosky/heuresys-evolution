...
export class SkillAnalyticsService {
  ...
  async getSkillTrends(
    ...
    } = { period: 'monthly', lookback_periods: 6 }
  ): Promise<SkillTrend[]> {
    const periodFormat = {
      monthly: 'YYYY-MM',
      quarterly: 'YYYY-"Q"Q',
    ...
    const truncUnit =
      options.period === 'quarterly' ? 'quarter' : options.period === 'yearly' ? 'year' : 'month';
    ...
    const params: (string | string[] | number)[] = [tenantId, totalLookback, intervalMultiplier];
    let paramIndex = 4;
