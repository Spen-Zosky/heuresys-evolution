...
export class TrainingRecommendationService {
    ...
    // --------------------------------------------------------------------------
    async getRecommendationsForEmployee(tenantId, employeeId, options = {}) {
        const { max_courses = 10, max_learning_paths = 3, preferred_delivery_methods = [], preferred_language, max_duration_hours, include_completed = false, target_skills = [] } = options;
        // Get employee info
        ...
        const courseRecommendations = await this.getCoursesForSkillGaps(tenantId, employeeId, skillGaps, {
            max_courses,
        ...
        const learningPathRecommendations = await this.getLearningPathRecommendations(tenantId, employeeId, skillGaps, max_learning_paths);
        // Calculate summary
        const summary = this.calculateSummary(skillGaps, courseRecommendations, learningPathRecommendations);
        return {
    ...
    // Get Employee Info
    // --------------------------------------------------------------------------
    async getEmployeeInfo(tenantId, employeeId) {
    ...
    // --------------------------------------------------------------------------
    async getEmployeeSkillGaps(tenantId, employeeId, targetSkills = []) {
    ...
    `;
        ...
        if (result.rows.length > 0 && result.rows[0].skill_gaps) {
            const gaps = result.rows[0].skill_gaps;
    ...
    // --------------------------------------------------------------------------
    async calculateSkillGaps(tenantId, employeeId) {
    ...
    // --------------------------------------------------------------------------
    async getCoursesForSkillGaps(tenantId, employeeId, skillGaps, options) {
    ...
    // Get Learning Path Recommendations
    // --------------------------------------------------------------------------
    async getLearningPathRecommendations(tenantId, _employeeId, // Reserved for future: exclude enrolled paths
    skillGaps, maxPaths) {
        if (skillGaps.length === 0) {
            return [];
        }
    ...
    // --------------------------------------------------------------------------
    async searchCoursesBySkillEmbedding(tenantId, skillQuery, limit = 10) {
    ...
    // --------------------------------------------------------------------------
    async getCourseCompletionRates(tenantId, courseIds) {
        const result = await this.pool.query(`
      SELECT
        c.id as course_id,
    ...
    // --------------------------------------------------------------------------
    async getEmployeePreferences(_tenantId, employeeId) {
    ...
    // Helper Methods
    // --------------------------------------------------------------------------
    calculatePriority(weightedScore, skillsCovered) {
    ...
    generateReason(skillsCovered, skillCoverage) {
        if (skillsCovered === 0)
            return 'General skill development';
        if (skillsCovered === 1)
            return `Addresses ${skillCoverage[0]?.skill_name || 'skill'} gap`;
        return `Covers ${skillsCovered} skill gaps`;
    }
    calculateSummary(skillGaps, courseRecommendations, learningPathRecommendations) {
        const totalHours = courseRecommendations.reduce((sum, r) => sum + (parseFloat(String(r.course.duration_hours)) || 0), 0);
...
//# sourceMappingURL=training-recommendation.service.js.map