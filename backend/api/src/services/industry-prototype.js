...
// SIZE CLASS MAPPINGS
// =============================================================================
function getSizeClassFromCount(employeeCount) {
    if (employeeCount < 10)
        return 'micro';
    if (employeeCount < 50)
        return 'small';
...
function getTypicalSpanOfControl(sizeClass) {
    switch (sizeClass) {
        case 'micro':
            return [3, 3, 3, 3, 3, 2];
        case 'small':
...
export class IndustryPrototypeService {
    ...
    async findPrototypeInDB(naceCode, sizeClass) {
        if (!naceCode)
            return null;
    ...
    generateDefaultPrototype(context, sizeClass) {
    ...
    getTypicalDepartments(sizeClass, naceSection) {
     ...
     */
    async getRelevantESCOOccupations(naceSection, limit = 50) {
     ...
     */
    async matchJobTitleToESCO(jobTitle) {
        const result = await pool.query(`
      SELECT
        code,
        preferred_label,
     ...
     */
    async getTemplatesForIndustry(naceSection, sizeClass) {
        const result = await pool.query(`
      SELECT
        t.id,
        t.name,
...
// FACTORY FUNCTION
// =============================================================================
export function createIndustryPrototypeService(tenantId) {
    return new IndustryPrototypeService(tenantId);
}
export default IndustryPrototypeService;
//# sourceMappingURL=industry-prototype.js.map