...
// HELPER FUNCTIONS
// =============================================================================
function calculateKsabaGap(current, required) {
    return {
...
function calculateGapSeverity(gapScore, importance) {
    // Negative gap = employee exceeds requirement
    if (gapScore <= 0)
        return 'none';
    // Adjust thresholds based on importance
    const importanceMultiplier = importance === 'essential' ? 1.5 :
...
// =============================================================================
export async function analyzeEmployeeVsRole(employeeId, roleId, tenantId) {
  ...
  `, [employeeId]);
    ...
    gaps.sort((a, b) => {
        ...
        const importanceOrder = { essential: 0, important: 1, nice_to_have: 2, developmental: 3 };
...
// =============================================================================
export async function analyzeTeamVsRole(employeeIds, roleId, aggregation = 'average', tenantId) {
    // Run individual analyses
    const individualResults = await Promise.all(employeeIds.map(empId => analyzeEmployeeVsRole(empId, roleId, tenantId)));
