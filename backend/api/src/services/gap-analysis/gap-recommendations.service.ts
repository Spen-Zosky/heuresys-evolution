...
import { SkillGap, GapAnalysisResult } from './gap-analysis.service.js';
...
export interface Recommendation {
  ...
  // Expected outcome
  expectedGapReduction: number; // 0-100%
  expectedLevelGain: number; // Expected composite score increase
...
function mapGapToPriority(severity: string): 'critical' | 'high' | 'medium' | 'low' {
  switch (severity) {
    case 'critical':
      return 'critical';
    case 'high':
...
function estimateEffortLevel(gapScore: number): 'minimal' | 'moderate' | 'significant' | 'major' {
  if (gapScore < 0.5) return 'minimal';
  if (gapScore < 1.0) return 'moderate';
  if (gapScore < 2.0) return 'significant';
  return 'major';
}

function estimateHoursForGap(gapScore: number, dimension: string): number {
...
async function findTrainingForSkill(
  skillId: string,
  skillName: string,
  gapScore: number,
  tenantId?: string
  ...
  try {
    ...
    for (const course of result.rows) {
      ...
      if (course.similarity < 0.5 && result.rows.indexOf(course) > 0) continue;
...
async function findMentorsForSkill(
  skillId: string,
  skillName: string,
  gapScore: number,
  employeeId: string,
...
function generateSelfStudyRecommendations(
  skillId: string,
  skillName: string,
  gap: SkillGap
): Recommendation[] {
...
export async function generateRecommendations(
  ...
  const skillRecommendations: RecommendationsResult['skillRecommendations'] = [];
  ...
  const eligibleGaps = gapAnalysis.gaps.filter(
    (g) => severityOrder.indexOf(g.severity) >= minSeverityIndex && g.gapScore > 0
  );

  // Generate recommendations for each gap
  for (const gap of eligibleGaps) {
    const skillRecs: Recommendation[] = [];

    // Training recommendations
    if (includeTypes.includes('training') || includeTypes.includes('certification')) {
      const trainingRecs = await findTrainingForSkill(
        gap.skillId,
        gap.skillName,
        gap.gapScore,
        tenantId
    ...
    if (includeTypes.includes('mentoring') && gapAnalysis.targetType === 'employee') {
      const mentoringRecs = await findMentorsForSkill(
        gap.skillId,
        gap.skillName,
        gap.gapScore,
        gapAnalysis.targetId,
  ...
  const totalEstimatedHours = allRecommendations.reduce(
