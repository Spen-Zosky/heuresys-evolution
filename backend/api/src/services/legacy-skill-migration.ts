...
export class LegacySkillMigrationService {
  ...
  async createMigrationJob(
    tenantId: string,
    jobType: MigrationJob['job_type'] = 'all'
   ...
   */
  async countUnmappedRecords(
    tenantId: string,
    jobType: MigrationJob['job_type']
    ...
    if (jobType === 'all' || jobType === 'unknown_skills') {
      const res = await this.pool.query(
        `SELECT COUNT(*) FROM unknown_skills
  ...
  async executeMigrationJob(
    ...
    const jobResult = await this.pool.query(
      `UPDATE skill_migration_jobs SET status = 'processing', started_at = NOW()
    ...
    try {
      ...
      if (job.job_type === 'all' || job.job_type === 'employee_skills') {
        const empStats = await this.migrateEmployeeSkills(
          job.tenant_id,
          confidenceThreshold,
          batchSize,
          createCustomSkills
      ...
      if (job.job_type === 'all' || job.job_type === 'extracted_skills') {
        const extStats = await this.migrateExtractedSkills(
          job.tenant_id,
          confidenceThreshold,
          batchSize
        );
      ...
      if (job.job_type === 'all' || job.job_type === 'unknown_skills') {
        const unkStats = await this.migrateUnknownSkills(
          job.tenant_id,
          confidenceThreshold,
          batchSize
        );
      ...
      await this.pool.query(
        `UPDATE skill_migration_jobs SET
         status = 'completed',
         completed_at = NOW(),
         processed_records = $2,
         matched_records = $3,
         failed_records = $4
         WHERE id = $1`,
        [jobId, stats.processed, stats.matched, stats.failed]
      );
    } catch (error) {
   ...
   */
  private async migrateEmployeeSkills(
    tenantId: string,
    confidenceThreshold: number,
    batchSize: number,
    createCustomSkills: boolean
    ...
    while (hasMore) {
      const batch = await this.pool.query(
        `SELECT id, custom_skill_name, employee_id
          ...
          if (match && match.confidence >= confidenceThreshold) {
            ...
            await this.pool.query(
              `UPDATE employee_skills SET
               ...
               updated_at = NOW()
               WHERE id = $3`,
              [match.esco_skill_id, match.confidence, row.id]
            );
            stats.matched++;
          } else if (createCustomSkills && match) {
            // Create as tenant custom skill if below threshold but has a match
            const customSkillId = await this.createTenantCustomSkill(
              tenantId,
              row.custom_skill_name,
              match.esco_skill_id,
              match.confidence
            );
            ...
            await this.pool.query(
              `UPDATE employee_skills SET
               ...
               updated_at = NOW()
               WHERE id = $3`,
              [match.confidence, customSkillId, row.id]
            );
   ...
   */
  private async migrateExtractedSkills(
    tenantId: string,
    confidenceThreshold: number,
    batchSize: number
  ): Promise<MigrationStats> {
   ...
   */
  private async migrateUnknownSkills(
    tenantId: string,
    confidenceThreshold: number,
    batchSize: number
  ): Promise<MigrationStats> {
      ...
      for (const row of batch.rows) {
        try {
          ...
          if (match) {
            ...
            await this.pool.query(
              `UPDATE unknown_skills SET
               suggested_esco_id = $1,
               suggested_confidence = $2,
               review_status = CASE WHEN $2 >= $4 THEN 'suggested' ELSE 'low_confidence' END,
               last_seen_at = NOW()
               WHERE id = $3`,
              [match.esco_skill_id, match.confidence, row.id, confidenceThreshold]
            );
  ...
  private async findBestEscoMatch(
    skillText: string
  ): Promise<{ esco_skill_id: string; skill_name: string; confidence: number } | null> {
   ...
   */
  private async findBestEscoMatchByText(
    skillText: string
  ): Promise<{ esco_skill_id: string; skill_name: string; confidence: number } | null> {
    ...
    let result = await this.pool.query(
      `SELECT id, preferred_label_en, 1.0 as similarity
       FROM esco_skills
   ...
   */
  private async createTenantCustomSkill(
    tenantId: string,
    skillName: string,
    nearestEscoId: string,
    similarity: number
  ): Promise<string> {
  ...
  async listJobs(
    ...
    const countResult = await this.pool.query(
      `SELECT COUNT(*) FROM skill_migration_jobs WHERE tenant_id = $1`,
      [tenantId]
    );

    const result = await this.pool.query(
      `SELECT * FROM skill_migration_jobs
       WHERE tenant_id = $1
   ...
   */
  async approveUnknownSkillMapping(
    skillId: string,
    approvedBy: string,
    usesuggested: boolean = true,
    overrideEscoId?: string
  ): Promise<void> {
    const escoId =
      overrideEscoId ||
      (usesuggested
   ...
   */
  async rejectUnknownSkillMapping(skillId: string, reviewedBy: string): Promise<void> {
    await this.pool.query(
      `UPDATE unknown_skills SET
   ...
   */
  async getMigrationSummary(tenantId: string): Promise<{
    employee_skills: { total: number; mapped: number; unmapped: number };
    extracted_skills: { total: number; mapped: number; unmapped: number };
    unknown_skills: { total: number; approved: number; pending: number; rejected: number };
    ...
    const [empSkills, extSkills, unkSkills, customSkills] = await Promise.all([
      ...
      this.pool.query(
        `SELECT
          ...
          COUNT(esco_skill_id) as mapped,
          COUNT(*) - COUNT(esco_skill_id) as unmapped
         FROM extracted_skills WHERE tenant_id = $1`,
        [tenantId]
      ),
      this.pool.query(
        `SELECT
         ...
         FROM unknown_skills WHERE tenant_id = $1`,
        [tenantId]
      ),
  ...
  // Helper methods
  private rowToJob(row: Record<string, unknown>): MigrationJob {
    return {
      id: row.id as string,
      tenant_id: row.tenant_id as string,
      job_type: row.job_type as MigrationJob['job_type'],
      ...
      matched_records: row.matched_records as number,
  ...
  private mergeStats(target: MigrationStats, source: MigrationStats): void {
