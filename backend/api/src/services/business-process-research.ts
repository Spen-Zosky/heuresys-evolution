...
import { AIOrchestrator, ChatMessage } from './ai-orchestrator.js';
...
// =============================================================================
// PROCESS LAYER TYPES (Migration 155)
// =============================================================================
...
export const VALUE_CHAIN_DESCRIPTIONS: Record<number, { name: string; description: string }> = {
...
const INDUSTRY_PROCESS_TEMPLATES: Record<string, BusinessProcess[]> = {
  K: [
    // Financial services
    {
      processCode: 'BP-K-01',
...
export class BusinessProcessResearchService {
   ...
   */
  async researchIndustryProcesses(
    naceCode: string,
    companySize: string
  ...
  private async getProcessesFromDatabase(naceCode: string): Promise<BusinessProcess[]> {
    const naceSection = naceCode.match(/^([A-Z])/i)?.[1]?.toUpperCase();
  ...
  private async buildResultFromDatabase(
    naceCode: string,
    companySize: string,
    processes: BusinessProcess[]
  ): Promise<ProcessResearchResult> {
  ...
  private async getCostCentersFromDatabase(naceCode: string): Promise<CostCenter[]> {
    const naceSection = naceCode.match(/^([A-Z])/i)?.[1]?.toUpperCase();
    const naceDivision = naceCode.match(/([0-9]{2})/)?.[1];
  ...
  private async buildResultFromTemplate(
    naceCode: string,
    companySize: string,
    naceSection: string
  ): Promise<ProcessResearchResult> {
  ...
  private generateCostCentersFromProcesses(processes: BusinessProcess[]): CostCenter[] {
  ...
  private async generateProcessesWithAI(
    naceCode: string,
    companySize: string
  ): Promise<ProcessResearchResult> {
    const industryName = await this.getIndustryName(naceCode);
...
Generate 15-20 business processes covering all value chain activities.`;
    ...
    const messages: ChatMessage[] = [{ role: 'user', content: processPrompt }];
    ...
    try {
      ...
      const processes: BusinessProcess[] = parsed.processes || [];
  ...
  mapToValueChain(processes: BusinessProcess[]): ValueChainMapping {
    ...
    return {
      primaryActivities: primaryActivities.sort(
        (a, b) => a.valueChainPosition - b.valueChainPosition
      ),
      supportActivities: supportActivities.sort(
        (a, b) => a.valueChainPosition - b.valueChainPosition
      ),
  ...
  async getCachedResearch(naceCode: string, companySize: string): Promise<CachedResearch | null> {
    const naceSection = naceCode.match(/^([A-Z])/i)?.[1]?.toUpperCase();
    const naceDivision = naceCode.match(/([0-9]{2})/)?.[1];
  ...
  private async getIndustryName(naceCode: string): Promise<string> {
    const naceSection = naceCode.match(/^([A-Z])/i)?.[1]?.toUpperCase();
    const naceDivision = naceCode.match(/([0-9]{2})/)?.[1];
  ...
  identifyCostCenters(process: BusinessProcess): CostCenter[] {
    ...
    const positionToCostType: Record<number, 'direct' | 'indirect' | 'overhead'> = {
      1: 'direct',
      2: 'direct',
      3: 'direct',
      4: 'indirect',
...
export function createBusinessProcessResearchService(
  tenantId: string
): BusinessProcessResearchService {
