...
export class HRIntelligenceService {
  ...
  async searchSkills(
  ...
  ): Promise<{ skills: ESCOSkill[]; total: number }> {
    ...
    const countResult = await pool.query(
      `SELECT COUNT(*) as total FROM esco_skills ${whereClause}`,
      values
    );

    values.push(limit, offset);
    const result = await pool.query(
      `SELECT * FROM esco_skills ${whereClause}
   ...
   */
  async getSkill(idOrUri: string): Promise<ESCOSkill | null> {
    const result = await pool.query(
      `SELECT * FROM esco_skills WHERE id::text = $1 OR esco_uri = $1`,
      [idOrUri]
    );
   ...
   */
  async getSkillHierarchy(skillUri: string): Promise<{
  ...
  async searchOccupations(
  ...
  ): Promise<{ occupations: ESCOOccupation[]; total: number }> {
    const { limit = 20, offset = 0 } = options;

    const countResult = await pool.query(
      `SELECT COUNT(*) as total FROM esco_occupations
    ...
    const result = await pool.query(
      `SELECT * FROM esco_occupations
       ...
       ORDER BY preferred_label
       LIMIT $2 OFFSET $3`,
      [`%${escapeILIKE(query)}%`, limit, offset]
    );
   ...
   */
  async getOccupationSkills(occupationUri: string): Promise<{
  ...
  async getEmployeeSkills(
    employeeId: string,
    options: { includeESCODetails?: boolean } = {}
  ...
  async updateEmployeeSkill(
    ...
    await pool.query(
      `UPDATE employee_skills SET ${setClauses.join(', ')}
   ...
   */
  async verifyEmployeeSkill(skillId: string, verifiedBy: string): Promise<void> {
    await pool.query(
      `UPDATE employee_skills
       SET is_verified = true, verified_by = $1, verified_at = NOW(), updated_at = NOW()
       WHERE id = $2 AND tenant_id = $3`,
      [verifiedBy, skillId, this.tenantId]
    );
  }
   ...
   */
  async deleteEmployeeSkill(skillId: string): Promise<void> {
   ...
   */
  async getJobMarketSources(): Promise<Record<string, unknown>[]> {
  ...
  async searchJobPostings(options: {
    ...
    limit?: number;
    offset?: number;
  }): Promise<{ postings: Record<string, unknown>[]; total: number }> {
    ...
    const countResult = await pool.query(
      `SELECT COUNT(*) as total FROM job_market_postings ${whereClause}`,
      values
    );
    ...
    const result = await pool.query(
      `SELECT * FROM job_market_postings ${whereClause}
  ...
  async getJobMarketStatistics(options: {
    ...
    const result = await pool.query(
      `SELECT * FROM job_market_statistics
  ...
  async getTrendingSkills(options: {
    ...
    const previousMap = new Map(
      previousResult.rows.map((r) => [r.skill_uri, parseInt(r.previous_count)])
    );

    // Get skill names and salary data
    const skillUris = currentResult.rows.map((r) => r.skill_uri);
    const skillsResult = await pool.query(
      `SELECT esco_uri, preferred_label FROM esco_skills WHERE esco_uri = ANY($1)`,
      [skillUris]
    );
    ...
    return currentResult.rows.map((row) => {
      ...
      return {
        ...
        demandScore: Math.min(
          100,
          Math.round((currentCount / (currentResult.rows[0]?.current_count || 1)) * 100)
        ),
  ...
  async extractSkills(
    text: string,
    options: { source?: string; jobType?: string } = {}
    ...
    const jobResult = await pool.query(
      ...
      RETURNING id`,
      [this.tenantId, options.jobType || 'document', text]
    );
    ...
    try {
      ...
      // Map to ESCO
      const mappedSkills: MappedSkill[] = [];
      ...
      // Update job
      await pool.query(
        `UPDATE skill_extraction_jobs SET
          status = 'completed',
          extracted_skills = $1,
          mapped_skills = $2,
          unmapped_skills = $3,
      ...
      await pool.query(
        `UPDATE skill_extraction_jobs SET status = 'failed', error_message = $1 WHERE id = $2`,
   ...
   */
  private async findBestESCOMatch(
    skillText: string
  ): Promise<{ uri: string; label: string; confidence: number } | null> {
    // Check aliases first
    const aliasResult = await pool.query(
      `SELECT sa.esco_skill_id, es.uri, es.preferred_label, sa.confidence_score
  ...
  async addSkillAlias(
    escoSkillId: string,
    aliasText: string,
    aliasType: string = 'synonym'
  ): Promise<string> {
    const result = await pool.query(
      `INSERT INTO skill_aliases (esco_skill_id, alias_text, alias_type, source)
       VALUES ($1, $2, $3, 'manual')
       RETURNING id`,
      [escoSkillId, aliasText, aliasType]
    );
    return result.rows[0].id;
  }
  ...
  async createSkillGapAnalysis(
    ...
    const reqResult = await pool.query(
      `SELECT psr.*, es.preferred_label as skill_name
       FROM position_skill_requirements psr
       LEFT JOIN esco_skills es ON psr.esco_skill_id = es.id
       WHERE psr.position_id = $1 AND psr.tenant_id = $2`,
      [targetPositionId, this.tenantId]
    );
    ...
    for (const req of requirements) {
      ...
      const employeeSkill = employeeSkills.find(
        (es) => es.escoSkillId === req.esco_skill_id || es.customSkillName === req.custom_skill_name
      );
    ...
    const coverageScore =
      requirements.length > 0 ? (coveredCount / requirements.length) * 100 : 100;
    const proficiencyScore = coveredCount > 0 ? (proficiencySum / coveredCount) * 100 : 0;

    // Get position name
    const posResult = await pool.query(
      `SELECT position_name FROM position_skill_requirements WHERE position_id = $1 LIMIT 1`,
      [targetPositionId]
    );
   ...
   */
  private generateRecommendations(gaps: SkillGap[]): string[] {
    const recommendations: string[] = [];
    const highPriorityGaps = gaps.filter((g) => g.priority === 'high');

    if (highPriorityGaps.length > 0) {
      recommendations.push(
        `Focus immediato su ${highPriorityGaps.length} competenze critiche: ${highPriorityGaps.map((g) => g.skillName).join(', ')}`
      );
    }
   ...
   */
  async getSkillGapAnalysis(analysisId: string): Promise<SkillGapAnalysis | null> {
    const result = await pool.query(
      `SELECT * FROM skill_gap_analyses WHERE id = $1 AND tenant_id = $2`,
      [analysisId, this.tenantId]
    );
  ...
  async listSkillGapAnalyses(options: {
    ...
    limit?: number;
    offset?: number;
  }): Promise<{ analyses: Record<string, unknown>[]; total: number }> {
    ...
    const countResult = await pool.query(
      `SELECT COUNT(*) as total FROM skill_gap_analyses ${whereClause}`,
      values
    );

    values.push(options.limit || 20, options.offset || 0);
    const result = await pool.query(
      `SELECT id, analysis_name, analysis_type, target_entity_type, target_entity_id,
  ...
  async generateSkillMatrix(
    ...
    const skillCoverage: Record<
      string,
      { count: number; totalProficiency: number; maxProficiency: number }
    > = {};
    ...
    for (const emp of employees) {
      ...
      for (const skill of skills) {
        ...
        skillCoverage[skillKey].maxProficiency = Math.max(
          skillCoverage[skillKey].maxProficiency,
    ...
    const coverageData: Record<
      string,
      { count: number; avgProficiency: number; maxProficiency: number }
    > = {};
   ...
   */
  async getBenchmarkConfigs(): Promise<Record<string, unknown>[]> {
    const result = await pool.query(
   ...
   */
  async createBenchmarkReport(
    configId: string | null,
    reportName: string,
    reportType: 'salary_benchmark' | 'skill_demand' | 'talent_availability'
  ): Promise<string> {
   ...
   */
  private async generateSalaryBenchmark(
    filters: Record<string, unknown>
  ): Promise<Record<string, unknown>> {
    const conditions: string[] = ['is_active = true', 'salary_min IS NOT NULL'];
    const values: unknown[] = [];
    let paramIndex = 1;

    if (filters.countries && (filters.countries as string[]).length > 0) {
      conditions.push(`country_code = ANY($${paramIndex})`);
      values.push(filters.countries);
      paramIndex++;
    }

    if (filters.industries && (filters.industries as string[]).length > 0) {
   ...
   */
  private async generateSkillDemandReport(
    ...
    const countries = _filters.countries as string[] | undefined;
    const industries = _filters.industries as string[] | undefined;
    if (countries?.[0]) trendingOptions.countryCode = countries[0];
    if (industries?.[0]) trendingOptions.industry = industries[0];
   ...
   */
  private async generateTalentAvailabilityReport(
    _filters: Record<string, unknown>
  ): Promise<Record<string, unknown>> {
    ...
    const talentSupply: Record<string, unknown> = {};
   ...
   */
  async getBenchmarkReport(reportId: string): Promise<Record<string, unknown> | null> {
    const result = await pool.query(
      `SELECT * FROM benchmark_reports WHERE id = $1 AND tenant_id = $2`,
      [reportId, this.tenantId]
    );
  ...
  async listBenchmarkReports(options: {
    ...
    const countResult = await pool.query(
      `SELECT COUNT(*) as total FROM benchmark_reports ${whereClause}`,
      values
    );

    values.push(options.limit || 20, options.offset || 0);
    const result = await pool.query(
      `SELECT id, config_id, report_name, report_type, report_date, created_at
  ...
  private mapESCOSkill(row: Record<string, unknown>): ESCOSkill {
    return {
      id: row.id as string,
      uri: row.esco_uri as string,
      preferredLabel: row.preferred_label as string,
      altLabels: (row.alt_labels as string[]) || [],
      description: row.description as string,
      skillType: row.skill_type as string,
      reuseLevel: row.reuse_level as string,
      isDigital: false, // Not in current schema
      isGreen: false, // Not in current schema
      broaderUri: ((row.broader_skills as string[]) || [])[0] || null,
      narrowerUris: (row.narrower_skills as string[]) || [],
      relatedUris: [], // Not directly in current schema
      iscoGroups: (row.isco_groups as string[]) || [],
    };
  }

  private mapESCOOccupation(row: Record<string, unknown>): ESCOOccupation {
    return {
      id: row.id as string,
      uri: row.esco_uri as string,
      preferredLabel: row.preferred_label as string,
      altLabels: (row.alt_labels as string[]) || [],
      description: row.description as string,
      iscoGroup: row.isco_group as string,
