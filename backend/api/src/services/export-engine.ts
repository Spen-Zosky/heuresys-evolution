...
import PDFGenerator, { type TableColumn } from './pdf-generator.js';
...
export class ExportEngineService {
  ...
  async exportBlueprintReport(runId: string, format: ExportFormat): Promise<Buffer> {
    const runResult = await this.dbClient.query(
      `SELECT br.*, bt.template_name
    ...
    const resultsResult = await this.dbClient.query(
      `SELECT id, result_type, entity_type, severity, title, description,
    ...
    const summaryResult = await this.dbClient.query(
  ...
  async exportSkillGapReport(orgUnitId: string, format: ExportFormat): Promise<Buffer> {
    const orgResult = await this.dbClient.query(
      `SELECT id, name, code, org_type FROM org_units WHERE id = $1`,
      [orgUnitId]
    );
    ...
    const gapResult = await this.dbClient.query(
      `SELECT
       ...
       ORDER BY e.last_name, e.first_name, esp.composite_score DESC NULLS LAST`,
      [orgUnitId]
    );
  ...
  async exportOrgChart(format: ExportFormat): Promise<Buffer> {
  ...
  private async orgChartToXlsx(rows: Record<string, any>[]): Promise<Buffer> {
    const wb = new ExcelJS.Workbook();
    wb.creator = 'Heuresys Platform';
    wb.created = new Date();
  ...
  private orgChartToPdf(rows: Record<string, any>[]): Promise<Buffer> {
    const pdf = new PDFGenerator({
      title: 'Organigramma',
  ...
  private async skillInventoryToXlsx(
    rows: Record<string, any>[],
    filters: SkillInventoryFilters
  ): Promise<Buffer> {
    const wb = new ExcelJS.Workbook();
    ...
    const filterParts: string[] = ['Skill Inventory'];
    if (filters.orgUnitId) filterParts.push(`Org Unit: ${filters.orgUnitId}`);
    if (filters.verificationStatus) filterParts.push(`Verifica: ${filters.verificationStatus}`);
    if (filters.minCompositeScore != null)
      filterParts.push(`Score min: ${filters.minCompositeScore}`);
    if (filters.skillType) filterParts.push(`Tipo: ${filters.skillType}`);
  ...
  private skillInventoryToPdf(
    ...
    const activeFilters: Array<{ label: string; value: string | number }> = [
      { label: 'Totale righe', value: rows.length },
    ];
    if (filters.orgUnitId) activeFilters.push({ label: 'Org Unit ID', value: filters.orgUnitId });
    if (filters.verificationStatus)
    ...
    if (filters.skillType) activeFilters.push({ label: 'Tipo skill', value: filters.skillType });
