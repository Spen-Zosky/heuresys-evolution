...
const MATCH_KEYS: Record<ImportType, string[]> = {
...
const TARGET_TABLES: Record<ImportType, string> = {
  employees: 'employees',
  org_units: 'org_units',
  roles: 'rbp_roles',
...
const EXCLUDED_FROM_UPDATE: string[] = ['id', 'tenant_id', 'created_at', 'created_by'];
...
export class ImportEngineService {
  ...
  private parseCsv(buffer: Buffer): ParsedData {
  ...
  private async parseExcel(buffer: Buffer | Uint8Array): Promise<ParsedData> {
    ...
    headerRow.eachCell((cell, colNumber) => {
      const val =
        cell.value?.toString().trim().toLowerCase().replace(/\s+/g, '_') || `col_${colNumber}`;
      headers.push(val);
    });
      ...
      headers.forEach((header, idx) => {
        ...
        if (cell.value !== null && cell.value !== undefined) {
          if (cell.value instanceof Date) {
            ...
            value = (cell.value as { result: unknown }).result?.toString() ?? null;
          } else {
  ...
  async validateData(
    ...
    if (mappedDbCols.has('_lookup_org_unit')) {
      const result = await this.dbClient.query(
        'SELECT id, code FROM org_units WHERE tenant_id = $1 AND code IS NOT NULL',
        [tenantId]
      );
      ...
      for (const entity of mapped.entities) {
        try {
          ...
          const updateCols = columns.filter(
            (col) => !matchKeyFields.includes(col) && !EXCLUDED_FROM_UPDATE.includes(col)
          );
