...
export class SkillRelationshipService {
  ...
  async getRelationships(
    ...
    let query: string;
    const params: string[] = [escoSkillId];
   ...
   */
  async getPrerequisites(escoSkillId: string): Promise<SkillRelationship[]> {
    const result = await pool.query(`
      SELECT
        sr.*,
   ...
   */
  async getComplementarySkills(escoSkillId: string): Promise<SkillRelationship[]> {
    const result = await pool.query(`
      SELECT
        sr.*,
        es.preferred_label as source_skill_label,
   ...
   */
  async getSubstitutionSkills(escoSkillId: string): Promise<SkillRelationship[]> {
    const result = await pool.query(`
      SELECT
        sr.*,
        es.preferred_label as source_skill_label,
   ...
   */
  async getBuildsOnSkills(escoSkillId: string): Promise<SkillRelationship[]> {
    const result = await pool.query(`
      SELECT
        sr.*,
        es.preferred_label as source_skill_label,
   ...
   */
  async getEnabledSkills(escoSkillId: string): Promise<SkillRelationship[]> {
    const result = await pool.query(`
      SELECT
        sr.*,
        es.preferred_label as source_skill_label,
  ...
  async validateRelationship(
    ...
    validatedBy: string,
    updates?: Partial<SkillRelationship>
  ): Promise<SkillRelationship> {
    const result = await pool.query(`
  ...
  async getAdjacencies(
    ...
    const conditions: string[] = ['(sa.skill_id = $1 OR sa.adjacent_skill_id = $1)'];
    const params: (string | number)[] = [escoSkillId];
    let paramIndex = 2;
   ...
   */
  async calculateJobPostingAdjacencies(escoSkillId: string): Promise<number> {
   ...
   */
  async calculateEmployeeAdjacencies(escoSkillId: string): Promise<number> {
    const result = await pool.query(`
      INSERT INTO skill_adjacencies (
        skill_id, adjacent_skill_id, adjacency_score, adjacency_type,
   ...
   */
  async getSkillGraph(escoSkillId: string, depth: number = 2): Promise<SkillGraph> {
  ...
  async getCareerPathSkills(
    ...
    const path: SkillPathNode[] = result.rows.map((row, idx) => ({
      skill_id: row.skill_id,
      preferred_label: row.preferred_label,
      primary_category: row.primary_category,
      cognitive_level: row.cognitive_level,
      distance: idx,
      path_type: 'builds_on' as const
    }));
   ...
   */
  async getSkillRelationshipSummary(escoSkillId: string): Promise<{
    skill_id: string;
    skill_label: string;
