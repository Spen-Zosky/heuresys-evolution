...
export class DashboardWidgetService {
    ...
    async createDashboard(input) {
        ...
        if (input.is_default) {
            await pool.query(`UPDATE dashboards SET is_default = false WHERE tenant_id = $1 AND created_by = $2`, [input.tenant_id, input.created_by]);
        }
        const query = `
     ...
     * Get dashboard by ID
     */
    async getDashboard(tenantId, dashboardId, userId) {
        const query = `
      SELECT d.*,
     ...
     */
    async listDashboards(tenantId, userId, options) {
     ...
     * Update dashboard
     */
    async updateDashboard(tenantId, dashboardId, userId, updates) {
        // Verify ownership
        const existing = await pool.query('SELECT id FROM dashboards WHERE id = $1 AND tenant_id = $2 AND created_by = $3', [dashboardId, tenantId, userId]);
        if (existing.rows.length === 0) {
            return null;
        }
        // If setting as default, unset others
        if (updates.is_default) {
            await pool.query(`UPDATE dashboards SET is_default = false WHERE tenant_id = $1 AND created_by = $2 AND id != $3`, [tenantId, userId, dashboardId]);
        }
        const setClauses = [];
        const params = [dashboardId, tenantId];
        let paramIndex = 3;
        const allowedFields = [
            'name',
            'description',
     ...
     * Delete dashboard
     */
    async deleteDashboard(tenantId, dashboardId, userId) {
        // Delete widgets first
        await pool.query('DELETE FROM dashboard_widgets WHERE dashboard_id = $1 AND tenant_id = $2', [
            dashboardId,
            tenantId,
        ]);
        const result = await pool.query('DELETE FROM dashboards WHERE id = $1 AND tenant_id = $2 AND created_by = $3 RETURNING id', [dashboardId, tenantId, userId]);
        return (result.rowCount ?? 0) > 0;
    }
    /**
     * Duplicate dashboard
     */
    async duplicateDashboard(tenantId, dashboardId, userId, newName) {
     ...
     */
    async getWidgets(tenantId, dashboardId) {
        const query = `
     ...
     * Update widget
     */
    async updateWidget(tenantId, widgetId, updates) {
        const setClauses = [];
     ...
     */
    async updateWidgetPositions(tenantId, dashboardId, positions) {
        const client = await pool.connect();
        try {
            await client.query('BEGIN');
            for (const item of positions) {
                await client.query(`UPDATE dashboard_widgets SET position = $1, updated_at = NOW()
           WHERE id = $2 AND dashboard_id = $3 AND tenant_id = $4`, [JSON.stringify(item.position), item.widget_id, dashboardId, tenantId]);
            }
    ...
    async deleteWidget(tenantId, widgetId) {
        const result = await pool.query('DELETE FROM dashboard_widgets WHERE id = $1 AND tenant_id = $2 RETURNING id', [widgetId, tenantId]);
        return (result.rowCount ?? 0) > 0;
    }
    // ==========================================================================
    // Widget Templates
     ...
     */
    async getWidgetTemplates(tenantId, options) {
        const conditions = [];
        const params = [];
        let paramIndex = 1;
     ...
     */
    async createWidgetTemplate(tenantId, input) {
        const id = uuidv4();
        const query = `
     ...
     */
    async fetchWidgetData(tenantId, widget) {
     ...
     */
    async fetchReportData(tenantId, reportId) {
        try {
     ...
     */
    async fetchQueryData(tenantId, queryConfig) {
        if (!queryConfig) {
            return { data: [] };
        }
        ...
        const fields = queryConfig.fields.map((f) => {
            return validateIdentifier(f.replace(/[^a-zA-Z0-9_]/g, ''), 'DashboardWidget.fetchQueryData.fields');
        });
        let selectClause;
        if (queryConfig.aggregate && queryConfig.aggregate.length > 0) {
            const aggFields = queryConfig.aggregate.map((a) => {
                const field = validateIdentifier(a.field.replace(/[^a-zA-Z0-9_]/g, ''), 'DashboardWidget.fetchQueryData.aggregate.field');
            ...
            const groupFields = queryConfig.group_by?.map((f) => validateIdentifier(f.replace(/[^a-zA-Z0-9_]/g, ''), 'DashboardWidget.fetchQueryData.group_by')) || [];
        ...
        if (queryConfig.group_by && queryConfig.group_by.length > 0) {
            const groupBy = queryConfig.group_by
                .map((f) => validateIdentifier(f.replace(/[^a-zA-Z0-9_]/g, ''), 'DashboardWidget.fetchQueryData.group_by'))
                .join(', ');
     ...
     * Get popular widgets
     */
    async getPopularWidgets(tenantId, limit = 10) {
        const query = `
      SELECT
        template_id,
