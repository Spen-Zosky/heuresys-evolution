...
export class ComparisonReportService {
     ...
     */
    async generateComparison(sessionId) {
    ...
    `, [sessionId, this.tenantId]);
        ...
        // Calculate summary
        const summary = await this.calculateSummary(sessionId, comparisons, fieldChangeCounts, totalConfidence);
        return {
            sessionId,
     ...
     */
    async calculateSummary(_sessionId, comparisons, fieldChangeCounts, totalConfidence) {
     ...
     */
    async getEmployeeComparison(sessionId, employeeId) {
     ...
     */
    async getComparisonsByChangeType(sessionId, changeType) {
        const report = await this.generateComparison(sessionId);
     ...
     */
    async getComparisonsByLevel(sessionId, level) {
        const report = await this.generateComparison(sessionId);
     ...
     */
    async getSignificantChanges(sessionId) {
        const report = await this.generateComparison(sessionId);
        return report.comparisons.filter((c) => c.totalChanges > 0 && c.changes.some((ch) => ch.significance === 'high'));
    }
    // ---------------------------------------------------------------------------
    // EXPORT FORMATS
    // ---------------------------------------------------------------------------
     ...
     */
    async exportDetailedCSV(sessionId) {
        const report = await this.generateComparison(sessionId);
        const headers = [
            'Employee ID',
            'Name',
     ...
     */
    async approveChanges(sessionId, approvedBy) {
        let approved = 0;
        let failed = 0;
     ...
     */
    async applyApprovedChanges(sessionId) {
        let updated = 0;
        let failed = 0;
    ...
    `, [sessionId, this.tenantId]);
        ...
        const stagingToEmployee = new Map();
        for (const row of result.rows) {
...
// FACTORY FUNCTION
// =============================================================================
export function createComparisonReportService(tenantId) {
