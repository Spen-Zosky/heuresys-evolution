...
import { SkillClassificationService, PrimaryCategory, CognitiveLevel } from './skill-classification.js';
...
export interface JobSkillDistribution {
  ...
  clusters: { code: string; name: string; count: number }[];
}
...
export class JobSkillsEnrichmentService {
  ...
  private classificationService: SkillClassificationService;
   ...
   */
  async enrichAllJobSkills(): Promise<EnrichmentResult> {
   ...
   */
  async enrichJobTemplateSkills(jobTemplateId: string): Promise<EnrichmentResult> {
    const result: EnrichmentResult = {
      total_skills: 0,
      enriched: 0,
      not_found: 0,
   ...
   */
  async suggestSkillsForJobTemplate(
    ...
    if (includeHard) categories.push('hard');
    if (includeSoft) categories.push('soft');
    if (includeHybrid) categories.push('hybrid');
   ...
   */
  async getJobSkillDistribution(jobTemplateId: string): Promise<JobSkillDistribution | null> {
    const result = await pool.query(`
      SELECT
   ...
   */
  async getSkillBalanceRecommendations(jobTemplateId: string): Promise<{
