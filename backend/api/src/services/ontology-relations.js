...
export class OntologyRelationsService {
    ...
    async getSkillClusters(options) {
        ...
        const countRes = await this.db.query(`SELECT COUNT(DISTINCT broader_uri)::int AS total
       FROM esco_skills
       WHERE broader_uri IS NOT NULL
         AND ($1::text IS NULL OR skill_type = $1)`, [skillType ?? null]);
        const total = countRes.rows[0].total;
        // Build clusters using window function for top-5 skills per group
        const res = await this.db.query(`WITH ranked AS (
         SELECT
           s.broader_uri,
           s.id,
       ...
       ORDER BY a.member_count DESC`, [skillType ?? null, limit, offset]);
        const clusters = res.rows.map((row, idx) => ({
     ...
     */
    async getCareerPathways(occupationId) {
        const sourceRes = await this.db.query(`SELECT id, COALESCE(preferred_label_en, preferred_label) AS label, isco_code
       FROM esco_occupations WHERE id = $1`, [occupationId]);
        if (sourceRes.rows.length === 0) {
            return { source: { id: occupationId, label: 'Unknown', iscoCode: null }, steps: [] };
        }
        const src = sourceRes.rows[0];
        // Source skill set
        const srcSkillsRes = await this.db.query(`SELECT os.skill_id AS id,
       ...
       WHERE os.occupation_id = $1`, [occupationId]);
        ...
        const srcSkillIds = srcSkillsRes.rows.map((r) => r.id);
        const srcSkillMap = new Map(srcSkillsRes.rows.map((r) => [
            r.id,
            { label: r.label, skillType: r.skill_type },
        ]));
        // Find occupations with >=60% skill overlap
        const OVERLAP_THRESHOLD = 0.6;
        const neighborsRes = await this.db.query(`SELECT
         o.id,
       ...
       LIMIT 5`, [srcSkillIds, srcSkillIds.length, occupationId, OVERLAP_THRESHOLD]);
        const steps = [];
        for (const neighbor of neighborsRes.rows) {
            const neighborSkillsRes = await this.db.query(`SELECT os.skill_id AS id,
                COALESCE(es.preferred_label_en, es.preferred_label) AS label,
                es.skill_type
         FROM esco_occupation_skills os
         JOIN esco_skills es ON es.id = os.skill_id
         WHERE os.occupation_id = $1`, [neighbor.id]);
            const neighborSkillIds = new Set(neighborSkillsRes.rows.map((r) => r.id));
            const neighborSkillMap = new Map(neighborSkillsRes.rows.map((r) => [
                r.id,
                { label: r.label, skillType: r.skill_type },
            ]));
            const skillsGained = [];
    ...
    async getProcessSkillImpact() {
       ...
       ORDER BY process_count DESC, skill_label`);
        const allSkills = res.rows.map((row) => ({
...
//# sourceMappingURL=ontology-relations.js.map