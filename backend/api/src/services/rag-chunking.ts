...
async function loadOpenAIApiKey(): Promise<string | null> {
  try {
    ...
    if (result.rows.length > 0 && result.rows[0].config_value) {
      return result.rows[0].config_value as string;
    }
  } catch (_err) {
...
export async function chunkDocument(
  ...
  const docResult = await pool.query(
    `SELECT id, filename, mime_type, file_path, metadata, knowledge_base_id
     FROM rag_documents
  ...
  if (doc.knowledge_base_id && (!options.chunkSize || !options.chunkOverlap)) {
    const kbResult = await pool.query(
      `SELECT chunk_size, chunk_overlap FROM rag_knowledge_bases WHERE id = $1 AND tenant_id = $2`,
      [doc.knowledge_base_id, tenantId]
    );
    if (kbResult.rows.length > 0) {
      const kb = kbResult.rows[0] as {
  ...
  if (!content) {
    throw new Error(
      `No content available for document ${documentId}. Store text in metadata.content or provide file_path.`
    );
  }
...
export async function processDocument(
  ...
  try {
    ...
    await pool.query(
      `UPDATE rag_documents SET status = 'processing', error_message = NULL
       WHERE id = $1 AND tenant_id = $2`,
      [documentId, tenantId]
    );
    ...
    for (const chunk of chunks) {
      ...
      storedChunkIds.push(insertResult.rows[0].id as string);
    ...
    for (let i = 0; i < storedChunkIds.length; i += batchSize) {
      ...
      try {
          ...
          if (chunkId && embedding) {
            await pool.query(
              `UPDATE rag_document_chunks SET
               ...
               WHERE id = $4`,
              [JSON.stringify(embedding), EMBEDDING_MODEL, EMBEDDING_DIMENSIONS, chunkId]
            );
            embeddingsGenerated++;
          }
        }
        ...
        logger.warn(
          `[rag-chunking] Embedding generation failed for batch starting at ${i}: ${error instanceof Error ? error.message : String(error)}`
        );
    ...
    await pool.query(
      `UPDATE rag_documents SET status = 'error', error_message = $1
       WHERE id = $2 AND tenant_id = $3`,
      [errorMessage, documentId, tenantId]
    );
...
export async function searchChunks(
  ...
  const params: unknown[] = [tenantId, JSON.stringify(queryEmbedding)];
  let paramIndex = 3;
  let extraFilters = '';
  ...
  return result.rows.map((row) => ({
    ...
    document_filename: row.document_filename as string,
    document_original_name: row.document_original_name as string,
  }));
}
...
export async function getDocumentChunks(
  ...
  return result.rows as StoredChunk[];
}
 ...
 */
export async function deleteDocumentChunks(documentId: string, tenantId: string): Promise<number> {
  const result = await pool.query(
    'DELETE FROM rag_document_chunks WHERE document_id = $1 AND tenant_id = $2',
    [documentId, tenantId]
  );
