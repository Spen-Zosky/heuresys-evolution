...
export class PDFGenerator {
    ...
    constructor(options) {
        ...
        const pageWidth = this.options.orientation === 'landscape'
    ...
    addMetricsSection(metrics) {
        ...
        metrics.forEach((metric, index) => {
            ...
            // Value
            const valueStr = typeof metric.value === 'number' ? this.formatNumber(metric.value) : metric.value;
            this.doc
                .fontSize(18)
            ...
            if (metric.change !== undefined) {
                ...
                this.doc
                    ...
                    .text(`${changeSymbol} ${Math.abs(metric.change).toFixed(1)}%`, x + cardWidth - 50, this.currentY + 8);
            }
            x += cardWidth + 10;
        });
 ...
 */
export async function generateHRDashboardPDF(data) {
 ...
 */
export async function generateCompensationPDF(data) {
    const pdf = new PDFGenerator({
 ...
 */
export async function generateWorkforcePlanningPDF(data) {
    const pdf = new PDFGenerator({
 ...
 */
export async function generateTimeAnalyticsPDF(data) {
    const pdf = new PDFGenerator({
 ...
 */
export async function generatePerformancePDF(data) {
    const pdf = new PDFGenerator({
 ...
 */
export async function generateGenericPDF(data, title) {
    const pdf = new PDFGenerator({
        title,
