...
export interface TurnoverRiskFactors {
  ...
  salary_band_position: number; // 0 to 1 (percentile in band)
...
export class PredictiveAnalyticsService {
   ...
   */
  async getActiveModel(tenantId: string, type: ModelType): Promise<Record<string, any>> {
  ...
  async listModels(
    tenantId: string,
    options?: { type?: ModelType; status?: ModelStatus }
  ): Promise<Record<string, any>[]> {
   ...
   */
  async calculateTurnoverRisk(tenantId: string, employeeId: string): Promise<PredictionResult> {
    // Gather factors
   ...
   */
  private async gatherTurnoverFactors(
    tenantId: string,
    employeeId: string
  ): Promise<TurnoverRiskFactors> {
    // Employee base data
    ...
    if (emp.manager_id) {
      ...
      if (mgrResult.rows[0]) {
        ...
        managerTenureMonths = Math.floor(
          (Date.now() - mgrHire.getTime()) / (30 * 24 * 60 * 60 * 1000)
        );
      }
    }
    ...
    const teamTurnoverRate =
      turnoverResult.rows[0].total > 0
   ...
   */
  private getDefaultTurnoverConfig(): ModelConfig {
    return {
      features: [
        'tenure_months',
        'performance_trend',
   ...
   */
  private calculateTurnoverScore(factors: TurnoverRiskFactors, config: ModelConfig): number {
   ...
   */
  private getRiskLevel(score: number, thresholds?: ModelConfig['thresholds']): RiskLevel {
    const t = thresholds || { low: 0.3, medium: 0.5, high: 0.7, critical: 0.85 };

    if (score >= (t.critical || 0.85)) return 'critical';
  ...
  private analyzeTurnoverFactors(
    factors: TurnoverRiskFactors,
    _config: ModelConfig
  ): PredictionResult['factors'] {
    const analyzed: PredictionResult['factors'] = [];
   ...
   */
  private calculateConfidence(factors: TurnoverRiskFactors): number {
   ...
   */
  private async storeTurnoverRisk(
    tenantId: string,
    employeeId: string,
   ...
   */
  async predictPerformance(tenantId: string, employeeId: string): Promise<PredictionResult> {
    // Gather factors
    ...
    const riskLevel: RiskLevel =
      predictedRating >= 4 ? 'low' : predictedRating >= 3 ? 'medium' : 'high';
   ...
   */
  private calculatePredictedRating(factors: PerformanceFactors): number {
    const weights = {
      historical: 0.35,
      goals: 0.2,
      training: 0.1,
   ...
   */
  private analyzePerformanceFactors(factors: PerformanceFactors): PredictionResult['factors'] {
    const analyzed: PredictionResult['factors'] = [];
    ...
    if (factors.historical_ratings.length >= 2) {
      const trend =
        (factors.historical_ratings[0] ?? 0) -
   ...
   */
  private calculatePerformanceConfidence(factors: PerformanceFactors): number {
   ...
   */
  private async storePerformancePrediction(
    tenantId: string,
    employeeId: string,
    result: PredictionResult
  ): Promise<void> {
   ...
   */
  async batchCalculateTurnoverRisk(
    tenantId: string
  ...
  async getHighRiskEmployees(
    tenantId: string,
    options?: { risk_level?: RiskLevel; limit?: number }
  ): Promise<Record<string, any>[]> {
    const conditions: string[] = ['trs.tenant_id = $1'];
    const params: unknown[] = [tenantId];
    let paramIndex = 2;
   ...
   */
  async getPredictionSummary(tenantId: string): Promise<Record<string, any>> {
    const turnoverResult = await pool.query(
      `
      SELECT
        risk_level,
