...
const ENTITY_CONFIGS = {
    employee: {
        ...
        contextTemplate: (r) => `${r.first_name} ${r.last_name} - ${r.job_title || ''} - ${Array.isArray(r.skills) ? r.skills.join(', ') : r.skills || ''}`,
    ...
    performance_review: {
        ...
        contextTemplate: (r) => `Performance Review (${r.review_type || 'annual'}) - Strengths: ${r.strengths || ''} - Areas: ${r.areas_for_improvement || ''}`,
        metadataColumns: ['employee_id', 'review_type'],
    },
    check_in: {
        table: 'check_ins',
        textColumns: ['meeting_type', 'employee_notes', 'manager_notes', 'action_items'],
        contextTemplate: (r) => `Check-in (${r.meeting_type || 'weekly'}) - ${r.employee_notes || ''} ${r.manager_notes || ''}`,
        metadataColumns: ['employee_id', 'meeting_type'],
    },
    feedback_360: {
        table: 'feedback_360',
        textColumns: ['strengths', 'areas_for_improvement', 'relationship_type'],
        contextTemplate: (r) => `360 Feedback (${r.relationship_type || 'peer'}) - ${r.strengths || ''} - ${r.areas_for_improvement || ''}`,
    ...
    skill_gap_analysis: {
        ...
        contextTemplate: (r) => `Skill Gap: ${r.analysis_name || ''} - ${r.recommendations || ''} - Priority: ${Array.isArray(r.priority_skills) ? r.priority_skills.join(', ') : r.priority_skills || ''}`,
    ...
    learning_path: {
        ...
        contextTemplate: (r) => `Learning Path: ${r.title} - ${r.description || ''} - Target: ${r.target_role || ''}`,
    ...
    candidate: {
        ...
        contextTemplate: (r) => `${r.first_name} ${r.last_name} - ${r.job_title || ''} at ${r.current_company || ''} - Skills: ${Array.isArray(r.skills) ? r.skills.join(', ') : r.skills || ''}`,
...
export class EmbeddingQueueProcessor {
    ...
    async fetchEntityData(entityType, entityId) {
        ...
        const validatedTable = validateTableName(config.table, `EmbeddingQueue.fetchEntityData(${entityType})`);
        ...
        const validatedColumns = columns.map((col) => validateIdentifier(col, `EmbeddingQueue.fetchEntityData(${entityType}).columns`));
        const result = await this.pool.query(`SELECT ${validatedColumns.join(', ')} FROM ${validatedTable} WHERE id = $1`, [entityId]);
     ...
     */
    startBackgroundProcessing(intervalMs = 30000, batchSize = 50) {
        if (this.processingInterval) {
            logger.info('Background processing already running');
            return;
        }
        logger.info(`Starting embedding queue processor (interval: ${intervalMs}ms, batch: ${batchSize})`);
        // Process immediately on start
        this.processBatch(batchSize)
            .then((result) => {
            if (result.processed > 0) {
                logger.info(`Embedding queue processed: ${result.succeeded}/${result.processed} succeeded`);
            }
        })
            .catch((err) => {
...
export function createEmbeddingQueueProcessor(pool) {
    if (!queueProcessor) {
        queueProcessor = new EmbeddingQueueProcessor(pool);
    }
    return queueProcessor;
}
export function getEmbeddingQueueProcessor() {
    return queueProcessor;
}
//# sourceMappingURL=embedding-queue-processor.js.map