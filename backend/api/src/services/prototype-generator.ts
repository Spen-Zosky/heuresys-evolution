...
import { BusinessProcessResearchService, BusinessProcess } from './business-process-research.js';
...
export class PrototypeGeneratorService {
  ...
  private processResearchService: BusinessProcessResearchService;
  ...
  async generateFromPrototype(
    ...
    try {
      ...
      const processResearch = await this.processResearchService.researchIndustryProcesses(
        naceCode,
        prototype.sizeClass
      );
      ...
      const staffingPlan = await this.generateStaffing(
        prototypeId,
        orgUnits,
        jobRoles,
        prototype.sizeClass
  ...
  private async createSession(_prototypeId: string): Promise<string> {
  ...
  private async updateSessionStatus(sessionId: string, status: GenerationStatus): Promise<void> {
    await pool.query(
      `
  ...
  private async completeSession(sessionId: string, result: GenerationResult): Promise<void> {
    await pool.query(
      `
      UPDATE org_chart_generation_sessions
  ...
  private async getPrototype(prototypeId: string): Promise<IndustryPrototype> {
    const result = await pool.query(
      `
    ...
    return {
      ...
      sizeClass: row.size_class.toLowerCase() as CompanySize,
  ...
  private groupProcessesToDivisions(
    primaryProcesses: BusinessProcess[],
    supportProcesses: BusinessProcess[]
  ): Record<string, BusinessProcess[]> {
    const divisions: Record<string, BusinessProcess[]> = {
      Operations: [],
      Commercial: [],
      'Finance & Admin': [],
      'Human Resources': [],
    ...
    for (const process of supportProcesses) {
      switch (process.valueChainPosition) {
        ...
        case 9: // Infrastructure
          if (
            process.processName.toLowerCase().includes('risk') ||
    ...
    return Object.fromEntries(
      Object.entries(divisions).filter(([, processes]) => processes.length > 0)
    );
  }

  private getDivisionCode(divisionName: string): string {
  ...
  private getItalianName(englishName: string): string {
  ...
  async generateJobRoles(prototype: IndustryPrototype, orgUnits: OrgUnit[]): Promise<JobRole[]> {
  ...
  private getRolesForOrgUnit(orgUnit: OrgUnit, sizeClass: CompanySize): JobRole[] {
    const roles: JobRole[] = [];
  ...
  async generateTasks(orgUnits: OrgUnit[], processes: BusinessProcess[]): Promise<Task[]> {
    const tasks: Task[] = [];
    let taskIndex = 0;

    for (const orgUnit of orgUnits) {
      if (orgUnit.level < 2) continue; // Skip CEO level
  ...
  private generateGenericTasks(orgUnit: OrgUnit): Task[] {
    const tasks: Task[] = [];
    const divisionCode = orgUnit.code.split('-')[1] ?? '';

    const genericTasksByDivision: Record<
      string,
      { name: string; frequency: string; complexity: number }[]
    > = {
      OPS: [
  ...
  async generateKPIs(orgUnits: OrgUnit[], _processes: BusinessProcess[]): Promise<KPI[]> {
    const kpis: KPI[] = [];

    for (const orgUnit of orgUnits) {
      if (orgUnit.level < 2) continue;
  ...
  private getKPIsForOrgUnit(orgUnit: OrgUnit): KPI[] {
    const kpis: KPI[] = [];
    const divisionCode = orgUnit.code.split('-')[1] ?? '';

    const kpiTemplates: Record<
      string,
      { name: string; unit: string; direction: string; benchmark: number }[]
    > = {
      OPS: [
  ...
  async distributeTasksToRoles(tasks: Task[], roles: JobRole[]): Promise<TaskDistribution[]> {
    const distributions: TaskDistribution[] = [];

    for (const task of tasks) {
      // Find roles in the same org unit
      const unitRoles = roles.filter(
        (r) => r.orgUnitCode === task.orgUnitCode || task.orgUnitCode.startsWith(r.orgUnitCode)
      );

      if (unitRoles.length === 0) continue;
      ...
      const perContributor =
        contributors.length > 0 ? Math.floor(remainingPercent / contributors.length) : 0;
  ...
  async distributeKPIsToRoles(kpis: KPI[], roles: JobRole[]): Promise<KPIDistribution[]> {
    const distributions: KPIDistribution[] = [];

    for (const kpi of kpis) {
      // Find roles in the same org unit
      const unitRoles = roles.filter(
        (r) => r.orgUnitCode === kpi.orgUnitCode || kpi.orgUnitCode.startsWith(r.orgUnitCode)
      );

      if (unitRoles.length === 0) continue;

      // Director is owner
      const director = unitRoles.find(
        (r) => r.jobCode.includes('-DIR') && !r.jobCode.includes('-VDIR')
      );
      if (director) {
        distributions.push({
      ...
      const managers = unitRoles.filter(
        (r) =>
          r.jobCode.includes('-MGR') || r.jobCode.includes('-SMGR') || r.jobCode.includes('-VDIR')
      );
   ...
   */
  async exportToExcalidraw(orgUnits: OrgUnit[]): Promise<object> {
  ...
  private groupByLevel(orgUnits: OrgUnit[]): Record<number, OrgUnit[]> {
    const groups: Record<number, OrgUnit[]> = {};
...
export function createPrototypeGeneratorService(tenantId: string): PrototypeGeneratorService {
