...
export class CareerPathService {
  ...
  async getAllPaths(
    tenantId: string,
    options: { pathType?: string; status?: string; search?: string } = {}
  ): Promise<CareerPath[]> {
    let query = `
      SELECT
        cp.id,
    ...
    return result.rows as CareerPath[];
  }
  ...
  async getPathDetails(
    tenantId: string,
    pathId: string
  ): Promise<(CareerPath & { levels: CareerPathLevel[] }) | null> {
    ...
    return {
      ...(pathResult.rows[0] as CareerPath),
      levels: levelsResult.rows as CareerPathLevel[],
    };
  }
  ...
  async getRecommendationsForEmployee(
    ...
    for (const path of paths) {
      try {
        const recommendation = await this.calculatePathRecommendation(
          tenantId,
          employeeId,
          path.id
        );
  ...
  async calculatePathRecommendation(
    tenantId: string,
    employeeId: string,
    pathId: string
  ): Promise<CareerPathRecommendation | null> {
    ...
    const skillMatchScore =
      levelAnalysis.reduce((sum, l) => sum + l.skillCoverage, 0) / levelAnalysis.length;

    // Aggregate skill gaps from future levels
    const futureAnalysis = levelAnalysis.slice(currentLevelIndex + 1);
    const allGaps: SkillGap[] = futureAnalysis.flatMap((l) => l.gaps);
    ...
    const monthsToTarget = futureAnalysis.reduce(
      (sum, l) => sum + this.estimateMonthsToLevel(l),
      0
    );
  ...
  async simulateCareerPath(tenantId: string, params: SimulationParams): Promise<CareerSimulation> {
    ...
    if (targetPathId) {
      ...
      pathsToAnalyze = result.rows as CareerPath[];
    } else {
  ...
  async getReachableRoles(
    tenantId: string,
    employeeId: string,
    maxGapThreshold: number = 2.0
  ): Promise<ReachableRole[]> {
    const employeeSkills = await this.getEmployeeSkills(employeeId);
    const paths = await this.getAllPaths(tenantId, { status: 'active' });
    const reachableRoles: ReachableRole[] = [];
      ...
      for (let i = currentLevelIndex + 1; i < pathDetails.levels.length; i++) {
        ...
        if (level.target_job_id && analysis.totalGap <= maxGapThreshold) {
          const jobResult = await this.pool.query(
            'SELECT id, title FROM tenant_jobs WHERE id = $1',
            [level.target_job_id]
          );

          if (jobResult.rows.length > 0) {
            const job = jobResult.rows[0] as { id: string; title: string };
            reachableRoles.push({
              job: job,
              path: path,
              level: level,
  ...
  private async getEmployeeSkills(employeeId: string): Promise<Map<string, number>> {
    const result = await this.pool.query(
      `
  ...
  private async analyzeLevelFit(
    ...
    const meetsRequirements =
      gaps.filter((g) => g.is_mandatory).length === 0 && skillCoverage >= 0.7;
  ...
  private classifyGapSeverity(
    ...
    if (isMandatory && gap > 1.5) return 'critical';
    if (importance === 'critical' && gap > 1) return 'critical';
    if (gap > 2) return 'high';
    if (gap > 1) return 'medium';
    return 'low';
  }

  private calculateOverallFitScore(levelAnalysis: LevelAnalysis[]): number {
    if (levelAnalysis.length === 0) return 0;
  ...
  private estimateMonthsToLevel(analysis: LevelAnalysis): number {
    if (analysis.totalGap === 0) return 0;
  ...
  private generateRecommendationReason(fitScore: number): string {
    if (fitScore >= 0.8) {
  ...
  private async getTrainingRecommendationsForGaps(
    gaps: SkillGap[]
    ...
    for (const gap of gaps.slice(0, 10)) {
      ...
      const courses: CourseInfo[] = coursesResult.rows.map(
        (row: {
          id: string;
          title: string;
          duration_hours: number;
          proficiency_level_gained: number | null;
        }) => ({
          id: row.id,
          title: row.title,
  ...
  private async reconstructRecommendationsFromCache(
    tenantId: string,
    cachedRows: Array<{
      path_id: string;
      fit_score: number;
  ...
  private async saveSimulation(simulation: {
    id: string;
    tenant_id: string;
    employee_id: string;
