...
export class OntologyEmbeddingService {
  ...
  async loadApiKeyFromDb(tenantId?: string): Promise<boolean> {
  ...
  private async openAIEmbedding(text: string): Promise<EmbeddingResult> {
  ...
  private async geminiEmbedding(text: string): Promise<EmbeddingResult> {
  ...
  private async openAIBatchEmbedding(
    texts: string[]
  ...
  async searchSkillsByVector(options: VectorSearchOptions): Promise<VectorSearchResult[]> {
    ...
    const params: (string | number | boolean)[] = [vectorStr, options.limit];
    let paramIndex = 3;
    ...
    if (options.includeCustomSkills && options.tenantId) {
      const customResults = await this.searchCustomSkillsByVector(
        vectorStr,
        options.tenantId,
        options.language,
        options.limit,
  ...
  private async searchCustomSkillsByVector(
    vectorStr: string,
    tenantId: string,
    language: 'en' | 'it',
    limit: number,
    ...
    return result.rows
      ...
      .map((row) => ({
        ...
        similarity: parseFloat(row.similarity.toFixed(4)),
        source: 'tenant_custom' as const,
      }));
  }

  private async fallbackTextSearch(options: VectorSearchOptions): Promise<VectorSearchResult[]> {
    const labelColumn = options.language === 'it' ? 'preferred_label_it' : 'preferred_label_en';
    const descColumn = options.language === 'it' ? 'description_it' : 'description_en';
  ...
  async createEmbeddingJob(
    targetTable: 'esco_skills' | 'tenant_custom_skills' | 'industry_classifications',
    ...
    if (targetTable === 'industry_classifications') {
      ...
      if (!tenantId) throw new Error('tenantId required for tenant_custom_skills');
      if (jobType === 'full') {
        countQuery =
  ...
  async getJobStatus(jobId: string): Promise<EmbeddingJobResult> {
    const result = await pool.query(
      `
      SELECT
  ...
  async processEmbeddingJob(jobId: string): Promise<EmbeddingJobResult> {
    // Get job details
    ...
    try {
      ...
      if (job.target_table === 'industry_classifications') {
        const result = await this.processIndustryClassificationsEmbeddings(
          job.job_type,
          jobId,
          (p, f, t) => {
            processed = p;
        ...
        const result = await this.processTenantSkillsEmbeddings(
          job.tenant_id,
          job.job_type,
          jobId,
          (p, f, t) => {
  ...
  private async processEscoSkillsEmbeddings(
    jobType: string,
    jobId: string,
    onProgress: (processed: number, failed: number, tokens: number) => void
    ...
    while (true) {
      ...
      if (skillsResult.rows.length === 0) break;
      ...
      for (const skill of skillsResult.rows) {
        ...
        const textIt =
          `${skill.preferred_label_it || skill.preferred_label_en || ''}. ${skill.description_it || skill.description_en || ''}`.trim();
  ...
  private async processTenantSkillsEmbeddings(
    tenantId: string,
    jobType: string,
    jobId: string,
    onProgress: (processed: number, failed: number, tokens: number) => void
    ...
    while (true) {
      ...
      for (const skill of skillsResult.rows) {
        ...
        const textIt =
          `${skill.name_it || skill.name_en || ''}. ${skill.description_it || skill.description_en || ''}`.trim();
  ...
  private async processIndustryClassificationsEmbeddings(
    jobType: string,
    jobId: string,
    onProgress: (processed: number, failed: number, tokens: number) => void
  ): Promise<{ processed: number; failed: number; tokens: number }> {
    ...
    while (true) {
      ...
      if (icResult.rows.length === 0) break;
      ...
      try {
        ...
        logger.error(
          `Industry classifications batch embedding failed at offset ${offset}:${error}`
        );
  ...
  async inferSkillRelations(
    similarityThreshold: number = 0.85,
    limit: number = 1000
  ): Promise<{ relationsFound: number; jobId: string }> {
...
export function createOntologyEmbeddingService(
