...
export class AdvancedSemanticSearchService {
   ...
   */
  async expandQuery(query: string, language: 'en' | 'it' = 'en'): Promise<QueryExpansion> {
    ...
    return results
      .map((result) => {
        ...
        const finalScore = Math.min(
          1,
          semanticSimilarity * 0.6 + contextRelevance * 0.25 + recencyBoost * 0.1 + typeBoost * 0.05
        );
   ...
   */
  async advancedSearch(options: AdvancedSearchOptions): Promise<AdvancedSearchResult> {
    ...
    // Group by type
    const resultsByType: Record<string, RankedResult[]> = {};
   ...
   */
  private async logSearchAnalytics(params: {
    tenantId: string;
    query: string;
   ...
   */
  async submitSearchFeedback(searchId: string, feedbackScore: number): Promise<void> {
    await pool.query(
      `
...
let serviceInstance: AdvancedSemanticSearchService | null = null;

export function getAdvancedSearchService(): AdvancedSemanticSearchService {
  if (!serviceInstance) {
