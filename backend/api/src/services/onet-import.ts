...
export class ONetImportService {
  ...
  private async initOpenAIKey(): Promise<void> {
    try {
      const result = await this.pool.query(
        `SELECT value FROM system_config WHERE key = 'openai_api_key' LIMIT 1`
      );
   ...
   * Create import job
   */
  async createImportJob(importType: string, sourceVersion?: string): Promise<ImportJobResult> {
    const result = await this.pool.query(
      `INSERT INTO onet_import_jobs (import_type, source_version, status)
       VALUES ($1, $2, 'pending')
       RETURNING *`,
      [importType, sourceVersion]
    );
   ...
   */
  async getImportJobStatus(jobId: string): Promise<ImportJobResult | null> {
  ...
  async importOccupations(
    ...
    await this.pool.query(
      `UPDATE onet_import_jobs SET status = 'processing', started_at = NOW(), total_records = $2 WHERE id = $1`,
      [jobId, occupations.length]
    );
    ...
    try {
      ...
      await this.pool.query(
        `UPDATE onet_import_jobs SET status = 'completed', completed_at = NOW() WHERE id = $1`,
        [jobId]
      );
    } catch (error) {
      await this.pool.query(
        `UPDATE onet_import_jobs SET status = 'failed', error_message = $2, completed_at = NOW() WHERE id = $1`,
   ...
   */
  async importSkills(jobId: string, skills: ONetSkill[]): Promise<ImportStats> {
    const stats: ImportStats = {
      occupations: 0,
      skills: 0,
      abilities: 0,
    ...
    try {
      for (const skill of skills) {
        try {
          await this.pool.query(
            `INSERT INTO onet_skills (element_id, element_name, description, category)
             VALUES ($1, $2, $3, $4)
             ON CONFLICT (element_id) DO UPDATE SET
               ...
               updated_at = NOW()`,
            [skill.element_id, skill.element_name, skill.description, skill.category]
          );
          stats.skills++;
        } catch (error) {
   ...
   */
  async importAbilities(jobId: string, abilities: ONetAbility[]): Promise<ImportStats> {
    const stats: ImportStats = {
      occupations: 0,
      skills: 0,
      abilities: 0,
    ...
    try {
      for (const ability of abilities) {
        try {
          await this.pool.query(
            `INSERT INTO onet_abilities (element_id, element_name, description, category)
             VALUES ($1, $2, $3, $4)
             ON CONFLICT (element_id) DO UPDATE SET
               element_name = EXCLUDED.element_name,
               description = EXCLUDED.description,
               category = EXCLUDED.category,
               updated_at = NOW()`,
            [ability.element_id, ability.element_name, ability.description, ability.category]
          );
          stats.abilities++;
        } catch (error) {
   ...
   */
  async importKnowledge(jobId: string, knowledgeAreas: ONetKnowledge[]): Promise<ImportStats> {
    const stats: ImportStats = {
      occupations: 0,
      skills: 0,
      abilities: 0,
    ...
    try {
      for (const ka of knowledgeAreas) {
        try {
          await this.pool.query(
            `INSERT INTO onet_knowledge (element_id, element_name, description, domain)
             VALUES ($1, $2, $3, $4)
             ON CONFLICT (element_id) DO UPDATE SET
               element_name = EXCLUDED.element_name,
               description = EXCLUDED.description,
               ...
               updated_at = NOW()`,
            [ka.element_id, ka.element_name, ka.description, ka.domain]
          );
          stats.knowledge++;
        } catch (error) {
   ...
   */
  async importWorkActivities(jobId: string, activities: ONetWorkActivity[]): Promise<ImportStats> {
    const stats: ImportStats = {
      occupations: 0,
      skills: 0,
      abilities: 0,
    ...
    try {
      for (const activity of activities) {
        try {
          await this.pool.query(
            `INSERT INTO onet_work_activities (element_id, element_name, description, activity_type, parent_element_id)
             VALUES ($1, $2, $3, $4, $5)
             ON CONFLICT (element_id) DO UPDATE SET
               element_name = EXCLUDED.element_name,
               description = EXCLUDED.description,
   ...
   */
  async importOccupationSkillLinks(
    jobId: string,
    links: ONetOccupationLink[]
  ): Promise<ImportStats> {
    const stats: ImportStats = {
      occupations: 0,
      skills: 0,
      ...
      for (const link of links) {
        try {
          ...
          const occResult = await this.pool.query(
            `SELECT id FROM onet_occupations WHERE onet_soc_code = $1`,
            [link.onet_soc_code]
          );
          ...
          const skillResult = await this.pool.query(
            `SELECT id FROM onet_skills WHERE element_id = $1`,
            [link.element_id]
          );
          ...
          await this.pool.query(
            ...
            [occupationId, skillId, link.importance, link.level]
          );
  ...
  async mapSkillsToEsco(
    ...
    const skills = await this.pool.query(
      `SELECT id, element_name, description
       FROM onet_skills
    ...
    for (const skill of skills.rows) {
      try {
        ...
        if (!data.data?.[0]?.embedding) continue;
        ...
        if (
          ...
          await this.pool.query(
            `UPDATE onet_skills SET
               ...
               embedding_generated_at = NOW()
             WHERE id = $3`,
            [vectorStr, EMBEDDING_MODEL, skill.id]
          );
        }
      } catch (error) {
   ...
   */
  async getSkillsForOccupation(occupationId: string): Promise<
    Array<{
      id: string;
      element_id: string;
      element_name: string;
  ...
  > {
    const result = await this.pool.query(
      `SELECT id, onet_soc_code, title, job_zone,
  ...
  async getUnifiedSkills(
    sources: string[] = ['esco', 'onet_skill', 'onet_ability', 'onet_knowledge'],
    limit: number = 100,
    offset: number = 0
  ): Promise<
    Array<{
  ...
  > {
    ...
    const result = await this.pool.query(
      `SELECT source, id, external_id, name, description, category
       FROM v_unified_skills
       WHERE source IN (${sourceFilter})
       ORDER BY source, name
       LIMIT $${sources.length + 1} OFFSET $${sources.length + 2}`,
  ...
  // Helper methods
  private rowToJob(row: Record<string, unknown>): ImportJobResult {
    return {
      id: row.id as string,
      import_type: row.import_type as string,
      status: row.status as ImportJobResult['status'],
      total_records: row.total_records as number,
      processed_records: row.processed_records as number,
      failed_records: row.failed_records as number,
