...
interface BlueprintFinding {
  resultType: BlueprintResult['resultType'];
...
export class BlueprintGeneratorService {
  ...
  async runBlueprint(input: BlueprintRunInput): Promise<BlueprintRunOutput> {
  ...
  private async runGreenfield(
  ...
  ): Promise<BlueprintFinding[]> {
    const findings: BlueprintFinding[] = [];

    // Load org_unit_templates for this template
    const templateUnits = await this.dbClient.query(
      `SELECT id, code, name_it, name_en, depth, level
    ...
    const processes = await this.dbClient.query(
    ...
    const mappings = await this.dbClient.query(
      `SELECT oupm.process_id, out2.code AS template_unit_code, out2.name_it AS template_unit_name
       FROM org_unit_process_mapping oupm
       JOIN org_unit_templates out2 ON out2.id = oupm.org_unit_template_id
       WHERE out2.template_id = $1`,
      [template.id]
    );

    const mappingsByProcess = new Map<string, Array<{ code: string; name: string }>>();
  ...
  private async runOverlay(
    ...
    const [orgFindings, processFindings, skillFindings, hierarchyFindings, kpiFindings] =
      await Promise.all([
  ...
  private async analyzeOrgUnits(
    ...
    try {
      ...
      const orgResult = await this.dbClient.query(
        `SELECT ou.id, ou.code, ou.name, ou.org_type, ou.org_level, ou.is_active,
      ...
      const realCodes = new Set(orgResult.rows.map((r: { code: string }) => r.code?.toLowerCase()));
  ...
  private async analyzeProcessCoverage(
    tenantId: string,
    profileId: string
  ): Promise<BlueprintFinding[]> {
    const findings: BlueprintFinding[] = [];
    ...
    try {
      ...
      const processResult = await this.dbClient.query(
        `SELECT bp.id, bp.process_code, bp.process_name, bp.process_category
         FROM business_processes bp
         WHERE bp.profile_id = $1`,
        [profileId]
      ...
      const mappingResult = await this.dbClient.query(
        `SELECT oupm.process_id, out2.code AS template_unit_code, out2.name_it AS template_unit_name
         ...
         WHERE bt.profile_id = $1`,
        [profileId]
      );

      const coveredProcessIds = new Set(
        mappingResult.rows.map((r: { process_id: string }) => r.process_id)
      );
      ...
      const realOrgUnits = await this.dbClient.query(
      ...
      const templateCodes = new Set(
        mappingResult.rows.map((r: { template_unit_code: string }) =>
          r.template_unit_code?.toLowerCase()
        )
      );
      ...
      logger.error(
        { err, context: 'analyzeProcessCoverage' },
        'Failed to analyze process coverage'
      );
    }
  ...
  private async analyzeSkillGaps(tenantId: string, profileId: string): Promise<BlueprintFinding[]> {
    const findings: BlueprintFinding[] = [];

    try {
      // Q4: Skills required by processes
      const requiredResult = await this.dbClient.query(
        `SELECT psr.id, psr.process_id, psr.esco_skill_id, psr.proficiency_level, psr.is_mandatory,
                bp.process_name,
      ...
      const employeeSkillsResult = await this.dbClient.query(
        `SELECT es2.esco_skill_id, es2.proficiency_level, COUNT(*) AS employee_count
         FROM employee_skills es2
         WHERE es2.tenant_id = $1 AND es2.esco_skill_id IS NOT NULL
         GROUP BY es2.esco_skill_id, es2.proficiency_level`,
        [tenantId]
      );

      // Build skill→proficiency→count map
      const skillMap = new Map<string, Map<number, number>>();
      for (const row of employeeSkillsResult.rows) {
        const profMap = skillMap.get(row.esco_skill_id) ?? new Map<number, number>();
  ...
  private async analyzeHierarchy(
    ...
    const validationRules = (config.validation_rules ?? {}) as Record<string, unknown>;
    const maxSpan =
      typeof validationRules.max_span_of_control === 'number'
        ? validationRules.max_span_of_control
        : DEFAULT_MAX_SPAN_OF_CONTROL;
    const expectedDepth =
      typeof config.expected_org_depth === 'number'
        ? config.expected_org_depth
        : DEFAULT_EXPECTED_ORG_DEPTH;

    try {
      // Q6: Span of control
      const spanResult = await this.dbClient.query(
        `SELECT e.manager_id, COUNT(*) AS direct_reports,
      ...
      const orphanResult = await this.dbClient.query(
      ...
      // Q8: Hierarchy depth
      const depthResult = await this.dbClient.query(
        `SELECT MAX(ou.org_level) AS max_depth FROM org_units ou
         WHERE ou.tenant_id = $1 AND ou.is_active = true AND ou.deleted_at IS NULL`,
        [tenantId]
      );
  ...
  private async analyzeKpiCoverage(profileId: string): Promise<BlueprintFinding[]> {
    const findings: BlueprintFinding[] = [];

    try {
      // Q9: Processes without KPIs
      const result = await this.dbClient.query(
        `SELECT bp.id, bp.process_code, bp.process_name,
  ...
  private async loadTemplate(templateId: string): Promise<BlueprintTemplate | null> {
    const result = await this.dbClient.query(
      `SELECT id, profile_id, template_name, template_version, description,
  ...
  private async createRun(params: {
    ...
    const result = await this.dbClient.query(
      `INSERT INTO blueprint_runs (tenant_id, template_id, run_mode, status, input_config, created_by)
  ...
  private async updateRunStatus(runId: string, status: BlueprintRun['status']): Promise<void> {
  ...
  private async completeRun(runId: string): Promise<BlueprintRun> {
    const result = await this.dbClient.query(
  ...
  private buildSummary(results: BlueprintResult[]): BlueprintRunOutput['summary'] {
  ...
  private computeAvgProficiency(profMap: Map<number, number> | undefined): number {
    if (!profMap || profMap.size === 0) return 0;
    let totalLevel = 0;
    let totalCount = 0;
    for (const [level, count] of profMap) {
  ...
  private mapRun(row: Record<string, unknown>): BlueprintRun {
    return {
      id: row.id as string,
      tenantId: row.tenant_id as string,
      templateId: row.template_id as string,
      runMode: row.run_mode as BlueprintRun['runMode'],
      status: row.status as BlueprintRun['status'],
  ...
  private mapResult(row: Record<string, unknown>): BlueprintResult {
    return {
      id: row.id as string,
      runId: row.run_id as string,
      resultType: row.result_type as BlueprintResult['resultType'],
      entityType: (row.entity_type as string) ?? null,
      entityId: (row.entity_id as string) ?? null,
      severity: (row.severity as BlueprintResult['severity']) ?? null,
      title: row.title as string,
      description: (row.description as string) ?? null,
      suggestedAction: (row.suggested_action as Record<string, unknown>) ?? null,
