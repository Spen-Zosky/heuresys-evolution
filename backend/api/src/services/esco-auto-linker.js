...
function classifyConfidence(similarity) {
...
function getApiKey() {
    ...
    if (!key) {
        throw new Error('OPENAI_API_KEY not configured. Set the environment variable in the api-gateway container.');
    }
    return key;
}
...
export class EscoAutoLinkerService {
    ...
    async findSimilarSkills(text, topN = 5, threshold = THRESHOLD_LOW) {
        const embedding = await generateEmbedding(text);
        const vectorLiteral = `[${embedding.join(',')}]`;
        const result = await this.dbClient.query(`SELECT id, preferred_label, skill_type,
              1 - (embedding_en <=> $1::vector) AS similarity
       FROM esco_skills
       WHERE embedding_en IS NOT NULL
       ORDER BY embedding_en <=> $1::vector
       LIMIT $2`, [vectorLiteral, topN]);
        return result.rows
    ...
    async autoLinkBatch(skillTexts, _tenantId) {
        ...
        for (let i = 0; i < uniqueTexts.length; i++) {
            ...
            const vectorLiteral = `[${embedding.join(',')}]`;
            const queryResult = await this.dbClient.query(`SELECT id, preferred_label, skill_type,
                1 - (embedding_en <=> $1::vector) AS similarity
         FROM esco_skills
         WHERE embedding_en IS NOT NULL
         ORDER BY embedding_en <=> $1::vector
         LIMIT 5`, [vectorLiteral]);
            const matches = queryResult.rows
                .filter((row) => parseFloat(row.similarity) >= THRESHOLD_LOW)
                .map((row) => ({
                escoSkillId: row.id,
                preferredLabel: row.preferred_label,
                similarity: parseFloat(parseFloat(row.similarity).toFixed(4)),
    ...
    async linkImportedSkills(importJobId, tenantId) {
        // Load import job and extract skill texts from preview_data
        const jobResult = await this.dbClient.query('SELECT import_type, preview_data, status FROM import_jobs WHERE id = $1', [importJobId]);
        ...
        for (const result of linkResults) {
            ...
            await this.dbClient.query(`INSERT INTO import_skill_links
          (import_job_id, tenant_id, input_text, esco_skill_id, similarity, confidence)
         VALUES ($1, $2, $3, $4, $5, $6)`, [importJobId, tenantId, result.inputText, escoSkillId, similarity, result.confidence]);
    ...
    extractSkillTexts(previewData, importType) {
        const texts = [];
...
//# sourceMappingURL=esco-auto-linker.js.map