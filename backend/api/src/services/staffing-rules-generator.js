...
export class StaffingRulesGeneratorService {
     ...
     */
    async calculateOptimalStaffing(prototypeId, sizeClass) {
        const log = [];
        ...
        log.push(`Prototype found: ${prototype.name} (NACE ${prototype.nace_section}.${prototype.nace_division})`);
        ...
        for (const orgUnit of orgUnits) {
            ...
            for (const job of orgJobs) {
                ...
                const existingRule = existingRules.find((r) => r.org_unit_template_id === orgUnit.id && r.job_template_id === job.id);
                let min, max, recommended, mandatory;
                if (existingRule) {
                    // Use existing rule
        ...
        log.push(`Total staffing: ${totalMinHeadcount}-${totalMaxHeadcount}, recommended: ${totalRecommendedHeadcount}`);
        return {
            prototypeId,
            sizeClass,
            rules,
    ...
    calculateRoleStaffing(job, sizeClass) {
     ...
     */
    async getIndustryBenchmarks(naceCode) {
    ...
    async getCachedBenchmark(naceCode) {
    ...
    `, [naceCode]);
        ...
        return {
            naceCode,
            naceName: proto.name,
            sizeClass: proto.company_size_code?.toLowerCase() || 'medium',
            totalEmployeeRange: {
                min: proto.min_employees || 50,
                max: proto.max_employees || 249,
            },
            ratios: [],
            spanOfControl: soc.map((optimal, index) => ({
                level: index + 1,
                levelName: ['CEO', 'Director', 'Manager', 'Team Lead', 'Supervisor', 'Staff'][index] ||
                    `Level ${index + 1}`,
    ...
    generateGenericBenchmark(naceCode) {
        return {
            naceCode,
     ...
     */
    async validateStaffingRatios(staffingPlan) {
        const issues = [];
        const warnings = [];
        ...
        if (maxUnitHeadcount > avgUnitHeadcount * 3) {
            ...
            recommendations.push('Review the org unit with maximum headcount for potential restructuring');
        }
        // Validate against prototype rules if provided
        if (staffingPlan.prototypeId) {
            const optimalStaffing = await this.calculateOptimalStaffing(staffingPlan.prototypeId, staffingPlan.sizeClass);
            // Check against recommended staffing
            const deviation = Math.abs(staffingPlan.totalHeadcount - optimalStaffing.totalRecommendedHeadcount);
            ...
            for (const rule of optimalStaffing.rules) {
                if (rule.isMandatory) {
                    ...
                    const matchingUnit = staffingPlan.byOrgUnit.find((u) => optimalStaffing.breakdown.find((b) => b.orgUnitCode === u.orgUnitCode &&
        ...
        for (const unit of staffingPlan.byOrgUnit) {
            const managementRoles = unit.roles.filter((r) => r.jobCode.includes('-DIR') || r.jobCode.includes('-MGR') || r.jobCode.includes('-EXEC'));
            const nonManagementRoles = unit.roles.filter((r) => !r.jobCode.includes('-DIR') && !r.jobCode.includes('-MGR') && !r.jobCode.includes('-EXEC'));
     ...
     */
    async getStaffingRules(prototypeId, sizeClass) {
        let query = `
      SELECT
        psr.*,
    ...
    `;
        ...
        return result.rows.map((row) => ({
            ...
            companySize: row.company_size.toLowerCase(),
    ...
    async getExistingRules(prototypeId, sizeClass) {
        const result = await pool.query(`
    ...
    async getOrgUnitTemplates(prototypeId) {
        const result = await pool.query(`
      SELECT out.*
    ...
    async getJobTemplates(prototypeId) {
        const result = await pool.query(`
      SELECT jt.*
...
// FACTORY FUNCTION
// =============================================================================
export function createStaffingRulesGeneratorService(tenantId) {
    return new StaffingRulesGeneratorService(tenantId);
}
export default StaffingRulesGeneratorService;
//# sourceMappingURL=staffing-rules-generator.js.map