...
export class PayrollIntegrationService {
   ...
   */
  async createIntegration(config: PayrollIntegrationConfig): Promise<string> {
   ...
   */
  async getIntegration(integrationId: string): Promise<Record<string, unknown> | null> {
    const result = await pool.query(
      `SELECT * FROM payroll_integrations WHERE id = $1 AND tenant_id = $2`,
  ...
  async listIntegrations(): Promise<Record<string, unknown>[]> {
    const result = await pool.query(
      `SELECT id, provider_name, provider_code, display_name, integration_type,
  ...
  async updateIntegration(
    integrationId: string,
    updates: Partial<PayrollIntegrationConfig>
  ): Promise<void> {
    ...
    await pool.query(
      `UPDATE payroll_integrations SET ${setClauses.join(', ')}
  ...
  private async testApiConnection(config: Record<string, unknown>): Promise<{
    success: boolean;
    message: string;
    details?: Record<string, unknown>;
  }> {
    // Simulate API test - in production would use actual provider SDK
    const endpoint = config.api_endpoint as string;
    if (!endpoint) {
  ...
  private async testSftpConnection(config: Record<string, unknown>): Promise<{
    success: boolean;
    message: string;
    details?: Record<string, unknown>;
  }> {
    const host = config.sftp_host as string;
    if (!host) {
   ...
   */
  async createExportJob(config: ExportJobConfig, createdBy?: string): Promise<string> {
    // Generate job number: PAY-YYYY-MM-NNN
    const jobNumberResult = await pool.query(
      `SELECT COUNT(*) as count FROM payroll_export_jobs
       WHERE tenant_id = $1 AND pay_period_year = $2 AND pay_period_month = $3`,
      [this.tenantId, config.payPeriodYear, config.payPeriodMonth]
    );
    ...
    const result = await pool.query(
      `INSERT INTO payroll_export_jobs (
   ...
   */
  async getExportJob(jobId: string): Promise<Record<string, unknown> | null> {
    const result = await pool.query(
      `SELECT j.*, i.provider_name, i.display_name as integration_name
  ...
  async listExportJobs(options: {
    ...
    const countResult = await pool.query(
      `SELECT COUNT(*) as total FROM payroll_export_jobs j WHERE ${whereClause}`,
      values
    );
   ...
   */
  async generateExport(jobId: string): Promise<ExportResult> {
    ...
    try {
      ...
      const integration = job.integration_id
        ? await this.getIntegration(job.integration_id as string)
        : null;

      const format = (integration?.export_format as ExportFormat) || 'csv';
      const sections = job.export_sections as ExportSection[];

      // Collect employees for export
      const employeesResult = await pool.query(
        `SELECT e.*, c.contract_type, c.ccnl_type, c.ccnl_level, c.gross_annual_salary,
      ...
      for (const employee of employees) {
        ...
        const exportRecord = await this.buildEmployeeExportRecord(
          employee,
          sections,
          job.period_start_date as Date,
          job.period_end_date as Date
        );
      ...
      await pool.query(
        `INSERT INTO payroll_export_files (
          job_id, tenant_id, file_type, file_name, file_path, file_size, file_hash, mime_type
        ) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`,
        [jobId, this.tenantId, 'main_export', fileName, filePath, fileSize, fileHash, 'text/csv']
      );

      // Update job with results
      await pool.query(
        `UPDATE payroll_export_jobs SET
          status = 'generated',
          ...
          progress_percent = 80
         WHERE id = $7`,
        [filePath, fileSize, fileHash, fileName, employees.length, totalExported, jobId]
      );
  ...
  private async buildEmployeeExportRecord(
    ...
    if (sections.includes('anagrafica')) {
      record.DTNAS = this.formatDate(employee.birth_date as Date | null);
      record.SESSO = employee.gender;
      record.EMAIL = employee.email;
      record.DTASS = this.formatDate(employee.hire_date as Date | null);
    ...
    if (sections.includes('presenze')) {
      ...
      const attendance = await pool.query(
        `SELECT SUM(hours_regular) as total_hours, COUNT(*) as days_present
         FROM employee_attendance
         WHERE employee_id = $1 AND tenant_id = $2
         AND attendance_date BETWEEN $3 AND $4`,
        [employee.id, this.tenantId, periodStart, periodEnd]
      );
    ...
    if (sections.includes('assenze')) {
      ...
      const leaves = await pool.query(
        `SELECT leave_type, SUM(days_requested) as days
  ...
  private formatExportData(
    ...
    const delimiter = (integration?.field_delimiter as string) || ';';
    ...
    switch (format) {
      ...
      case 'xml': {
        const xmlLines: string[] = ['<?xml version="1.0" encoding="UTF-8"?>', '<payroll_export>'];
        for (const record of data) {
   ...
   */
  async validateExport(jobId: string): Promise<ValidationResult> {
    ...
    try {
      ...
      const rulesResult = await pool.query(
        `SELECT * FROM payroll_validation_rules
         WHERE (tenant_id IS NULL OR tenant_id = $1) AND is_active = true
         ORDER BY is_blocking DESC, category, rule_code`,
        [this.tenantId]
      );
      ...
      const status: JobStatus = blockedCount > 0 ? 'validation_failed' : 'validation_complete';
  ...
  private async applyValidationRule(
    ...
    switch (ruleType) {
      ...
      case 'value_range': {
        ...
        const min = ruleConfig.min as number | undefined;
        const max = ruleConfig.max as number | undefined;
      ...
      case 'comparison': {
        const check = ruleConfig.check as string;

        if (check === 'contract_covers_period') {
          const contractStart = employee.contract_start as Date | null;
          const contractEnd = employee.contract_end as Date | null;
        ...
        if (check === 'all_approved') {
          const pending = await pool.query(
            `SELECT COUNT(*) as count FROM employee_time_off_requests
             WHERE employee_id = $1 AND tenant_id = $2
             AND status = 'pending'
             AND start_date <= $4 AND end_date >= $3`,
            [employee.id, this.tenantId, periodStart, periodEnd]
  ...
  private async detectAnomalies(
    ...
    const warnings: ValidationWarning[] = [];
    const periodStart = job.period_start_date as Date;
    const periodEnd = job.period_end_date as Date;

    // Calculate average overtime for comparison
    const overtimeResult = await pool.query(
      `SELECT AVG(total_ot) as avg_overtime, STDDEV(total_ot) as stddev_overtime
       FROM (
      ...
      [this.tenantId, periodStart, periodEnd]
    );
    ...
    for (const employee of employees) {
      const empOvertime = await pool.query(
        `SELECT SUM(hours) as total FROM employee_overtime
         WHERE employee_id = $1 AND tenant_id = $2
         AND overtime_date BETWEEN $3 AND $4`,
        [employee.id, this.tenantId, periodStart, periodEnd]
      );
   ...
   */
  async transmitExport(
    jobId: string,
    options: { confirmTransmission: boolean; method?: string }
    ...
    await pool.query(
      `UPDATE payroll_export_jobs SET transmission_started_at = NOW() WHERE id = $1`,
      [jobId]
    );
    ...
    try {
      ...
      await pool.query(
        `UPDATE payroll_transmission_log SET
      ...
      if (transmissionResult.success) {
        ...
        await pool.query(
          `UPDATE payroll_export_jobs SET
            ...
            last_retry_at = NOW()
           WHERE id = $3`,
          [transmissionResult.errorCode, transmissionResult.errorMessage, jobId]
        );
      }
  ...
  private async transmitViaApi(job: Record<string, unknown>): Promise<TransmissionResult> {
    ...
    return {
      ...
      recordsAccepted: job.exported_employees as number,
      recordsRejected: 0,
    };
  }

  private async transmitViaSftp(job: Record<string, unknown>): Promise<TransmissionResult> {
  ...
  private async logTransmission(jobId: string, method: string, _phase: string): Promise<string> {
    const result = await pool.query(
      `INSERT INTO payroll_transmission_log (
        job_id, tenant_id, transmission_type, request_timestamp
      ) VALUES ($1, $2, $3, NOW()) RETURNING id`,
      [jobId, this.tenantId, method]
    );
    return result.rows[0].id;
  }
   ...
   */
  async completeExport(jobId: string): Promise<void> {
    await pool.query(
      `UPDATE payroll_export_jobs SET
        status = 'completed',
  ...
  async getExportHistory(options: {
    ...
    const summaryResult = await pool.query(
      `SELECT
       ...
       WHERE tenant_id = $1
       ${options.year ? 'AND pay_period_year = $2' : ''}`,
      options.year ? [this.tenantId, options.year] : [this.tenantId]
    );
   ...
   */
  async getExportFiles(jobId: string): Promise<Record<string, unknown>[]> {
    const result = await pool.query(
      `SELECT * FROM payroll_export_files
   ...
   */
  async getTransmissionLog(jobId: string): Promise<Record<string, unknown>[]> {
    const result = await pool.query(
      `SELECT * FROM payroll_transmission_log
       WHERE job_id = $1 AND tenant_id = $2
   ...
   */
  async compareWithPreviousPeriod(jobId: string): Promise<{
    ...
    const prevYear =
      current.pay_period_month === 1
        ? (current.pay_period_year as number) - 1
        : (current.pay_period_year as number);
    const prevMonth =
      current.pay_period_month === 1 ? 12 : (current.pay_period_month as number) - 1;

    const previousResult = await pool.query(
      `SELECT * FROM payroll_export_jobs
       WHERE tenant_id = $1
       AND pay_period_year = $2 AND pay_period_month = $3
       AND status = 'completed'
       ORDER BY created_at DESC LIMIT 1`,
      [this.tenantId, prevYear, prevMonth]
    );
    ...
    if (previous) {
      differences.push({
        ...
        change: (current.total_employees as number) - (previous.total_employees as number),
      });
      differences.push({
        metric: 'exported_employees',
        current: current.exported_employees,
        previous: previous.exported_employees,
        change: (current.exported_employees as number) - (previous.exported_employees as number),
      });
    }
  ...
  async getAnnualSummary(year: number): Promise<{
    ...
    const summaryResult = await pool.query(
      `SELECT
       ...
       WHERE tenant_id = $1
       AND job_id IN (SELECT id FROM payroll_export_jobs WHERE pay_period_year = $2)`,
      [this.tenantId, year]
    );
  ...
  private async updateJobStatus(
    ...
    await pool.query(
      `UPDATE payroll_export_jobs SET ${updates.join(', ')}
       WHERE id = $${paramIndex} AND tenant_id = $${paramIndex + 1}`,
      values
    );
  }
   ...
   * Get validation rules
   */
  async getValidationRules(): Promise<Record<string, unknown>[]> {
    const result = await pool.query(
      `SELECT * FROM payroll_validation_rules
       WHERE (tenant_id IS NULL OR tenant_id = $1) AND is_active = true
   ...
   * Get field mappings
   */
  async getFieldMappings(integrationId?: string): Promise<Record<string, unknown>[]> {
    const result = await pool.query(
      `SELECT * FROM payroll_field_mappings
       WHERE (tenant_id IS NULL OR tenant_id = $1)
       ${integrationId ? 'AND (integration_id IS NULL OR integration_id = $2)' : ''}
       AND is_active = true
       ORDER BY section, priority`,
      integrationId ? [this.tenantId, integrationId] : [this.tenantId]
    );
    return result.rows;
  }
  ...
  async getStatistics(): Promise<Record<string, unknown>> {
    const result = await pool.query(
      `SELECT
        ...
        (SELECT SUM(exported_employees) FROM payroll_export_jobs WHERE tenant_id = $1 AND status = 'completed') as total_employees_exported`,
      [this.tenantId]
    );
