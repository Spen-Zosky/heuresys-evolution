...
export class EmployeeAssignmentService {
     ...
     */
    async assignEmployeesToPositions(sessionId, orgChart) {
    ...
    `, [sessionId]);
        try {
            ...
            const managerAssignments = await this.assignByManagerChain(sessionId, employees, positionSlots, orgChart);
            // Phase 2: Assign by job title similarity
            const unassignedAfterManagers = employees.filter((e) => !managerAssignments.assignedEmployeeIds.has(e.id));
            const titleAssignments = await this.assignByJobTitle(sessionId, unassignedAfterManagers, positionSlots);
            // Phase 3: Assign by department
            const unassignedAfterTitle = unassignedAfterManagers.filter((e) => !titleAssignments.assignedEmployeeIds.has(e.id));
            const deptAssignments = await this.assignByDepartment(sessionId, unassignedAfterTitle, positionSlots);
            // Phase 4: Assign remaining to level 7 positions
            const stillUnassigned = unassignedAfterTitle.filter((e) => !deptAssignments.assignedEmployeeIds.has(e.id));
     ...
     */
    async assignByManagerChain(sessionId, employees, positionSlots, _orgChart) {
        ...
        // Assign CEO
        if (potentialCEO) {
            const ceoSlot = Array.from(positionSlots.values()).find((s) => s.position.level === 1 && s.remainingSlots > 0);
            if (ceoSlot) {
     ...
     * Phase 2: Assign by job title similarity
     */
    async assignByJobTitle(sessionId, employees, positionSlots) {
        const assignedEmployeeIds = new Set();
        for (const employee of employees) {
            ...
            if (matchingSlot && matchingSlot.remainingSlots > 0) {
                const confidence = this.calculateTitleMatchConfidence(employee.jobTitle, matchingSlot.position.titleIt, matchingSlot.position.titleEn);
                await this.createStagingRecord(sessionId, employee, matchingSlot, 'job_title_match', confidence);
     ...
     * Phase 3: Assign by department
     */
    async assignByDepartment(sessionId, employees, positionSlots) {
        const assignedEmployeeIds = new Set();
        for (const employee of employees) {
            if (!employee.department && !employee.orgUnitId)
                continue;
            // Find unit matching department
            const matchingSlot = this.findSlotByDepartment(positionSlots, employee.department || '', employee.orgUnitId);
            if (matchingSlot && matchingSlot.remainingSlots > 0) {
     ...
     */
    async assignRemainingToStaff(sessionId, employees, positionSlots) {
    ...
    // ---------------------------------------------------------------------------
    async createStagingRecord(sessionId, employee, slot, method, confidence) {
     ...
     */
    async buildStagingHierarchy(sessionId, positionSlots) {
    ...
    `, [sessionId]);
        ...
        const positionToStaging = new Map();
        ...
        for (const record of stagingRecords) {
            if (record.hierarchy_level === 1)
                continue; // CEO reports to no one
            ...
            if (!managerId && slot) {
                const parentUnit = Array.from(positionSlots.values()).find((s) => s.unit.code === slot.unit.parentCode);
                if (parentUnit) {
            ...
            if (!managerId) {
                const higherLevelRecord = stagingRecords.find((r) => r.hierarchy_level === record.hierarchy_level - 1);
    ...
    // HELPER METHODS
    // ---------------------------------------------------------------------------
    estimateLevelFromReports(directReports, jobTitle) {
        // Level estimation based on direct reports
        if (directReports > 20)
            return 2; // C-Level
        if (directReports > 10)
            return 3; // VP/Director
        if (directReports > 5)
            return 4; // Director
        if (directReports > 2)
            return 5; // Manager
        ...
        if (jobTitle) {
            ...
            if (title.includes('ceo') || title.includes('amministratore delegato'))
                return 1;
            ...
            if (title.includes('director') ||
                ...
                return 4;
            if (title.includes('manager') || title.includes('capo'))
                return 5;
        ...
        const candidates = Array.from(positionSlots.values())
            ...
            if (employee.department) {
                ...
                const deptMatchB = b.unit.name.toLowerCase().includes(employee.department.toLowerCase())
                    ? 0
                    : 1;
                if (deptMatchA !== deptMatchB)
                    return deptMatchA - deptMatchB;
            }
            return 0;
        });
        return candidates[0] || null;
    }
    findSlotByTitleSimilarity(positionSlots, jobTitle) {
    ...
    findSlotByDepartment(positionSlots, orgUnitName, _orgUnitId) {
        ...
        for (const slot of positionSlots.values()) {
            ...
            if (unitName.includes(deptLower) ||
    ...
    determineChangeType(employee, slot) {
    ...
    calculateDiffFields(employee, slot) {
        const diffs = [];
    ...
    calculateTitleMatchConfidence(originalTitle, targetTitleIt, targetTitleEn) {
        const original = originalTitle.toLowerCase();
        const targetIt = targetTitleIt.toLowerCase();
        const targetEn = targetTitleEn.toLowerCase();
        if (original === targetIt || original === targetEn)
            return 1.0;
        if (targetIt.includes(original) || targetEn.includes(original))
            return 0.9;
        if (original.includes(targetIt) || original.includes(targetEn))
            return 0.8;
        // Word overlap
    ...
    // STATISTICS
    // ---------------------------------------------------------------------------
    async calculateAssignmentStats(sessionId) {
        const result = await pool.query(`
      SELECT
        COUNT(*) as total,
...
// FACTORY FUNCTION
// =============================================================================
export function createEmployeeAssignmentService(tenantId) {
