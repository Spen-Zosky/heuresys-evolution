...
export class SuccessionPlanningService {
    ...
    // --------------------------------------------------------------------------
    async getSuccessionPlan(tenantId, roleId, options = {}) {
    ...
    `;
        ...
        const candidates = await this.findCandidates(tenantId, requirements, minSkillMatch, maxCandidates, options.org_unit_id);
        // Generate development plans if requested
        if (includeDevelopmentPlans) {
            for (const candidate of candidates) {
                candidate.development_plan = await this.generateDevelopmentPlan(tenantId, candidate.skill_gaps);
            }
        }
        // Calculate risk assessment
        const riskAssessment = this.assessRoleRisk(role, candidates);
        // Calculate summary
        const qualifiedCandidates = candidates.filter((c) => c.skill_match_percentage >= minSkillMatch * 100);
        const avgReadiness = candidates.length > 0
            ? candidates.reduce((sum, c) => sum + c.readiness_score, 0) / candidates.length
            : 0;
        const avgSkillMatch = candidates.length > 0
    ...
    // --------------------------------------------------------------------------
    async findCandidates(tenantId, requirements, minSkillMatch, maxCandidates, orgUnitId) {
    ...
    `;
        const params = [tenantId, skillIds];
        let paramIndex = 3;
        if (orgUnitId) {
    ...
    `;
      ...
      `;
            ...
            const employeeSkills = new Map(skillMatchResult.rows.map((s) => [
                s.skill_id,
            ...
            if (skillMatchPercentage < minSkillMatch * 100)
                continue;
    ...
    // --------------------------------------------------------------------------
    calculateReadinessScore(skillMatchPercentage, skillGaps) {
    ...
    // --------------------------------------------------------------------------
    determineReadinessLevel(readinessScore, skillGaps) {
    ...
    // --------------------------------------------------------------------------
    async generateDevelopmentPlan(tenantId, skillGaps) {
        const plan = [];
        for (const gap of skillGaps) {
            ...
            const priority = gap.importance === 'critical' ? 'high' : gap.importance === 'high' ? 'medium' : 'low';
    ...
    // Assess Role Risk
    // --------------------------------------------------------------------------
    assessRoleRisk(role, candidates) {
