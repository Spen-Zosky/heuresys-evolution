...
// HELPERS
// =============================================================================
function classifyPosition(tenantValue, industryAvg) {
    if (tenantValue === null || industryAvg === null || industryAvg === 0)
        return 'no_data';
 ...
 */
async function fetchPeerMetrics(naceCode, excludeTenantId) {
    ...
    return {
        ...
        metrics: {
            avg_employees_per_org_unit: row.avg_employees_per_org_unit != null ? parseFloat(row.avg_employees_per_org_unit) : null,
            avg_skills_per_employee: row.avg_skills_per_employee != null ? parseFloat(row.avg_skills_per_employee) : null,
            ...
            avg_kpi_count_per_process: row.avg_kpi_count_per_process != null ? parseFloat(row.avg_kpi_count_per_process) : null,
        },
    };
}
/**
 ...
 */
async function fetchTenantMetrics(tenantId) {
    const result = await pool.query(`
    WITH org_metrics AS (
...
export class BenchmarkingService {
    tenantId;
    tenantName;
    constructor(tenantId, tenantName) {
     ...
     */
    async getIndustryBenchmark(naceCode) {
        try {
            const peer = await fetchPeerMetrics(naceCode, this.tenantId);
            const metrics = Object.entries(METRIC_LABELS).map(([key, meta]) => ({
                metric: key,
                label: meta.label,
                unit: meta.unit,
    ...
    async getTenantComparison() {
        try {
            ...
            const tenantResult = await pool.query(`SELECT t.company_size, ip.nace_class_code
         FROM tenants t
         LEFT JOIN industry_profiles ip ON ip.id = t.industry_profile_id
         WHERE t.id = $1`, [this.tenantId]);
            const tenantRow = tenantResult.rows[0] ?? {};
            const naceCode = tenantRow.nace_class_code ?? null;
            ...
            const metricsComparison = Object.entries(METRIC_LABELS).map(([key, meta]) => {
                const tenantValue = tenantMetrics[key] ?? null;
                const industryAvg = peer.metrics[key] ?? null;
                return {
                    metric: key,
                    label: meta.label,
                    unit: meta.unit,
