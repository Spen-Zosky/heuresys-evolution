 ...
 */
export function validateTableName(tableName, context) {
    if (!ALLOWED_TABLES.has(tableName)) {
        throw new Error(`[SQL_SAFETY] Invalid table name '${tableName}' in ${context}. ` +
 ...
 */
export function validateIdentifier(identifier, context) {
    if (!ALLOWED_IDENTIFIER_PATTERN.test(identifier)) {
        throw new Error(`[SQL_SAFETY] Invalid SQL identifier '${identifier}' in ${context}. ` +
 ...
 */
const ALLOWED_EMBEDDING_COLUMNS = new Set([
    // Embedding columns
    'embedding',
    'embedding_en',
    'embedding_it',
 ...
 */
export function validateEmbeddingColumn(columnName, context) {
    if (!ALLOWED_EMBEDDING_COLUMNS.has(columnName)) {
        throw new Error(`[SQL_SAFETY] Invalid embedding column '${columnName}' in ${context}. ` +
 ...
 */
export function escapeILIKE(input) {
    return input
