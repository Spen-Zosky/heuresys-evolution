...
import { INFOTYPE_DEFINITIONS, VALUE_TRANSFORMERS, PA_INFOTYPES, HRP_INFOTYPES } from './infotype-definitions.js';
/**
...
export class SAPParser {
     ...
     * Detect infotype from filename or headers
     */
    detectInfotype(filename, headers) {
        // Check filename
        if (filename) {
     ...
     */
    mapToRecord(headers, values, definition, lineNumber) {
        const record = {};
     ...
     */
    async parseXML(content, _options = {}) {
        // Simple XML parser for SAP IDoc format
        const data = [];
        const recordsByInfotype = {};
        const infotypesFound = new Set();
     ...
     */
    detectInfotypeFromSegment(segmentName, record) {
        // E1P0001 -> PA0001
     ...
     * Parse JSON content
     */
    async parseJSON(content, _options = {}) {
     ...
     */
    getInfotypeDefinition(infotype) {
        if (!infotype)
            return null;
     ...
     */
    validateRecord(record, infotype) {
...
//# sourceMappingURL=sap-parser.js.map