...
import { HeuresysError, ErrorCodes, DatabaseContext } from './types.js'
...
export interface PostgresError extends Error {
  ...
  detail?: string         // Additional detail message
  hint?: string           // Hint for fixing the problem
  position?: string       // Error position in query
  internalPosition?: string
  internalQuery?: string
  where?: string          // Context in PL/pgSQL
 ...
 */
export const PostgresErrorMapping: Record<string, {
  heuresysCode: string
  httpStatus: number
 ...
 */
export function extractDatabaseContext(error: PostgresError): DatabaseContext {
  return {
 ...
 */
function extractFromMessage(message: string): { table?: string | undefined; column?: string | undefined; constraint?: string | undefined } {
  const result: { table?: string | undefined; column?: string | undefined; constraint?: string | undefined } = {}

  // Pattern per estrarre tabella: "relation \"tablename\""
  const tableMatch = message.match(/relation "([^"]+)"/i)
  if (tableMatch && tableMatch[1]) result.table = tableMatch[1]

  // Pattern per estrarre colonna: "column \"columnname\""
  const columnMatch = message.match(/column "([^"]+)"/i)
  if (columnMatch && columnMatch[1]) result.column = columnMatch[1]

  // Pattern per estrarre constraint: "constraint \"constraintname\""
  const constraintMatch = message.match(/constraint "([^"]+)"/i)
  if (constraintMatch && constraintMatch[1]) result.constraint = constraintMatch[1]
 ...
 */
function getConstraintMessage(constraint: string, table?: string, column?: string): string {
 ...
 */
export function handleDatabaseError(error: PostgresError, query?: string): HeuresysError {
  ...
  switch (sqlState) {
    ...
    case '23503': // FOREIGN KEY violation
      userMessage = constraint
        ? getConstraintMessage(constraint, table, column)
        : `Impossibile ${error.detail?.includes('update or delete') ? 'eliminare/modificare' : 'inserire'} il record - ${
            error.detail?.includes('update or delete')
 ...
 */
function getSuggestionForError(sqlState: string, _table?: string, column?: string, _constraint?: string): string {
  switch (sqlState) {
    case '23505':
      return column
...
export function isPostgresError(error: unknown): error is PostgresError {
  return (
    ...
    (typeof (error as any).code === 'string' ||
      ['ERROR', 'FATAL', 'PANIC', 'WARNING', 'NOTICE'].includes((error as any).severity))
  )
}
