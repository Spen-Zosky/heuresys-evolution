...
export type ErrorCategory =
  ...
  | 'RESOURCE'      // Resource not found/access
  | 'RATE_LIMIT'    // Rate limiting
  | 'TENANT'        // Multi-tenant errors
  | 'CONFIG'        // Configuration errors
  | 'PERMISSION'    // Permission/access control errors
  | 'SYSTEM'        // System-level errors
  | 'AI'            // AI-related errors
...
// ============================================================================
// ERROR CONTEXT - Request & Debug Information
// ============================================================================
...
export interface DatabaseContext {
  ...
  sqlState?: string | undefined;  // Alias for errorCode (PostgreSQL SQLSTATE)
...
export interface HeuresysError {
  ...
  code: string;                         // Error code (e.g., ERR-DB-3005)
  ...
  // Messages
  message: string;                      // User-friendly message (IT)
  messageEN?: string | undefined;       // English message (optional)
  technicalMessage?: string | undefined;// Technical details (dev only)

  // Context
  requestContext?: RequestContext | undefined;
  debugContext?: DebugContext | undefined;      // Only in development
  databaseContext?: DatabaseContext | undefined;// Only for DB errors

  // Additional details
  details?: Record<string, unknown> | undefined;// Additional context data

  // Resolution guidance
  retryable: boolean;                   // Can retry the operation
  retryAfter?: number | undefined;      // Seconds to wait before retry
  suggestion?: string | undefined;      // What to try (IT)
  suggestionEN?: string | undefined;    // English suggestion
  ...
  // Metadata
  timestamp: string;
  correlationId?: string | undefined;   // For distributed tracing
  parentErrorId?: string | undefined;   // For error chains
}
...
export const ErrorMessageRegistry: Record<string, ErrorMessages> = {
  // Database Errors
  'ERR-DB-3001': {
...
export const PostgresErrorMapping: Record<string, { code: string; category: ErrorCategory; severity: ErrorSeverity }> = {
