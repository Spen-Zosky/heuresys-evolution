  ...
  try {
    ...
    Sentry.init({
      ...
      beforeSend(event, _hint) {
        ...
        if (event.extra) {
          event.extra = filterSensitiveData(event.extra as Record<string, any>);
        }
        ...
        if (event.request?.data) {
          event.request.data = filterSensitiveData(
 ...
 */
export function sentryErrorLogger(error: HeuresysError): void {
...
export function captureException(
  ...
  Sentry.withScope((scope) => {
    if (context) {
      scope.setContext('additional', filterSensitiveData(context as Record<string, any>));
    }
...
export function setUser(
  ...
  Sentry.setUser({
    ...
    ...(additionalData && filterSensitiveData(additionalData as Record<string, any>)),
  });
}
...
export function addBreadcrumb(
  ...
  if (data) {
    breadcrumb.data = filterSensitiveData(data as Record<string, any>);
  }
...
export async function flush(timeout: number = 2000): Promise<boolean> {
  if (!isEnabled || !isInitialized) return true;
