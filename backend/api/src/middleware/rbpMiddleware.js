...
async function l2Set(roleCode, ctx) {
    try {
        const redis = getRedis();
        if (!redis)
            return;
...
// ---------------------------------------------------------------------------
export function invalidateRbpCache(roleCode) {
    if (roleCode) {
        l1Cache.delete(roleCode);
    }
    else {
...
// ---------------------------------------------------------------------------
async function loadRbpContext(roleCode) {
    ...
    const permissions = permResult.rows.map((r) => ({
    ...
    // 2. Get scope rules
    const scopeResult = await pool.query(`SELECT sr.scope_type, sr.sql_template, sr.parameters
     FROM rbp_scope_rules sr
     JOIN rbp_roles r ON r.id = sr.role_id
     WHERE r.code = $1`, [roleCode]);
    const scopeRules = scopeResult.rows;
    // 3. Get field policies
    const fpResult = await pool.query(`SELECT dc.code AS classification_code, fp.action
     ...
     WHERE r.code = $1
     ORDER BY dc.sensitivity_level`, [roleCode]);
    const fieldPolicies = fpResult.rows;
    // 4. Get role metadata
    const roleResult = await pool.query(`SELECT r.is_system_role, r.default_dashboard_code, d.layout_path
     FROM rbp_roles r
    ...
    const dashResult = await pool.query(`SELECT d.code
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const dashboards = dashResult.rows.map((r) => r.code);
    return {
        permissions,
        scopeRules,
        fieldPolicies,
...
// ---------------------------------------------------------------------------
async function getRbpContext(roleCode) {
    // L1 check
...
export function requirePermission(area, action) {
    ...
    if (!USE_RBP_FRAMEWORK) {
        const legacyRole = mapAreaToLegacyRole(area);
...
// ---------------------------------------------------------------------------
export function applyScopeFilter(area) {
    if (!USE_RBP_FRAMEWORK)
    ...
    return async (req, res, next) => {
        try {
            ...
            const ctx = authReq.rbpContext;
            if (!ctx)
                return next(); // No RBP context = no filtering
...
// ---------------------------------------------------------------------------
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function resolveSessionValue(req, path) {
    switch (path) {
...
// ---------------------------------------------------------------------------
function mapAreaToLegacyRole(area) {
    const areaToRole = {
        PLATFORM: 'SUPERUSER',
...
// ---------------------------------------------------------------------------
export async function getUserPermissions(roleCode) {
...
// ---------------------------------------------------------------------------
export async function getDashboardNavItems(dashboardCode) {
    const result = await pool.query(`SELECT
     ...
     ORDER BY ni.section, ni.sort_order`, [dashboardCode]);
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
...
export function applyFieldPolicy() {
    ...
    return (req, res, next) => {
        const authReq = req;
        const ctx = authReq.rbpContext;
        if (!ctx || ctx.fieldPolicies.length === 0)
            return next();
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function maskFields(data, policyMap) {
    if (data === null || data === undefined)
        return data;
    if (Array.isArray(data))
        return data.map((item) => maskFields(item, policyMap));
...
//# sourceMappingURL=rbpMiddleware.js.map