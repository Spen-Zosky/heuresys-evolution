 ...
 */
async function getTenant(identifier, type) {
 ...
 */
async function setTenantRLS(tenantId) {
 ...
 */
async function acquireTenantClient(tenantId) {
...
export async function tenantContextMiddleware(req, _res, next) {
    try {
        let tenantId;
        let tenantCode;
        ...
        if (!tenantId && !tenantCode) {
            ...
            tenantCode = (req.params['tenantCode'] || req.params['code']);
        }
        ...
        if (user) {
            ...
            if (!isSuperuser && userTenantId && userTenantId !== tenant.id) {
                logger.warn(`[SECURITY] Cross-tenant access attempt blocked: ` +
                ...
                throw createAppError('Cross-tenant access denied: you do not have permission to access this tenant', 403, ErrorCodes.FORBIDDEN, {
...
export function requireTenant(req, _res, next) {
    if (!req.tenantId && !req.allTenants) {
        return next(createAppError('Tenant context required', 400, ErrorCodes.VALIDATION_ERROR, {
 ...
 */
export function clearTenantCache(tenantId, tenantCode) {
    if (tenantId) {
 ...
 */
export function getTenantIdOrThrow(req) {
    if (!req.tenantId) {
 ...
 */
export function getTenantIdOrAll(req) {
    if (req.allTenants) {
