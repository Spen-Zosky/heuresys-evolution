...
async function loginAs(page: Page, username: string, password: string, expectRedirect: string) {
...
test.describe('Tenant Selector — Role-Based Visibility', () => {
  // This test does NOT use shared auth state — each test logs in independently
  test.use({ storageState: { cookies: [], origins: [] } })

  for (const user of USERS) {
    test(`${user.label}: tenant selector ${user.expectTenantSelector ? 'VISIBILE' : 'NON visibile'}`, async ({ page }) => {
      await loginAs(page, user.username, user.password, user.expectRedirect)

      // Verify we landed on the correct page
      expect(page.url()).toContain(user.expectRedirect)
      ...
      if (user.expectTenantSelector) {
        ...
        await expect(tenantButton.first()).toBeVisible({ timeout: 5000 })
        ...
        await expect(tuttiOption).toBeVisible({ timeout: 5000 })

        // Verifica che ci siano anche i tenant individuali
        await expect(page.locator('[role="menuitem"]:has-text("RTL Bank")')).toBeVisible()
        await expect(page.locator('[role="menuitem"]:has-text("SmartFood")')).toBeVisible()
        await expect(page.locator('[role="menuitem"]:has-text("EcoNova")')).toBeVisible()
        await expect(page.locator('[role="menuitem"]:has-text("Heuresys System")')).toBeVisible()
        ...
        await expect(tenantButton).toHaveCount(0)
        ...
        await expect(outlineButtons).toHaveCount(0)
      }
  ...
  test('SUPERUSER: switch tenant filtra i dati', async ({ page }) => {
    ...
    await expect(updatedButton).toBeVisible({ timeout: 10000 })
