...
test.describe('Wireframe Preview Component', () => {
  test('should render SVG wireframe viewer', async ({ page }) => {
    ...
    expect(excalidrawInfo.hasContainer).toBe(true);
  });
  ...
  test('should switch between wireframes', async ({ page }) => {
    await page.goto('/admin/design/wireframes', {
      waitUntil: 'domcontentloaded',
      timeout: 60000
    });
    ...
    expect(await canvas1.isVisible().catch(() => false)).toBe(true);
    ...
    expect(await canvas2.isVisible().catch(() => false)).toBe(true);
