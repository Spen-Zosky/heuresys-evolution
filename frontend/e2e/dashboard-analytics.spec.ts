 ...
 */
test.describe('4. Dashboard - Tab Analytics', () => {
  ...
  test('4.1 Analytics Tab - Verify RecruitingFunnelChart displays real pipeline data', async ({ page, request }) => {
    ...
    expect(api.data.recruiting_pipeline).toBeDefined()
    expect(Array.isArray(api.data.recruiting_pipeline)).toBeTruthy()
  ...
  test('4.2 Verify GoalCompletionChart shows real data', async ({ page, request }) => {
    // Get overview data
    const res = await request.get(`${API_BASE}/api/v1/dashboard/overview`, { headers: HEADERS })
    if (!res.ok()) { console.log('[SKIP] API returned', res.status()); return }
    const api = await res.json()
    if (!api.data?.goals) { console.log('[SKIP] No goals data'); return }
  ...
  test('4.3 Verify recruiting pipeline consistency with database', async ({ request }) => {
    // Get pipeline from HR metrics
    const hrRes = await request.get(`${API_BASE}/api/v1/dashboard/hr-metrics`, { headers: HEADERS })
    if (!hrRes.ok()) { console.log('[SKIP] API returned', hrRes.status()); return }
    const hrData = await hrRes.json()
    if (!hrData.data?.recruiting_pipeline) { console.log('[SKIP] No recruiting pipeline data'); return }
    ...
    if (appsRes.ok()) {
      ...
      if (appsData.meta?.total) {
        ...
        expect(totalFromApi).toBe(dbTotal)
      }
    }
  })
})
