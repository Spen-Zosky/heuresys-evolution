 ...
 */
test.describe('Compensation & Benefits (E-COMP-01)', () => {
  ...
  test.describe('1. Salary Bands API', () => {

    test('1.1 Get Salary Bands - Returns band catalog', async ({ request }) => {
      ...
      if (data.data.length > 0) {
        ...
        expect(band).toHaveProperty('band_name')
        expect(band).toHaveProperty('min_salary')
        expect(band).toHaveProperty('max_salary')
        console.log(`[Salary Bands]: ${data.meta.total} bands in system`)
      }
    })

    test('1.2 Get Salary Band Stats - Returns statistics', async ({ request }) => {
      ...
      expect(data.data).toHaveProperty('job_levels')

      console.log(`[Band Stats]: ${data.data.total} total, ${data.data.active} active, ${data.data.job_levels} job levels`)
    })

    test('1.3 Get Single Salary Band - Returns details', async ({ request }) => {
      ...
      if (listData.data?.length > 0) {
        ...
        expect(data).toHaveProperty('success', true)
        expect(data).toHaveProperty('data')
        expect(data.data).toHaveProperty('id', bandId)
        expect(data.data).toHaveProperty('assignment_count')
  ...
  test.describe('2. Bonus Plans API', () => {

    test('2.1 Get Bonus Plans - Returns plan list', async ({ request }) => {
      ...
      if (data.data.length > 0) {
        ...
        expect(plan).toHaveProperty('bonus_type')
        expect(plan).toHaveProperty('status')
        console.log(`[Bonus Plans]: ${data.meta.total} plans in system`)
      }
    })

    test('2.2 Get Bonus Plan Stats - Returns statistics', async ({ request }) => {
    ...
    test('2.3 Get Active Bonus Plans - Returns active plans', async ({ request }) => {
    ...
    test('2.4 Get Bonus Plan Details - Returns full info', async ({ request }) => {
      ...
      if (listData.data?.length > 0) {
        ...
        expect(data).toHaveProperty('success', true)
        expect(data).toHaveProperty('data')
        expect(data.data).toHaveProperty('id', planId)
        expect(data.data).toHaveProperty('allocation_count')

        console.log(`[Plan Details]: "${data.data.name}" - ${data.data.allocation_count} allocations`)
      }
    })

    test('2.5 Get Bonus Allocations - Returns employee allocations', async ({ request }) => {
      const listRes = await request.get(`${API_BASE}/api/v1/bonus-plans?limit=1`, { headers: HEADERS })
      const listData = await listRes.json()
      ...
      if (listData.data?.length > 0) {
        ...
        if (data.data.length > 0) {
          expect(data.data[0]).toHaveProperty('employee_name')
        }
  ...
  test.describe('3. Benefits API', () => {

    test('3.1 Get Benefits - Returns benefit list', async ({ request }) => {
      ...
      if (data.data.length > 0) {
        ...
        expect(benefit).toHaveProperty('benefit_name')
        expect(benefit).toHaveProperty('benefit_type')
        console.log(`[Benefits]: ${data.meta.total} benefits available`)
      }
    })

    test('3.2 Get Benefit Stats - Returns statistics', async ({ request }) => {
      ...
      expect(data).toHaveProperty('success', true)
      expect(data).toHaveProperty('data')
      expect(data.data).toHaveProperty('total_benefits')
      expect(data.data).toHaveProperty('active_benefits')
      expect(data.data).toHaveProperty('total_enrollments')

      console.log(`[Benefit Stats]: ${data.data.total_benefits} benefits, ${data.data.active_enrollments} active enrollments`)
    })

    test('3.3 Get Benefit Types - Returns type breakdown', async ({ request }) => {
    ...
    test('3.4 Get Benefit Details - Returns full info', async ({ request }) => {
      ...
      if (listData.data?.length > 0) {
        ...
        expect(data).toHaveProperty('success', true)
        expect(data).toHaveProperty('data')
        expect(data.data).toHaveProperty('id', benefitId)
        expect(data.data).toHaveProperty('active_enrollments')

        console.log(`[Benefit Details]: "${data.data.benefit_name}" - ${data.data.active_enrollments} enrolled`)
      }
    })

    test('3.5 Get Benefit Enrollments - Returns enrollment list', async ({ request }) => {
      const listRes = await request.get(`${API_BASE}/api/v1/benefits?limit=1`, { headers: HEADERS })
      const listData = await listRes.json()
  ...
  test.describe('4. Pay Stubs API', () => {

    test('4.1 Get Pay Stubs - Returns payroll list', async ({ request }) => {
      ...
      if (data.data.length > 0) {
        ...
        expect(stub).toHaveProperty('employee_name')
        expect(stub).toHaveProperty('gross_pay')
        expect(stub).toHaveProperty('net_pay')
        console.log(`[Pay Stubs]: ${data.meta.total} pay stubs in system`)
      }
    })

    test('4.2 Get Pay Stub Stats - Returns overview', async ({ request }) => {
    ...
    test('4.3 Get Single Pay Stub - Returns details', async ({ request }) => {
      ...
      if (listData.data?.length > 0) {
        ...
        expect(data).toHaveProperty('success', true)
        expect(data).toHaveProperty('data')
        expect(data.data).toHaveProperty('id', stubId)
  ...
  test.describe('5. Complete Compensation Flow', () => {

    test('5.1 Full Compensation Analysis Flow', async ({ request }) => {
      ...
      if (!bandStatsRes.ok()) { console.log('[SKIP] Salary band stats API returned', bandStatsRes.status()); return }
      ...
      if (!bandsRes.ok()) { console.log('[SKIP] Salary bands API returned', bandsRes.status()); return }
      ...
      if (!bonusStatsRes.ok()) { console.log('[SKIP] Bonus stats API returned', bonusStatsRes.status()); return }
      ...
      if (!activePlansRes.ok()) { console.log('[SKIP] Active plans API returned', activePlansRes.status()); return }
      ...
      if (!benefitStatsRes.ok()) { console.log('[SKIP] Benefit stats API returned', benefitStatsRes.status()); return }
      ...
      if (!typesRes.ok()) { console.log('[SKIP] Benefit types API returned', typesRes.status()); return }
      ...
      if (!payrollRes.ok()) { console.log('[SKIP] Payroll API returned', payrollRes.status()); return }
  ...
  test.describe('6. Data Verification', () => {
    ...
    test('6.1 Verify Salary Band Data Quality', async ({ request }) => {
      const res = await request.get(`${API_BASE}/api/v1/salary-bands?limit=20`, { headers: HEADERS })
      if (!res.ok()) { console.log('[SKIP] API returned', res.status()); return }
      ...
      for (const band of data.data.slice(0, 5)) {
        ...
        expect(band.band_name).toBeTruthy()
        expect(parseFloat(band.max_salary)).toBeGreaterThan(parseFloat(band.min_salary))
      }

      console.log(`[Verification]: ${data.data.length} salary bands verified`)
    })

    test('6.2 Verify Bonus Plan Data', async ({ request }) => {
      const res = await request.get(`${API_BASE}/api/v1/bonus-plans?limit=20`, { headers: HEADERS })
      if (!res.ok()) { console.log('[SKIP] API returned', res.status()); return }
      ...
      for (const plan of data.data.slice(0, 5)) {
        ...
        expect(plan.bonus_type).toBeTruthy()
      }

      console.log(`[Verification]: ${data.data.length} bonus plans verified`)
    })

    test('6.3 Verify Benefits Data', async ({ request }) => {
      const res = await request.get(`${API_BASE}/api/v1/benefits?limit=20`, { headers: HEADERS })
      if (!res.ok()) { console.log('[SKIP] API returned', res.status()); return }
      ...
      for (const benefit of data.data.slice(0, 5)) {
        ...
        expect(benefit.benefit_name).toBeTruthy()
        expect(benefit.benefit_type).toBeTruthy()
      }

      console.log(`[Verification]: ${data.data.length} benefits verified`)
    })

    test('6.4 Verify Pay Stub Data', async ({ request }) => {
      const res = await request.get(`${API_BASE}/api/v1/pay-stubs?limit=20`, { headers: HEADERS })
      if (!res.ok()) { console.log('[SKIP] API returned', res.status()); return }
      ...
      for (const stub of data.data.slice(0, 5)) {
        ...
        expect(stub.employee_name).toBeTruthy()
        expect(parseFloat(stub.gross_pay)).toBeGreaterThanOrEqual(parseFloat(stub.net_pay))
      }
