 ...
 */
test.describe('Headcount Trend Chart Validation', () => {
  ...
  test('Trend Organico must show correct headcount from API', async ({ page, request }) => {
    ...
    if (!overviewRes.ok() || !headcountRes.ok()) {
      console.log('[SKIP] API returned', overviewRes.status(), headcountRes.status()); return
    }
    ...
    expect(latestHeadcount, 'Headcount trend must match active employees').toBe(expectedActiveEmployees)
    ...
    if (cardText?.includes('Dati trend non disponibili')) {
      ...
      if (uiHeadcount > 0) {
        ...
        expect(uiHeadcount, `Organico attuale MISMATCH! UI shows ${uiHeadcount} but DB has ${expectedActiveEmployees}`).toBe(expectedActiveEmployees)
