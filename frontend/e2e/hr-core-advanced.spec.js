 ...
 */
test_1.test.describe('HR Core Advanced Features', () => {
    // Use auth state
    test_1.test.use({ storageState: '.auth/admin.json' });
    ...
    // ============================================
    test_1.test.describe('Employee Documents', () => {
        (0, test_1.test)('should display employee documents page', async ({ page, request }) => {
            // Get an employee from API
            const res = await request.get(`${API_BASE}/api/v1/employees`, { headers: HEADERS });
            if (!res.ok()) {
                console.log('[SKIP] Employees API returned', res.status());
                return;
            }
        ...
        (0, test_1.test)('should show document categories', async ({ page, request }) => {
            const res = await request.get(`${API_BASE}/api/v1/employees`, { headers: HEADERS });
            const api = await res.json();
            const employees = api.data?.employees || api.data || [];
            if (employees.length === 0) {
    ...
    test_1.test.describe('Employee Skills Profile', () => {
        (0, test_1.test)('should display employee skills page', async ({ page, request }) => {
            const res = await request.get(`${API_BASE}/api/v1/employees`, { headers: HEADERS });
            if (!res.ok()) {
                console.log('[SKIP] Employees API returned', res.status());
                return;
        ...
        (0, test_1.test)('should show skill categories and levels', async ({ page, request }) => {
            const res = await request.get(`${API_BASE}/api/v1/employees`, { headers: HEADERS });
            const api = await res.json();
            const employees = api.data?.employees || api.data || [];
            if (employees.length === 0) {
    ...
    test_1.test.describe('Employee History Timeline', () => {
        (0, test_1.test)('should display employee history page', async ({ page, request }) => {
            const res = await request.get(`${API_BASE}/api/v1/employees`, { headers: HEADERS });
            if (!res.ok()) {
                console.log('[SKIP] Employees API returned', res.status());
                return;
        ...
        (0, test_1.test)('should show timeline events', async ({ page, request }) => {
            const res = await request.get(`${API_BASE}/api/v1/employees`, { headers: HEADERS });
            const api = await res.json();
            const employees = api.data?.employees || api.data || [];
            if (employees.length === 0) {
    ...
    test_1.test.describe('Employee Contracts', () => {
        (0, test_1.test)('should display employee contracts page', async ({ page, request }) => {
            const res = await request.get(`${API_BASE}/api/v1/employees`, { headers: HEADERS });
            if (!res.ok()) {
                console.log('[SKIP] Employees API returned', res.status());
                return;
        ...
        (0, test_1.test)('should show contract details', async ({ page, request }) => {
            const res = await request.get(`${API_BASE}/api/v1/employees`, { headers: HEADERS });
            const api = await res.json();
            const employees = api.data?.employees || api.data || [];
            if (employees.length === 0) {
    ...
    // ============================================
    test_1.test.describe('Department Tree View', () => {
        (0, test_1.test)('should toggle between table and tree view', async ({ page }) => {
            await page.goto('/admin/departments');
            await page.waitForLoadState('domcontentloaded');
            await page.waitForLoadState('networkidle');
            // Look for view mode toggle buttons
            const tableViewBtn = page.getByRole('button', { name: /tabella|table/i }).or(page.locator('button').filter({ has: page.locator('[class*="LayoutGrid"]') }));
            const treeViewBtn = page.getByRole('button', { name: /albero|tree/i }).or(page.locator('button').filter({ has: page.locator('[class*="Network"]') }));
        ...
        (0, test_1.test)('should expand and collapse tree nodes', async ({ page }) => {
            await page.goto('/admin/departments');
            await page.waitForLoadState('domcontentloaded');
            await page.waitForLoadState('networkidle');
            // Switch to tree view
    ...
    test_1.test.describe('Department Statistics', () => {
        (0, test_1.test)('should display department statistics page', async ({ page, request }) => {
            const res = await request.get(`${API_BASE}/api/v1/departments`, { headers: HEADERS });
            if (!res.ok()) {
                console.log('[SKIP] Departments API returned', res.status());
                return;
            }
            const api = await res.json();
        ...
        (0, test_1.test)('should show KPI cards and charts', async ({ page, request }) => {
            const res = await request.get(`${API_BASE}/api/v1/departments`, { headers: HEADERS });
            const api = await res.json();
            const departments = api.data || [];
            if (departments.length === 0) {
        ...
        (0, test_1.test)('should have time range selector', async ({ page, request }) => {
            const res = await request.get(`${API_BASE}/api/v1/departments`, { headers: HEADERS });
            const api = await res.json();
            const departments = api.data || [];
            if (departments.length === 0) {
    ...
    // ============================================
    test_1.test.describe('Employee Bulk Operations', () => {
        (0, test_1.test)('should show checkboxes for multi-select', async ({ page }) => {
            await page.goto('/admin/employees');
            await page.waitForLoadState('domcontentloaded');
            await page.waitForLoadState('networkidle');
            // Check for checkbox in table header
            const headerCheckbox = page.locator('thead input[type="checkbox"]').or(page.locator('thead button[role="checkbox"]'));
            const hasHeaderCheckbox = await headerCheckbox.isVisible().catch(() => false);
            // Check for checkboxes in table rows
            const rowCheckboxes = page.locator('tbody input[type="checkbox"]').or(page.locator('tbody button[role="checkbox"]'));
        ...
        (0, test_1.test)('should show bulk action bar when items selected', async ({ page }) => {
            await page.goto('/admin/employees');
            await page.waitForLoadState('domcontentloaded');
            await page.waitForLoadState('networkidle');
            // Find and click first row checkbox
            const firstCheckbox = page.locator('tbody button[role="checkbox"]').first().or(page.locator('tbody input[type="checkbox"]').first());
            if (await firstCheckbox.isVisible().catch(() => false)) {
                await firstCheckbox.click();
                await page.waitForTimeout(500);
                // Check for floating action bar
                const actionBar = page.locator('[class*="fixed"]').filter({ hasText: /selezionat/i }).or(page.locator('[class*="bottom"]').filter({ hasText: /selezionat/i }));
        ...
        (0, test_1.test)('should select all with header checkbox', async ({ page }) => {
            await page.goto('/admin/employees');
            await page.waitForLoadState('domcontentloaded');
            await page.waitForLoadState('networkidle');
    ...
    // ============================================
    test_1.test.describe('Employee Advanced Search', () => {
        (0, test_1.test)('should have advanced search toggle', async ({ page }) => {
            await page.goto('/admin/employees');
            await page.waitForLoadState('domcontentloaded');
            await page.waitForLoadState('networkidle');
            // Look for advanced search toggle button
            const advancedToggle = page.getByRole('button', { name: /filtri avanzati|advanced/i }).or(page.locator('button').filter({ has: page.locator('[class*="SlidersHorizontal"]') }));
        ...
        (0, test_1.test)('should show quick filter presets', async ({ page }) => {
            await page.goto('/admin/employees');
            await page.waitForLoadState('domcontentloaded');
            await page.waitForLoadState('networkidle');
        ...
        (0, test_1.test)('should apply date range filter', async ({ page }) => {
            await page.goto('/admin/employees');
            await page.waitForLoadState('domcontentloaded');
            await page.waitForLoadState('networkidle');
            // Open advanced search
        ...
        (0, test_1.test)('should show save search dialog', async ({ page }) => {
            await page.goto('/admin/employees');
            await page.waitForLoadState('domcontentloaded');
            await page.waitForLoadState('networkidle');
            // Open advanced search
            ...
            if (await advancedToggle.isVisible().catch(() => false)) {
                ...
                const saveBtn = page.getByRole('button', { name: /salva ricerca|save search/i }).or(page.locator('button').filter({ has: page.locator('[class*="Bookmark"]') }));
        ...
        (0, test_1.test)('should clear all filters', async ({ page }) => {
            await page.goto('/admin/employees');
            await page.waitForLoadState('domcontentloaded');
            await page.waitForLoadState('networkidle');
            // Open advanced search
    ...
    // MOBILE RESPONSIVE
    // ============================================
    test_1.test.describe('Mobile Responsive', () => {
        test_1.test.use({ viewport: { width: 375, height: 667 } });
        (0, test_1.test)('should display employees page correctly on mobile', async ({ page }) => {
            await page.goto('/admin/employees');
            await page.waitForLoadState('domcontentloaded');
            await page.waitForLoadState('networkidle');
            // Verify page loads
        ...
        (0, test_1.test)('should display departments page correctly on mobile', async ({ page }) => {
            await page.goto('/admin/departments');
            await page.waitForLoadState('domcontentloaded');
            await page.waitForLoadState('networkidle');
            const heading = page.getByRole('heading', { level: 1 });
            await (0, test_1.expect)(heading).toContainText('Dipartimenti');
            await page.screenshot({ path: 'test-results/hr-core-mobile-departments.png', fullPage: true });
        });
        (0, test_1.test)('should show mobile-friendly bulk action bar', async ({ page }) => {
            await page.goto('/admin/employees');
            await page.waitForLoadState('domcontentloaded');
            await page.waitForLoadState('networkidle');
            // Select an employee
    ...
    // ============================================
    test_1.test.describe('Navigation Integration', () => {
        (0, test_1.test)('should navigate from employee detail to sub-pages', async ({ page, request }) => {
            const res = await request.get(`${API_BASE}/api/v1/employees`, { headers: HEADERS });
            const api = await res.json();
            const employees = api.data?.employees || api.data || [];
            if (employees.length === 0) {
            ...
            for (const subPage of subPages) {
                const link = page.getByRole('button', { name: new RegExp(subPage, 'i') }).or(page.getByRole('link', { name: new RegExp(subPage, 'i') }));
        ...
        (0, test_1.test)('should navigate from department list to stats', async ({ page, request }) => {
            const res = await request.get(`${API_BASE}/api/v1/departments`, { headers: HEADERS });
            const api = await res.json();
            const departments = api.data || [];
            if (departments.length === 0) {
            ...
            await page.waitForLoadState('networkidle');
            // Look for stats link
            const statsLink = page.getByRole('link', { name: /statistiche|stats/i }).or(page.getByRole('button', { name: /statistiche|stats/i }));
...
//# sourceMappingURL=hr-core-advanced.spec.js.map