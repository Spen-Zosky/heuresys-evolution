...
async function getCardText(page, filterText) {
 ...
 */
test_1.test.describe('1. Dashboard - Tab Overview', () => {
    test_1.test.beforeAll(async () => {
        const token = await (0, api_auth_helper_1.getAuthToken)();
        HEADERS = { ...HEADERS, 'Authorization': `Bearer ${token}` };
    });
    (0, test_1.test)('1.1 KPI Cards - Dipendenti Attivi', async ({ page, request }) => {
        ...
        if (!cardText) {
            console.log('[SKIP] Dipendenti card not found');
            return;
        }
        const uiValue = extractNumber(cardText);
        if (!uiValue) {
            console.log('[SKIP] Could not extract UI value');
            return;
        }
        console.log(`[Dipendenti Attivi] API=${expected} UI=${uiValue}`);
        (0, test_1.expect)(uiValue, 'Dipendenti Attivi MISMATCH').toBe(expected);
    });
    (0, test_1.test)('1.2 KPI Cards - Obiettivi in Corso', async ({ page, request }) => {
        const res = await request.get(`${API_BASE}/api/v1/dashboard/overview`, { headers: HEADERS });
        if (!res.ok()) {
            console.log('[SKIP] API request failed');
            return;
        ...
        if (!cardText) {
            console.log('[SKIP] Obiettivi card not found');
            return;
        }
        const uiValue = extractNumber(cardText);
        if (!uiValue) {
        ...
        (0, test_1.expect)(uiValue, 'Obiettivi in Corso MISMATCH').toBe(expected);
    });
    (0, test_1.test)('1.3 KPI Cards - Valutazione Media', async ({ page, request }) => {
        const res = await request.get(`${API_BASE}/api/v1/dashboard/overview`, { headers: HEADERS });
        if (!res.ok()) {
            console.log('[SKIP] API request failed');
            return;
        ...
        if (!cardText) {
            console.log('[SKIP] Valutazione card not found');
            return;
        }
        ...
        (0, test_1.expect)(uiValue, 'Valutazione Media MISMATCH').toBe(expected);
    });
    (0, test_1.test)('1.4 KPI Cards - Corsi Attivi', async ({ page, request }) => {
        const res = await request.get(`${API_BASE}/api/v1/dashboard/overview`, { headers: HEADERS });
        if (!res.ok()) {
            console.log('[SKIP] API request failed');
            return;
        ...
        (0, test_1.expect)(expectedCourses, 'Total courses should be > 0').toBeGreaterThan(0);
    ...
    (0, test_1.test)('1.5 KPI Cards - Riconoscimenti', async ({ page, request }) => {
        const res = await request.get(`${API_BASE}/api/v1/dashboard/overview`, { headers: HEADERS });
        if (!res.ok()) {
            console.log('[SKIP] API request failed');
            return;
        ...
        if (!cardText) {
            console.log('[SKIP] Riconoscimenti card not found on dashboard');
            return;
        }
        ...
        (0, test_1.expect)(Number(expected), 'Riconoscimenti API value should be > 0').toBeGreaterThan(0);
    });
    (0, test_1.test)('1.6 KPI Cards - Posizioni Aperte', async ({ page, request }) => {
        ...
        if (!cardText) {
            console.log('[SKIP] Posizioni card not found');
            return;
        }
        const uiValue = extractNumber(cardText);
        if (!uiValue) {
        ...
        (0, test_1.expect)(uiValue, 'Posizioni Aperte MISMATCH').toBe(expected);
    });
    (0, test_1.test)('1.7 KPI Cards - Ferie Pending', async ({ page, request }) => {
        const res = await request.get(`${API_BASE}/api/v1/dashboard/hr-metrics`, { headers: HEADERS });
        if (!res.ok()) {
            console.log('[SKIP] API request failed');
            return;
        ...
        if (!cardText) {
            console.log('[SKIP] Ferie card not found');
            return;
        }
        const uiValue = extractNumber(cardText);
        if (!uiValue) {
        ...
        (0, test_1.expect)(uiValue, 'Ferie Pending MISMATCH').toBe(expected);
    });
    (0, test_1.test)('1.8 Chart - Trend Organico (Organico attuale)', async ({ page, request }) => {
        ...
        if (!isVisible) {
            console.log('[SKIP] Trend chart card not visible');
            return;
        }
        const cardText = await chartCard.textContent({ timeout: 5000 }).catch(() => null);
        if (!cardText) {
            console.log('[SKIP] Could not get chart text');
            return;
        }
        ...
        (0, test_1.expect)(uiValue, 'Organico attuale MISMATCH').toBe(expected);
    });
    (0, test_1.test)('1.9 Chart - Distribuzione Organico (totale)', async ({ page, request }) => {
        const res = await request.get(`${API_BASE}/api/v1/dashboard/overview`, { headers: HEADERS });
        if (!res.ok()) {
            console.log('[SKIP] API request failed');
            return;
        ...
        const isVisible = await chartCard.isVisible().catch(() => false);
        if (!isVisible) {
            console.log('[SKIP] Distribution chart card not visible');
            return;
        }
        const cardText = await chartCard.textContent({ timeout: 5000 }).catch(() => null);
        if (!cardText) {
        ...
        (0, test_1.expect)(uiCount, 'Distribuzione Organico total MISMATCH').toBe(expected);
    });
});
//# sourceMappingURL=systematic-verification.spec.js.map