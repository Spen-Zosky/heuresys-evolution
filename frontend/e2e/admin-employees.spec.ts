...
test.describe('Admin Employees Page', () => {
  ...
  test('should load the employees page', async ({ page }) => {
    ...
    await expect(h1).toContainText('Dipendenti')
  ...
  test('should display employee stats cards', async ({ page }) => {
    ...
    expect(hasSearch || hasTable).toBeTruthy()

    await page.screenshot({ path: 'test-results/employees-page-content.png', fullPage: true })
  })

  test('should load employees data from API', async ({ page }) => {
    ...
    expect(hasTable || hasNoEmployees || hasError || mainVisible).toBeTruthy()

    await page.screenshot({ path: 'test-results/employees-data-loaded.png', fullPage: true })
  })

  test('should display employee table with columns', async ({ page }) => {
    await page.waitForLoadState('networkidle')
    ...
    if (headerCount > 0) {
      ...
      await expect(page.locator('th:has-text("Dipendente")')).toBeVisible()
      await expect(page.locator('th:has-text("Ruolo")')).toBeVisible()
      await expect(page.locator('th:has-text("Dipartimento")')).toBeVisible()
  ...
  test('should display pagination when there are many employees', async ({ page }) => {
    await page.waitForLoadState('networkidle')
  ...
  test('should navigate from dashboard to employees via sidebar', async ({ page }) => {
    ...
    await expect(page.locator('h1')).toContainText('Dipendenti')

    await page.screenshot({ path: 'test-results/employees-navigation.png', fullPage: true })
  })

  test('should display employee action menu', async ({ page }) => {
    await page.waitForLoadState('networkidle')
