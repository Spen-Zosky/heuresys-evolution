 ...
 */
test_1.test.describe('HR Core Module', () => {
    // Use auth state
    ...
    test_1.test.describe('Org Units', () => {
        (0, test_1.test)('should display org units list page', async ({ page, request }) => {
            // Verify API
            ...
            await (0, test_1.expect)(heading).toContainText('Unità Organizzative');
            // Verify stats cards are present
            await (0, test_1.expect)(page.getByText('Totale Unità')).toBeVisible();
            // Verify table has data
            if (orgUnits.length > 0) {
                const firstUnit = orgUnits[0];
                await (0, test_1.expect)(page.getByText(firstUnit.name)).toBeVisible();
            }
            await page.screenshot({ path: 'test-results/hr-core-org-units-list.png', fullPage: true });
        });
        (0, test_1.test)('should navigate to org unit detail page', async ({ page, request }) => {
            // Get first org unit from API
            const res = await request.get(`${API_BASE}/api/v1/org-units`, { headers: HEADERS });
            const api = await res.json();
            const orgUnits = api.data || [];
            if (orgUnits.length === 0) {
                console.log('[SKIP] No org units found');
                return;
            }
            const firstUnit = orgUnits[0];
        ...
        (0, test_1.test)('should display new org unit form', async ({ page }) => {
            ...
            if (hasForm) {
                await (0, test_1.expect)(page.getByLabel(/codice/i)).toBeVisible();
                await (0, test_1.expect)(page.getByRole('button', { name: /crea|salva/i })).toBeVisible();
    ...
    test_1.test.describe('Locations', () => {
        (0, test_1.test)('should display locations list page', async ({ page, request }) => {
            // Verify API
            ...
            // Verify page title
            const heading = page.getByRole('heading', { level: 1 });
            await (0, test_1.expect)(heading).toContainText('Sedi');
            // Verify stats cards
            await (0, test_1.expect)(page.getByText('Totale Sedi')).toBeVisible();
            // Verify table has data
            if (locations.length > 0) {
                const firstLoc = locations[0];
                await (0, test_1.expect)(page.getByText(firstLoc.name)).toBeVisible();
            }
            await page.screenshot({ path: 'test-results/hr-core-locations-list.png', fullPage: true });
        });
        (0, test_1.test)('should navigate to location detail page', async ({ page, request }) => {
            const res = await request.get(`${API_BASE}/api/v1/locations`, { headers: HEADERS });
            const api = await res.json();
            const locations = api.data || [];
            if (locations.length === 0) {
            ...
            await page.waitForLoadState('domcontentloaded');
            await (0, test_1.expect)(page.getByRole('heading', { level: 1 })).toContainText(firstLoc.name);
            await page.screenshot({ path: 'test-results/hr-core-location-detail.png', fullPage: true });
        });
        (0, test_1.test)('should display new location form', async ({ page }) => {
            await page.goto('/admin/locations/new');
            await page.waitForLoadState('domcontentloaded');
            await (0, test_1.expect)(page.getByLabel(/nome/i)).toBeVisible();
            await (0, test_1.expect)(page.getByLabel(/codice/i)).toBeVisible();
            await (0, test_1.expect)(page.getByLabel(/città/i)).toBeVisible();
            await (0, test_1.expect)(page.getByRole('button', { name: /crea|salva/i })).toBeVisible();
            await page.screenshot({ path: 'test-results/hr-core-location-new.png', fullPage: true });
        });
    });
    test_1.test.describe('Cost Centers', () => {
        (0, test_1.test)('should display cost centers list page', async ({ page, request }) => {
            // Verify API
            ...
            // Verify page title
            const heading = page.getByRole('heading', { level: 1 });
            await (0, test_1.expect)(heading).toContainText('Centri di Costo');
            // Verify stats cards
            await (0, test_1.expect)(page.getByText('Totale Centri')).toBeVisible();
            await (0, test_1.expect)(page.getByText('Budget Totale')).toBeVisible();
            // Verify table has data
            if (costCenters.length > 0) {
                const first = costCenters[0];
                await (0, test_1.expect)(page.getByText(first.name)).toBeVisible();
            }
            await page.screenshot({ path: 'test-results/hr-core-cost-centers-list.png', fullPage: true });
        });
        (0, test_1.test)('should navigate to cost center detail page', async ({ page, request }) => {
            const res = await request.get(`${API_BASE}/api/v1/cost-centers`, { headers: HEADERS });
            const api = await res.json();
            const costCenters = api.data || [];
            if (costCenters.length === 0) {
            ...
            await page.waitForLoadState('domcontentloaded');
            await (0, test_1.expect)(page.getByRole('heading', { level: 1 })).toContainText(first.name);
            await page.screenshot({ path: 'test-results/hr-core-cost-center-detail.png', fullPage: true });
        });
        (0, test_1.test)('should display new cost center form', async ({ page }) => {
    ...
    test_1.test.describe('Org Chart', () => {
        (0, test_1.test)('should display interactive org chart', async ({ page, request }) => {
            // Verify API
            const res = await request.get(`${API_BASE}/api/v1/org-units`, { headers: HEADERS });
            if (!res.ok()) {
                console.log('[SKIP] API returned', res.status());
            ...
            // Verify page title
            const heading = page.getByRole('heading', { level: 1 });
            await (0, test_1.expect)(heading).toContainText('Organigramma');
            // Verify React Flow canvas is present
            await (0, test_1.expect)(page.locator('.react-flow')).toBeVisible();
            // Verify controls are present (zoom, minimap)
            await (0, test_1.expect)(page.locator('.react-flow__controls')).toBeVisible();
            // Verify stats cards
            await (0, test_1.expect)(page.getByText('Totale Unità')).toBeVisible();
            await page.screenshot({ path: 'test-results/hr-core-org-chart.png', fullPage: true });
        });
        (0, test_1.test)('should interact with org chart nodes', async ({ page, request }) => {
            const res = await request.get(`${API_BASE}/api/v1/org-units`, { headers: HEADERS });
            const api = await res.json();
            const orgUnits = api.data || [];
            if (orgUnits.length === 0) {
    ...
    test_1.test.describe('Navigation', () => {
        (0, test_1.test)('should have working sidebar navigation for HR Core', async ({ page }) => {
            await page.goto('/admin');
            await page.waitForLoadState('domcontentloaded');
            // Find Organigramma in sidebar
            const orgMenu = page.getByRole('navigation').getByText('Organigramma');
            await (0, test_1.expect)(orgMenu).toBeVisible();
            ...
            if (hasSubnav) {
                await (0, test_1.expect)(vistaGrafico).toBeVisible();
