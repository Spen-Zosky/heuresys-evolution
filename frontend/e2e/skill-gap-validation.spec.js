 ...
 */
test_1.test.describe('Skill Gap Analysis - Real Data Validation', () => {
    test_1.test.beforeAll(async () => {
        const token = await (0, api_auth_helper_1.getAuthToken)();
        HEADERS = { ...HEADERS, 'Authorization': `Bearer ${token}` };
    });
    (0, test_1.test)('Verify skill gap chart shows real database data', async ({ page, request }) => {
        ...
        (0, test_1.expect)(api.data).toBeDefined();
        (0, test_1.expect)(api.data.length).toBeGreaterThan(0);
    ...
    (0, test_1.test)('Verify skill gap values match API data', async ({ request }) => {
        ...
        const leadershipData = api.data.find((d) => d.skill === 'Leadership');
        if (leadershipData) {
            ...
            (0, test_1.expect)(isHardcoded, 'Leadership data should NOT be hardcoded values').toBeFalsy();
        }
        // Verify data has expected structure (allowing null values)
        for (const skill of api.data) {
            (0, test_1.expect)(skill.skill).toBeDefined();
            // Allow null values as the API may return nulls for missing data
            if (skill.current !== null && skill.target !== null) {
                (0, test_1.expect)(typeof skill.current).toBe('number');
                (0, test_1.expect)(typeof skill.target).toBe('number');
            }
        }
        console.log('[PASS] Skill gap API returns real database data');
    });
});
//# sourceMappingURL=skill-gap-validation.spec.js.map