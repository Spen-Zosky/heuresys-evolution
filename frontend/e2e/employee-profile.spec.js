...
const STORAGE_STATE = path_1.default.join(__dirname, '../.auth/admin.json');
test_1.test.use({ storageState: STORAGE_STATE });
...
test_1.test.describe('Employee Profile - Multi-Tab Page', () => {
    test_1.test.beforeEach(async ({ page }) => {
        await page.goto(EMPLOYEE_URL, { waitUntil: 'domcontentloaded', timeout: 30000 });
        await page.waitForLoadState('networkidle');
    });
    (0, test_1.test)('shows employee name and tab navigation', async ({ page }) => {
    ...
    (0, test_1.test)('Panoramica tab shows identity and address data', async ({ page }) => {
        const heading = await page.locator('h1').first().isVisible({ timeout: 10000 }).catch(() => false);
        if (!heading) {
            console.log('[SKIP] Employee profile did not load');
            return;
        }
    ...
    (0, test_1.test)('Panoramica tab shows education data (not dashes)', async ({ page }) => {
        const heading = await page.locator('h1').first().isVisible({ timeout: 10000 }).catch(() => false);
        if (!heading) {
            console.log('[SKIP] Employee profile did not load');
            return;
    ...
    (0, test_1.test)('Panoramica tab shows emergency contact', async ({ page }) => {
        const heading = await page.locator('h1').first().isVisible({ timeout: 10000 }).catch(() => false);
        if (!heading) {
            console.log('[SKIP] Employee profile did not load');
            return;
    ...
    (0, test_1.test)('Organizzazione tab shows department and manager name', async ({ page }) => {
        const heading = await page.locator('h1').first().isVisible({ timeout: 10000 }).catch(() => false);
        if (!heading) {
            console.log('[SKIP] Employee profile did not load');
            return;
        ...
        if (!tabVisible) {
            console.log('[SKIP] Organizzazione tab not found');
            return;
        }
        await orgTab.click();
        await page.waitForLoadState('networkidle');
    ...
    (0, test_1.test)('Organizzazione tab shows SAP and lifecycle', async ({ page }) => {
        const heading = await page.locator('h1').first().isVisible({ timeout: 10000 }).catch(() => false);
        if (!heading) {
            console.log('[SKIP] Employee profile did not load');
            return;
    ...
    (0, test_1.test)('Contratti tab loads contract data', async ({ page }) => {
        const heading = await page.locator('h1').first().isVisible({ timeout: 10000 }).catch(() => false);
        if (!heading) {
            console.log('[SKIP] Employee profile did not load');
            return;
        ...
        if (!tabVisible) {
            console.log('[SKIP] Contratti tab not found');
            return;
        }
        await tab.click();
        await page.waitForLoadState('networkidle');
    ...
    (0, test_1.test)('Performance tab loads reviews and check-ins', async ({ page }) => {
        const heading = await page.locator('h1').first().isVisible({ timeout: 10000 }).catch(() => false);
        if (!heading) {
            console.log('[SKIP] Employee profile did not load');
            return;
        ...
        const tabVisible = await tab.isVisible().catch(() => false);
        if (!tabVisible) {
            console.log('[SKIP] Performance tab not found');
            return;
        }
        await tab.click();
        await page.waitForLoadState('networkidle');
    ...
    (0, test_1.test)('Obiettivi tab loads goals', async ({ page }) => {
        const heading = await page.locator('h1').first().isVisible({ timeout: 10000 }).catch(() => false);
        if (!heading) {
            console.log('[SKIP] Employee profile did not load');
            return;
        ...
        const tabVisible = await tab.isVisible().catch(() => false);
        if (!tabVisible) {
            console.log('[SKIP] Obiettivi tab not found');
            return;
        }
        await tab.click();
        await page.waitForLoadState('networkidle');
    ...
    (0, test_1.test)('Documenti tab loads documents', async ({ page }) => {
        const heading = await page.locator('h1').first().isVisible({ timeout: 10000 }).catch(() => false);
        if (!heading) {
            console.log('[SKIP] Employee profile did not load');
            return;
        ...
        const tabVisible = await tab.isVisible().catch(() => false);
        if (!tabVisible) {
            console.log('[SKIP] Documenti tab not found');
            return;
        }
        await tab.click();
        await page.waitForLoadState('networkidle');
    ...
    (0, test_1.test)('all tabs can be clicked without errors', async ({ page }) => {
        const heading = await page.locator('h1').first().isVisible({ timeout: 10000 }).catch(() => false);
        if (!heading) {
            console.log('[SKIP] Employee profile did not load');
            return;
        ...
        for (const tabName of tabs) {
            ...
            (0, test_1.expect)(crashError).toBe(0);
        }
        await page.screenshot({ path: 'test-results/employee-profile-all-tabs.png' });
    });
});
//# sourceMappingURL=employee-profile.spec.js.map