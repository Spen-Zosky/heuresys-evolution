 ...
 */
test_1.test.describe('3. Dashboard - Tab Talent', () => {
    test_1.test.beforeAll(async () => {
        const token = await (0, api_auth_helper_1.getAuthToken)();
        HEADERS = { ...HEADERS, 'Authorization': `Bearer ${token}` };
    });
    (0, test_1.test)('3.1 Talent Tab - Verify TurnoverAnalysisChart is visible', async ({ page, request }) => {
    ...
    (0, test_1.test)('3.2 Verify department distribution data', async ({ page, request }) => {
        ...
        if (!api.data?.headcount_by_department) {
            console.log('[SKIP] No headcount by department data');
            return;
        }
        console.log('[HR Metrics - Headcount by Department]:', api.data.headcount_by_department);
        // Calculate total
        const total = api.data.headcount_by_department.reduce((acc, d) => acc + parseInt(d.count), 0);
        ...
        (0, test_1.expect)(total).toBeGreaterThan(0);
        console.log(`[PASS] Department distribution API returns ${total} employees`);
    });
    (0, test_1.test)('3.3 Verify turnover-summary API returns valid data', async ({ request }) => {
        ...
        // Verify structure
        (0, test_1.expect)(data.data.monthlyData).toBeDefined();
        (0, test_1.expect)(Array.isArray(data.data.monthlyData)).toBeTruthy();
        (0, test_1.expect)(data.data.monthlyData.length).toBe(12); // 12 months
        // Verify each month has required fields
        const firstMonth = data.data.monthlyData[0];
        (0, test_1.expect)(firstMonth).toHaveProperty('month');
        (0, test_1.expect)(firstMonth).toHaveProperty('voluntary');
        (0, test_1.expect)(firstMonth).toHaveProperty('involuntary');
        (0, test_1.expect)(firstMonth).toHaveProperty('total');
        console.log(`[PASS] Turnover summary API returns valid data structure`);
    });
});
//# sourceMappingURL=dashboard-talent.spec.js.map