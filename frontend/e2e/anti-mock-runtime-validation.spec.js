...
function containsSuspiciousPatterns(obj, path = '') {
    const issues = [];
...
function validateUUIDs(obj, path = '') {
    const issues = [];
...
test_1.test.describe('Anti-Mock Runtime Validation', () => {
    let authToken = null;
    ...
    test_1.test.describe('API Endpoint Validation', () => {
        for (const endpoint of CRITICAL_ENDPOINTS) {
            (0, test_1.test)(`${endpoint.description} (${endpoint.path}) should return real data`, async ({ request }) => {
                const headers = {
                ...
                if (endpoint.expectArray) {
                    ...
                    if (endpoint.minItems) {
                        (0, test_1.expect)(items.length, `${endpoint.path} should return at least ${endpoint.minItems} items`).toBeGreaterThanOrEqual(endpoint.minItems);
                    }
                }
                // Check for suspicious patterns in response
                const suspiciousIssues = containsSuspiciousPatterns(responseData);
                (0, test_1.expect)(suspiciousIssues, `${endpoint.path} contains suspicious mock data patterns:\n${suspiciousIssues.join('\n')}`).toHaveLength(0);
    ...
    test_1.test.describe('Page Content Validation', () => {
        ...
        (0, test_1.test)('Portal payroll page should display real pay stubs', async ({ page }) => {
            ...
            for (const pattern of SUSPICIOUS_PATTERNS) {
                (0, test_1.expect)(content, `Page contains mock pattern: ${pattern.source}`).not.toMatch(pattern);
            }
        });
        (0, test_1.test)('Portal learning page should display real courses', async ({ page }) => {
        ...
        (0, test_1.test)('Platform page should display real metrics', async ({ page }) => {
            ...
            (0, test_1.expect)(tenantsText).toBeTruthy();
            const content = await page.content();
            for (const pattern of SUSPICIOUS_PATTERNS) {
                (0, test_1.expect)(content, `Page contains mock pattern: ${pattern.source}`).not.toMatch(pattern);
            }
        });
        (0, test_1.test)('Admin career page should display real analytics', async ({ page }) => {
            await page.goto('/admin/career');
            await page.waitForLoadState('domcontentloaded');
            await page.waitForLoadState('networkidle');
            const content = await page.content();
            // Verify page loaded with real content (not showing error state)
            (0, test_1.expect)(content).not.toContain('API Error');
            (0, test_1.expect)(content).not.toContain('Failed to load');
            for (const pattern of SUSPICIOUS_PATTERNS) {
                (0, test_1.expect)(content, `Page contains mock pattern: ${pattern.source}`).not.toMatch(pattern);
            }
        });
        (0, test_1.test)('Root page should not show component library demo', async ({ page }) => {
            await page.goto('/');
            await page.waitForLoadState('domcontentloaded');
            const content = await page.content();
            // Should not have "Component Library" text from old demo page
            (0, test_1.expect)(content).not.toContain('Component Library');
            (0, test_1.expect)(content).not.toContain('sampleEmployee');
            (0, test_1.expect)(content).not.toContain('sampleRequest');
    ...
    test_1.test.describe('API Response Structure', () => {
        (0, test_1.test)('All endpoints should have consistent response format', async ({ request }) => {
            const headers = {
                'Content-Type': 'application/json',
                'x-tenant-code': DEFAULT_TENANT,
            };
...
//# sourceMappingURL=anti-mock-runtime-validation.spec.js.map