 ...
 */
test.describe('Data Validation - Dashboard vs API', () => {
  ...
  test('KPI values must match API data', async ({ page, request }) => {
    ...
    if (!overviewRes.ok() || !hrRes.ok()) {
      console.log('[SKIP] API returned', overviewRes.status(), hrRes.status()); return
    }
    ...
    if (!overview.data?.employees || !overview.data?.goals) { console.log('[SKIP] Incomplete overview data'); return }
    ...
    expect(Number(expectedActiveEmployees), 'Active employees should be > 0').toBeGreaterThan(0)
    expect(Number(expectedGoalsInProgress), 'Goals in progress should be >= 0').toBeGreaterThanOrEqual(0)
    expect(Number(expectedActiveCourses), 'Active courses should be > 0').toBeGreaterThan(0)
  ...
  test('Second row KPIs validation', async ({ page, request }) => {
    const apiBase = API_BASE
    const headers = HEADERS
    ...
    const overview = await overviewRes.json()
    const hrMetrics = await hrRes.json()
    if (!overview.data?.recognition || !hrMetrics.data) { console.log('[SKIP] Incomplete data'); return }
    ...
    expect(Number(expectedRecognitions), 'Recognitions should be >= 0').toBeGreaterThanOrEqual(0)
    expect(Number(expectedOpenPositions), 'Open positions should be >= 0').toBeGreaterThanOrEqual(0)
    expect(Number(expectedFeriePending), 'Pending time off should be >= 0').toBeGreaterThanOrEqual(0)
