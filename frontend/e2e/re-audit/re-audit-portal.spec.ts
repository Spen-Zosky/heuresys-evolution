...
test.describe('Re-Audit Portal', () => {
  test.beforeEach(async ({ page }) => {
    await loginAsSysadmin(page)
  })

  test('BUG-133: payroll page loads', async ({ page }) => {
    ...
    for (const path of paths) {
      ...
      if (!is404 && !finalUrl.includes('/login')) break
    }
  ...
  test('BUG-134: documents page loads', async ({ page }) => {
