...
test_1.test.describe('Re-Audit HR Core', () => {
    test_1.test.beforeEach(async ({ page }) => {
        await (0, login_helper_1.loginAsSysadmin)(page);
    });
    (0, test_1.test)('BUG-071: employees filter by department works (no DB error)', async ({ page }) => {
    ...
    (0, test_1.test)('BUG-072: department column shows name not raw code', async ({ page }) => {
        await page.goto('/admin/employees');
        await page.waitForLoadState('networkidle');
        await page.waitForTimeout(2000);
        ...
        if (hasTable) {
            ...
            for (let i = 0; i < Math.min(cellCount, 50); i++) {
                ...
                if (text && /^[0-9a-f]{8}-[0-9a-f]{4}/.test(text.trim())) {
                    foundUUID = true;
                    break;
                }
            }
    ...
    (0, test_1.test)('BUG-076: employee creation - department dropdown works', async ({ page }) => {
    ...
    (0, test_1.test)('BUG-074: employee detail time-off section', async ({ page }) => {
        await page.goto('/admin/employees');
        await page.waitForLoadState('networkidle');
        await page.waitForTimeout(2000);
    ...
    (0, test_1.test)('BUG-077/078: onboarding list loads', async ({ page }) => {
    ...
    (0, test_1.test)('BUG-079: departments tree view with hierarchy', async ({ page }) => {
    ...
    (0, test_1.test)('BUG-080/081: departments detail - employee count (API + UI)', async ({ page }) => {
        await page.goto('/admin/departments');
        await page.waitForLoadState('networkidle');
        await page.waitForTimeout(2000);
...
//# sourceMappingURL=re-audit-hr.spec.js.map