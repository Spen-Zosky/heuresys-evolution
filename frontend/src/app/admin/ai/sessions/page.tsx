...
export default function AiSessionsPage() {
  ...
  const [data, setData] = useState<RagSession[]>([]);
  ...
  const fetchData = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      const response = await apiClient.get<any>('/api/v1/rag-sessions');
      const items =
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2">
            <Brain className="h-6 w-6" /> Sessioni AI
          </h1>
              ...
              <TableBody>
                {data.map((session) => (
                  <TableRow key={session.id}>
                    ...
                    <TableCell>{session.user_name || session.user_id || 'N/A'}</TableCell>
                    <TableCell>
                      <Badge variant="outline">{session.model || 'default'}</Badge>
                    </TableCell>
