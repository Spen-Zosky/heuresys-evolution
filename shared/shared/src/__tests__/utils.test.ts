...
describe('UUID Utilities', () => {
  ...
  test('isValidUUID should validate correct UUIDs', () => {
    expect(isValidUUID('d5855519-3ed1-4427-865f-fe75f1e42c4c')).toBe(true);
    expect(isValidUUID('0c54b84a-1234-4567-89ab-cdef01234567')).toBe(true);
  });

  test('isValidUUID should reject invalid UUIDs', () => {
    expect(isValidUUID('not-a-uuid')).toBe(false);
    expect(isValidUUID('')).toBe(false);
    expect(isValidUUID('12345678-1234-1234-1234-123456789012')).toBe(false); // version not 4
  });
});

describe('Italian Fiscal Code Utilities', () => {
  test('isValidFiscalCode should validate correct format', () => {
    expect(isValidFiscalCode('RSSMRA85M01H501Z')).toBe(true);
    expect(isValidFiscalCode('VRDLGI80A01F205X')).toBe(true);
  });
  ...
  test('isValidFiscalCode should reject invalid format', () => {
    ...
    expect(isValidFiscalCode('12345678901234567')).toBe(false);
  });
  ...
  test('validateFiscalCodeChecksum should validate and reject checksums', () => {
    ...
    const result = validateFiscalCodeChecksum('RSSMRA85M01H501Z');
    expect(typeof result).toBe('boolean');

    // Invalid format should always return false
    expect(validateFiscalCodeChecksum('INVALID')).toBe(false);
    expect(validateFiscalCodeChecksum('')).toBe(false);
  });
});
...
describe('Date Utilities', () => {
  ...
  test('formatDateIT should format date in Italian format', () => {
    ...
    expect(formatDateIT(date)).toBe('15/03/2024');
  });

  test('parseDateIT should parse Italian date format', () => {
    const date = parseDateIT('15/03/2024');
  ...
  test('parseDateIT should return null for invalid format', () => {
    expect(parseDateIT('invalid')).toBeNull();
    expect(parseDateIT('2024-03-15')).toBeNull();
  });
  ...
  test('getBusinessDays should count weekdays', () => {
    ...
    expect(getBusinessDays(monday, nextMonday)).toBe(6);
  });
});
...
describe('String Utilities', () => {
  test('slugify should convert string to URL-safe slug', () => {
    ...
    expect(slugify('Città di Milano')).toBe('citta-di-milano');
...
describe('Error Utilities', () => {
  ...
  test('createError should include details when provided', () => {
    const error = createError(ErrorCodes.VALIDATION_ERROR, 'Invalid input', { field: 'email' });
